...
- **`postedit.md`** — working on a live PostEdit-mirrored site: "work on {alias} {page}" startup, resolve the alias from `postedit.xml`, resolve its wamcp `db_id`, launch debug Chrome, edit→auto-sync→refresh→**nudge 1px**→screenshot.
