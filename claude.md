...
Two startup phrases select how a session works: **"work on {alias} [page]"** (a PostEdit-mirrored live site) and **"work on wasql [page]"** (local WaSQL core/framework dev, no PostEdit watcher — the deliberate exception to "never modify core" above, since here you ARE the framework developer). Recognize them on the first message and **ACT, don't narrate** — the **`wasql-workon` skill** owns startup mechanics (the one-command `workon.php`/`workon.py` script, flags, browser choice, alias-resolution rules, wamcp `db_id` resolution, the post-task `?_menu=synchronize` step, and the manual fallback): invoke it via the Skill tool in the same turn, before asking what to work on. Deep flag/gotcha reference remains in `workon.md`.

Once startup finishes: **"work on {alias} {page}"** hands off to **`postedit.md`**'s edit loop — mirror files under `postedit/postEditFiles/{alias}/...` are the source of truth for that site's page code (Read/Edit them directly), not wamcp; all logic stays in the DB, "never modify core" is in full force. **"work on wasql"** means editing core `php/`/`wfiles/` repo files directly — no mirror, no watcher, changes affect every site built on WaSQL, so change with care.
