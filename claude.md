...
## The core-bug tracker — `wasql_bugs.db` / the `/bugs` page
Known bugs in **core files shared by every WaSQL site** (`php/common.php`, `php/database.php`, `php/cron.php`, `wfiles/js/extras/wacss.js`, …) live in **`wasql_bugs.db`** (SQLite, repo root), managed through the **`/bugs`** page on localhost — a dashboard + checklist with add / update / mark-fixed / won't-fix actions. `wasql_bugs.md` is now only a pointer stub. The page keeps all logic in its own `_pages` record and reads/writes the `.db` file directly (via `getWasqlPath()`); no core files are involved.
...
**⚠️ ACT, don't narrate.** Recognizing the phrase means *doing* the startup steps in the SAME turn — run `workon` so the PostEdit watcher is up and the mirror inventory is in hand **before** you ask the user what to work on. Do NOT reply with only a description of the mode ("I'm in X mode, here's how it works, what would you like to do?") — that is the wrong response and wastes a round-trip. (No browser at startup — see below.)
...
```
No browser and no `--shot` by default — startup launches the PostEdit watcher and prints the mirror inventory, and assumes a browser is already open. Per "Ask before doing post-change QA" above, add `--browse` (drive a debug browser to the page) or `--shot=<scratchpad>/shot.png` (implies `--browse`, then Read the PNG) only once the developer has actually asked you to look at the page.
...
- **"work on {site} {page}"** (or "let's work on {site} {page}") → a **PostEdit-mirrored live site**. Resolve `{site}` and follow **`postedit.md`** (start the watcher, edit the mirrored `postEditFiles`, which auto-sync to the site DB — then refresh/screenshot only if the developer asks, which is when the browser gets launched). Default page = `index` when omitted. All logic stays in the DB — the "never modify core" rule above is in **full force**.
...
- **⚠️ Alias resolution is literal, not semantic — don't pattern-match on wording.** `{site}` in "work on {site} [page]" must be looked up against `postedit.xml`'s `alias` list first, exactly as typed. `postedit.xml` can have (and does have) an alias literally named `localhost` — a real PostEdit-mirrored site, distinct from the `wasql` local-framework trigger below even though both ultimately serve from `http://localhost`. "work on localhost" means that PostEdit alias (run `workon.php localhost` — watcher, mirror, DB-driven) and is NOT the same as "work on wasql" (no watcher, core repo files edited directly). Only the exact phrase **"work on wasql"** selects local framework mode; do not infer it just because "localhost" appears in the request.
...
2d. **Conversely, a field's FINAL `<?php` must BE closed** — `evalPHP` matches islands with `/\<\?(.+?)\?\>/sm`, so an unterminated block is never executed and gets **echoed as literal text** (no PHP error at all; the page shows your controller source and every controller variable is empty). Rewriting a whole `controller`/`functions` field? Keep the trailing close tag. `php -l` passes either way.
...
- **Querying a different DB connection** (postgres/hana/mssql/c-tree registered in `config.xml`) uses the **`db*` family** with the connection name first: `dbQueryResults('conn',$sql)`, `dbGetRecord('conn',$sql)`, `dbGetCount`, `dbListRecords`, `dbExecuteSQL`. ⚠️ A **failed** `db*` SELECT returns an **empty array**, not an error string — "0 rows" and "the query blew up" look identical, so check **`dbLastError()`** right after every read (`''` = genuinely no rows). → full detail in `wasql_reference.md`.
...
- **`workon.md`** — full mechanics of the `workon.php` one-command session-startup script: what each step does, gotchas, the permission-wildcard check, `--browse`/`--reshoot`/`--no-chrome`, and the manual fallback if the script fails.
