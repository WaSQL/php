"""
Installation
    LibPQ.jl package for PostgreSQL database connectivity
...
References
    https://invenia.github.io/LibPQ.jl/stable/
...
function get_connection_key(params::Dict)
    ...
    dbuser = get(params, "dbuser", "postgres")
    dbname = get(params, "dbname", "postgres")
    schema = get(params, "schema", "public")
...
function sanitize_error_message(err)
    ...
    elseif occursin("access denied", lowercase(err_str)) || occursin("authentication failed", lowercase(err_str))
        return "Access denied"
    elseif occursin("database", lowercase(err_str)) && occursin("does not exist", lowercase(err_str))
        return "Database not found"
    elseif occursin("could not connect", lowercase(err_str)) || occursin("connection refused", lowercase(err_str))
...
function connect(params::Dict)
    ...
    dbname = get(params, "dbname", "postgres")
    schema = get(params, "schema", "public")
    sslmode = get(params, "sslmode", "prefer")
    connect_timeout = get(params, "connect_timeout", "10")

    try
        # Build connection string
        connstr = "host=$dbhost port=$dbport user=$dbuser password=$dbpass dbname=$dbname sslmode=$sslmode connect_timeout=$connect_timeout"

        # Add schema if specified
        if schema != "public"
            connstr *= " options='-c search_path=$schema'"
        end
        ...
        if conn === nothing
            ...
            @error "PostgreSQL Connection Error after $MAX_RETRIES attempts" error=string(last_error) sanitized=sanitized_error
            return nothing
        end
        ...
        @error "PostgreSQL Connection Error" error=string(err) sanitized=sanitized_error
        return nothing
    end
end
...
function executeSQL(query::String, params::Dict=Dict())
    ...
    # Warn if query looks like it might contain user input
    if occursin(r"['\"].*\$", query)
...
function executePS(query::String, args::Vector, params::Dict=Dict())
    ...
    # Validate that number of placeholders roughly matches number of args
    # PostgreSQL uses $1, $2, etc. syntax
    max_placeholder = 0
    ...
    if max_placeholder > 0 && max_placeholder != length(args)
        error_msg = "Parameter mismatch: query expects $max_placeholder parameters but $(length(args)) arguments provided"
        @error error_msg
        return error_msg
    end
