/* Reference: https://unpkg.com/flatpickr@1.6.5/index.html */
...
flatpickr.init = function (element, instanceConfig) {
    ...
    init = function () {
        ...
        if(self.config.defaultDate){
            if(self.config.defaultDate.toLowerCase() == 'today'){
                self.config.defaultDate=uDate(new Date(), true);
            }
            else if(!isNaN(parseFloat(self.config.defaultDate)) && isFinite(self.config.defaultDate)){
                let d = new Date();
                let n=parseInt(self.config.defaultDate);
                if(n < 0){
                    n=Math.abs(n);
                    d.setDate(d.getDate() - n);
                    self.config.defaultDate = uDate(d, true);
                }
                else{
                    d.setDate(d.getDate() + n);
                    self.config.defaultDate = uDate(d, true);
                ...
                self.config.defaultDate = uDate(self.config.defaultDate, true);
            }
        }
        ...
        if ( self.element.value || self.config.defaultDate )
            self.selectedDateObj = uDate(self.element.value||self.config.defaultDate);

        //minDate
        if(self.config.minDate){
            if(self.config.minDate.toLowerCase() == 'today'){
                self.config.minDate=uDate(new Date(), true);
            }
            else if(!isNaN(parseFloat(self.config.minDate)) && isFinite(self.config.minDate)){
                let d = new Date();
                let n=parseInt(self.config.minDate);
                if(n < 0){
                    n=Math.abs(n);
                    d.setDate(d.getDate() - n);
                    self.config.minDate = uDate(d, true);
                }
                else{
                    d.setDate(d.getDate() + n);
                    self.config.minDate = uDate(d, true);
        ...
        if(self.config.maxDate){
            if(self.config.maxDate.toLowerCase() == 'today'){
                self.config.maxDate=uDate(new Date(), true);
            }
            else if(!isNaN(parseFloat(self.config.maxDate)) && isFinite(self.config.maxDate)){
                let d = new Date();
                let n=parseInt(self.config.maxDate);
                if(n < 0){
                    n=Math.abs(n);
                    d.setDate(d.getDate() - n);
                    self.config.maxDate = uDate(d, true);
                }
                else{
                    d.setDate(d.getDate() + n);
                    self.config.maxDate = uDate(d, true);
    ...
    isDisabled = function(date){
        //look for disableweekends
        if(self.config.disableWeekends){
    ...
    bind = function () {
        ...
        if ( self.config.enableTime ){
            ...
            hourElement.addEventListener("mousewheel", timeWrapper, {passive:false});
            hourElement.addEventListener("DOMMouseScroll", timeWrapper, {passive:false});
            minuteElement.addEventListener("mousewheel", timeWrapper, {passive:false});
            minuteElement.addEventListener("DOMMouseScroll", timeWrapper, {passive:false});

            hourElement.addEventListener("mouseout", updateValue, {passive:false});
            minuteElement.addEventListener("mouseout", updateValue, {passive:false});
            ...
            am_pm.addEventListener("mousewheel", am_pm_toggle, {passive:false});
            am_pm.addEventListener("DOMMouseScroll", am_pm_toggle, {passive:false});
            am_pm.addEventListener("mouseout", updateValue, {passive:false});
