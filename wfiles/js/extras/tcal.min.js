...
function f_tcalParseDate(s_date,s_format){if(!s_date)return;var s_char,s_regexp='^',a_tokens={},a_options,n_token=0;for(var n=0;n<s_format.length;n++){s_char=s_format.charAt(n);if(A_TCALTOKENS_IDX[s_char]){a_tokens[s_char]=++n_token;s_regexp+='('+A_TCALTOKENS_IDX[s_char].r+')'}else if(s_char==' ')
s_regexp+='\\s';else s_regexp+=(s_char.match(/[\w\d]/)?'':'\\')+s_char}
...
return!1;var a_classes=s_classes.split(' ');for(var n=0;n<a_classes.length;n++)
...
return!1;var a_classes=s_classes.split(' '),a_newClasses=[];for(var n=0;n<a_classes.length;n++){if(a_classes[n]==s_class)
continue;a_newClasses[a_newClasses.length]=a_classes[n]}
e_elem.className=a_newClasses.join(' ');return!0}
...
function f_tcalAddOnload(f_func){if(document.addEventListener){window.addEventListener('load',f_func,!1)}else if(window.attachEvent){window.attachEvent('onload',f_func)}else{var f_onLoad=window.onload;if(typeof window.onload!='function'){window.onload=f_func}else{window.onload=function(){f_onLoad();f_func()}}}}
