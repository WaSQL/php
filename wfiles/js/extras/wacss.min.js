var wacss={version:'2024.0124',author:'WaSQL.com',chartjs:{},EOL:'\n',CRLF:'\r\n',processing:'<span class="icon-spin4 w_spin"></span>',hoverDiv:'',loadJsText:function(name,body){let code='this.f = function '+name+'() {'+body+'}';eval(code);return!0},addClass:function(element,classToAdd){element=wacss.getObject(element);if(undefined==element){return!1}
...
xmlhttp.open("GET",aurl.toString(),!0);xmlhttp.send();return!1},ajaxPost:function(frm,div){let xmlhttp=new XMLHttpRequest();let url=frm.getAttribute('action');xmlhttp.div=div;xmlhttp.onreadystatechange=function(){if(this.readyState==XMLHttpRequest.DONE){let div=document.getElementById(this.div);if(this.status==200||this.status==201){div.innerHTML=this.responseText}else{div.innerHTML=JSON.stringify(this)}}};let data=new FormData(frm);xmlhttp.open("POST",url,!0);xmlhttp.send(data);return!1},ajaxUniqueID:function(){return"10000000000000000000".replace(/[018]/g,c=>(c^crypto.getRandomValues(new Uint8Array(1))[0]&15>>c/4).toString(16))},blink:function(el){el=wacss.getObject(el);if(undefined==el){return}
...
return tag},centerObject:function(obj){let sObj=wacss.getObject(obj);if(undefined==sObj){return!1}
...
if(y<10){y=10}
sObj.style.top=y+'px';return new Array(x,y)},checkAllElements:function(att,val,ck){let list;switch(att.toLowerCase()){case 'class':list=document.querySelectorAll('input[type="checkbox"].'+val);break;case 'id':list=document.querySelectorAll('input[type="checkbox"]#'+val);break;default:list=document.querySelectorAll('input[type="checkbox"]['+att+'="'+val+'"]');break}
...
return!1},chartjsDrawTotals:function(chart){let width=chart.chart.width,height=chart.chart.height,ctx=chart.chart.ctx;ctx.restore();let fontSize=(height/60).toFixed(2);ctx.font=fontSize+"em sans-serif";ctx.textBaseline="middle";let text=chart.config.centerText.text,textX=Math.round((width-ctx.measureText(text).width)/2),textY=height-20;ctx.fillText(text,textX,textY);ctx.save()},hexToRgb:function(hex){let result=/^#?([a-f\d]{2})([a-f\d]{2})([a-f\d]{2})$/i.exec(hex);return"rgb("+parseInt(result[1],16)+","+parseInt(result[2],16)+","+parseInt(result[3],16)+")"},initChartJsBehavior:function(chartid){let list=document.querySelectorAll('[data-behavior="chartjs"]');if(undefined!=chartid){list=document.querySelectorAll('#'+chartid)}
...
let shorthandRegex=/^#?([a-f\d])([a-f\d])([a-f\d])$/i;hex=hex.replace(shorthandRegex,function(m,r,g,b){return r+r+g+g+b+b});let result=/^#?([a-f\d]{2})([a-f\d]{2})([a-f\d]{2})$/i.exec(hex);return result?{r:parseInt(result[1],16),g:parseInt(result[2],16),b:parseInt(result[3],16)}:{r:255,g:255,b:255}},implode:function(glue,pieces){let i='',retVal='',tGlue='';if(arguments.length===1){pieces=glue;glue=''}
...
return retVal}}else{return pieces}},initCodeMirror:function(){let list=document.querySelectorAll('textarea.code[data-mode]');if(undefined==list||list.length==0){return!1}
...
if(obj.className.indexOf(myclass1)!=-1){wacss.removeClass(obj,myclass1);wacss.addClass(obj,myclass2)}else if(obj.className.indexOf(myclass2)!=-1){wacss.removeClass(obj,myclass2);wacss.addClass(obj,myclass1)}else{wacss.addClass(obj,myclass1)}}},initHovers:function(){let hoverEls=document.querySelectorAll('[data-hover]');if(hoverEls.length>0){if(wacss.hoverDiv==''){wacss.hoverDiv=document.createElement('div');let d=document.createElement('div');d.className='hover_content';d.innerHTML='wacss hover default text';wacss.hoverDiv.appendChild(d);wacss.hoverDiv.className='wacss_hover';wacss.hoverDiv.dataset.position='above';document.body.appendChild(wacss.hoverDiv);wacss.hoverDiv.addEventListener('mouseout',function(){if(undefined!=this.hoverParent&&this.hoverParent.matches(':hover')){return!1}
