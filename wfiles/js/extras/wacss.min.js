...
return!1},init:function(){wacss.initWacssEdit();wacss.initChartJs();wacss.initCodeMirror()},chartjsDrawTotals:function(chart){var width=chart.chart.width,height=chart.chart.height,ctx=chart.chart.ctx;ctx.restore();var fontSize=(height/60).toFixed(2);ctx.font=fontSize+"em sans-serif";ctx.textBaseline="middle";var text=chart.config.centerText.text,textX=Math.round((width-ctx.measureText(text).width)/2),textY=height-20;ctx.fillText(text,textX,textY);ctx.save()},initChartJs:function(initid){let list=document.querySelectorAll('div.chartjs');let gcolors=new Array('rgb(255, 159, 64)','rgb(75, 192, 192)','rgb(255, 99, 132)','rgb(54, 162, 235)','rgb(153, 102, 255)','rgb((218,165,32)','rgb(233,150,122)','rgb(189,183,107)','rgb(154,205,50)','rgb(255,228,196)','rgb(244,164,96)','rgb(176,196,222)','rgb(188,143,143)','rgb(255,228,225)','rgb(201, 203, 207)');for(let i=0;i<list.length;i++){if(undefined==list[i].id){console.log('missing id',list[i]);continue}
...
break}}
return!0},initCodeMirror:function(){let list=document.querySelectorAll('textarea.code[data-mode]');if(undefined==list||list.length==0){return!1}
let defaults={mode:'text/x-sql',indentUnit:4,indentWithTabs:!0,smartIndent:!0,lineNumbers:!0,lineWrapping:!1,matchBrackets:!0,autofocus:!0,extraKeys:{"Ctrl-Space":"autocomplete"}};if(undefined==this.codemirror){this.codemirror={}}
for(let i=0;i<list.length;i++){if(undefined!=list[i].dataset.initialized){continue}
list[i].dataset.initialized=1;let params={};for(k in list[i].dataset){if(k=='debug'||k=='initialized'){continue}
let v=list[i].dataset[k];if(typeof v==='string'||v instanceof String){switch(v){case 'true':v=!0;break;case 'false':v=!1;break}}
params[k]=v}
switch(params.mode.toLowerCase()){case 'css':params.mode='text/css';break;case 'html':params.mode='text/html';defaults.htmlMode=!0;break;case 'javascript':params.mode='text/javascript';defaults.continueComments='Enter';defaults.extraKeys["Ctrl-Q"]='toggleComment';break;case 'json':params.mode='application/x-json';defaults.autoCloseBrackets=!0;defaults.matchBrackets=!0;defaults.lineWrapping=!0;break;case 'lua':params.mode='text/x-lua';break;case 'perl':params.mode='text/x-perl';break;case 'php':params.mode='application/x-httpd-php';break;case 'python':params.mode={name:'python',version:3,singleLineStringErrors:!1};break;case 'sql':params.mode='text/x-sql';break;case 'vbscript':params.mode='text/vbscript';break;case 'xml':params.mode='application/xml';break}
...
let cm=CodeMirror.fromTextArea(list[i],params);cm.on('change',function(cm){cm.save()})}},initWacssEdit:function(){let list=document.querySelectorAll('textarea.wacssedit');for(let i=0;i<list.length;i++){if(undefined==list[i].id){continue}
