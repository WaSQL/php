...
if(undefined!=datasets[d].dataset.bordercolor){dataset.borderColor=datasets[d].dataset.bordercolor}else if(undefined!=datasets[d].dataset.borderColor){dataset.borderColor=datasets[d].dataset.borderColor}
...
for(let i=0;i<hoverEls.length;i++){hoverEls[i].addEventListener('mouseover',function(){let txt='';wacss.hoverDiv.style.display='initial';if(hoverEls[i].dataset.hover.indexOf('id:')===0){console.log("hover id");let txtid=trim(str_replace('id:','',hoverEls[i].dataset.hover));let txtel=document.querySelector('#'+txtid);if(undefined!=txtel){txt=txtel.innerHTML}}else if(hoverEls[i].dataset.hover.indexOf('js:')===0){console.log("hover js");let f=trim(str_replace('js:','',hoverEls[i].dataset.hover));let jsfunc=new Function(f);txt=jsfunc()}else{txt=hoverEls[i].dataset.hover}
wacss.hoverDiv.style.width='initial';wacss.hoverDiv.style.height='initial';wacss.hoverDiv.querySelector('.hover_content').innerHTML=txt;wacss.hoverDiv.hoverParent=this;let drect=this.getBoundingClientRect();let hrect=wacss.hoverDiv.getBoundingClientRect();console.log(drect);console.log(hrect);let vw=Math.max(document.documentElement.clientWidth||0,window.innerWidth||0);let vh=Math.max(document.documentElement.clientHeight||0,window.innerHeight||0);switch(this.dataset.position.toLowerCase()){case 'above':wacss.hoverDiv.dataset.position='above';wacss.hoverDiv.style.top=parseInt(drect.top-hrect.height-2)+'px';wacss.hoverDiv.style.left=parseInt(drect.left)+'px';if(!wacss.inViewport(wacss.hoverDiv)){console.log('not in viewport')
wacss.hoverDiv.dataset.position='below';wacss.hoverDiv.style.top=parseInt(drect.top+drect.height+2)+'px';wacss.hoverDiv.style.left=parseInt(drect.left)+'px'}
break;case 'below':default:wacss.hoverDiv.dataset.position='below';wacss.hoverDiv.style.top=parseInt(drect.top+drect.height+2)+'px';wacss.hoverDiv.style.left=parseInt(drect.left)+'px';if(!wacss.inViewport(wacss.hoverDiv)){console.log('not in viewport')
wacss.hoverDiv.dataset.position='above';wacss.hoverDiv.style.top=parseInt(drect.top-hrect.height-2)+'px';wacss.hoverDiv.style.left=parseInt(drect.left)+'px'}
break;case 'right':wacss.hoverDiv.dataset.position='right';wacss.hoverDiv.style.top=parseInt(drect.top+2)+'px';wacss.hoverDiv.style.left=parseInt(drect.left+drect.width+2)+'px';if(!wacss.inViewport(wacss.hoverDiv)){console.log('not in viewport');wacss.hoverDiv.style.width=parseInt(vw-drect.right-5)+'px';wacss.hoverDiv.style.left=parseInt(drect.right)+'px'}
