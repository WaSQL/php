...
return!0},ajaxGet:function(url,div,params){let xmlhttp=new XMLHttpRequest();xmlhttp.recenter='';xmlhttp.div=div;if(typeof(div)==='string'){switch(div.toLowerCase()){case 'centerpop':case 'wacss_centerpop':xmlhttp.div=wacss.getObject('wacss_centerpop');if(undefined==xmlhttp.div){xmlhttp.recenter='wacss_centerpop'}
xmlhttp.div=wacss.createCenterpop(params.title);break;case 'centerpop1':case 'wacss_centerpop1':xmlhttp.div=wacss.getObject('wacss_centerpop1');if(undefined==xmlhttp.div){xmlhttp.recenter='wacss_centerpop1'}
xmlhttp.div=wacss.createCenterpop(params.title,1);break;case 'centerpop2':case 'wacss_centerpop2':xmlhttp.div=wacss.getObject('wacss_centerpop2');if(undefined==xmlhttp.div){xmlhttp.recenter='wacss_centerpop2'}
xmlhttp.div=wacss.createCenterpop(params.title,2);break;case 'centerpop3':case 'wacss_centerpop3':xmlhttp.div=wacss.getObject('wacss_centerpop3');if(undefined==xmlhttp.div){xmlhttp.recenter='wacss_centerpop2'}
xmlhttp.div=wacss.createCenterpop(params.title,3);break}}
...
let cp=wacss.getObject('wacss_centerpop'+x);if(undefined!=cp){return cp.querySelector('div.wacss_centerpop_content')}
cp=document.createElement('div');cp.id='wacss_centerpop'+x;cp.className='wacss_centerpop';cp.setAttribute('data-movable','true');let cpt=document.createElement('div');cpt.className='wacss_centerpop_title';let cpt_text=document.createElement('div');cpt_text.style.flex=1;cpt_text.innerHTML=title;cpt.appendChild(cpt_text);let cpt_processing=document.createElement('div');cpt_processing.id=cp.id+'_processing';cpt_processing.className='wacss_centerpop_processing';cpt_processing.innerText='*';cpt.appendChild(cpt_processing);let cpt_close=document.createElement('div');cpt_close.className='icon-close wacss_centerpop_close';cpt_close.title='Close';cpt_close.closeid=cp.id;cpt_close.onclick=function(){wacss.removeId(this.closeid)}
cpt.appendChild(cpt_close);let cpc=document.createElement('div');cpc.id='centerpop'+x+'_content';cpc.className='wacss_centerpop_content';cpc.innerHTML='<div class="align-center">......</div>';cp.appendChild(cpt);cp.appendChild(cpc);document.body.appendChild(cp);wacss.centerObject(cp);wacss.makeMovable(cp,cpt);return cpc},dismiss:function(el){if(el.parentElement.querySelector(':hover')==el){let wtimer=parseInt(el.timer)*3;setTimeout(function(){wacss.dismiss(el)},wtimer);return}
