...
let nav=opts.nav||elobj.dataset.nav||pli.dataset.nav||pul.dataset.nav||ptd.dataset.nav||ptr.dataset.nav||elobj.getAttribute('href');if(nav=='id'&&undefined!=elobj.dataset.parent){let target=wacss.getObject(div);let ptarget=wacss.getObject(elobj.dataset.parent);let childs=ptarget.querySelectorAll('div[id]');for(let c=0;c<childs.length;c++){childs[c].style.display='none'}
