...
let lis=p.nextSibling.querySelectorAll('ul li');for(let x=0;x<lis.length;x++){lis[x].className='wacssform_many'}}},initWhiteboard:function(){let list=document.querySelectorAll('textarea[data-behavior="whiteboard"]');for(let i=0;i<list.length;i++){let wrapper=document.createElement('div');wrapper.style.width=list[i].style.width;wrapper.style.height=list[i].style.height;wrapper.style.display='block';wrapper.style.position='relative';wrapper.style.boxShadow='rgba(0, 0, 0, 0.16) 0px 3px 6px, rgba(0, 0, 0, 0.23) 0px 3px 6px';wrapper.id='whiteboard_wrapper_1';wrapper.style.backgroundColor=list[i].dataset.fill||'#fff';wrapper.shapes=new Array();wrapper.drawShapes=function(){this.canvas.ctx.clearRect(0,0,this.canvas.width,this.canvas.height);this.canvas.txtarea.innerText='';let dshapes=this.shapes;dshapes.push(this.canvas.shape);for(let i=0;i<dshapes.length;i++){let shape=dshapes[i];switch(shape.shape){case 'circle':this.canvas.ctx.beginPath();this.canvas.ctx.arc(shape.x,shape.y,shape.radius,0,Math.PI*2);if(shape.fillcolor.length){this.canvas.ctx.fillStyle=shape.fillcolor;this.canvas.ctx.lineWidth=shape.size;this.canvas.ctx.fill()}else{this.canvas.ctx.lineWidth=shape.size;this.canvas.ctx.stroke()}
...
dshapes=[]}
list[i].parentNode.insertBefore(wrapper,list[i].nextSibling);let wcanvas=document.createElement('canvas');let toolbar=document.createElement('div');let params={};wrapper.canvas=wcanvas;toolbar.canvas=wcanvas;toolbar.wrapper=wrapper;toolbar.style='display:flex;justify-content:flex-end;align-items:center;width:100%;background:#f0f0f0;height:34px;box-shadow: rgba(17, 17, 26, 0.05) 0px 1px 0px, rgba(17, 17, 26, 0.1) 0px 0px 8px;';wrapper.dataset.shape='pencil';let shapes={'pencil':'Pencil','line':'Line','circle':'Circle','rectangle':'Rectangle',};params={style:'margin-left:10px;width:100px;padding:3px;'}
...
this.parentNode.drawShapes();if(this.isDown==!0&&typeof this.toDataURL==='function'){this.txtarea.innerText=this.toDataURL('image/png')}};wrapper.onmouseout=function(e){e=e||window.event;this.canvas.isDown=!1;switch(this.parentNode.dataset.shape){default:break;case 'line':break;case 'circle':break;case 'rectangle':break}
if(this.canvas.isDown==!0&&typeof this.canvas.toDataURL==='function'){this.canvas.txtarea.innerText=this.canvas.toDataURL('image/png')}}
document.body.addEventListener('touchmove',function(e){e=e||window.event;e.preventDefault()},!1);wrapper.appendChild(wcanvas);wrapper.appendChild(toolbar);list[i].style.display='none'}},isNum:function(n){return!isNaN(parseFloat(n))&&isFinite(n)},listen:function(evnt,elem,func){if(elem.addEventListener){elem.addEventListener(evnt,func,!1)}else if(elem.attachEvent){let r=elem.attachEvent("on"+evnt,func);return r}else{console.log('wacss.listen failed. Browser does not support event listeners')}},loadCSS:function(file,notify){let link=document.createElement('link');if(undefined!=notify&&notify==1){link.onload=function(){wacss.toast(this.getAttribute('href')+' loaded successfully')}}
