...
params.txt=txt;if('speechSynthesis' in window){if(undefined==params.multiple){if(undefined!=params.debug){console.log('canceling any speach already playing')}
window.speechSynthesis.cancel()}
let voices=window.speechSynthesis.getVoices();if(undefined!=params.debug||undefined!=params.showvoices){console.log('voices');console.log(voices)}
if(voices.length>0){var msg=new SpeechSynthesisUtterance();if(undefined!=params.name){for(let i=0;i<voices.length;i++){if(voices[i].name.toLowerCase().indexOf(params.name.toLowerCase())!=-1){msg.voice=voices[i];break}}}
...
if(undefined!=params.onstart){msg.func_start=params.onstart;msg.addEventListener('start',function(event){window[this.func_start](event.utterance)})}
if(params.txt.indexOf('<')!=-1){console.log('ssml');msg.input={ssml:params.txt}}else{msg.text=params.txt}
if(undefined!=params.debug){console.log(msg)}
window.utterances.push(msg);window.speechSynthesis.speak(msg);return!1}else{window.speechSynthesis.params=params;window.speechSynthesis.onvoiceschanged=function(){let params=window.speechSynthesis.params;window.utterances=[];wacss.speak(params.txt,params)}}}else{console.log('wacss.speak error: speechSynthesis is not supported in your browser or OS')}
