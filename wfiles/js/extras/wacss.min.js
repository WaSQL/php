...
return!1},init:function(){wacss.initWacssEdit();wacss.initChartJs();wacss.initCodeMirror();wacss.initEditor()},chartjsDrawTotals:function(chart){var width=chart.chart.width,height=chart.chart.height,ctx=chart.chart.ctx;ctx.restore();var fontSize=(height/60).toFixed(2);ctx.font=fontSize+"em sans-serif";ctx.textBaseline="middle";var text=chart.config.centerText.text,textX=Math.round((width-ctx.measureText(text).width)/2),textY=height-20;ctx.fillText(text,textX,textY);ctx.save()},initChartJs:function(initid){let list=document.querySelectorAll('div.chartjs');let gcolors=new Array('rgb(255, 159, 64)','rgb(75, 192, 192)','rgb(255, 99, 132)','rgb(54, 162, 235)','rgb(153, 102, 255)','rgb((218,165,32)','rgb(233,150,122)','rgb(189,183,107)','rgb(154,205,50)','rgb(255,228,196)','rgb(244,164,96)','rgb(176,196,222)','rgb(188,143,143)','rgb(255,228,225)','rgb(201, 203, 207)');for(let i=0;i<list.length;i++){if(undefined==list[i].id){console.log('missing id',list[i]);continue}
...
els[e].setAttribute('data-initialized',1);let mh=els[e].style.minHeight||'200px';els[e].style.display='none';let editor=document.createElement('div');editor.setAttribute('contenteditable','true');editor.style.minHeight=mh;editor.saveto=els[e];els[e].editor=editor;editor.onkeydown=function(e){if(e.keyCode==9){e.preventDefault();let sel=document.getSelection();let range=sel.getRangeAt(0);let tabNodeValue='\u0009'
let tabNode=document.createTextNode(tabNodeValue);range.insertNode(tabNode);range.setStartAfter(tabNode);range.setEndAfter(tabNode);return!1}}
editor.addEventListener("input",function(ie){this.saveto.textContent=this.textContent});editor.save=function(){this.saveto.textContent=this.textContent};editor.textContent=els[e].textContent;els[e].parentNode.insertBefore(editor,els[e].nextSibling)}},initWacssEdit:function(){let list=document.querySelectorAll('textarea.wacssedit');for(let i=0;i<list.length;i++){if(undefined==list[i].id){continue}
