...
let div=opts.div||elobj.dataset.div||pli.dataset.div||ptd.dataset.div||ptr.dataset.div||'main_content';let has_confirm=opts.confirm||elobj.dataset.confirm||pli.dataset.confirm||ptd.dataset.confirm||ptr.dataset.confirm;if(undefined!=has_confirm&&has_confirm.length>0){let txt=has_confirm.replace(/\[newline\]/g,"\n");if(txt.indexOf('id:')==0){let cid=txt.replace('id:','');let cidobj=document.querySelector('#'+cid);if(undefined!=cidobj){txt=cidobj.innerText}}
...
let params={};let checkbox=elobj.dataset.checkbox||pli.dataset.checkbox||ptd.dataset.checkbox||ptr.dataset.checkbox;if(undefined!=checkbox){let elsearch='input[type="checkbox"][name="'+checkbox+'[]"]:checked';let checkboxes=document.querySelectorAll(elsearch);params.checkboxes=new Array();for(let c=0;c<checkboxes.length;c++){params.checkboxes.push(checkboxes[c].value)}}
let frm=elobj.dataset.form||pli.dataset.form||ptd.dataset.form||ptr.dataset.form;if(undefined!=frm){frm=wacss.getObject(frm);if(undefined!=frm){let els=frm.querySelectorAll('input[name],select[name]');for(let i=0;i<els.length;i++){params[els[i].name]=els[i].value}}}
...
let nav=opts.nav||elobj.dataset.nav||pli.dataset.nav||ptd.dataset.nav||ptr.dataset.nav||elobj.getAttribute('href');let sp=opts.sp||elobj.dataset.sp||pli.dataset.sp||ptd.dataset.sp||ptr.dataset.sp||document.querySelector('#setprocessing');if(undefined!=sp){params.setprocessing=sp}
let pmpt=opts.prompt||elobj.dataset.prompt||pli.dataset.prompt||ptd.dataset.prompt||ptr.dataset.prompt||'';if(undefined!=pmpt&&pmpt.length>0){params.prompt=prompt(pmpt);if(undefined==params.prompt||params.prompt.length==0){return!1}}
let title=opts.title||elobj.dataset.title||pli.dataset.title||ptd.dataset.title||ptr.dataset.title;if(undefined!=title){params.title=title;params.cp_title=title}
