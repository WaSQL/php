...
let frm=elobj.dataset.form||pli.dataset.form||pul.dataset.form||ptd.dataset.form||ptr.dataset.form;if(undefined!=frm){frm=wacss.getObject(frm);if(undefined!=frm){let els=frm.querySelectorAll('input[name],select[name]');for(let i=0;i<els.length;i++){params[els[i].name]=els[i].value}}}
...
let nav=opts.nav||elobj.dataset.nav||pli.dataset.nav||pul.dataset.nav||ptd.dataset.nav||ptr.dataset.nav||elobj.getAttribute('href');if(nav=='id'&&undefined!=elobj.dataset.parent){console.log('debug_1');let target=wacss.getObject(div);let ptarget=wacss.getObject(elobj.dataset.parent);let childs=ptarget.querySelectorAll('[id]');for(let c=0;c<childs.length;c++){console.log(childs[c]);childs[c].style.display='none'}
target.style.display='block';return!1}
