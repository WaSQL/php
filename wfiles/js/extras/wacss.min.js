...
return!1},init:function(){wacss.initWacssEdit();wacss.initChartJs();wacss.initTruncate()},initChartJs:function(initid){let list=document.querySelectorAll('div.chartjs');let colors=new Array('rgb(255, 159, 64)','rgb(75, 192, 192)','rgb(255, 99, 132)','rgb(54, 162, 235)','rgb(153, 102, 255)','rgb(201, 203, 207)');for(let i=0;i<list.length;i++){if(undefined==list[i].id){console.log('missing id',list[i]);continue}
...
break;case 'line':case 'bar':if(undefined!=wacss.chartjs[list[i].id]){let ck=list[i].querySelector('canvas');if(undefined!=ck){let udatasets=datadiv.querySelectorAll('dataset');for(let ud=0;ud<udatasets.length;ud++){if(undefined==udatasets[ud].getAttribute('data-label')){continue}
...
let xkey=list[i].getAttribute('data-x');let ykey=list[i].getAttribute('data-y');let lconfig={type:type,data:{labels:[],datasets:[]}};lconfig.options={responsive:!0,scales:{xAxes:[{type:'time',distribution:'series',ticks:{source:'data',autoSkip:!0}}],yAxes:[{scaleLabel:{display:!0,labelString:list[i].getAttribute('data-ylabel')}}]},tooltips:{intersect:!1,mode:'index',callbacks:{label:function(tooltipItem,myData){var label=myData.datasets[tooltipItem.datasetIndex].label||'';if(label){label+=': '}
label+=parseFloat(tooltipItem.value).toFixed(2);return label}}},plugins:{datalabels:{display:!1,formatter:Math.round}}};if(undefined!=list[i].getAttribute('data-ysteps')){if(undefined==lconfig.options.scales.yAxes.ticks){lconfig.options.scales.yAxes.ticks={}}
lconfig.options.scales.yAxes.ticks.steps=parseInt(list[i].getAttribute('data-ysteps'))}
if(undefined!=list[i].getAttribute('data-ystepvalue')){if(undefined==lconfig.options.scales.yAxes.ticks){lconfig.options.scales.yAxes.ticks={}}
lconfig.options.scales.yAxes.ticks.stepValue=parseInt(list[i].getAttribute('data-ystepvalue'))}
if(undefined!=list[i].getAttribute('data-ystepsize')){if(undefined==lconfig.options.scales.yAxes.ticks){lconfig.options.scales.yAxes.ticks={}}
lconfig.options.scales.yAxes.ticks.stepSize=parseInt(list[i].getAttribute('data-ystepsize'))}
if(undefined!=list[i].getAttribute('data-ymin')){if(undefined==lconfig.options.scales.yAxes.ticks){lconfig.options.scales.yAxes.ticks={}}
lconfig.options.scales.yAxes.ticks.min=parseInt(list[i].getAttribute('data-ymin'))}
if(undefined!=list[i].getAttribute('data-ymax')){if(undefined==lconfig.options.scales.yAxes.ticks){lconfig.options.scales.yAxes.ticks={}}
lconfig.options.scales.yAxes.ticks.max=parseInt(list[i].getAttribute('data-ymax'))}
if(undefined!=list[i].getAttribute('data-ybeginatzero')){if(undefined==lconfig.options.scales.yAxes.ticks){lconfig.options.scales.yAxes.ticks={}}
lconfig.options.scales.yAxes.ticks.beginAtZero=!0}
console.log(lconfig.options);let labels=[];let datasets=datadiv.querySelectorAll('dataset');for(let d=0;d<datasets.length;d++){if(undefined==datasets[d].getAttribute('data-label')){continue}
