var wacss={version:'2022.0501',author:'WaSQL.com',chartjs:{},loadJsText:function(name,body){console.log(name);console.log(body);var code='this.f = function '+name+'() {'+body+'}';eval(code);return!0},addClass:function(element,classToAdd){element=wacss.getObject(element);if(undefined==element){return!1}
...
return!1},init:function(){wacss.initWacssEdit();wacss.initChartJs();wacss.initTabs();wacss.initCodeMirror();wacss.initEditor();wacss.initWhiteboard();wacss.initSignaturePad()},chartjsDrawTotals:function(chart){let width=chart.chart.width,height=chart.chart.height,ctx=chart.chart.ctx;ctx.restore();let fontSize=(height/60).toFixed(2);ctx.font=fontSize+"em sans-serif";ctx.textBaseline="middle";let text=chart.config.centerText.text,textX=Math.round((width-ctx.measureText(text).width)/2),textY=height-20;ctx.fillText(text,textX,textY);ctx.save()},initChartJsBehavior:function(chartid){let list=document.querySelectorAll('[data-behavior="chartjs"]');if(undefined!=chartid){list=document.querySelectorAll('#'+chartid)}
...
let cm=CodeMirror.fromTextArea(list[i],params);list[i].codemirror=cm;cm.on('change',function(cm){cm.save()})}},callFunc:function(params){if(undefined==params){return!1}
