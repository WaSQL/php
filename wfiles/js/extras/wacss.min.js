...
return!0},ajaxGet:function(url,div,params){let xmlhttp=new XMLHttpRequest();xmlhttp.recenter='';if(typeof(div)==='string'){switch(div.toLowerCase()){case 'centerpop':case 'wacss_centerpop':xmlhttp.div=wacss.getObject('wacss_centerpop');if(undefined==xmlhttp.div){xmlhttp.recenter='wacss_centerpop'}
...
xmlhttp.onreadystatechange=function(){if(this.readyState==XMLHttpRequest.DONE){if(this.status==200||this.status==201){if(undefined==this.div){console.log(this.responseText)}else{if(undefined!=this.processing){this.processing.innerHTML=this.processing.previous}
...
wacss.removeId('geolocationmap_popup');return!1},getObject:function(obj){if(typeof(obj)=='object'){return obj}else if(typeof(obj)=='string'){let qso=document.querySelector('#'+obj);if(typeof(qso)=='object'){return qso}
...
return!1},init:function(){wacss.initWacssEdit();wacss.initChartJs();wacss.initTabs();wacss.initCodeMirror();wacss.initEditor();wacss.initWhiteboard();wacss.initSignaturePad();wacss.initDrag();wacss.initHovers();return!1},initDrag:function(){let draggables=document.querySelectorAll('[draggable="true"]');for(let i=0;i<draggables.length;i++){if(undefined!=draggables[i].dataset.draggable_initialized){continue}
