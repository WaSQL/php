...
let defaults={mode:'text/x-sql',indentUnit:4,indentWithTabs:!0,smartIndent:!0,lineNumbers:!0,lineWrapping:!1,matchBrackets:!0,autofocus:!0,autoRefresh:!0,extraKeys:{"Ctrl-Space":"autocomplete"}};if(undefined==this.codemirror){this.codemirror={}}
...
let cm=CodeMirror.fromTextArea(list[i],params);list[i].codemirror=cm;cm.on('change',function(cm){cm.save()})}},initWacssEdit:function(){let list=document.querySelectorAll('textarea.wacssedit');for(let i=0;i<list.length;i++){if(undefined==list[i].id){continue}
