...
document.body.appendChild(d);centerObject(d);z=z-2;let v=document.createElement('div');v.style.zIndex=z;v.style.display='block';v.style.width='3000px';v.style.height='2000px';v.style.position='absolute';v.style.top='0px';v.style.left='0px';v.style.background='rgba(0,0,0,0.5)';v.id=d.id+'_overlay';v.setAttribute('data-target',d.id);v.onclick=function(){removeDiv(this.getAttribute('data-target'));removeDiv(this.id)};document.body.appendChild(v)},speak:function(txt,params){if(undefined==params){params={}}
params.txt=txt;if('speechSynthesis' in window){let voices=window.speechSynthesis.getVoices();if(undefined!=params.debug){console.log(voices)}
