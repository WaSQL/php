var wacss={version:'2024.0124',author:'WaSQL.com',chartjs:{},EOL:'\n',CRLF:'\r\n',processing:'<span class="icon-spin4 w_spin"></span>',hoverDiv:'',loadJsText:function(name,body){var code='this.f = function '+name+'() {'+body+'}';eval(code);return!0},addClass:function(element,classToAdd){element=wacss.getObject(element);if(undefined==element){return!1}
...
list[i].checked=ck}
return!1},checkMouseLeave:function(element,evt){if(element.contains&&undefined!=evt.toElement){return!element.contains(evt.toElement)}else if(evt.relatedTarget){return!containsDOM(element,evt.relatedTarget)}},copy2Clipboard:function(str,msg){if(undefined==msg){msg='Copy Successful'}
...
return rv},getViewportSize:function(w){w=w||window;if(w.innerWidth!=null)return{w:w.innerWidth,h:w.innerHeight};var d=w.document;if(document.compatMode=="CSS1Compat")
...
break;case 'select':for(var j=0;j<cidobj.options.length;j++){if(cidobj.options[j].selected){cidv=cidobj.options[cidobj.selectedIndex].value;cidvals.push(cidv)}}
...
params.txt=txt;if('speechSynthesis' in window){window.speechSynthesis.cancel();let voices=window.speechSynthesis.getVoices();if(voices.length==0){window.speechSynthesis.params=params;window.speechSynthesis.onvoiceschanged=function(){let params=window.speechSynthesis.params;wacss.speak(params.txt,params)}}else{var msg=new SpeechSynthesisUtterance();if(undefined!=params.name){for(let i=0;i<voices.length;i++){if(voices[i].name.toLowerCase().indexOf(params.name.toLowerCase())!=-1){msg.voice=voices[i];break}}}
