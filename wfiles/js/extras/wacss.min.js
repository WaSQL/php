...
return null},getParent:function(obj,name){if(undefined==obj){return null}
if(undefined!=name){var count=1;while(count<1000){if(undefined==obj.parentNode){return null}
...
var pobj=cObj.parentNode;if(typeof(cObj.parentNode)=="object"){return cObj.parentNode}else{return wacss.getParent(pobj)}},getSiblings:function(elem){var siblings=[];var sibling=elem.parentNode.firstChild;while(sibling){if(sibling.nodeType===1&&sibling!==elem){siblings.push(sibling)}
sibling=sibling.nextSibling}
return siblings},guid:function(){function _p8(s){var p=(Math.random().toString(16)+"000000000").substr(2,8);return s?"-"+p.substr(0,4)+"-"+p.substr(4,4):p}
...
list[i].setAttribute('data-initialized',1);let type=list[i].getAttribute('data-type').toLowerCase();let datadiv=wacss.getObject(list[i].id+'_data');let colorsdiv=datadiv.querySelector('colors');if(undefined!=colorsdiv){let colorsjson=wacss.trim(colorsdiv.innerText);colors=JSON.parse(colorsjson)}else{colors=gcolors}
...
let foundchart=0;switch(type){case 'guage':if(undefined!=wacss.chartjs[list[i].id]){let ck=list[i].querySelector('canvas');if(undefined!=ck){let gv=parseInt(datadiv.innerText);let gv1=parseInt(180*(gv/100));if(gv1>180){gv1=180}
...
let gcanvas=document.createElement('canvas');list[i].appendChild(gcanvas);let gctx=gcanvas.getContext('2d');wacss.chartjs[list[i].id]=new Chart(gctx,gconfig);if(undefined!=list[i].getAttribute('data-onclick')){gcanvas.parentobj=list[i];gcanvas.chartobj=wacss.chartjs[list[i].id];gcanvas.onclick_func=list[i].getAttribute('data-onclick');gcanvas.clicked=0;gcanvas.onclick=function(evt){if(this.clicked==0){this.clicked=1;this.timeout=setTimeout(function(obj){obj.clicked=0},250,this);let activePoints=this.chartobj.getElementsAtEventForMode(evt,'point',this.chartobj.options);if(activePoints.length>0){let firstPoint=activePoints[0];let params={};params.type=this.parentobj.getAttribute('data-type');params.label=this.chartobj.data.labels[firstPoint._index]||this.chartobj.data.datasets[firstPoint._datasetIndex].label;params.value=this.chartobj.data.datasets[firstPoint._datasetIndex].data[firstPoint._index];window[this.onclick_func](params)}}}}}
break;case 'line':case 'bar':case 'horizontalbar':case 'doughnut':if(undefined!=wacss.chartjs[list[i].id]){let ck=list[i].querySelector('canvas');if(undefined!=ck){if(labels.length>0){wacss.chartjs[list[i].id].config.data.labels=labels}
let udatasets=datadiv.querySelectorAll('dataset');let datasetLabels=new Array();for(let ud=0;ud<udatasets.length;ud++){let json=JSON.parse(udatasets[ud].innerText);let udataset={backgroundColor:udatasets[ud].getAttribute('data-backgroundColor')||colors[ud],type:udatasets[ud].getAttribute('data-type')||list[i].getAttribute('data-type'),data:json,fill:!1};if(undefined!=udatasets[ud].getAttribute('data-yaxis')){udataset.yAxisID=udatasets[ud].getAttribute('data-yaxis')}
...
wacss.chartjs[list[i].id].config.options=options;if(labels.length==0&&datasetLabels.length>0){wacss.chartjs[list[i].id].config.data.labels=datasetLabels}
...
return!1},setActiveTab:function(el){let p=wacss.getParent(el,'ul');if(p===null){return!1}
...
return!1},toast:function(msg,params){if(undefined==params){params={color:'w_red',timer:3}}
if(undefined==params.color){params.color=wacss.color()}
