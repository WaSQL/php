...
if(undefined!=sObj.dataset.position){if(sObj.dataset.position!='unset'){sObj.style.position=sObj.dataset.position}}else{sObj.style.position='fixed'}
let w=sObj.offsetWidth||sObj.innerWidth||getWidth(sObj)||100;let h=sObj.offsetHeight||sObj.innerHeight||getHeight(sObj)||100;let vp=wacss.getViewportSize();let x=Math.round((vp.w/2)-(w/2));let y=Math.round((vp.h/2)-(h/2));sObj.style.left=x+'px';if(undefined==y){y=10}
if(y<10){y=10}
...
modal.setAttribute('data-position','inherit');wacss.centerObject(modal);return modal},nav:function(el,opts){let elobj=wacss.getObject(el);if(undefined==elobj){return!1}
