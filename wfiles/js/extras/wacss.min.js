...
break;case 'line':case 'bar':case 'horizontalbar':if(undefined!=wacss.chartjs[list[i].id]){let ck=list[i].querySelector('canvas');if(undefined!=ck){let udatasets=datadiv.querySelectorAll('dataset');for(let ud=0;ud<udatasets.length;ud++){if(undefined==udatasets[ud].getAttribute('data-label')){continue}
...
if(foundchart==0){let lconfig={type:list[i].getAttribute('data-type'),data:{labels:[],datasets:[]}};let labelsdiv=datadiv.querySelector('labels');if(undefined!=labelsdiv){let labelsjson=wacss.trim(labelsdiv.innerText);lconfig.data.labels=JSON.parse(labelsjson)}
let optionsdiv=datadiv.querySelector('options');if(undefined!=optionsdiv){let optionsjson=wacss.trim(optionsdiv.innerText);lconfig.options=JSON.parse(optionsjson)}
let datasets=datadiv.querySelectorAll('dataset');for(let d=0;d<datasets.length;d++){if(undefined==datasets[d].getAttribute('data-label')){continue}
