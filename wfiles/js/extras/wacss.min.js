...
let aurl=new URL(url);let aparams=aurl.searchParams;for(k in params){if(undefined==typeof(params[k])||params[k]===null||params[k].length==0){continue}
aparams.append(k,params[k])}
xmlhttp.open("GET",aurl.toString(),!0);xmlhttp.send();return!1},ajaxPost:function(frm,div){let xmlhttp=new XMLHttpRequest();let url=frm.getAttribute('action');xmlhttp.div=div;xmlhttp.onreadystatechange=function(){if(this.readyState==XMLHttpRequest.DONE){let div=document.getElementById(this.div);if(this.status==200||this.status==201){div.innerHTML=this.responseText}else{div.innerHTML=JSON.stringify(this)}}};let data=new FormData(frm);xmlhttp.open("POST",url,!0);xmlhttp.send(data);return!1},blink:function(el){el=wacss.getObject(el);if(undefined==el){return}
