...
return!1},options)}
return!1},geoLocationMap:function(lat,long,params){if(undefined==params){params={}}
...
params.zoom=params.zoom||13;let myLatlng={lat:params.lat,lng:params.long};let map=new google.maps.Map(params.div,{center:myLatlng,mapTypeId:'roadmap',zoom:params.zoom,});let infoWindow=new google.maps.InfoWindow({content:params.displayname,position:myLatlng,});infoWindow.open(map);infoWindow.setContent('Current Location');map.params=params;map.addListener("click",(mapsMouseEvent)=>{let latlon=mapsMouseEvent.latLng.toJSON();let latlonval='['+latlon.lat+','+latlon.lng+']';infoWindow.close();infoWindow=new google.maps.InfoWindow({position:mapsMouseEvent.latLng,});let htm=JSON.stringify(mapsMouseEvent.latLng.toJSON(),null,2);htm=htm+'<div class="align-right" style="margin-top:10px;"><span data-latlon="'+latlonval+'" data-input="'+map.params.input+'" class="w_pointer" onclick="setText(wacss.getObject(this.dataset.input),this.dataset.latlon);removeId(\'geolocationmap_popup\');"><span class="w_bigger w_gray icon-save w_pointer"></span> Save</span></div>';infoWindow.setContent(htm);infoWindow.open(map)})},getObject:function(obj){if(typeof(obj)=='object'){return obj}else if(typeof(obj)=='string'){let qso=document.querySelector('#'+obj);if(typeof(qso)=='object'){return qso}
qso=document.querySelector(obj);if(typeof(qso)=='object'){return qso}
if(undefined!=document.getElementById(obj)){return document.getElementById(obj)}else if(undefined!=document.getElementsByName(obj)){var els=document.getElementsByName(obj);if(els.length==1){return els[0]}}else if(undefined!=document.all[obj]){return document.all[obj]}}
