...
cp=wacss.createCenterpop(params.title,3);xmlhttp.div=cp.querySelector('.wacss_centerpop_content');break;case 'modal':params.overlay=1;let modal=wacss.modalPopup(wacss.setprocessing,params.title,params)
xmlhttp.div=modal.querySelector('#wacss_modal_content');break}}
...
if(undefined!=div.id&&div.id.indexOf('_modal_')==-1){wacss.modalClose()}else if(undefined!=div.id&&div.id.indexOf('centerpop')==-1){wacss.removeId('centerpop')}},ajaxUniqueID:function(){return"10000000000000000000".replace(/[018]/g,c=>(c^crypto.getRandomValues(new Uint8Array(1))[0]&15>>c/4).toString(16))},blink:function(el){el=wacss.getObject(el);if(undefined==el){return}
