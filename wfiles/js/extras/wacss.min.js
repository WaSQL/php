...
return!1},init:function(){wacss.initWacssEdit();wacss.initChartJs();wacss.initCodeMirror();wacss.initEditor();wacss.initWhiteboard()},chartjsDrawTotals:function(chart){let width=chart.chart.width,height=chart.chart.height,ctx=chart.chart.ctx;ctx.restore();let fontSize=(height/60).toFixed(2);ctx.font=fontSize+"em sans-serif";ctx.textBaseline="middle";let text=chart.config.centerText.text,textX=Math.round((width-ctx.measureText(text).width)/2),textY=height-20;ctx.fillText(text,textX,textY);ctx.save()},initChartJsBehavior:function(chartid){let list=document.querySelectorAll('[data-behavior="chartjs"]');if(undefined!=chartid){list=document.querySelectorAll('#'+chartid)}
...
let lconfig={type:list[i].dataset.type,data:{labels:[],datasets:[]},options:{responsive:!0,events:!1,animation:{animateScale:!1,animateRotate:!0},title:{display:!1},tooltips:{enabled:!1,intersect:!1,mode:'index'},plugins:{labels:{fontColor:function(data){let rgb={};rgb=wacss.hexToRgb(data.dataset.backgroundColor[data.index]);if(undefined==rgb.r){return'#FFF'}
let threshold=140;let luminance=0.299*rgb.r+0.587*rgb.g+0.114*rgb.b;return luminance>threshold?'black':'white'},precision:0,showActualPercentages:!0}}}};let labelsdiv=datadiv.querySelector('labels');if(undefined!=labelsdiv){let labelsjson=wacss.trim(labelsdiv.innerText);lconfig.data.labels=JSON.parse(labelsjson)}
...
let datasets=datadiv.querySelectorAll('dataset');let datasetLabels=new Array();for(let d=0;d<datasets.length;d++){let json=JSON.parse(datasets[d].innerText);let fill=datasets[d].getAttribute('data-fill')||list[i].getAttribute('data-fill');if(undefined!=fill){if(fill.indexOf('true')!=-1){fill=!0}else if(fill=='1'){fill=!0}else{fill=!1}}else{fill=!1}
let dataset={backgroundColor:datasets[d].getAttribute('data-backgroundColor')||colors[d],type:datasets[d].getAttribute('data-type')||list[i].getAttribute('data-type'),data:json,fill:fill,pointBackgroundColor:[],pointBorderColor:[]};if(undefined!=datasets[d].getAttribute('data-yaxis')){dataset.yAxisID=datasets[d].getAttribute('data-yaxis')}
if(undefined!=datasets[d].getAttribute('data-showLine')&&datasets[d].getAttribute('data-showLine')=='false'){dataset.showLine=!1}
if(undefined!=datasets[d].getAttribute('data-label')){dataset.label=datasets[d].getAttribute('data-label');let dlabel=datasets[d].getAttribute('data-label');datasetLabels.push(dlabel)}
...
let pcanvas=document.createElement('canvas');list[i].appendChild(pcanvas);let pctx=pcanvas.getContext('2d');wacss.chartjs[list[i].id]=new Chart(pctx,pconfig);if(undefined!=list[i].getAttribute('data-onclick')){pcanvas.parentobj=list[i];pcanvas.chartobj=wacss.chartjs[list[i].id];pcanvas.onclick_func=list[i].getAttribute('data-onclick');pcanvas.clicked=0;pcanvas.onclick=function(evt){if(this.clicked==0){this.clicked=1;this.timeout=setTimeout(function(obj){obj.clicked=0},250,this);let activePoints=this.chartobj.getElementsAtEventForMode(evt,'point',this.chartobj.options);if(activePoints.length>0){let firstPoint=activePoints[0];let params={};params.parent=this.parentobj;params.chart=this.chartobj;params.type=this.parentobj.getAttribute('data-type');params.label=this.chartobj.data.labels[firstPoint._index]||this.chartobj.data.datasets[firstPoint._datasetIndex].label;params.value=this.chartobj.data.datasets[firstPoint._datasetIndex].data[firstPoint._index];window[this.onclick_func](params)}}}}}
break}}
...
for(e=0;e<els.length;e++){if(undefined!=els[e].getAttribute('data-initialized')){continue}
...
let lis=p.nextSibling.querySelectorAll('ul li');for(let x=0;x<lis.length;x++){lis[x].className='wacssform_many'}}},initWhiteboard:function(){let list=document.querySelectorAll('textarea[data-behavior="whiteboard"]');for(let i=0;i<list.length;i++){if(undefined!=list[i].dataset.initialized){continue}
list[i].dataset.initialized+=1;list[i].style.display='none';let wrapper=document.createElement('div');wrapper.style.width=list[i].style.width;wrapper.style.height=list[i].style.height;wrapper.style.display='block';wrapper.style.position='relative';wrapper.style.boxShadow='rgba(0, 0, 0, 0.16) 0px 3px 6px, rgba(0, 0, 0, 0.23) 0px 3px 6px';wrapper.id='whiteboard_wrapper_1';wrapper.style.backgroundColor=list[i].dataset.fill||'#fff';wrapper.shapes=new Array();wrapper.drawShapes=function(){this.canvas.ctx.clearRect(0,0,this.canvas.width,this.canvas.height);this.canvas.txtarea.innerText='';let dshapes=this.shapes;dshapes.push(this.canvas.shape);let simg=undefined;for(let i=0;i<dshapes.length;i++){let shape=dshapes[i];if(undefined==shape){continue}
if(undefined==shape.shape){continue}
console.log(i);switch(shape.shape){case 'image':simg=new Image();simg.canvas=this.canvas;simg.crossOrigin='anonymous';simg.src=shape.src;break;case 'circle':this.canvas.ctx.beginPath();this.canvas.ctx.arc(shape.x,shape.y,shape.radius,0,Math.PI*2);if(shape.fillcolor.length){this.canvas.ctx.fillStyle=shape.fillcolor;this.canvas.ctx.lineWidth=shape.size;this.canvas.ctx.fill()}else{this.canvas.ctx.lineWidth=shape.size;this.canvas.ctx.strokeStyle=shape.pencolor;this.canvas.ctx.stroke()}
break;case 'rectangle':if(shape.fillcolor.length){this.canvas.ctx.fillStyle=shape.fillcolor;this.canvas.ctx.lineWidth=shape.size;this.canvas.ctx.fillRect(shape.x,shape.y,shape.width,shape.height)}else{this.canvas.ctx.strokeStyle=shape.pencolor;this.canvas.ctx.lineWidth=shape.size;this.canvas.ctx.strokeRect(shape.x,shape.y,shape.width,shape.height)}
break;case 'line':this.canvas.ctx.beginPath();this.canvas.ctx.moveTo(shape.x,shape.y);this.canvas.ctx.strokeStyle=shape.pencolor;this.canvas.ctx.lineTo(shape.x2,shape.y2);this.canvas.ctx.lineWidth=shape.size;this.canvas.ctx.stroke();this.canvas.ctx.closePath();break;default:this.canvas.ctx.beginPath();this.canvas.ctx.moveTo(shape.x,shape.y);this.canvas.ctx.strokeStyle=shape.pencolor;this.canvas.ctx.lineWidth=shape.size;this.canvas.ctx.lineTo(shape.x2,shape.y2);this.canvas.ctx.stroke();this.canvas.ctx.closePath();break}}
dshapes=[];if(typeof this.canvas.toDataURL==='function'){if(simg){simg.onload=function(){this.canvas.ctx.drawImage(this,0,0);this.canvas.txtarea.innerText=this.canvas.toDataURL('image/png')}}else{this.canvas.txtarea.innerText=this.canvas.toDataURL('image/png')}}else{}}
list[i].parentNode.insertBefore(wrapper,list[i].nextSibling);let wcanvas=document.createElement('canvas');let toolbar=document.createElement('div');let params={};wrapper.canvas=wcanvas;wcanvas.onload=function(){console.log('canvas load');console.log(this)}
toolbar.canvas=wcanvas;toolbar.wrapper=wrapper;toolbar.style='display:flex;justify-content:flex-end;align-items:center;width:100%;background:#f0f0f0;height:34px;box-shadow: rgba(17, 17, 26, 0.05) 0px 1px 0px, rgba(17, 17, 26, 0.1) 0px 0px 8px;';wrapper.dataset.shape='pencil';let shapes={'pencil':'Pencil','line':'Line','circle':'Circle','rectangle':'Rectangle',};params={style:'margin-left:10px;width:100px;padding:3px;'}
toolbar.shape=wacss.buildFormSelect('shape',shapes,params);toolbar.shape.onchange=function(){let shape=this.options[this.selectedIndex].value;this.parentNode.wrapper.dataset.shape=shape};toolbar.shape.title="Shape";toolbar.appendChild(toolbar.shape);wrapper.dataset.size='1';let sizes={'1':'1px','3':'3px','5':'5px','7':'7px','9':'9px','11':'11px','13':'13px','15':'15px','17':'17px','19':'19px'};params={style:'margin-left:10px;width:100px;padding:3px;'}
toolbar.size=wacss.buildFormSelect('size',sizes,params);toolbar.size.onchange=function(){let size=this.options[this.selectedIndex].value;this.parentNode.wrapper.dataset.size=size};toolbar.size.title="Pen Size";toolbar.appendChild(toolbar.size);wrapper.dataset.pencolor='#000';let pencolors={'#000':'Black','#213a9a':'Blue','#05abff':'Light Blue','#05a04d':'Green','#66d81f':'Light Green','#ff0081':'Pink','#f16115':'Orange','#f43940':'Red','#fee213':'Yellow'};params={style:'margin-left:10px;width:100px;padding:3px;'}
toolbar.pencolor=wacss.buildFormSelect('pencolor',pencolors,params);toolbar.pencolor.onchange=function(){let pencolor=this.options[this.selectedIndex].value;this.parentNode.wrapper.dataset.pencolor=pencolor};toolbar.pencolor.title="Pen Color";toolbar.appendChild(toolbar.pencolor);let fillcolors={'':'None','#000':'Black','#213a9a':'Blue','#05abff':'Light Blue','#05a04d':'Green','#66d81f':'Light Green','#ff0081':'Pink','#f16115':'Orange','#f43940':'Red','#fee213':'Yellow'};wrapper.dataset.fillcolor='';params={style:'margin-left:10px;width:100px;padding:3px;'}
toolbar.fillcolor=wacss.buildFormSelect('fillcolor',fillcolors,params);toolbar.fillcolor.onchange=function(){let fillcolor=this.options[this.selectedIndex].value;this.parentNode.wrapper.dataset.fillcolor=fillcolor};toolbar.fillcolor.title="Fill Color";toolbar.appendChild(toolbar.fillcolor);toolbar.clear=document.createElement('span');toolbar.clear.className='icon-erase w_pointer';toolbar.clear.setAttribute('style','margin-left:10px;margin-right:10px;');toolbar.clear.title='Erase Whiteboard';toolbar.appendChild(toolbar.clear);toolbar.clear.onclick=function(e){if(!confirm('Erase Whiteboard?')){return!1}
this.parentNode.canvas.ctx.clearRect(0,0,this.parentNode.canvas.width,this.parentNode.canvas.height);this.parentNode.canvas.txtarea.innerText='';this.parentNode.parentNode.shapes=new Array()};wrapper.canvas=wcanvas;wrapper.roinit=0;wcanvas.txtarea=list[i];wrapper.ro=new ResizeObserver(entries=>{for(let entry of entries){if(undefined!=entry.target.canvas&&undefined!=entry.target.clientWidth){if(entry.target.roinit==1){entry.target.canvas.setAttribute('width',entry.target.clientWidth);entry.target.canvas.setAttribute('height',parseInt(entry.target.clientHeight)-30)}
entry.target.roinit=1}}});wrapper.ro.observe(wrapper);wcanvas.width=wrapper.clientWidth;let h=parseInt(wrapper.clientHeight)-parseInt(toolbar.style.height)-2;wcanvas.height=h;wcanvas.style.position='relative';wcanvas.style.cursor='crosshair';wcanvas.ctx=wcanvas.getContext("2d");wcanvas.ctx.strokeStyle="#000";if(list[i].innerHTML.length){let wshape={shape:'image',src:list[i].innerHTML}
wrapper.shapes.push(wshape);wrapper.drawShapes()}
wcanvas.isDown=!1;wcanvas.canvasX=0;wcanvas.canvasY=0;wcanvas.lineWidth=list[i].dataset.size||1;wcanvas.onmousedown=function(e){e=e||window.event;let rect=e.target.getBoundingClientRect();this.x=parseInt(e.pageX-rect.left);this.y=parseInt(e.pageY-rect.top);this.isDown=!0;switch(this.parentNode.dataset.shape){default:this.shape={shape:'pencil',x:this.x,y:this.y,pencolor:this.parentNode.dataset.pencolor,size:this.parentNode.dataset.size};break;case 'line':this.shape={shape:'line',x:this.x,y:this.y,pencolor:this.parentNode.dataset.pencolor,size:this.parentNode.dataset.size};break;case 'circle':this.shape={shape:'circle',x:this.x,y:this.y,pencolor:this.parentNode.dataset.pencolor,size:this.parentNode.dataset.size};if(this.parentNode.dataset.fillcolor.length){this.shape.fillcolor=this.parentNode.dataset.fillcolor}else{this.shape.fillcolor=''}
break;case 'rectangle':this.shape={shape:'rectangle',x:this.x,y:this.y,pencolor:this.parentNode.dataset.pencolor,size:this.parentNode.dataset.size};if(this.parentNode.dataset.fillcolor.length){this.shape.fillcolor=this.parentNode.dataset.fillcolor}else{this.shape.fillcolor=''}
break}};wcanvas.onmousemove=function(e){e=e||window.event;if(this.isDown!==!1){let rect=e.target.getBoundingClientRect();let x=parseInt(e.pageX-rect.left);let y=parseInt(e.pageY-rect.top);switch(this.parentNode.dataset.shape){default:this.shape.x2=x;this.shape.y2=y;this.parentNode.shapes.push(this.shape);this.shape={shape:'pencil',x:x,y:y,pencolor:this.parentNode.dataset.pencolor};break;case 'line':this.shape.x2=x;this.shape.y2=y;break;case 'circle':let r1=Math.abs(x-this.shape.x);let r2=Math.abs(y-this.shape.y);if(r1>r2){this.shape.radius=r1}else{this.shape.radius=r2}
break;case 'rectangle':this.shape.width=Math.abs(x-this.shape.x);this.shape.height=Math.abs(y-this.shape.y);break}
this.parentNode.drawShapes()}};wcanvas.onmouseup=function(e){e=e||window.event;this.isDown=!1;switch(this.parentNode.dataset.shape){default:this.parentNode.shapes.push(this.shape);break;case 'line':break;case 'circle':this.parentNode.shapes.push(this.shape);break;case 'rectangle':this.parentNode.shapes.push(this.shape);break}
this.parentNode.drawShapes()};wrapper.onmouseout=function(e){e=e||window.event;this.canvas.isDown=!1;switch(this.parentNode.dataset.shape){default:break;case 'line':break;case 'circle':break;case 'rectangle':break}}
document.body.addEventListener('touchmove',function(e){e=e||window.event;e.preventDefault()},!1);wrapper.appendChild(wcanvas);wrapper.appendChild(toolbar)}},isNum:function(n){return!isNaN(parseFloat(n))&&isFinite(n)},listen:function(evnt,elem,func){if(elem.addEventListener){elem.addEventListener(evnt,func,!1)}else if(elem.attachEvent){let r=elem.attachEvent("on"+evnt,func);return r}else{console.log('wacss.listen failed. Browser does not support event listeners')}},loadCSS:function(file,notify){let link=document.createElement('link');if(undefined!=notify&&notify==1){link.onload=function(){wacss.toast(this.getAttribute('href')+' loaded successfully')}}
