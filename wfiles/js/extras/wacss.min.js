...
let nav=opts.nav||elobj.dataset.nav||pli.dataset.nav||pul.dataset.nav||ptd.dataset.nav||ptr.dataset.nav||elobj.getAttribute('href');if(nav=='id'&&undefined!=elobj.dataset.parent){let target=wacss.getObject(div);let ptarget=wacss.getObject(elobj.dataset.parent);for(let c=0;c<ptarget.childNodes.length;c++){if(undefined==ptarget.childNodes[c].id){continue}
...
if(elobj.dataset[k].indexOf('id:')==0){let cid=elobj.dataset[k].replace('id:','');let cidobj=document.querySelector('#'+cid);if(undefined!=cidobj){params[k]=cidobj.value||cidobj.innerHTML}else{params[k]=elobj.dataset[k]}}else{params[k]=elobj.dataset[k]}}
for(k in opts){params[k]=opts[k]}
