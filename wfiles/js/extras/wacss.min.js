...
let labels={};let labelsdiv=datadiv.querySelector('labels');if(undefined!=labelsdiv){let labelsjson=wacss.trim(labelsdiv.innerText);labels=JSON.parse(labelsjson)}
...
break;case 'line':case 'bar':case 'horizontalbar':case 'doughnut':if(undefined!=wacss.chartjs[list[i].id]){let ck=list[i].querySelector('canvas');if(undefined!=ck){let udatasets=datadiv.querySelectorAll('dataset');for(let ud=0;ud<udatasets.length;ud++){if(undefined==udatasets[ud].getAttribute('data-label')){continue}
let json=JSON.parse(udatasets[ud].innerText);let udataset={label:udatasets[ud].getAttribute('data-label'),backgroundColor:udatasets[ud].getAttribute('data-backgroundColor')||colors[ud],borderColor:udatasets[ud].getAttribute('data-borderColor')||colors[ud],pointColor:udatasets[ud].getAttribute('data-pointColor')||colors[ud],type:list[i].getAttribute('data-type'),pointRadius:udatasets[ud].getAttribute('data-pointRadius')||0,data:json,fill:!1,lineTension:udatasets[ud].getAttribute('data-lineTension')||0,borderWidth:udatasets[ud].getAttribute('data-borderWidth')||2};wacss.chartjs[list[i].id].config.data.datasets[ud]=udataset}
wacss.chartjs[list[i].id].config.options=options;wacss.chartjs[list[i].id].config.data.labels=labels;wacss.chartjs[list[i].id].update();foundchart=1}}
if(foundchart==0){let lconfig={type:list[i].getAttribute('data-type'),data:{labels:labels,datasets:[]},options:options};let datasets=datadiv.querySelectorAll('dataset');for(let d=0;d<datasets.length;d++){if(undefined==datasets[d].getAttribute('data-label')){continue}
let json=JSON.parse(datasets[d].innerText);let dataset={label:datasets[d].getAttribute('data-label'),backgroundColor:datasets[d].getAttribute('data-backgroundColor')||colors[d],borderColor:datasets[d].getAttribute('data-borderColor')||colors[d],pointColor:datasets[d].getAttribute('data-pointColor')||colors[d],type:list[i].getAttribute('data-type'),pointRadius:datasets[d].getAttribute('data-pointRadius')||0,data:json,fill:!1,lineTension:datasets[d].getAttribute('data-lineTension')||0,borderWidth:datasets[d].getAttribute('data-borderWidth')||2};lconfig.data.datasets.push(dataset)}
