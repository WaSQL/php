...
popup_title.appendChild(popup_title_close);popup.appendChild(popup_title);let popup_content=document.createElement('div');popup_content.id='geolocationmap_content';popup_content.style='flex:1;';popup.appendChild(popup_content);params.div=popup_content;wacss.geoLocationMapContent(params);document.body.appendChild(popup);centerObject(popup);let viewport=document.querySelector('meta[name="viewport"]');if(undefined!=viewport){viewport.setAttribute('content','width=device-width, minimum-scale=1.0, initial-scale=1.0, maximum-scale=1.0')}},geoLocationMapContent:function(params){if(undefined==params){params={}}
params.zoom=params.zoom||13;let myLatlng={lat:params.lat,lng:params.long};let map_params={center:myLatlng,mapTypeId:'roadmap',zoom:params.zoom,styles:[]};if(undefined!=params.hide&&params.hide.toLowerCase().indexOf('poi')!=-1){let poi={featureType:'poi',elementType:'labels',stylers:[{visibility:'off'}]};map_params.styles.push(poi)}
