...
return!1},init:function(){wacss.initWacssEdit();wacss.initChartJs();wacss.initTruncate()},chartjsDrawTotals:function(chart){var width=chart.chart.width,height=chart.chart.height,ctx=chart.chart.ctx;ctx.restore();var fontSize=(height/60).toFixed(2);ctx.font=fontSize+"em sans-serif";ctx.textBaseline="middle";var text=chart.config.centerText.text,textX=Math.round((width-ctx.measureText(text).width)/2),textY=height-10;ctx.fillText(text,textX,textY);ctx.save()},initChartJs:function(initid){Chart.Chart.pluginService.register({beforeDraw:function(chart){if(undefined!=chart.config.centerText){if(undefined!=chart.config.centerText.display){wacss.chartjsDrawTotals(chart)}}},});let list=document.querySelectorAll('div.chartjs');let gcolors=new Array('rgb(255, 159, 64)','rgb(75, 192, 192)','rgb(255, 99, 132)','rgb(54, 162, 235)','rgb(153, 102, 255)','rgb((218,165,32)','rgb(233,150,122)','rgb(189,183,107)','rgb(154,205,50)','rgb(255,228,196)','rgb(244,164,96)','rgb(176,196,222)','rgb(188,143,143)','rgb(255,228,225)','rgb(201, 203, 207)');for(let i=0;i<list.length;i++){if(undefined==list[i].id){console.log('missing id',list[i]);continue}
...
if(foundchart==0){let gv=parseInt(datadiv.innerText);let max=list[i].dataset.max||180;let gv1=parseInt(max*(gv/100));if(gv1>max){gv1=max}
let gv2=max-gv1;let color=list[i].dataset.color||'#009300';let gconfig={type:'doughnut',data:{datasets:[{data:[gv1,gv2],backgroundColor:colors,borderWidth:0}]},options:{title:{display:!1},circumference:Math.PI,rotation:-1*Math.PI,responsive:!0,legend:{display:!1},animation:{animateScale:!1,animateRotate:!0}},centerText:{display:!0,text:"123"}};if(undefined!=list[i].getAttribute('data-title')){let title=list[i].getAttribute('data-title');gconfig.options.title={display:!0,padding:0,position:'bottom',text:title}}
