var wacss={version:'2023.0211',author:'WaSQL.com',chartjs:{},hoverDiv:'',loadJsText:function(name,body){var code='this.f = function '+name+'() {'+body+'}';eval(code);return!0},addClass:function(element,classToAdd){element=wacss.getObject(element);if(undefined==element){return!1}
if(undefined==element.className){element.className=classToAdd;return!0}
let currentClassValue=element.className;if(currentClassValue.indexOf(classToAdd)==-1){if((currentClassValue==null)||(currentClassValue==="")){element.className=classToAdd}else{element.className+=" "+classToAdd}}},ajaxGet:function(url,div,params){let xmlhttp=new XMLHttpRequest();xmlhttp.div=div;xmlhttp.onreadystatechange=function(){if(this.readyState==XMLHttpRequest.DONE){let div=document.getElementById(this.div);if(this.status==200||this.status==201){div.innerHTML=this.responseText}else{div.innerHTML=JSON.stringify(this)}}};if(url.indexOf('http')==-1){url=window.location.origin+url}
...
return tag},checkAllElements:function(att,val,ck){let list;switch(att.toLowerCase()){case 'class':list=document.querySelectorAll('input[type="checkbox"].'+val);break;case 'id':list=document.querySelectorAll('input[type="checkbox"]#'+val);break;default:list=document.querySelectorAll('input[type="checkbox"]['+att+'="'+val+'"]');break}
for(let i=0;i<list.length;i++){if(undefined!=list[i].dataset.onclick){wacss.simulateEvent(list[i],'click')}
list[i].checked=ck}
return!1},copy2Clipboard:function(str,msg){if(undefined==msg){msg='Copy Successful'}
...
p.querySelector('input[type="checkbox"]').checked=!1},dismiss:function(el){if(el.parentElement.querySelector(':hover')==el){let wtimer=parseInt(el.timer)*3;setTimeout(function(){wacss.dismiss(el)},wtimer);return}
