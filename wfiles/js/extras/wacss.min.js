...
let n=blink+1;el.setAttribute('data-blink',n);switch(blink){case 0:case 2:case 4:el.style.boxShadow='0 8px 12px 0 rgba(0,0,0,0.7),0 10px 24px 0 rgba(0,0,0,0.69)';setTimeout(function(){wacss.blink(el)},150);break;case 1:case 3:case 5:el.style.boxShadow='0 8px 12px 0 rgba(0,0,0,0.2),0 10px 24px 0 rgba(0,0,0,0.19)';setTimeout(function(){wacss.blink(el)},150);break;default:el.style.boxShadow=el.getAttribute('data-boxshadow');el.setAttribute('data-blink',0);break}},copy2Clipboard:function(str,msg){if(undefined==msg){msg='Copy Successful'}
...
document.body.appendChild(d);centerObject(d);z=z-2;let v=document.createElement('div');v.style.zIndex=z;v.style.display='block';v.style.width='3000px';v.style.height='2000px';v.style.position='absolute';v.style.top='0px';v.style.left='0px';v.style.background='rgba(0,0,0,0.5)';v.id=d.id+'_overlay';v.setAttribute('data-target',d.id);v.onclick=function(){removeDiv(this.getAttribute('data-target'));removeDiv(this.id)};document.body.appendChild(v)},speak:function(msg,params){if('speechSynthesis' in window){window.speechSynthesis.params=params;window.speechSynthesis.onvoiceschanged=function(){let msg=new SpeechSynthesisUtterance();let params=window.speechSynthesis.params;let voices=window.speechSynthesis.getVoices();if(undefined!=params.name){for(let i=0;i<voices.length;i++){if(voices[i].name==params.name){msg.voice=voices[i];break}}}else if(undefined!=params.lang){for(let i=0;i<voices.length;i++){if(voices[i].lang==params.lang){msg.voice=voices[i];break}}}
if(undefined!=params.volume){msg.volume=params.volume}else if(undefined!=params.whisper){msg.volume=0.2}
if(undefined!=params.rate){msg.rate=params.rate}
if(undefined!=params.pitch){msg.pitch=params.pitch}
window.speechSynthesis.speak(msg)}}else{console.log('wacss.speak error: speechSynthesis is not supported in your browser or OS')}
