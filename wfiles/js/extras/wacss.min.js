...
break}}
editor.addEventListener("paste",function(e){e.preventDefault();let text=e.clipboardData.getData('text/plain');document.execCommand("insertHTML",!1,text)});editor.addEventListener("input",function(ie){this.saveto.textContent=this.textContent});editor.save=function(){this.saveto.textContent=this.textContent};editor.textContent=els[e].textContent;els[e].parentNode.insertBefore(editor,els[e].nextSibling)}},initWacssEdit:function(){let list=document.querySelectorAll('textarea.wacssedit');for(let i=0;i<list.length;i++){if(undefined==list[i].id){continue}
