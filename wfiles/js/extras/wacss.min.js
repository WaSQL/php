...
el.className='toast dismiss';setTimeout(function(){wacss.removeObj(el)},1000)},function_exists:function(function_name){if(typeof function_name=='string'){return(typeof window[function_name]=='function')}else{return(function_name instanceof Function)}},getAllAttributes:function(obj){var node=wacss.getObject(obj);var rv={};for(var i=0;i<node.attributes.length;i++){if(node.attributes.item(i).specified){rv[node.attributes.item(i).nodeName]=node.attributes.item(i).nodeValue}}
return rv},geoLocation:function(fld,opts){console.log('wacss.geoLocation');console.log(fld);fld=wacss.getObject(fld);if(undefined==fld){console.log("wacss.getGeoLocation error: "+fld+' is undefined');return!1}
if(navigator.geolocation){let options={enableHighAccuracy:!0,timeout:10000,maximumAge:5000};if(undefined!=opts){for(let k in opts){options[k]=opts[k]}}
navigator.geoSetFld=fld;navigator.geolocation.getCurrentPosition(function(position){console.log(position);console.log(navigator.geoSetFld);if(wacss.function_exists(navigator.geoSetFld)){console.log('function');window[navigator.geoSetFld](position.coords.latitude,position.coords.longitude)}else{console.log('field');navigator.geoSetFld.value='['+position.coords.latitude+','+position.coords.longitude+']'}
return!1},function(err){console.log(err);return!1},options)}
return!1},getObject:function(obj){if(typeof(obj)=='object'){return obj}else if(typeof(obj)=='string'){if(undefined!=document.getElementById(obj)){return document.getElementById(obj)}else if(undefined!=document.getElementsByName(obj)){var els=document.getElementsByName(obj);if(els.length==1){return els[0]}}else if(undefined!=document.all[obj]){return document.all[obj]}
...
return!1},init:function(){wacss.initWacssEdit();wacss.initChartJs();wacss.initTruncate()},chartjsDrawTotals:function(chart){var width=chart.chart.width,height=chart.chart.height,ctx=chart.chart.ctx;ctx.restore();var fontSize=(height/60).toFixed(2);ctx.font=fontSize+"em sans-serif";ctx.textBaseline="middle";var text=chart.config.centerText.text,textX=Math.round((width-ctx.measureText(text).width)/2),textY=height-20;ctx.fillText(text,textX,textY);ctx.save()},initChartJs:function(initid){let list=document.querySelectorAll('div.chartjs');let gcolors=new Array('rgb(255, 159, 64)','rgb(75, 192, 192)','rgb(255, 99, 132)','rgb(54, 162, 235)','rgb(153, 102, 255)','rgb((218,165,32)','rgb(233,150,122)','rgb(189,183,107)','rgb(154,205,50)','rgb(255,228,196)','rgb(244,164,96)','rgb(176,196,222)','rgb(188,143,143)','rgb(255,228,225)','rgb(201, 203, 207)');for(let i=0;i<list.length;i++){if(undefined==list[i].id){console.log('missing id',list[i]);continue}
...
let gv2=max-gv1;let color=list[i].dataset.color||'#009300';let gconfig={type:'doughnut',data:{datasets:[{data:[gv1,gv2],backgroundColor:colors,borderWidth:0}]},options:{title:{display:!1},circumference:Math.PI,rotation:-1*Math.PI,responsive:!0,plugins:{datalabels:{display:!0}},legend:{display:!1},animation:{animateScale:!1,animateRotate:!0}},centerText:{display:!0,text:gv1}};if(undefined!=list[i].dataset.labels&&list[i].dataset.labels=='false'){gconfig.options.plugins.datalabels.display=!1}
...
wacss.chartjs[list[i].id].config.data.datasets[0].data=data;if(labels.length>0){wacss.chartjs[list[i].id].config.data.labels=labels}else{wacss.chartjs[list[i].id].config.data.labels=pielabels}
wacss.chartjs[list[i].id].update();foundchart=1}}
...
z=z||1020;let d=document.createElement('div');d.id="modal1";d.tabindex=0;d.style.zIndex=z;d.style.display='block';d.style.background='#FFF';d.style.padding='15px';d.style.border='1px outset #747392';d.style.borderRadius='3px';d.style.position='absolute';d.style.maxWidth='60%';d.style.maxHeight='800px';d.style.transform='scaleX(1) scaleY(1)';let i=document.createElement('img');i.src=el.getAttribute('src');i.style.maxWidth='100%';i.style.maxHeight='770px';d.appendChild(i)
document.body.appendChild(d);centerObject(d);z=z-2;let v=document.createElement('div');v.style.zIndex=z;v.style.display='block';v.style.width='3000px';v.style.height='2000px';v.style.position='absolute';v.style.top='0px';v.style.left='0px';v.style.background='rgba(0,0,0,0.5)';v.id=d.id+'_overlay';v.setAttribute('data-target',d.id);v.onclick=function(){removeDiv(this.getAttribute('data-target'));removeDiv(this.id)};document.body.appendChild(v)},toast:function(msg,params){if(undefined==params){params={color:'w_red',timer:3}}
if(undefined==params.color){params.color=wacss.color()}
