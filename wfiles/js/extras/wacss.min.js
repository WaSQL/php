...
let datasets=datadiv.querySelectorAll('dataset');let datasetLabels=new Array();for(let d=0;d<datasets.length;d++){let json=JSON.parse(datasets[d].innerText);let dataset={backgroundColor:datasets[d].getAttribute('data-backgroundColor')||colors[d],type:datasets[d].getAttribute('data-type')||list[i].getAttribute('data-type'),data:json,fill:!1,};if(undefined!=datasets[d].getAttribute('data-yaxis')){dataset.yAxisID=datasets[d].getAttribute('data-yaxis')}
