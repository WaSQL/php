...
let threshold=140;let luminance=0.299*rgb.r+0.587*rgb.g+0.114*rgb.b;return luminance>threshold?'black':'white'},precision:0,showActualPercentages:!0}}}}
...
let datasets=datadiv.querySelectorAll('dataset');for(let d=0;d<datasets.length;d++){let datasetjson=wacss.trim(datasets[d].innerText);let json=JSON.parse(datasetjson);if(undefined!=list[i].dataset.debug){console.log('datasetjson'):console.log(datasetjson);console.log('json');console.log(json)}
...
wrapper.pad.txtarea=list[i];wrapper.pad.addEventListener('endStroke',function(){this.txtarea.innerText=this.toDataURL('image/png')});wrapper.toolbar=document.createElement('div');wrapper.toolbar.style.position='absolute';wrapper.toolbar.style.bottom='0px';wrapper.toolbar.style.left='0px';wrapper.toolbar.style.display='flex';wrapper.toolbar.style.justifyContent='flex-end';wrapper.toolbar.style.alignItems='center';wrapper.toolbar.style.width='100%';wrapper.toolbar.style.height='30px';wrapper.toolbar.style.borderTop='1px solid #ccc';wrapper.appendChild(wrapper.toolbar);params={style:'flex:1;color:#999;font-size:0.8rem;height:2.1em;',class:'input w_small'};wrapper.toolbar.txt=wacss.buildFormText('txt',params);wrapper.toolbar.txt.onkeyup=function(){let fontname=this.parentNode.parentNode.dataset.font;let pad=this.parentNode.parentNode.pad;let ctx=pad._ctx;let w=pad.canvas.width;let h=pad.canvas.height;let px=parseInt(h/2)+5;pad.clear();ctx.font=px+'px '+fontname;ctx.textAlign='start';let x=15;let y=parseInt(h/2)+10;let m=w-10;ctx.fillText(this.value,x,y,m);return!0};wrapper.toolbar.txt.onfocus=function(){this.style.outline='none'}
wrapper.toolbar.txt.title="Signature";wrapper.toolbar.txt.style.borderColor='transparent';wrapper.toolbar.txt.style.borderRight='1px solid #ccc';wrapper.toolbar.appendChild(wrapper.toolbar.txt);wrapper.dataset.font='andragogy';let fonts={'andragogy':'Andragogy','high_summit':'High Summit','julialauren':'Julia Lauren','katrineholland':'Katrine Holland','sandrabelhock':'Sandra Belhock','yasminerothem':'Yasmine Rothem',};Object.keys(fonts).forEach(function(fontname){let el=document.createElement('div');el.style.float='left';el.style.fontSize='1px';el.style.fontFamily=fontname;el.innerText='.';wrapper.toolbar.appendChild(el)});params={style:'border-color:transparent;border-right:1px solid #ccc;color:#999;margin-left:2px;width:60px;font-size:0.8rem;padding:3px;'};wrapper.toolbar.font=wacss.buildFormSelect('font',fonts,params);wrapper.toolbar.font.onchange=function(){let fontname=this.options[this.selectedIndex].value;this.parentNode.parentNode.dataset.font=fontname;let pad=this.parentNode.parentNode.pad;let ctx=pad._ctx;let w=pad.canvas.width;let h=pad.canvas.height;let px=parseInt(h/2)+5;pad.clear();ctx.font=px+'px '+fontname;ctx.textAlign='start';let x=15;let y=parseInt(h/2)+10;let m=w-10;ctx.fillText(this.parentNode.txt.value,x,y,m);return!0};wrapper.toolbar.font.onfocus=function(){this.style.outline='none'}
wrapper.toolbar.font.title="Signature Font";wrapper.toolbar.font.style.borderColor='#000000';wrapper.toolbar.appendChild(wrapper.toolbar.font);wrapper.dataset.pencolor='#000';let pencolors={'#000000':'Black','#213a9a':'Blue','#05abff':'Light Blue','#05a04d':'Green','#66d81f':'Light Green','#ff0081':'Pink','#f16115':'Orange','#f43940':'Red','#fee213':'Yellow'};params={style:'background-color:#000;color:#fff;margin-left:2px;width:60px;font-size:0.8rem;padding:3px;'};wrapper.toolbar.pencolor=wacss.buildFormSelect('pencolor',pencolors,params);wrapper.toolbar.pencolor.onchange=function(){let pencolor=this.options[this.selectedIndex].value;this.parentNode.parentNode.pad.penColor=pencolor;this.parentNode.parentNode.dataset.pencolor=pencolor;this.style.borderColor=pencolor;this.style.backgroundColor=pencolor};wrapper.toolbar.pencolor.onfocus=function(){this.style.outline='none'}
wrapper.toolbar.pencolor.title="Pen Color";wrapper.toolbar.pencolor.style.borderColor='#000000';wrapper.toolbar.appendChild(wrapper.toolbar.pencolor);wrapper.toolbar.undo=document.createElement('span');wrapper.toolbar.undo.className='btn btn-small w_blue icon-undo';wrapper.toolbar.undo.title="Undo";wrapper.toolbar.undo.style.marginLeft='2px';wrapper.toolbar.undo.onclick=function(){let data=this.parentNode.parentNode.pad.toData();if(data){data.pop();this.parentNode.parentNode.pad.fromData(data)}
...
wrapper.toolbar.appendChild(wrapper.toolbar.undo);wrapper.toolbar.clear=document.createElement('span');wrapper.toolbar.clear.className='btn btn-small w_red icon-erase';wrapper.toolbar.clear.title='Clear';wrapper.toolbar.clear.style.marginLeft='2px';wrapper.toolbar.clear.onclick=function(){if(!confirm('Clear?')){return!1}
this.parentNode.parentNode.pad.clear();this.parentNode.undo.style.display='inline-block';this.parentNode.parentNode.pad.penColor=this.parentNode.parentNode.dataset.pencolor}
