var MojoZoom=(function(){var $=function(id){return document.getElementById(id)};var dc=function(tag){return document.createElement(tag)};var defaultWidth=256;var defaultHeight=256;function addEvent(element,ev,handler){var doHandler=function(e){return handler(e||window.event)}
if(element.addEventListener){element.addEventListener(ev,doHandler,!1)}else if(element.attachEvent){element.attachEvent("on"+ev,doHandler)}}
function getElementPos(element){var x=element.offsetLeft;var y=element.offsetTop;var parent=element.offsetParent;while(parent){x+=parent.offsetLeft-parent.scrollLeft;y+=parent.offsetTop-parent.scrollTop;parent=parent.offsetParent}
return{x:x,y:y}}
...
var isInImage=!1;if(!alwaysShow){addEvent(zoomInput,"mouseout",function(e){var target=e.target||e.srcElement;if(!target)return;if(target.nodeName!="DIV")return;var relTarget=e.relatedTarget||e.toElement;if(!relTarget)return;while(relTarget!=target&&relTarget.nodeName!="BODY"&&relTarget.parentNode){relTarget=relTarget.parentNode}
if(relTarget!=target){isInImage=!1;ctr.style.display="none";zoomImgCtr.style.visibility="hidden"}});addEvent(document.body,"mouseover",function(e){if(isInImage&&!(e.toElement==zoomBorder||e.target==zoomBorder)){ctr.style.display="none";zoomImgCtr.style.visibility="hidden";isInImage=!1}})}
addEvent(zoomInput,"mousemove",function(e){isInImage=!0;var imgPos=getElementPos(img);if(useDefaultCtr){zoomImgCtr.style.left=w+imgPos.x+"px";zoomImgCtr.style.top=imgPos.y+"px"}
...
return{addEvent:addEvent,makeZoomable:makeZoomable}})();