...
var event,bubblesValue=eventInitDict&&eventInitDict[bubbles]!==undefined?eventInitDict[bubbles]:!1,cancelableValue=eventInitDict&&eventInitDict[cancelable]!==undefined?eventInitDict[cancelable]:!1;if('createEvent' in doc){event=doc.createEvent(EVENT);event.initEvent(type,bubblesValue,cancelableValue)}else{event=doc.createEventObject();event[etype]=type;event[bubbles]=bubblesValue;event[cancelable]=cancelableValue}
return event}}
...
var element=this,type=event[etype];try{if(!event[bubbles]){event[cancelBubble]=!0;var cancelBubbleEvent=function(event){event[cancelBubble]=!0;(element||win).detachEvent('on'+type,cancelBubbleEvent)};this.attachEvent('on'+type,cancelBubbleEvent)}
