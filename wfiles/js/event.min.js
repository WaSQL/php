var MouseX=0;var MouseY=0;var cursor={x:0,y:0};function eventJSInit(){if(undefined!=this.event_initialized){return false;}
this.event_initialized=1;if(document.onmousemove){document.onmousemove=mouseMove;}
else if(document.addEventListener){document.addEventListener("mousedown",mouseMove,commonPassiveEventListener(false));document.addEventListener("mouseup",mouseMove,commonPassiveEventListener(false));document.addEventListener("mousemove",mouseMove,commonPassiveEventListener(false));}
...
function eventBuildOnLoad(){if(document.readyState==="complete"){eventProcessBuildOnLoad();}
else{if(window.addEventListener)
window.addEventListener("load",function(){eventProcessBuildOnLoad();},false);else if(window.attachEvent){var r=window.attachEvent("onload",function(){eventProcessBuildOnLoad();});return r;}}}
...
function dragSortEnableDragItem(item){item.setAttribute("draggable",true);item.ondrag=dragSortHandleDrag;item.ondragend=dragSortHandleDrop;}
...
list=document.querySelectorAll('.w_sticky');for(i=0;i<list.length;i++){if(undefined!=tlist[i].nodeName&&tlist[i].nodeName=='TABLE'){continue;}
else if(undefined!=tlist[i].tagName&&tlist[i].tagName=='TABLE'){continue;}
t=list[i].offsetTop;list[i].style.position='sticky';list[i].style.top=t+'px';}}
function marquee(id){var mobj=getObject(id);if(undefined==mobj){return false;}
...
switch(direction.toLowerCase()){case'left':mobj.style.textAlign='right';mobj.style.whiteSpace='nowrap';mobj.style.paddingLeft='0px';if(undefined!=mobj.style.paddingRight){x=mobj.style.paddingRight;if(x.length){x=x.replace('px','');x=parseInt(x,10);}
else{x=0;}}
if(x<mwh[0]-pxwh[0]-2){mobj.style.paddingRight=parseInt(x+2,10)+'px';}
else{mobj.style.paddingRight='0px';}
break;case'right':mobj.style.textAlign='left';mobj.style.whiteSpace='nowrap';mobj.style.paddingRight='0px';if(undefined!=mobj.style.paddingLeft){x=mobj.style.paddingLeft;if(x.length){x=x.replace('px','');x=parseInt(x,10);}
else{x=0;}}
if(x<mwh[0]-pxwh[0]-2){mobj.style.paddingLeft=parseInt(x+2,10)+'px';}
else{mobj.style.paddingLeft='0px';}
break;default:return false;break;}}
TimoutArray[id]=setTimeout("marquee('"+id+"')",timer);return false;}
function mouseMove(e){if(!e){e=window.event;}
if(e.pageX||e.pageY){cursor.x=e.pageX;cursor.y=e.pageY;}
else if(e.clientX||e.clientY){if(document.body){if(document.documentElement){cursor.x=e.clientX+document.body.scrollLeft+document.documentElement.scrollLeft;cursor.y=e.clientY+document.body.scrollTop+document.documentElement.scrollTop;}}}
...
function pageRefresh(page,div,opts,nosetprocess){if(undefined==document.getElementById(div)){return false;}
ajaxGet('/'+page,div,opts,'',60000,nosetprocess);return false;}
function processMultiComboBox(tid,cid,tcnt,cm,showvalues){var tbox=document.getElementById(tid);var cbox=document.getElementById(cid);if(cm&&cbox){if(cbox.checked){cbox.checked=false;}
...
popUpDiv('',params);setCenterPopText(divid,txt,{drag:true,close_bot:false});centerObject(divid);setStyle(divid,'zIndex','99999');return false;}
...
obj=getObject(obj);if(tooltipDivObj==obj){return false;}
tooltipDivObj=obj;let params={};params.position=obj.getAttribute('data-tooltip_position')||obj.getAttribute('data-tip_position')||'';let txt=obj.getAttribute('data-tooltip')||obj.getAttribute('data-tip')||'';if(txt.indexOf('id:')===0){let divid=trim(str_replace('id:','',txt));txt=getText(divid)||'';}
else if(txt.indexOf('js:')===0){let f=trim(str_replace('js:','',txt));let jsfunc=new Function(f);txt=jsfunc();}
else if(txt.indexOf('ajax:')===0){params.ajax=trim(str_replace('ajax:','',txt));txt='';}
else if(txt.indexOf('att:')===0){let att=trim(str_replace('att:','',txt));txt=obj.getAttribute(att)||'';}
...
showTooltip(obj,txt,params);return false;}
function showTooltip(obj,txt,params){obj=getObject(obj);if(undefined==obj){return false;}
if(undefined==params){params={};}
if(undefined==params.position){if(obj.nodeName.toLowerCase()==='img'){params.position='bottom';}
else{params.position='right';}}
...
else{masterdiv=document.createElement("div");masterdiv.setAttribute("id",opt['id']);masterdiv.style.zIndex='9999';masterdiv.style.position='absolute';var t=document.createElement("table");t.border=0;t.align="left";t.cellPadding=0;t.cellSpacing=0;if(undefined!=opt['width']){t.style.width=opt['width']+'px';}
...
var tb=document.createElement("tbody");if(undefined==param['notop']){var toprow=document.createElement("div");var titlecell=document.createElement("td");titlecell.noWrap=true;titlecell.align='right';titlecell.style.fontFamily='arial';titlecell.style.fontSize='11px';titlecell.style.backgroundColor=bgcolor;if(opt.titlecolor){titlecell.style.color=opt.titlecolor;}
...
masterdiv.style.display='block';if(opt.center){var xy=centerObject(masterdiv);var x=0;var y=0;var cvalue=opt.center+'';if(cvalue.indexOf('x')!=-1){x=xy[0];}
else if(cvalue.indexOf('y')!=-1){y=xy[1];}
else{x=xy[0];y=xy[1];}
if(undefined!=opt.x){xvalue=opt.x+'';if(xvalue.indexOf('+')!=-1){x=Math.round(MouseX+parseInt(xvalue,10));}
else if(xvalue.indexOf('-')!=-1){x=Math.round(MouseX-Math.abs(parseInt(xvalue,10)));}
else{x=Math.round(opt.x);}
if(x<0){x=0;}}
if(undefined!=opt.y){yvalue=opt.y+'';if(yvalue.indexOf('+')!=-1){y=Math.round(MouseY+parseInt(yvalue,10));}
...
else if(opt.screen){showOnScreen(masterdiv);}
else if(opt.mouse){x=0;y=0;cvalue=opt.mouse+'';if(cvalue.indexOf('x')!=-1){x=MouseX;}
...
if(opt.showtime){t=Math.round(opt.showtime*1000);popupdiv_timeout=setTimeout("removeDivOnExit('"+opt.id+"',1)",t);}
...
function removeId(divid){var obj=getObject(divid);if(undefined==obj){return false;}
...
return null;}
function setObjectPos(obj,x,y){var sObj=getObject(obj);if(undefined==sObj){return false;}
...
let sObj=getObject(obj);if(undefined==sObj){return false;}
...
if(undefined!=e){if(checkMouseLeave(this,e)){this.style.display='none';}}};return false;}
...
function getViewPort(){let viewport={};if(typeof window.innerWidth!='undefined'){viewport.w=window.innerWidth;viewport.h=window.innerHeight;}
else if(typeof document.documentElement!='undefined'&&typeof document.documentElement.clientWidth!='undefined'&&document.documentElement.clientWidth!=0){viewport.w=document.documentElement.clientWidth;viewport.h=document.documentElement.clientHeight;}
...
function initSpinWheel(){let wheels=document.querySelectorAll('select[data-behavior="spinwheel"]');if(undefined==wheels||wheels.length==0){return false;}
...
wheels[w].dataset.initialized=1;if(undefined!=wheels[w].dataset.colors){wcolors=wheels[w].dataset.colors.split(',');}
else{wcolors=colors;}
...
function clicked(evt){let w=parseInt(evt.currentTarget.dataset.wheel_index);if(undefined==wheels[w].dataset.clicked){wheels[w].dataset.clicked=1;}
else{wheels[w].dataset.clicked=parseInt(wheels[w].dataset.clicked)+1;}
if(undefined!=wheels[w].dataset.max){let max=parseInt(wheels[w].dataset.max);if(wheels[w].dataset.clicked>max){return false;}}
wheels[w].angVel=rand(0.25,0.35);return false;}
...
return false;};}
navbars=document.querySelectorAll('[data-toggle="dropdown"]');for(n=0;n<navbars.length;n++){navbars[n].onclick=function(e){cancel(e);let p=getParent(this);if(undefined!=p){if(p.className.indexOf('open')==-1){addClass(p,'open');}
else{removeClass(p,'open');}}
return false;};}
navbars=document.querySelectorAll('[data-toggle="collapse"]');for(n=0;n<navbars.length;n++){navbars[n].onclick=function(){var t=this.getAttribute('data-target');if(undefined==t){return false;}
var tdiv=document.querySelector(t);if(undefined==tdiv){return false;}
...
setText(navEls[n],'');try{paypal.Buttons(paypal_checkout).render('#'+paypal_checkout.renderid);}
catch(e){if(undefined!=navEls[n].dataset.onfail){parent.window[navEls[n].dataset.onfail](e);}
else{console.log('paypal_checkout - paypal.Buttons render failed');console.log(e);}}}}
...
for(let i=0;i<els.length;i++){els[i].textContent=v;simulateEvent(els[i],'change');}});}}
else{console.log("data-copyvalue error - missing data-id attribute");}}
...
if(in_array("utcclock",behaviors)){var id=navEls[n].getAttribute('id');if(id){startUTCClock(id,1);}}
else if(in_array("countdown",behaviors)){var id=navEls[n].getAttribute('id');if(id){countDown(id);}}
else if(in_array("countdowntime",behaviors)){var id=navEls[n].getAttribute('id');if(id){countdownTime(id);}}
else if(in_array("countdowndate",behaviors)){let id=navEls[n].getAttribute('id');if(id){let year=navEls[n].dataset.year||navEls[n].getAttribute('year');let month=navEls[n].dataset.month||navEls[n].getAttribute('month');let day=navEls[n].dataset.day||navEls[n].getAttribute('day');let hour=navEls[n].dataset.hour||navEls[n].getAttribute('hour');let minute=navEls[n].dataset.minute||navEls[n].getAttribute('minute');let tz=navEls[n].dataset.tz||navEls[n].getAttribute('tz');countDownDate(id,year,month,day,hour,minute,tz);}}
if(in_array("drag",behaviors)){let head=navEls[n].dataset.head||navEls[n].getAttribute('head');let headobj=getObject(head);navEls[n].style.position='relative';if(undefined==headobj){Drag.init(navEls[n]);}
else{Drag.init(headobj,navEls[n]);}}
if(in_array("email-link",behaviors)){navEls[n].setAttribute('data-behavior','processed');if(undefined!=navEls[n].dataset.href){let email=navEls[n].dataset.href;email=email.replace(/^[\/]+/,'',email);email=email.replace(/[\/]+$/,'',email);email=email.replace(/[\/]/,'@',email);email=email.replace(/[\/]/,'.',email);navEls[n].href='mailto:'+email;}
else{let email=navEls[n].dataset.value||navEls[n].innerText;email=email.replace(/^[\/]+/,'',email);email=email.replace(/[\/]+$/,'',email);email=email.replace(/[\/]/,'@',email);email=email.replace(/[\/]/,'.',email);navEls[n].href='mailto:'+email;navEls[n].innerText=email;}}
...
if(in_array("fileupload",behaviors)){if(window.File&&window.FileReader&&window.FileList&&window.Blob){navEls[n].setAttribute('_bgcolor',navEls[n].style.backgroundColor||'');addEventHandler(navEls[n],"dragover",function(evt){cancelBubble(evt);},false);addEventHandler(navEls[n],"dragenter",function(evt){cancelBubble(evt);this.style.backgroundColor=this.getAttribute('data-color-over')||'rgba(249,249,153,0.35)';},false);addEventHandler(navEls[n],"drop",function(evt){evt=evt||window.event;cancelBubble(evt);this.style.backgroundColor=this.getAttribute('_bgcolor');fileUploadBehavior(evt,this);},false);}
else{setText(navEls[n],'Fileupload via dragdrop is not supported in your browser.');}}
...
else if(in_array("dropdown",behaviors)){var dname=navEls[n].dataset.display||navEls[n].getAttribute('display');if(dname){navEls[n].style.position='relative';addEventHandler(navEls[n],'click',function(e){let dname=navEls[n].dataset.display||navEls[n].getAttribute('display');dObj=getObject(dname);if(dObj){if(dObj.style.display=='block'){return true;}
...
if(undefined!=e){if(checkMouseLeave(this,e)){let dname=navEls[n].dataset.display||navEls[n].getAttribute('display');dObj=getObject(dname);if(dObj){var hide=0;if(undefined!=dObj.className){if(dObj.className.indexOf("current")==-1){hide++;}}
...
else if(in_array("menu",behaviors)){var dname=navEls[n].dataset.display||navEls[n].getAttribute('display');if(dname){addEventHandler(navEls[n],'click',function(e){var dname=this.dataset.display||this.getAttribute('display');dObj=getObject(dname);if(undefined!=dObj){if(undefined!=dObj.dataset.display){if(dObj.style.display==dObj.dataset.display){return true;}
dObj.style.display=dObj.dataset.display;}
else{if(dObj.style.display=='block'){return true;}
...
dObj.style.position='absolute';dObj.style.top=y+"px";dObj.style.left=x+"px";}});addEventHandler(navEls[n],'mouseover',function(e){var dname=this.dataset.display||this.getAttribute('display');dObj=getObject(dname);if(undefined!=dObj){if(undefined!=dObj.dataset.display){if(dObj.style.display==dObj.dataset.display){return true;}
dObj.style.display=dObj.dataset.display;}
else{if(dObj.style.display=='block'){return true;}
dObj.style.display='block';}}
var dmouse=this.getAttribute('mouse');var dx=this.getAttribute('x');var dy=this.getAttribute('y');if(undefined!=dmouse){var x=0;var y=0;if(dmouse.indexOf('x')!=-1){x=MouseX;}
...
if(undefined!=e){if(checkMouseLeave(this,e)){var dname=this.dataset.display||this.getAttribute('display');dObj=getObject(dname);if(dObj){var hide=0;if(undefined!=dObj.className){if(dObj.className.indexOf("current")==-1){hide++;}}
else{var cclass=dObj.getAttribute('class');if(undefined==cclass){hide++;}
else{if(cclass.indexOf("current")==-1){hide++;}}}
if(hide){dObj.style.display='none';}}
var onhide=this.getAttribute('onhide');if(onhide){let jsfunc=new Function(onhide);jsfunc();}}}});}}
...
else{txt+='<div id="'+id+'_nav_'+n+'" data-tooltip="'+navtitle+'" data-tooltip_position="bottom" class=""  onclick="slideShow(\''+id+'\','+n+');"></div>';}}
setText(navobj,txt);}}
slideShow(id,0,t);}}
...
if(in_array("stopwatch",behaviors)){var id=navEls[n].getAttribute('id');if(id){stopWatch(id,0);}}
else if(in_array("timer_verbose",behaviors)){var id=navEls[n].getAttribute('id');if(undefined!=id){var t=parseInt(getText(navEls[n]),10);if(!isNaN(t)){navEls[n].setAttribute('timer_verbose',t);timerVerbose(id,1);}}}
else if(in_array("time_verbose",behaviors)){var s=getText(navEls[n]);if(!isNaN(s)){var secs=parseInt(s,10);if(!isNaN(secs)){var t=verboseTime(secs);if(t.length){setText(navEls[n],t);}}}}
...
function initEnlighterJs(){let els=document.querySelectorAll('[data-enlighter-language]');for(let i=0;i<els.length;i++){if(undefined==els[i].dataset.enlightened){let params={language:els[i].dataset.enlighterLanguage,rawButton:false,infoButton:false,windowButton:false};if(undefined!=els[i].dataset.enlighterRawbutton&&els[i].dataset.enlighterRawbutton.indexOf('false')==-1){params.rawButton=true;}
if(undefined!=els[i].dataset.enlighterWindowbutton&&els[i].dataset.enlighterWindowbutton.indexOf('false')==-1){params.windowButton=true;}
if(undefined!=els[i].dataset.enlighterInfobutton&&els[i].dataset.enlighterInfobutton.indexOf('false')==-1){params.infoButton=true;}
...
return false;}
function initFlatpickr(){let els=document.querySelectorAll('input[data-behavior="flatpickr"]');if(!els.length){return false;}
...
els[i].dataset.initflatpickr=1;let lang=els[i].dataset.lang||nlang;let config={errorHandler:function(err){alert(err);return false;}};config.prevArrow="<span class='icon-arrow-left'></span>";config.nextArrow="<span class='icon-arrow-right'></span>";for(k in els[i].dataset){let v=els[i].dataset[k];switch(k.toLowerCase()){case'altformat':k='altFormat';break;case'altinput':k='altInput';break;case'altinputclass':k='altInputClass';break;case'allowinput':k='allowInput';break;case'allowinvalidpreload':k='allowInvalidPreload';break;case'appendto':k='appendTo';v=document.querySelector(v)||getObject(v);break;case'ariadateformat':k='ariaDateFormat';break;case'conjunction':k='conjunction';break;case'clickopens':k='clickOpens';break;case'dateformat':k='dateFormat';break;case'defaultdate':if(undefined!=els[i].value&&els[i].value.length){k='';}
else{k='defaultDate';}
...
let letter=fels[i].dataset.navigateFocus.replace('Ctrl+','');if(letter.length==1){let str=span.innerText;if(undefined!=span.dataset.navigateColor){span.innerHTML=str.replace(letter,'<span style="border-bottom:1px solid '+span.dataset.navigateColor+';">'+letter+'</span>');}
else{span.innerHTML=str.replace(letter,'<span style="border-bottom:1px solid #d70000;">'+letter+'</span>');}}}
...
let letter=fels[i].dataset.navigateFocus.replace('Alt+','');if(letter.length==1){let str=span.innerText;if(undefined!=span.dataset.navigateColor){span.innerHTML=str.replace(letter,'<span style="border-bottom: 1px solid '+span.dataset.navigateColor+';">'+letter+'</span>');}
else{span.innerHTML=str.replace(letter,'<span style="border-bottom:1px solid #0000e8;">'+letter+'</span>');}}}
...
navels[i].setAttribute('data-navigate',i);let group='navigate';if(undefined==navels[i].dataset.navigateGroup){navels[i].setAttribute('data-navigate-group',group);}
else{group=navels[i].dataset.navigateGroup;}
...
document.onkeydown=function(e){e=e||window.event;if(undefined==e){return true;}
...
if(nav.shift){nav.focus='Shift+'+nav.key;}
else if(nav.ctrl&&nav.alt){nav.focus='Ctrl+Alt+'+nav.key;}
else if(nav.alt){nav.focus='Alt+'+nav.key;}
else if(nav.ctrl){nav.focus='Ctrl+'+nav.key;}
else{nav.focus=nav.key;}
if(nav.focus.length){nav.el=document.querySelector('[data-navigate-focus="'+nav.focus+'" i]');if(undefined!=nav.el){e.preventDefault();e.stopPropagation();nav.el.focus();simulateEvent(nav.el,'click');return false;}}
let fel=document.activeElement;if(undefined==fel){return true;}
if(undefined==fel.dataset.navigate){return true;}
if(undefined!=fel.dataset.navigateAll){e.preventDefault();e.stopPropagation();let func=fel.dataset.navigateAll;if(function_exists(func)){window[func](fel);}
else{eval(func);}
return false;}
let index=parseInt(fel.dataset.navigate);let group=fel.dataset.navigateGroup;let gindex=parseInt(fel.dataset.navigateGroupIndex);let next=gindex+1;let nextel=document.querySelector('[data-navigate-group="'+group+'"][data-navigate-group-index="'+next+'"]');let prev=gindex-1;let prevel=document.querySelector('[data-navigate-group="'+group+'"][data-navigate-group-index="'+prev+'"]');switch(parseInt(nav.keycode)){case 13:if(undefined!=fel.dataset.navigateEnter){e.preventDefault();e.stopPropagation();let func=fel.dataset.navigateEnter;let funcEl=document.querySelector(func);if((func.indexOf('Ctrl')==0||func.indexOf('Alt')==0)&&func.indexOf('+')!=-1){return navigateSetFocus(func);}
else if(func.length==0||func=='false'||func.indexOf('return false')==0){}
else if(func.indexOf('group:')==0){let group=func.replace('group:','');return navigateSetFocusGroup(group);}
else if(function_exists(func)){window[func](fel);}
else{eval(func);}
return false;}
break;case 13:if(undefined!=fel.dataset.navigateTab){e.preventDefault();e.stopPropagation();let func=fel.dataset.navigateTab;if((func.indexOf('Ctrl')==0||func.indexOf('Alt')==0)&&func.indexOf('+')!=-1){return navigateSetFocus(func);}
else if(func.length==0||func=='false'||func.indexOf('return false')==0){}
else if(func.indexOf('group:')==0){let group=func.replace('group:','');return navigateSetFocusGroup(group);}
else if(function_exists(func)){window[func](fel);}
else{eval(func);}
return false;}
break;case 38:if(undefined!=fel.dataset.navigateUp){e.preventDefault();e.stopPropagation();let func=fel.dataset.navigateUp;if((func.indexOf('Ctrl')==0||func.indexOf('Alt')==0)&&func.indexOf('+')!=-1){return navigateSetFocus(func);}
else if(func.length==0||func=='false'||func.indexOf('return false')==0){}
else if(func.indexOf('group:')==0){let group=func.replace('group:','');return navigateSetFocusGroup(group);}
else if(function_exists(func)){window[func](fel);}
else{eval(func);}
return false;}
if(undefined!=prevel){e.preventDefault();e.stopPropagation();simulateEvent(fel,'mouseout');prevel.focus();prevel.setAttribute('data-navigate-key',nav.keycode);if(undefined!=prevel.dataset.navigateClick){let clickFunc=prevel.dataset.navigateClick;if(clickFunc.length==0||clickFunc=='false'||clickFunc.indexOf('return false')==0){}
else if(function_exists(clickFunc)){window[clickFunc](fel);}
else{eval(clickFunc);}
return false;}
simulateEvent(prevel,'click');return false;}
break;case 40:if(undefined!=fel.dataset.navigateDown){e.preventDefault();e.stopPropagation();let func=fel.dataset.navigateDown;if((func.indexOf('Ctrl')==0||func.indexOf('Alt')==0)&&func.indexOf('+')!=-1){return navigateSetFocus(func);}
else if(func.length==0||func=='false'||func.indexOf('return false')==0){}
else if(func.indexOf('group:')==0){let group=func.replace('group:','');return navigateSetFocusGroup(group);}
else if(function_exists(func)){window[func](fel);}
else{eval(func);}
return false;}
if(undefined!=nextel){e.preventDefault();e.stopPropagation();simulateEvent(fel,'mouseout');nextel.focus();nextel.setAttribute('data-navigate-key',nav.keycode);if(undefined!=nextel.dataset.navigateClick){let clickFunc=nextel.dataset.navigateClick;if(clickFunc.length==0||clickFunc=='false'||clickFunc.indexOf('return false')==0){}
else if(function_exists(clickFunc)){window[clickFunc](fel);}
else{eval(clickFunc);}
return false;}
simulateEvent(nextel,'click');return false;}
break;case 39:if(undefined!=fel.dataset.navigateRight){e.preventDefault();e.stopPropagation();let func=fel.dataset.navigateRight;if((func.indexOf('Ctrl')==0||func.indexOf('Alt')==0)&&func.indexOf('+')!=-1){return navigateSetFocus(func);}
else if(func.length==0||func=='false'||func.indexOf('return false')==0){}
else if(func.indexOf('group:')==0){let group=func.replace('group:','');return navigateSetFocusGroup(group);}
else if(function_exists(func)){window[func](fel);}
else{eval(func);}
...
return false;}
simulateEvent(nextel,'click');return false;}
break;case 37:if(undefined!=fel.dataset.navigateLeft){e.preventDefault();e.stopPropagation();let func=fel.dataset.navigateLeft;if((func.indexOf('Ctrl')==0||func.indexOf('Alt')==0)&&func.indexOf('+')!=-1){return navigateSetFocus(func);}
else if(func.length==0||func=='false'||func.indexOf('return false')==0){}
else if(func.indexOf('group:')==0){let group=func.replace('group:','');return navigateSetFocusGroup(group);}
else if(function_exists(func)){window[func](fel);}
else{eval(func);}
...
if(scrollPosition<sticky_start&&list[i].style.position!="relative"){list[i].style.position="relative";list[i].style.top="";list[i].style.width="";if(undefined!=list[i].getAttribute('sticky_z')){list[i].style.zIndex=list[i].getAttribute('sticky_z');}
else{list[i].style.zIndex='';}}}}
function removeTinyMCE(id){if(tinyMCE.getInstanceById(id)){tinyMCE.execCommand('mceRemoveControl',false,id);}}
...
function addEventHandler(elem,evnt,func){if(elem.addEventListener){elem.addEventListener(evnt,func,commonPassiveEventListener(false));}
...
function removeEventHandler(elem,evnt,func){if(elem.removeEventListener){elem.removeEventListener(evnt,func,false);}
...
var tinymceInitialized=0;function tinymceInitialize(txtid,px){var bar1="bold,italic,underline,|,forecolor,backcolor,|,justifyleft,justifycenter,justifyright,|,bullist,numlist,outdent,indent,|,charmap,image,media,link,unlink,formatselect,code,fullscreen,fontsizeselect";var bar2="";var bar3="";var ok=tinyMCE.init({mode:"none",theme:"advanced",plugins:"table,advhr,inlinepopups,contextmenu,media,paste,fullscreen,visualchars,nonbreaking,xhtmlxtras,template,advlist,advimage",submit_patch:false,paste_auto_cleanup_on_paste:true,paste_text_replacements:[[/\u2026/g,"..."],[/[\x93\x94\u201c\u201d]/g,'"'],[/[\x60\x91\x92\u2018\u2019]/g,"'"],[/.*((https?|ssh|ftp|file):\/\/\S+).*/gi,'<a href="$1">$1</a>'],[/.*((https?|ssh|ftp|file):\/\/\S+((\.)bmp|gif|jpe?g|png|psd|tif?f)).*/gi,'<img src="$1" alt="image" />']],theme_advanced_buttons1:bar1,theme_advanced_buttons2:bar2,theme_advanced_buttons3:bar3,theme_advanced_buttons4:"",theme_advanced_toolbar_location:"top",theme_advanced_toolbar_align:"left",theme_advanced_resizing:true});}
function fullScreenToggle(id){var obj=getObject(id);if(undefined==obj){return false;}
var set=obj.getAttribute('fullscreen');if(undefined==set||set==0){var wh=getWidthHeight(obj);obj.setAttribute('fullscreen',1);obj.setAttribute('width_ori',wh[0]);obj.setAttribute('height_ori',wh[1]);obj.style.width=document.body.clientWidth+'px';obj.style.height=document.body.clientHeight+'px';}
else{obj.setAttribute('fullscreen',0);obj.style.width=obj.getAttribute('width_ori')+'px';obj.style.height=obj.getAttribute('height_ori')+'px';}}
...
function replaceText(text,textarea){if(typeof(textarea.caretPos)!="undefined"&&textarea.createTextRange){var caretPos=textarea.caretPos;caretPos.text=caretPos.text.charAt(caretPos.text.length-1)==' '?text+' ':text;caretPos.select();}
else if(typeof(textarea.selectionStart)!="undefined"){var begin=textarea.value.substr(0,textarea.selectionStart);var end=textarea.value.substr(textarea.selectionEnd);var scrollPos=textarea.scrollTop;textarea.value=begin+text+end;if(textarea.setSelectionRange){textarea.focus();textarea.setSelectionRange(begin.length+text.length,begin.length+text.length);}
textarea.scrollTop=scrollPos;}
else{textarea.value+=text;textarea.focus(textarea.value.length-1);}}
function surroundText(text1,text2,textarea){if(typeof(textarea.caretPos)!="undefined"&&textarea.createTextRange){var caretPos=textarea.caretPos;caretPos.text=caretPos.text.charAt(caretPos.text.length-1)==' '?text1+caretPos.text+text2+' ':text1+caretPos.text+text2;caretPos.select();}
...
var objs=divobj.getElementsByTagName(tag);if(objs.length==0){alert('SlideShow Error: - No '+tag+' elements found');return false;}
...
if(undefined!=navobj){var navdiv=getObject(divid+'_nav_'+i);addClass(navdiv,'active');}}
else{removeClass(objs[i],'opaque');if(undefined!=navobj){var navdiv=getObject(divid+'_nav_'+i);removeClass(navdiv,'active');}
caption='';}
setText(divid+'_caption',caption);}
idx=Math.round(idx+1);TimoutArray[id]=setTimeout("slideShow('"+divid+"',"+idx+","+s+")",ms);}
function stopWatch(id){clearTimeout(TimoutArray[id]);var obj=getObject(id);if(undefined==obj){return false;}
if(obj.dataset.behavior.indexOf('stopwatch')==-1){return false;}
if(undefined!=obj.dataset.stop&&parseInt(obj.dataset.stop)==1){return false;}
...
var attr=getAllAttributes(obj);var number;if(undefined!=attr['data-countdown']){number=parseInt(attr['data-countdown']);}
...
number--;obj.setAttribute('data-countdown',number);var timer;if(undefined!=attr['data-timer']){timer=parseInt(attr['data-timer']);}
else if(undefined!=attr['timer']){timer=parseInt(attr['timer']);}
else{timer=75;}
var skip=0;if(undefined!=document.activeElement&&undefined!=document.activeElement.type&&document.activeElement.type.indexOf('text')!=-1){skip=1;}
else if(undefined!=getObject('skip_ajax_timer')){skip=1;}
obj.setAttribute('data-skip',skip);if(skip==0&&number<=0){if(undefined!=attr['url']||undefined!=attr['data-url']){var url;if(undefined!=attr['url']){url=attr['url'];}
...
params['showprocessing']=false;params['timer']=timer;ajaxGet(parts[0],attr['id'],params);}
else if(undefined!=attr['function']||undefined!=attr['data-function']){var func;if(undefined!=attr['function']){func=attr['function'];}
...
let cb='';if(undefined!=obj.getAttribute('data-callback')){cb=obj.getAttribute('data-callback');}
else if(undefined!=obj.getAttribute('callback')){cb=obj.getAttribute('callback');}
let stop=0;if(cb.length>0){let cbfunc=cb+"('"+id+"','"+number+"')";let jsfunc=new Function(cbfunc);jsfunc();}
else if(number==0){if(undefined!=obj.dataset.end){let func=obj.dataset.end;console.log(func);if(function_exists(func)){window[func](obj);}
else{let fn=new Function(func)();console.log(fn);}}
else if(undefined!=obj.dataset.onzero){let func=obj.dataset.onzero;if(function_exists(func)){window[func](obj);}}
stop=1;}
...
if(undefined!=divobj.dataset.second){bstr=bstr+':'+divobj.dataset.second;}
else{bstr=bstr+':00';}
...
function countdownTime(id){let el=getObject(id);if(undefined==el){return false;}
...
if(repeat==0){el.innerHTML='00:00:00';if(undefined!=el.dataset.end){let func=el.dataset.end;if(function_exists(func)){window[func](el);}}}
else{el.innerText=h+':'+m+':'+s;}}
else if(timeArray.length==2){let t=new Date(c.getFullYear(),c.getMonth(),c.getDay(),c.getHours(),timeArray[0],timeArray[1]-1);let m=t.getMinutes();let s=t.getSeconds();if(m==0&&s==0){repeat=0;}
if(m.toString().length==1){m='0'+m;}
if(s.toString().length==1){s='0'+s;}
if(repeat==0){el.innerHTML='00:00';if(undefined!=el.dataset.end){let func=el.dataset.end;if(function_exists(func)){window[func](el);}}}
...
function timerVerbose(id,live){var obj=getObject(id);if(undefined==obj){return false;}
...
function startUTCClock(id,live){if(undefined==live){live=false;}
...
var h=h+offset;var m=dt.getUTCMinutes();var s=dt.getUTCSeconds();var p=" am";if(h>24){h=h-24;p='am';}
...
try{let jsfunc=new Function(str);str=jsfunc();setText(id,str);}
...
return false;}
var Drag={obj:null,init:function(o,oRoot,minX,maxX,minY,maxY,bSwapHorzRef,bSwapVertRef,fXMapper,fYMapper){o=getObject(o);o.onmousedown=Drag.start;o.hmode=bSwapHorzRef?false:true;o.vmode=bSwapVertRef?false:true;o.root=oRoot&&oRoot!=null?oRoot:o;if(o.hmode&&isNaN(parseInt(o.root.style.left))){o.root.style.left="0px";}
...
el=getObject(el);wasqlMarkerLoad();fontIconCursor(el,cl);addEventHandler(el,'click',function(e){cancelBubble(e);if(e.altKey&&e.ctrlKey){wasqlMarkerForm();return false;}
return false;});}
