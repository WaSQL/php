...
var onhide=this.getAttribute('onhide');if(onhide){let jsfunc=new Function(onhide);jsfunc()}}}})}}else if(in_array("menu",behaviors)){var dname=navEls[n].dataset.display||navEls[n].getAttribute('display');if(dname){addEventHandler(navEls[n],'click',function(e){var dname=this.dataset.display;if(undefined==dname){dname=this.getAttribute('display')}
...
dObj.style.position='absolute';dObj.style.top=y+"px";dObj.style.left=x+"px"}});addEventHandler(navEls[n],'mouseover',function(e){var dname=navEls[n].dataset.display||navEls[n].getAttribute('display');dObj=getObject(dname);if(undefined!=dObj){if(undefined!=dObj.dataset.display){if(dObj.style.display==dObj.dataset.display){return!0}
dObj.style.display=dObj.dataset.display}else{if(dObj.style.display=='block'){return!0}
dObj.style.display='block'}}
var dmouse=this.getAttribute('mouse');var dx=this.getAttribute('x');var dy=this.getAttribute('y');if(undefined!=dmouse){var x=0;var y=0;if(dmouse.indexOf('x')!=-1){x=MouseX}else if(dmouse.indexOf('y')!=-1){y=MouseY}else{x=MouseX;y=MouseY}
if(undefined!=dx){xvalue=dx+'';if(xvalue.indexOf('+')!=-1){x=Math.round(MouseX+parseInt(xvalue))}else if(xvalue.indexOf('-')!=-1){x=Math.round(MouseX-Math.abs(parseInt(xvalue)))}else{x=Math.round(Math.abs(parseInt(xvalue)))}}
...
if(undefined!=e){if(checkMouseLeave(this,e)){var dname=navEls[n].dataset.display||navEls[n].getAttribute('display');dObj=getObject(dname);if(dObj){var hide=0;if(undefined!=dObj.className){if(dObj.className.indexOf("current")==-1){hide++}}else{var cclass=dObj.getAttribute('class');if(undefined==cclass){hide++}else{if(cclass.indexOf("current")==-1){hide++}}}
if(hide){dObj.style.display='none'}}
...
function countDownDate(divid,yr,m,d,hr,min,tz){let args=new Array(divid,yr,m,d,hr,min,tz);if(undefined==tz){tz=''}
