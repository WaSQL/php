...
const rand=(m,M)=>Math.random()*(M-m)+m;const PI=Math.PI;const TAU=2*PI;const audio=new Audio('/wfiles/tick.mp3');const friction=0.991;let angVel=0;let ang=0;let colors=['#e70697','#fff200','#f6989d','#ee1c24','#3cb878','#f26522','#a186be','#00aef0'];let isw=this;for(let w=0;w<wheels.length;w++){if(undefined!=wheels[w].dataset.initialized){continue}
...
wheels[w].tot=wheels[w].sectors.length;wheels[w].dia=wheels[w].ctx.canvas.width;wheels[w].rad=wheels[w].dia/2;wheels[w].arc=TAU/wheels[w].sectors.length;wheels[w].angVel=0;for(let s=0;s<wheels[w].sectors.length;s++){wheels[w].ang=wheels[w].arc*s;wheels[w].ctx.save();wheels[w].ctx.beginPath();wheels[w].ctx.fillStyle=wheels[w].sectors[s].color;wheels[w].ctx.moveTo(wheels[w].rad,wheels[w].rad);wheels[w].ctx.arc(wheels[w].rad,wheels[w].rad,wheels[w].rad,wheels[w].ang,wheels[w].ang+wheels[w].arc);wheels[w].ctx.lineTo(wheels[w].rad,wheels[w].rad);wheels[w].ctx.fill();wheels[w].ctx.translate(wheels[w].rad,wheels[w].rad);wheels[w].ctx.rotate(wheels[w].ang+wheels[w].arc/2);wheels[w].ctx.textAlign="right";wheels[w].ctx.fillStyle="#000";wheels[w].ctx.font="bold 20px sans-serif";wheels[w].ctx.fillText(wheels[w].sectors[s].label,wheels[w].rad-10,10);wheels[w].ctx.restore()}}
