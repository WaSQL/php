...
tipdiv.innerHTML=txt;let pos=findPos(obj);let x=y=h=w=th=0;h=obj.offsetHeight||obj.innerHeight||commonGetHeight(obj)||24;w=obj.offsetWidth||obj.innerWidth||commonGetWidth(obj)||24;if(params.position=='bottom'){y=pos.y+h+6;x=pos.x}else{tipdiv.setAttribute("class",'left');y=pos.y;x=pos.x+w+6}
...
let w=getWidth(sObj);let h=getHeight(sObj);let vp=getViewportSize();let x=Math.round((vp.w/2)-(w/2));let y=Math.round((vp.h/2)-(h/2));sObj.style.position='fixed';sObj.style.left=x+'px';if(undefined==y){y=10}
if(y<10){y=10}
