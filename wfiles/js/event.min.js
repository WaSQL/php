...
wheels[w].button.dataset.wheel_index=w;wheels[w].button.addEventListener("click",clicked,!1);wheels[w].wrapper.appendChild(wheels[w].button);wheels[w].sectors=new Array();let c=0;for(let i=0;i<wheels[w].options.length;i++){let label=trim(wheels[w].options[i].text);if(label.length==0){continue}
