...
if(in_array("email-link",behaviors)){var email=navEls[n].dataset.value||navEls[n].innerText;email=email.replace(/^[\/]+/,'',email);email=email.replace(/[\/]+$/,'',email);email=email.replace(/[\/]/,'@',email);email=email.replace(/[\/]/,'.',email);navEls[n].href='mailto:'+email;navEls[n].setAttribute('data-behavior','processed');navEls[n].innerText=email}
if(in_array("loadtextfile",behaviors)){loadTextFileInit(navEls[n])}
if(in_array("markers",behaviors)){wasqlMarkerInit(navEls[n])}
if(in_array("signature",behaviors)){let pencolor=navEls[n].getAttribute('data-color')||'#000';signaturePad=new SignaturePad(navEls[n],{penColor:pencolor})}
...
simulateEvent(prevel,'click');return!1}
break}};return!0}
async function eventRecorderAudio(el){if(undefined==el.recorder){el.recorder={}}
if(undefined==el.recordedchunks){el.recordedchunks=[]}
if(undefined!=el.recorder.state&&el.recorder.state=="recording"){el.recorder.stop();el.stream.getAudioTracks()[0].stop();let mic=el.querySelector('span.icon-mic');if(undefined!=mic){mic.classList.remove('w_red')}}else{el.stream=null;try{el.stream=await navigator.mediaDevices.getUserMedia({audio:!0})}catch(err){console.log(err);return!1}
if(undefined!=el.stream){el.recorder=new MediaRecorder(el.stream);el.recorder.el=el;el.recorder.ondataavailable=function(e){if(e.data.size>0){this.el.recordedchunks.push(e.data);let blob=new Blob(this.el.recordedchunks,{type:"audio/webm"});let url=URL.createObjectURL(blob);let reader=new FileReader();reader.readAsDataURL(blob);reader.el=this.el;console.log(reader);reader.onloadend=function(){let base64String=this.result;let fld=document.querySelector('#'+this.el.dataset.id);console.log(this.el.dataset.id);console.log(fld);if(undefined!=fld){fld.textContent='input1.webm;input1.webm;'+base64String}}}};el.recorder.start();let mic=el.querySelector('span.icon-mic');if(undefined!=mic){mic.classList.add('w_red')}}}}
...
return!1})}
function resizeSignatureWidthHeight(){var list=document.querySelectorAll('.w_signature');for(var i=0;i<list.length;i++){var pwh=getWidthHeight(list[i]);list[i].width=pwh[0];list[i].height=pwh[1]}}
