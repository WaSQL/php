var MouseX=0;var MouseY=0;var cursor={x:0,y:0};function eventJSInit(){if(undefined!=this.event_initialized){return!1}
this.event_initialized=1;if(document.onmousemove){document.onmousemove=mouseMove}else if(document.addEventListener){document.addEventListener("mousedown",mouseMove,commonPassiveEventListener(!1));document.addEventListener("mouseup",mouseMove,commonPassiveEventListener(!1));document.addEventListener("mousemove",mouseMove,commonPassiveEventListener(!1))}else if(document.attachEvent){document.attachEvent("onmousedown",mouseMove);document.attachEvent("onmouseup",mouseMove);document.attachEvent("onmousemove",mouseMove)}else if(document.captureEvents){document.captureEvents(Event.MOUSEDOWN|Event.MOUSEMOVE|Event.MOUSEUP)}
addEventHandler(window,'resize',function(){resizeSignatureWidthHeight()})}
function eventBuildOnLoad(){if(document.readyState==="complete"){eventProcessBuildOnLoad()}else{if(window.addEventListener)
window.addEventListener("load",function(){eventProcessBuildOnLoad()},!1);else if(window.attachEvent){var r=window.attachEvent("onload",function(){eventProcessBuildOnLoad()});return r}}}
function eventProcessBuildOnLoad(){let list=document.querySelectorAll('[data-onload]');for(let i=0;i<list.length;i++){if(undefined!=list[i].getAttribute('data-onload-ex')){continue}
list[i].setAttribute('data-onload-ex',new Date().getTime());let str=list[i].getAttribute('data-onload');try{let strfunc=new Function(str);strfunc()}catch(error){console.log('eventProcessBuildOnLoad Javascript Failed:');console.log(' - Dynamic function:');console.log(str);console.log(' - Error message:');console.log(error)}}}
function dragSortEnable(querystring){let sortableLists=document.querySelectorAll(querystring);Array.prototype.map.call(sortableLists,list=>{dragSortEnableDragList(list)})}
function dragSortEnableDragList(list){Array.prototype.map.call(list.children,item=>{dragSortEnableDragItem(item)})}
function dragSortEnableDragItem(item){item.setAttribute("draggable",!0);item.ondrag=dragSortHandleDrag;item.ondragend=dragSortHandleDrop}
function dragSortHandleDrag(item){let selectedItem=item.target;let list=selectedItem.parentNode;let x=event.clientX;let y=event.clientY;selectedItem.classList.add("dragsort-active");let swapItem=document.elementFromPoint(x,y)===null?selectedItem:document.elementFromPoint(x,y);if(list===swapItem.parentNode){swapItem=swapItem!==selectedItem.nextSibling?swapItem:swapItem.nextSibling;list.insertBefore(selectedItem,swapItem)}}
function dragSortHandleDrop(item){item.target.classList.remove("dragsort-active")}
function loadTextFileInit(el){var loadTextFile=getObject(el);if(undefined==el){console.log('Error: invalid element',el);return}
loadTextFile.origBgcolor=loadTextFile.style.backgroundColor;addEventHandler(loadTextFile,"dragover",function(evt){evt.stopPropagation();evt.preventDefault()});addEventHandler(loadTextFile,"dragenter",function(evt){evt.stopPropagation();evt.preventDefault();this.style.backgroundColor='#f0f0f0'});addEventHandler(loadTextFile,"dragexit",function(evt){evt.stopPropagation();evt.preventDefault();this.style.backgroundColor=this.origBgcolor});addEventHandler(loadTextFile,"drop",function(evt){evt.stopPropagation();evt.preventDefault();this.style.backgroundColor=this.origBgcolor;var files=evt.dataTransfer.files;var obj=this;if(files.length==0&&undefined!=evt.dataTransfer.getData('text')){let txt=evt.dataTransfer.getData('text');let last=this.getAttribute('data-insert');if(txt.length&&last!=txt){this.setAttribute('data-insert',txt);insertAtCursor(this,txt)}
return!1}
for(var i=0;i<files.length;i++){var f=files[i];if(!f.type.match('text.*')){setText(this,'ERROR! '+f.name+' is not a text file');continue}
var reader=new FileReader();reader.targetObj=this;reader.onload=(function(theFile){return function(e){setText(this.targetObj,e.target.result)}})(f);reader.readAsText(f);loadTextFileInit(this);break}})}
function eventInitSticky(){var tlist=document.querySelectorAll('table.w_sticky');for(var t=0;t<tlist.length;t++){var tabletop=tlist[t].offsetTop;if(undefined!=tlist[t].getAttribute('data-top')){tabletop=tlist[t].getAttribute('data-top')}
var list=tlist[t].querySelectorAll('thead th');for(var i=0;i<list.length;i++){list[i].style.position='sticky';list[i].style.top=tabletop+'px'}}
list=document.querySelectorAll('.w_sticky');for(i=0;i<list.length;i++){if(undefined!=tlist[i].nodeName&&tlist[i].nodeName=='TABLE'){continue}else if(undefined!=tlist[i].tagName&&tlist[i].tagName=='TABLE'){continue}
t=list[i].offsetTop;list[i].style.position='sticky';list[i].style.top=t+'px'}}
function marquee(id){var mobj=getObject(id);if(undefined==mobj){return!1}
var mid=mobj.id;clearTimeout(TimoutArray[id]);var attr=getAllAttributes(mobj);if(undefined==attr.m){mobj.setAttribute('m',0);attr.m=0}
if(typeof(mobj.onmouseover)!='undefined'){mobj.onmouseover=function(){this.setAttribute('m',1)};mobj.onmouseout=function(e){if(undefined==e){e=fixE(e)}
if(undefined!=e){if(checkMouseLeave(this,e)){this.setAttribute('m',0)}}}}
if(undefined==attr.m||attr.m===0){var pxwh=getTextPixelWidthHeight(mobj);var mwh=getWidthHeight(mobj);var timer=20;if(undefined!=attr.timer){speed=attr.timer}
var direction='left';if(undefined!=attr.direction){direction=attr.direction}
switch(direction.toLowerCase()){case 'left':mobj.style.textAlign='right';mobj.style.whiteSpace='nowrap';mobj.style.paddingLeft='0px';if(undefined!=mobj.style.paddingRight){x=mobj.style.paddingRight;if(x.length){x=x.replace('px','');x=parseInt(x,10)}else{x=0}}
if(x<mwh[0]-pxwh[0]-2){mobj.style.paddingRight=parseInt(x+2,10)+'px'}else{mobj.style.paddingRight='0px'}
break;case 'right':mobj.style.textAlign='left';mobj.style.whiteSpace='nowrap';mobj.style.paddingRight='0px';if(undefined!=mobj.style.paddingLeft){x=mobj.style.paddingLeft;if(x.length){x=x.replace('px','');x=parseInt(x,10)}else{x=0}}
if(x<mwh[0]-pxwh[0]-2){mobj.style.paddingLeft=parseInt(x+2,10)+'px'}else{mobj.style.paddingLeft='0px'}
break;default:return!1;break}}
TimoutArray[id]=setTimeout("marquee('"+id+"')",timer);return!1}
function mouseMove(e){if(!e){e=window.event}
if(e.pageX||e.pageY){cursor.x=e.pageX;cursor.y=e.pageY}else if(e.clientX||e.clientY){if(document.body){if(document.documentElement){cursor.x=e.clientX+document.body.scrollLeft+document.documentElement.scrollLeft;cursor.y=e.clientY+document.body.scrollTop+document.documentElement.scrollTop}}}
...
if(undefined!=param.popup&&param.popup==1){popUpDiv(htm,param)}else{setText(param.id,htm)}}
...
this.cx+=(pX+this.sx-this.cx)/8;this.cy+=(pY+this.sy-this.cy)/8;this.sP(this.cx,this.cy);setTimeout(this.id+"_obj.floatIt()",40)};return el}
var changeState=new Array();var changeValue=new Array();var nicEditors=new Array();var OnLoad="";var schedulePageID='';function schedulePageRefresh(page,div,opts,ms){var id='schedulePageRefresh';clearTimeout(TimoutArray[id]);if(undefined==ms){ms=60000}
TimoutArray[id]=setTimeout('pageRefresh("'+page+'","'+div+'","'+opts+'")',ms)}
function scheduleAjaxGet(id,page,div,opts,ms,nosetprocess){clearTimeout(TimoutArray[id]);if(undefined==ms){ms=60000}
if(undefined==nosetprocess){nosetprocess=0}
TimoutArray[id]=setTimeout('pageRefresh("'+page+'","'+div+'","'+opts+'",'+nosetprocess+')',ms)}
function pageRefresh(page,div,opts,nosetprocess){if(undefined==document.getElementById(div)){return!1}
ajaxGet('/'+page,div,opts,'',60000,nosetprocess);return!1}
function processMultiComboBox(tid,cid,tcnt,cm,showvalues){var tbox=document.getElementById(tid);var cbox=document.getElementById(cid);if(cm&&cbox){if(cbox.checked){cbox.checked=!1}else{cbox.checked=!0}}
var list=GetElementsByAttribute('input','name',tid);var cnt=0;var val='';for(var i=0;i<list.length;i++){if(list[i].checked){if(showvalues){val+=list[i].value+","}
cnt++}}
if(showvalues){tbox.value=val}else{tbox.value=cnt+"/"+tcnt+" selected"}}
function setChangeState(tid){var el=document.getElementById(tid);changeState[tid]=el.value}
function setChangeValue(tid,val){changeValue[tid]=val}
function evalChange(tid){let jsfunc=new Function(changeValue[tid]);jsfunc()}
function hasChanged(tid){var el=document.getElementById(tid);var changed=0;var val=el.value;var old=changeState[tid];if(val!=old){changed++}
if(changed){return!0}
return!1}
function iframePopup(url,opts){if(undefined==opts){opts=new Object}
var htm='';if(undefined==opts.iwidth){opts.iwidth=500}
if(undefined==opts.iheight){opts.iheight=300}
if(undefined==opts.iscrolling){opts.iscrolling='auto'}
htm+='<div class="w_centerpop_title">'+opts.title+'</div>'+"\n";htm+='<div class="w_centerpop_content" style="padding:0px;">'+"\n";htm+='<iframe seamless="1" src="'+url+'" width="'+opts.iwidth+'" height="'+opts.iheight+'" frameborder="0" marginwidth="0" marginheight="0" scrolling="'+opts.iscrolling+'" align="center">Your browser does not support iframes.</iframe>';htm+='</div>'+"\n";centerpopDiv(htm,null,0);return!1}
function w_shareButton(url,t){window.open(url,'_blank','scrollbars=no, location=no, width=600, height=400, status=no, toolbar=no, menubar=no',!1);return!1}
function ajaxPopup(url,params,useropts){var pid='ajaxPopupDiv';var opt={id:pid,drag:1};if(useropts){var key;for(key in opt){if(undefined!=useropts[key]){opt[key]=useropts[key]}}
for(key in useropts){if(undefined==opt[key]){opt[key]=useropts[key]}}}
popUpDiv('<div class="w_bold w_lblue w_big"><img src="/wfiles/loading_blu.gif"> loading...please wait.</div>',opt);ajaxGet(url,opt.id+'_Body',params)}
function cancelBubble(e){var evt=e?e:window.event;if(evt.preventDefault){evt.preventDefault()}
if(evt.stopPropagation){evt.stopPropagation()}
if(evt.cancelBubble!==null){evt.cancelBubble=!0}}
function centerpopDiv(txt,rtimer,x){if(undefined==x){x=''}
var divid='centerpop'+x;if(undefined!=document.getElementById(txt)){txt=getText(txt)}
var params={id:divid,drag:1,notop:1,nobot:1,noborder:1,nobackground:1,bodystyle:"padding:0px;border:0px;background:none;"};if(undefined!=rtimer&&rtimer>0){params.showtime=rtimer}
popUpDiv('',params);setCenterPopText(divid,txt,{drag:!0,close_bot:!1});centerObject(divid);setStyle(divid,'zIndex','99999');return!1}
var tooltipDivObj='';function tooltipDiv(obj,rtimer){if(undefined!=commonFadeIdTimeouts.w_tooltip){clearTimeout(commonFadeIdTimeouts.w_tooltip)}
obj=getObject(obj);if(tooltipDivObj==obj){return!1}
tooltipDivObj=obj;let params={};params.position=obj.getAttribute('data-tooltip_position')||obj.getAttribute('data-tip_position')||'';let txt=obj.getAttribute('data-tooltip')||obj.getAttribute('data-tip')||'';if(txt.indexOf('id:')===0){let divid=trim(str_replace('id:','',txt));txt=getText(divid)||''}else if(txt.indexOf('js:')===0){let f=trim(str_replace('js:','',txt));let jsfunc=new Function(f);txt=jsfunc()}else if(txt.indexOf('ajax:')===0){params.ajax=trim(str_replace('ajax:','',txt));txt=''}else if(txt.indexOf('att:')===0){let att=trim(str_replace('att:','',txt));txt=obj.getAttribute(att)||''}
if(txt.length===0||txt==='false'||!txt){txt=' '}
let cObj=getObject('w_tooltip');if(undefined!=cObj){tooltipDivObj='';removeId(cObj)}
if(undefined!=rtimer){params.rtimer=rtimer}
showTooltip(obj,txt,params);return!1}
function showTooltip(obj,txt,params){obj=getObject(obj);if(undefined==obj){return!1}
if(undefined==params){params={}}
if(undefined==params.position){if(obj.nodeName.toLowerCase()==='img'){params.position='bottom'}else{params.position='right'}}
let tipdiv=getObject('w_tooltip');if(undefined==tipdiv){tipdiv=document.createElement("div");tipdiv.setAttribute("id",'w_tooltip');tipdiv.style.zIndex='698999';tipdiv.style.position='absolute';document.body.appendChild(tipdiv)}
tipdiv.innerHTML=txt;let pos=findPos(obj);let x=y=h=w=th=0;h=obj.offsetHeight||obj.innerHeight||getHeight(obj)||24;w=obj.offsetWidth||obj.innerWidth||getWidth(obj)||24;th=tipdiv.offsetHeight||tipdiv.innerHeight||getHeight(tipdiv)||24;tw=tipdiv.offsetWidth||tipdiv.innerWidth||getWidth(tipdiv)||24;let viewport=getViewPort();if(params.position=='bottom'){y=parseInt(pos.y)+h+6;x=parseInt(pos.x)}else{tipdiv.setAttribute("class",'left');y=parseInt(pos.y);x=parseInt(pos.x)+w+6;if(x+tw>parseInt(viewport.w)){}}
tipdiv.style.top=y+"px";tipdiv.style.left=x+"px";if(undefined!=params.ajax){ajaxGet(params.ajax,'w_tooltip')}
return!1}
function popUpDiv(content,param){var s="position:absolute;top:200px;left:200px;margin:5px;z-index:99999;";var bs="padding:5px;border:1px solid #d6dee7;background:#FFF;";clearTimeout('popupdiv_timeout');if(undefined!=param.width){s+='width:'+param.width+'px;'}
if(undefined!=param.height){bs+='height:'+param.height+'px;overflow:auto;'}
var opt={id:'w'+new Date().getTime(),style:s,title:"",closestyle:"cursor:pointer;",close:'<span class="icon-cancel w_danger"></span>',bodystyle:bs,titleleft:20,body:content};if(param){var key;for(key in opt){if(undefined!=param[key]){opt[key]=param[key]}}
for(key in param){if(undefined==opt[key]){opt[key]=param[key]}}}
var masterdiv;if(undefined!=document.getElementById(opt.id)){removeDiv(opt.id)}
if(undefined!=document.getElementById(opt.id)){masterdiv=document.getElementById(opt.id);var bodyid=opt.id+'_Body';masterdiv.style.display='block';if(undefined!=document.getElementById(bodyid)){setText(bodyid,opt.body)}}else{masterdiv=document.createElement("div");masterdiv.setAttribute("id",opt.id);masterdiv.style.zIndex='9999';masterdiv.style.position='absolute';var t=document.createElement("table");t.border=0;t.align="left";t.cellPadding=0;t.cellSpacing=0;if(undefined!=opt.width){t.style.width=opt.width+'px'}
var bgcolor='#49495a';if(opt.titlebgcolor){bgcolor=opt.titlebgcolor}else if(param.titlebgcolor){bgcolor=param.titlebgcolor}
if(undefined==param.noborder){t.style.border='1px solid '+bgcolor}else{t.style.border='0px solid '+bgcolor}
var tb=document.createElement("tbody");if(undefined==param.notop){var toprow=document.createElement("div");var titlecell=document.createElement("td");titlecell.noWrap=!0;titlecell.align='right';titlecell.style.fontFamily='arial';titlecell.style.fontSize='11px';titlecell.style.backgroundColor=bgcolor;if(opt.titlecolor){titlecell.style.color=opt.titlecolor}else{titlecell.style.color='#FFFFFF'}
var titlediv=document.createElement("div");var titletxt='<div id="'+opt.id+'_Title'+'" style="float:left;margin-left:10px;margin-top:1px">'+opt.title+'</div>';titletxt+='<a href="#" style="font-weight:bold;font-size:12px;font-family:arial;color:#970000;text-decoration:none;padding:0 3px 0 0;" onclick="ajaxAbort(\''+opt.id+'\',1);return false;">X</a>';titlediv.innerHTML=titletxt;titlecell.appendChild(titlediv);toprow.appendChild(titlecell);tb.appendChild(toprow)}
var bodyrow=document.createElement("tr");bodyrow.height='100%';var bodycell=document.createElement("td");if(undefined==param.nobackground){bodycell.style.backgroundColor='#FFFFFF'}
var bodydiv=document.createElement("div");var bodycontent='<div id="'+opt.id+'_Body'+'">'+opt.body;bodycontent+='</div>';bodydiv.innerHTML=bodycontent;bodycell.appendChild(bodydiv);bodyrow.appendChild(bodycell);tb.appendChild(bodyrow);if(undefined==param.nobot){var botrow=document.createElement("tr");var botcell=document.createElement("td");botcell.noWrap=!0;botcell.align='right';botcell.style.fontFamily='arial';botcell.style.fontSize='11px';bgcolor='#FFFFFF';if(opt.botbgcolor){bgcolor=opt.botbgcolor}else if(param.botbgcolor){bgcolor=param.botbgcolor}
botcell.style.backgroundColor=bgcolor;var botdiv=document.createElement("div");var bottxt='<a href="#" class="w_red w_bold w_link"" onclick="ajaxAbort(\''+opt.id+'\',1);return false;">Close</a>';botdiv.innerHTML=bottxt;botcell.appendChild(botdiv);botrow.appendChild(botcell);tb.appendChild(botrow)}
...
masterdiv.style.display='block';if(opt.center){var xy=centerObject(masterdiv);var x=0;var y=0;var cvalue=opt.center+'';if(cvalue.indexOf('x')!=-1){x=xy[0]}else if(cvalue.indexOf('y')!=-1){y=xy[1]}else{x=xy[0];y=xy[1]}
if(undefined!=opt.x){xvalue=opt.x+'';if(xvalue.indexOf('+')!=-1){x=Math.round(MouseX+parseInt(xvalue,10))}else if(xvalue.indexOf('-')!=-1){x=Math.round(MouseX-Math.abs(parseInt(xvalue,10)))}else{x=Math.round(opt.x)}
if(x<0){x=0}}
if(undefined!=opt.y){yvalue=opt.y+'';if(yvalue.indexOf('+')!=-1){y=Math.round(MouseY+parseInt(yvalue,10))}else if(yvalue.indexOf('-')!=-1){y=Math.round(MouseY-Math.abs(parseInt(yvalue,10)))}else{y=Math.round(opt.y)}
if(y<0){y=0}}
if(x<0){x=0}
if(y<0){y=0}
masterdiv.style.position='absolute';masterdiv.style.left=x+"px";masterdiv.style.top=y+"px"}else if(param.topright){masterdiv.style.position='absolute';masterdiv.style.top=param.topright+"px";masterdiv.style.right=param.topright+"px"}else if(param.topleft){masterdiv.style.position='absolute';masterdiv.style.top=param.topleft+"px";masterdiv.style.left=param.topleft+"px"}else if(param.botright){masterdiv.style.position='absolute';masterdiv.style.bottom=param.botright+"px";masterdiv.style.right=param.botright+"px"}else if(param.botleft){masterdiv.style.position='absolute';masterdiv.style.bottom=param.botleft+"px";masterdiv.style.left=param.botleft+"px"}else if(opt.screen){showOnScreen(masterdiv)}else if(opt.mouse){x=0;y=0;cvalue=opt.mouse+'';if(cvalue.indexOf('x')!=-1){x=MouseX}else if(cvalue.indexOf('y')!=-1){y=MouseY}
if(undefined!=opt.x){xvalue=opt.x+'';if(xvalue.indexOf('+')!=-1){x=Math.round(MouseX+parseInt(xvalue,10))}else if(xvalue.indexOf('-')!=-1){x=Math.round(MouseX-Math.abs(parseInt(xvalue,10)))}else{x=MouseX}}
if(undefined!=opt.y){yvalue=opt.y+'';if(yvalue.indexOf('+')!=-1){y=Math.round(MouseY+parseInt(yvalue,10))}else if(yvalue.indexOf('-')!=-1){y=Math.round(MouseY-Math.abs(parseInt(yvalue,10)))}else{y=MouseY}}
if(x<0){x=0}
if(y<0){y=0}
masterdiv.style.top=y+"px";masterdiv.style.left=x+"px"}else{if(undefined!=opt.x){masterdiv.style.left=opt.x+"px"}
if(undefined!=opt.y){masterdiv.style.top=opt.y+"px"}}
if(opt.showtime){t=Math.round(opt.showtime*1000);popupdiv_timeout=setTimeout("removeDivOnExit('"+opt.id+"',1)",t)}else if(opt.fade){masterdiv.onmouseout=function(e){if(undefined==e){e=fixE(e)}
if(undefined!=e){if(checkMouseLeave(this,e)){fadeId(this.id,1)}}}}}
function createTable(){var t=document.createElement("table");tb=document.createElement("tbody");t.setAttribute("border","1");var tr=document.createElement("tr");var td;var d;d=document.createElement("div");d.style.backgroundColor="red";d.style.minHeight="20px";d.style.width="50px";td=document.createElement("td");td.appendChild(d);tr.appendChild(td);d=document.createElement("div");d.style.backgroundColor="green";d.style.minHeight="20px";d.style.width="50px";td=document.createElement("td");td.appendChild(d);tr.appendChild(td);tb.appendChild(tr);t.appendChild(tb);document.getElementById("ajaxstatus").appendChild(t)}
function removeDiv(divid){return removeId(divid)}
function removeId(divid){var obj=getObject(divid);if(undefined==obj){return!1}
...
if(!isMouseOver(divid)){if(undefined!=fade&&fade==1){fadeId(divid,1)}else{removeDiv(divid)}
return}
if(undefined!=fade&&fade==1){obj.onmouseout=function(e){if(undefined==e){e=fixE(e)}
if(undefined!=e){if(checkMouseLeave(this,e)){fadeId(this.id,1)}}}}else{obj.onmouseout=function(e){if(undefined==e){e=fixE(e)}
if(undefined!=e){if(checkMouseLeave(this,e)){removeDiv(this.id)}}}}}
function isMouseOver(id){var exy=getXY(id);if(undefined==exy){return!0}
var ewh=getWidthHeight(id);if(MouseX>=exy[0]&&MouseX<=exy[0]+ewh[0]&&MouseY>=exy[1]&&MouseY<=exy[1]+ewh[1]){return!0}
return!1}
...
return null}
function setObjectPos(obj,x,y){var sObj=getObject(obj);if(undefined==sObj){return!1}
if(undefined==x){x=MouseX||10}
if(undefined==y){y=MouseY||10}
sObj.style.position='absolute';sObj.style.left=x+'px';sObj.style.top=y+'px';return new Array(x,y)}
function centerObject(obj,fade){if(undefined==fade){fade=0}
let sObj=getObject(obj);if(undefined==sObj){return!1}
let w=sObj.offsetWidth||sObj.innerWidth||getWidth(sObj)||100;let h=sObj.offsetHeight||sObj.innerHeight||getHeight(sObj)||100;let vp=getViewportSize();let x=Math.round((vp.w/2)-(w/2));let y=Math.round((vp.h/2)-(h/2));sObj.style.position=sObj.dataset.position||'fixed';sObj.style.left=x+'px';if(undefined==y){y=10}
if(y<10){y=10}
sObj.style.top=y+'px';if(fade==1){sObj.onmouseout=function(e){if(undefined==e){e=fixE(e)}
if(undefined!=e){if(checkMouseLeave(this,e)){fadeId(this.id,1)}}}}
return new Array(x,y)}
function hideOnExit(obj){var sObj=getObject(obj);if(undefined==sObj){return!1}
sObj.onmouseout=function(e){if(undefined==e){e=fixE(e)}
if(undefined!=e){if(checkMouseLeave(this,e)){this.style.display='none'}}};return!1}
function showOnScreen(obj){var sObj=getObject(obj);if(undefined==sObj){return!1}
sObj.style.position='absolute';sObj.style.visibility='hidden';sObj.style.display='block';var w=sObj.offsetWidth||sObj.innerWidth||getWidth(sObj)||100;var h=sObj.offsetHeight||sObj.innerHeight||getHeight(sObj)||100;var screen=getViewPort();var sw=getWidth();var sh=getHeight();var x=cursor.x;var y=cursor.y;if(x+w+20>sw){var z=x-w;while(z<0){z++}
x=z}
if(y+h+20>sh){z=y-h;while(z<0){z++}
y=z}
sObj.style.left=x+'px';sObj.style.top=y+'px';sObj.style.visibility='visible';return new Array(x,y)}
function getViewPort(){let viewport={};if(typeof window.innerWidth!='undefined'){viewport.w=window.innerWidth;viewport.h=window.innerHeight}else if(typeof document.documentElement!='undefined'&&typeof document.documentElement.clientWidth!='undefined'&&document.documentElement.clientWidth!=0){viewport.w=document.documentElement.clientWidth;viewport.h=document.documentElement.clientHeight}else{viewport.w=document.getElementsByTagName('body')[0].clientWidth;viewport.h=document.getElementsByTagName('body')[0].clientHeight}
return viewport}
sfHover=function(){var navEls=GetElementsByAttribute('ul','id','w_nav');for(var n=0;n<navEls.length;n++){var sfEls=navEls[n].getElementsByTagName("LI");for(var i=0;i<sfEls.length;i++){sfEls[i].onmouseover=function(){this.className="sfhover"};sfEls[i].onmouseout=function(){this.className=""}}}};addEventHandler(window,'load',sfHover);var calledStickyMenus=0;function initCarousels(){let list=document.querySelectorAll('.w_carousel');for(let i=0;i<list.length;i++){if(undefined!=list[i].getAttribute('initialized')){continue}
list[i].setAttribute('initialized',1);let e=document.createElement('span');e.className="icon-arrow-right w_carousel_next";e.onclick=function(){let list=this.parentNode.querySelectorAll('.slide');for(let i=0;i<list.length;i++){if(list[i].className.indexOf('active')!=-1){removeClass(list[i],'active');let n=i+1;if(undefined!=list[n]){addClass(list[n],'active')}else{addClass(list[0],'active')}
break}}}
list[i].appendChild(e)}}
function initDisplayif(){let els=document.querySelectorAll('form [data-displayif]');for(let i=0;i<els.length;i++){let frm=getParent(els[i],'form');if(undefined!=frm){if(undefined==frm.getAttribute('onchange')){frm.setAttribute('onchange','formChanged(this);')}
formChanged(frm)}}}
function initCustomValidity(){let els=document.querySelectorAll('textarea[data-required_msg],input[data-required_msg],select[data-required_msg]');for(let i=0;i<els.length;i++){els[i].oninvalid=function(e){e.target.setCustomValidity('');if(!e.target.validity.valid){e.target.setCustomValidity(this.dataset.required_msg)}};els[i].oninput=function(e){e.target.setCustomValidity('')}}
els=document.querySelectorAll('input[data-pattern_msg]');for(let i=0;i<els.length;i++){els[i].oninvalid=function(e){e.target.setCustomValidity('');if(!e.target.validity.valid){e.target.setCustomValidity(this.dataset.pattern_msg)}};els[i].oninput=function(e){e.target.setCustomValidity('')}}}
function initQuill(){let quills=document.querySelectorAll('textarea[data-behavior="quill"],textarea[data-behavior="richtext"],textarea[data-behavior="nicedit"]');for(i=0;i<quills.length;i++){if(undefined!=quills[i].processed){continue}
quills[i].processed=1;let qdiv=document.createElement('div');quills[i].insertAdjacentElement('afterEnd',qdiv);quills[i].style.display='none';let toolbarOptions=new Array();if(undefined==quills[i].dataset.toolbar){toolbarOptions.push('bold','italic','underline','strike','blockquote','code-block',{'header':1},{'header':2},{'list':'ordered'},{'list':'bullet'},{'align':[]},{'script':'sub'},{'script':'super'},{'indent':'-1'},{'indent':'+1'},{'direction':'rtl'},{'size':['small',!1,'large','huge']},{'header':[1,2,3,4,5,6,!1]},'image','attach',{'color':[]},{'background':[]},{'font':[]},'clean')}else{let bars=quills[i].dataset.toolbar.split(/[ ,]+/);for(let b=0;b<bars.length;b++){switch(bars[b].toLowerCase()){case 'h1':toolbarOptions.push({'header':1});break;case 'h2':toolbarOptions.push({'header':2});break;case 'background':toolbarOptions.push({'background':[]});break;case 'color':toolbarOptions.push({'color':[]});break;case 'font':toolbarOptions.push({'font':[]});break;case 'header':toolbarOptions.push({'header':[1,2,3,4,5,6,!1]});break;case 'size':toolbarOptions.push({'size':['small',!1,'large','huge']});break;case 'direction':toolbarOptions.push({'direction':'rtl'});break;case 'ol':toolbarOptions.push({'list':'ordered'});break;case 'ul':toolbarOptions.push({'list':'bullet'});break;case 'align':toolbarOptions.push({'align':[]});break;case 'sub':toolbarOptions.push({'script':'sub'});break;case 'sup':toolbarOptions.push({'script':'super'});break;case 'outdent':toolbarOptions.push({'indent':'-1'});break;case 'indent':toolbarOptions.push({'indent':'+1'});break;case 'bold':case 'italic':case 'underline':case 'strike':case 'blockquote':case 'code-block':case 'link':case 'image':case 'video':case 'clean':case 'attach':toolbarOptions.push(bars[b].toLowerCase());break}}}
let myquill=new Quill(qdiv,{modules:{toolbar:toolbarOptions},theme:'snow'});let toolbar=myquill.getModule('toolbar');let attach=document.querySelector('button.ql-attach');if(undefined!=attach){attach.setAttribute('title','Attachment');let fileInput=document.createElement('input');fileInput.setAttribute('type','file');fileInput.classList.add('ql-attach');fileInput.addEventListener('change',function(){if(fileInput.files!=null&&fileInput.files[0]!=null){let reader=new FileReader();reader.filename=fileInput.files[0].name;reader.onload=function(e){let range=myquill.getSelection(!0);let img=Quill.import('formats/image');img.className='ql-attachment';Quill.register(img,!0);let fname=this.filename;myquill.insertEmbed(range.index||0,'image',this.result,'user');img.className='ql-image';Quill.register(img,!0);fileInput.value="";decorateQuillAttachments(fname)};reader.readAsDataURL(fileInput.files[0])}});toolbar.container.appendChild(fileInput);attach.file=fileInput;attach.onclick=function(){this.file.click()}}
for(let i=0;i<toolbar.controls.length;i++){let name=toolbar.controls[i][0]
let ctrl=toolbar.controls[i][1];switch(name.toLowerCase()){case 'header':ctrl.setAttribute('title','H'+ctrl.value);break;case 'indent':if(ctrl.value=='+1'){ctrl.setAttribute('title','Indent')}else{ctrl.setAttribute('title','Outdent')}
break;case 'direction':ctrl.setAttribute('title','Direction: '+ctrl.value);break;default:ctrl.setAttribute('title',ctrl.value||name);break}}
if(quills[i].innerText.length){const delta=myquill.clipboard.convert(quills[i].innerText);myquill.setContents(delta,'silent')}
myquill.textarea=quills[i];myquill.on('text-change',function(delta,source){myquill.textarea.innerHTML='<div class="ql-editor ql-snow">'+myquill.root.innerHTML+'</div>'})}
decorateQuillAttachments()}
function initSpinWheel(){let wheels=document.querySelectorAll('select[data-behavior="spinwheel"]');if(undefined==wheels||wheels.length==0){return!1}
const rand=(m,M)=>Math.random()*(M-m)+m;const PI=Math.PI;const TAU=2*PI;const friction=0.991;let angVel=0;let ang=0;let colors=['#e70697','#fff200','#f6989d','#ee1c24','#3cb878','#f26522','#a186be','#00aef0'];let isw=this;for(let w=0;w<wheels.length;w++){if(undefined!=wheels[w].dataset.initialized){continue}
wheels[w].dataset.initialized=1;if(undefined!=wheels[w].dataset.colors){wcolors=wheels[w].dataset.colors.split(',')}else{wcolors=colors}
wheels[w].audiofile=wheels[w].dataset.audio||'/wfiles/tick.mp3';wheels[w].audio=new Audio(wheels[w].audiofile);let width_height=wheels[w].dataset.width||wheels[w].dataset.height||300;wheels[w].wrapper=document.createElement('div');wheels[w].wrapper.setAttribute('class','spinwheel');wheels[w].wrapper.setAttribute('style','width:'+width_height+'px;height:'+width_height+'px;');wheels[w].wrapper.dataset.wheel_index=w;if(undefined!=wheels[w].dataset.displayif){wheels[w].wrapper.displayif=wheels[w].dataset.displayif}
wheels[w].parentNode.insertBefore(wheels[w].wrapper,wheels[w].nextSibling);wheels[w].canvas=document.createElement('canvas');wheels[w].canvas.setAttribute('width',width_height);wheels[w].canvas.setAttribute('height',width_height);wheels[w].ctx=wheels[w].canvas.getContext('2d');wheels[w].wrapper.appendChild(wheels[w].canvas);wheels[w].canvas.dataset.wheel_index=w;wheels[w].button=document.createElement('button');wheels[w].button.setAttribute('class','spin');wheels[w].button.setAttribute('type','button');wheels[w].button.textContent=wheels[w].dataset.button_text||'spin';wheels[w].button.style.background=wheels[w].dataset.button_background||'#fff';wheels[w].button.style.color=wheels[w].dataset.button_color||'#000';if(width_height<=100){wheels[w].button.style.font='0.7em/0 sans-serif'}else if(width_height<=200){wheels[w].button.style.font='1.0em/0 sans-serif'}else if(width_height<=300){wheels[w].button.style.font='1.3em/0 sans-serif'}else{wheels[w].button.style.font='1.5em/0 sans-serif'}
wheels[w].button.dataset.wheel_index=w;wheels[w].button.addEventListener("click",clicked,!1);wheels[w].wrapper.appendChild(wheels[w].button);wheels[w].sectors=new Array();let c=0;for(let i=0;i<wheels[w].options.length;i++){if(trim(wheels[w].options[i].value).length==0){continue}
let label=trim(wheels[w].options[i].text);if(label.length==0){continue}
let value=trim(wheels[w].options[i].value)||label;let sector={label:label,color:wcolors[c],value:value};wheels[w].sectors.push(sector);c=c+1;if(undefined==wcolors[c]){c=0}}
wheels[w].tot=wheels[w].sectors.length;wheels[w].dia=wheels[w].ctx.canvas.width;wheels[w].rad=wheels[w].dia/2;wheels[w].arc=TAU/wheels[w].sectors.length;wheels[w].angVel=0;wheels[w].volume=0;let font=wheels[w].dataset.font||'bold 20px sans-serif';let fillstyle=wheels[w].dataset.fillstyle||'#000';let textalign=wheels[w].dataset.textalign||'right';for(let s=0;s<wheels[w].sectors.length;s++){wheels[w].ang=wheels[w].arc*s;wheels[w].ctx.save();wheels[w].ctx.beginPath();wheels[w].ctx.fillStyle=wheels[w].sectors[s].color;wheels[w].ctx.moveTo(wheels[w].rad,wheels[w].rad);wheels[w].ctx.arc(wheels[w].rad,wheels[w].rad,wheels[w].rad,wheels[w].ang,wheels[w].ang+wheels[w].arc);wheels[w].ctx.lineTo(wheels[w].rad,wheels[w].rad);wheels[w].ctx.fill();wheels[w].ctx.translate(wheels[w].rad,wheels[w].rad);wheels[w].ctx.rotate(wheels[w].ang+wheels[w].arc/2);wheels[w].ctx.textAlign=textalign;wheels[w].ctx.fillStyle=fillstyle;wheels[w].ctx.font=font;wheels[w].ctx.fillText(wheels[w].sectors[s].label,wheels[w].rad-10,10);wheels[w].ctx.restore()}}
function clicked(evt){let w=parseInt(evt.currentTarget.dataset.wheel_index);if(undefined==wheels[w].dataset.clicked){wheels[w].dataset.clicked=1}else{wheels[w].dataset.clicked=parseInt(wheels[w].dataset.clicked)+1}
if(undefined!=wheels[w].dataset.max){let max=parseInt(wheels[w].dataset.max);if(wheels[w].dataset.clicked>max){return!1}}
wheels[w].angVel=rand(0.25,0.35);return!1}
function rotate(w){let s=Math.floor(wheels[w].tot-wheels[w].ang/TAU*wheels[w].tot)%wheels[w].tot;let sector=wheels[w].sectors[s];wheels[w].canvas.style.transform=`rotate(${wheels[w].ang - PI / 2}rad)`;let label=!wheels[w].angVel?sector.label:sector.label;if(undefined==wheels[w].label||wheels[w].label!=label){playSound(w);wheels[w].label=label;wheels[w].value=sector.value;wheels[w].button.textContent=label;wheels[w].button.style.background=sector.color}}
function frame(){for(w in wheels){if(!isNum(w)){continue}
if(wheels[w].angVel==0){continue}
wheels[w].angVel*=friction;if(wheels[w].angVel<0.002){wheels[w].angVel=0;wheels[w].volume=0;simulateEvent(wheels[w],'change')}
wheels[w].ang+=wheels[w].angVel;wheels[w].ang%=TAU;rotate(w)}}
function spinwheelengine(){frame();requestAnimationFrame(spinwheelengine)}
function playSound(w){if(!isNum(w)){return}
if(undefined!=wheels[w].dataset.volume&&wheels[w].dataset.volume==0){return}
wheels[w].audio.pause();wheels[w].audio.currentTime=0;wheels[w].audio.play();wheels[w].volume=parseFloat(wheels[w].volume,3)+0.025;if(wheels[w].volume>1){wheels[w].volume=1}
wheels[w].audio.v=wheels[w].dataset.volume||wheels[w].volume;wheels[w].audio.onplay=function(){this.volume=this.v}}
spinwheelengine()}
function decorateQuillAttachments(fname){let els=document.querySelectorAll('.ql-editor img[alt],.ql-editor img.ql-attachment');for(e=0;e<els.length;e++){let cfname=els[e].alt||fname||'attachment';if(els[e].processed==1){continue}
els[e].processed=1;if(els[e].src.indexOf('data:image')!=-1){continue}
els[e].alt=cfname;els[e].style.textDecoration='underline';els[e].style.color='#487da6';els[e].style.cursor='pointer';els[e].className='ql-attachment';els[e].onclick=function(){let link=document.createElement('a');link.href=this.src;link.target='_blank';link.download=this.alt;document.body.appendChild(link);link.click();document.body.removeChild(link)}}}
function initBehaviors(ajaxdiv){try{initQuill()}catch(e){}
...
let navel=document.querySelector('[data-navigate]');if(undefined!=navel){try{initNavigate()}catch(e){}}
let displayif=document.querySelector('form [data-displayif]');if(undefined!=displayif){try{initDisplayif()}catch(e){}}
navel=document.querySelector('[data-enlighter-language]');if(undefined!=navel){try{initEnlighterJs()}catch(e){}}
var buttons=document.querySelectorAll('[data-toggle="buttons"] .btn');for(var i=0;i<buttons.length;i++){buttons[i].onclick=function(){var kids=this.parentNode.childNodes;for(var k=0;k<kids.length;k++){removeClass(kids[k],'active')}
addClass(this,'active')}}
if(!window.jQuery){var navbars=document.querySelectorAll('[data-toggle="modal"]');for(var n=0;n<navbars.length;n++){navbars[n].onclick=function(e){cancel(e);var t=this.getAttribute('data-target');var tdiv=getObject(t);var state=tdiv.getAttribute('aria-hidden');switch(state.toLowerCase()){case 'f':case 'false':case 0:tdiv.setAttribute('aria-hidden','true');removeClass(tdiv,'in');break;default:tdiv.setAttribute('aria-hidden','false');addClass(tdiv,'in');break}
return!1}}
navbars=document.querySelectorAll('[data-toggle="dropdown"]');for(n=0;n<navbars.length;n++){navbars[n].onclick=function(e){cancel(e);let p=getParent(this);if(undefined!=p){if(p.className.indexOf('open')==-1){addClass(p,'open')}else{removeClass(p,'open')}}
return!1}}
navbars=document.querySelectorAll('[data-toggle="collapse"]');for(n=0;n<navbars.length;n++){navbars[n].onclick=function(){var t=this.getAttribute('data-target');if(undefined==t){return!1}
var tdiv=document.querySelector(t);if(undefined==tdiv){return!1}
var state=this.getAttribute('aria-expanded');if(undefined=='state'){state=''}
if(state===null){state=''}
switch(state.toLowerCase()){case 'f':case 'false':case '0':case '':this.setAttribute('aria-expanded','true');addClass(tdiv,'in');break;default:this.setAttribute('aria-expanded','false');removeClass(tdiv,'in');break}
return!1}}}else{}
eventInitSticky();var navEls=GetElementsByAttribute('*','data-behavior','.+');var navEls2=GetElementsByAttribute('*','_behavior','.+');for(n=0;n<navEls2.length;n++){navEls2[n].setAttribute('data-behavior',navEls2[n].getAttribute('_behavior'));navEls2[n].removeAttribute('_behavior');navEls[navEls.length]=navEls2[n]}
for(n=0;n<navEls.length;n++){var str=navEls[n].getAttribute('data-behavior').toLowerCase();var behaviors=str.split(/[\ \;]+/);if(in_array("ajax",behaviors)){var attr=getAllAttributes(navEls[n]);if(undefined!=attr.id&&(undefined!=attr.url||undefined!=attr['data-url']||undefined!=attr['data-function'])&&(undefined!=attr['data-timer']||undefined!=attr.timer)){ajaxTimer(attr.id)}}
if(in_array("animate",behaviors)){addEventHandler(navEls[n],'mouseover',function(e){animateGrow(this.id,parseInt(this.getAttribute('min'),10),parseInt(this.getAttribute('max'),10))});addEventHandler(navEls[n],'mouseout',function(e){if(undefined==e){e=fixE(e)}
if(undefined!=e){if(checkMouseLeave(this,e)){animateShrink(this.id,parseInt(this.getAttribute('max'),10),parseInt(this.getAttribute('min'),10))}}})}
if(in_array("autogrow",behaviors)){navEls[n].setAttribute('data-autogrow',navEls[n].style.height);addEventHandler(navEls[n],'focus',function(e){autoGrow(this)});addEventHandler(navEls[n],'keypress',function(e){autoGrow(this)});addEventHandler(navEls[n],'blur',function(e){var h=this.getAttribute('data-autogrow');this.style.height=h})}
if(in_array("tabs",behaviors)){navEls[n].onkeydown=function(e){if(e.keyCode==9||e.which==9){e.preventDefault();var s=this.selectionStart;this.value=this.value.substring(0,this.selectionStart)+"\t"+this.value.substring(this.selectionEnd);this.selectionEnd=s+1}}}
if(in_array("paypal_checkout",behaviors)){if(undefined!=navEls[n].id){let paypal_checkout={style:{shape:navEls[n].dataset.shape||'pill',color:navEls[n].dataset.color||'blue',layout:navEls[n].dataset.layout||'vertical',label:navEls[n].dataset.label||'paypal'}};paypal_checkout.renderid=navEls[n].id;paypal_checkout.cartinfo=JSON.parse(trim(getText(navEls[n])));paypal_checkout.createOrder=function(data,actions){return actions.order.create(paypal_checkout.cartinfo)};paypal_checkout.debug=navEls[n].dataset.debug||0;if(undefined!=navEls[n].dataset.onapprove){if(paypal_checkout.debug==1){console.log('paypal_checkout - onapprove set')}
paypal_checkout.onapprove_func=navEls[n].dataset.onapprove;paypal_checkout.onApprove=function(data,actions){return actions.order.capture().then(function(details){let rtn={order_id:details.id,order_cdate:details.create_time,order_edate:details.update_time,order_status:details.status,order_amount:details.purchase_units[0].amount.value,order_reference_id:details.purchase_units[0].reference_id||'',order_custom_id:details.purchase_units[0].custom_id||'',payee_email:details.purchase_units[0].payee.email_address,payee_merchant_id:details.purchase_units[0].payee.merchant_id,payer_id:details.payer.payer_id,payer_email:details.payer.email_address,shipping_full_name:details.purchase_units[0].shipping.name.full_name,shipping_address_line_1:details.purchase_units[0].shipping.address.address_line_1,shipping_address_line_2:details.purchase_units[0].shipping.address.address_line_2,shipping_city:details.purchase_units[0].shipping.address.admin_area_2,shipping_state:details.purchase_units[0].shipping.address.admin_area_1,shipping_postal_code:details.purchase_units[0].shipping.address.postal_code,shipping_country_code:details.purchase_units[0].shipping.address.country_code,payer_country_code:details.payer.address.country_code,payer_firstname:details.payer.name.given_name,payer_lastname:details.payer.name.surname,payment_status:details.purchase_units[0].payments.captures[0].status,payment_id:details.purchase_units[0].payments.captures[0].id};if(paypal_checkout.debug==1){console.log('paypal_checkout - onApprove called');console.log(details)}
parent.window[paypal_checkout.onapprove_func](rtn)})}}
if(undefined!=navEls[n].dataset.oncancel){paypal_checkout.oncancel_func=navEls[n].dataset.oncancel;if(paypal_checkout.debug==1){console.log('paypal_checkout - onCancel set')}
paypal_checkout.onCancel=function(data){if(paypal_checkout.debug==1){console.log('paypal_checkout - onCancel called')}
parent.window[paypal_checkout.oncancel_func]()}}
setText(navEls[n],'');try{paypal.Buttons(paypal_checkout).render('#'+paypal_checkout.renderid)}catch(e){if(undefined!=navEls[n].dataset.onfail){parent.window[navEls[n].dataset.onfail](e)}else{console.log('paypal_checkout - paypal.Buttons render failed');console.log(e)}}}}
if(in_array("chart",behaviors)){var chart_type=navEls[n].getAttribute('data-type')||'bar';chart_type=chart_type.toLowerCase();var chart_labels=navEls[n].getAttribute('data-labels')||'';chart_labels=chart_labels.split(',');var chart_datasets=new Array();var chart_id=navEls[n].getAttribute('data-datasets')||'';var chart_ids=GetElementsByAttribute('div','id',chart_id);for(i=0;i<chart_ids.length;i++){var txt=getText(chart_ids[i]);var dataset=parseJSONString(txt);chart_datasets.push(dataset)}
var chart_data={labels:chart_labels,datasets:chart_datasets};var chart_options=navEls[n].getAttribute('data-options')||'';if(undefined!=document.getElementById(chart_options)){chart_options=getText(chart_options)}
chart_options=parseJSONString(chart_options);var myLine;switch(chart_type){case 'bar':myLine=new Chart(navEls[n].getContext("2d")).Bar(chart_data,chart_options);break;case 'line':myLine=new Chart(navEls[n].getContext("2d")).Line(chart_data,chart_options);break;case 'pie':myLine=new Chart(navEls[n].getContext("2d")).Pie(chart_data,chart_options);break;default:myLine='';break}
var chart_image=navEls[n].getAttribute('data-image')||'';if(chart_image.length){var img=navEls[n].toDataURL();setText(chart_image,img)}}
if(in_array("copyvalue",behaviors)){if(undefined!=navEls[n].dataset.id){let copyel=document.querySelector('#'+navEls[n].dataset.id);if(undefined!=copyel&&undefined==copyel.dataset.copyvalue_init){copyel.dataset.copyvalue_init=1;let els=document.querySelectorAll('[data-behavior="copyvalue"][data-id="'+copyel.id+'"]');let v=copyel.value||copyel.textContent
for(let i=0;i<els.length;i++){els[i].textContent=v;simulateEvent(els[i],'change')}
copyel.addEventListener("keyup",function(event){let els=document.querySelectorAll('[data-behavior="copyvalue"][data-id="'+this.id+'"]');let v=this.value||this.textContent
for(let i=0;i<els.length;i++){els[i].textContent=v;simulateEvent(els[i],'change')}});copyel.addEventListener("change",function(event){let els=document.querySelectorAll('[data-behavior="copyvalue"][data-id="'+this.id+'"]');let v=this.value||this.textContent
for(let i=0;i<els.length;i++){els[i].textContent=v;simulateEvent(els[i],'change')}})}}else{console.log("data-copyvalue error - missing data-id attribute")}}
if(in_array("clock",behaviors)){var id=navEls[n].getAttribute('id');if(id){startClock(id,1)}}
if(in_array("clone",behaviors)){cloneDiv(navEls[n])}
if(in_array("utcclock",behaviors)){var id=navEls[n].getAttribute('id');if(id){startUTCClock(id,1)}}else if(in_array("countdown",behaviors)){var id=navEls[n].getAttribute('id');if(id){countDown(id)}}else if(in_array("countdowntime",behaviors)){var id=navEls[n].getAttribute('id');if(id){countdownTime(id)}}else if(in_array("countdowndate",behaviors)){let id=navEls[n].getAttribute('id');if(id){let year=navEls[n].dataset.year||navEls[n].getAttribute('year');let month=navEls[n].dataset.month||navEls[n].getAttribute('month');let day=navEls[n].dataset.day||navEls[n].getAttribute('day');let hour=navEls[n].dataset.hour||navEls[n].getAttribute('hour');let minute=navEls[n].dataset.minute||navEls[n].getAttribute('minute');let tz=navEls[n].dataset.tz||navEls[n].getAttribute('tz');countDownDate(id,year,month,day,hour,minute,tz)}}
if(in_array("drag",behaviors)){let head=navEls[n].dataset.head||navEls[n].getAttribute('head');let headobj=getObject(head);navEls[n].style.position='relative';if(undefined==headobj){Drag.init(navEls[n])}else{Drag.init(headobj,navEls[n])}}
if(in_array("email-link",behaviors)){navEls[n].setAttribute('data-behavior','processed');if(undefined!=navEls[n].dataset.href){let email=navEls[n].dataset.href;email=email.replace(/^[\/]+/,'',email);email=email.replace(/[\/]+$/,'',email);email=email.replace(/[\/]/,'@',email);email=email.replace(/[\/]/,'.',email);navEls[n].href='mailto:'+email}else{let email=navEls[n].dataset.value||navEls[n].innerText;email=email.replace(/^[\/]+/,'',email);email=email.replace(/[\/]+$/,'',email);email=email.replace(/[\/]/,'@',email);email=email.replace(/[\/]/,'.',email);navEls[n].href='mailto:'+email;navEls[n].innerText=email}}
if(in_array("loadtextfile",behaviors)){loadTextFileInit(navEls[n])}
if(in_array("markers",behaviors)){wasqlMarkerInit(navEls[n])}
if(in_array("signature",behaviors)){let pencolor=navEls[n].getAttribute('data-color')||'#000';signaturePad=new SignaturePad(navEls[n],{penColor:pencolor});signaturePad.saveSignature()}
if(in_array("tab_enable",behaviors)){navEls[n].onkeydown=function(e){if(e.keyCode==9||e.which==9){e.preventDefault();var s=this.selectionStart;this.value=this.value.substring(0,this.selectionStart)+"\t"+this.value.substring(this.selectionEnd);this.selectionEnd=s+1}}}
if(in_array("zoom",behaviors)){if(undefined==navEls[n].getAttribute('id')){navEls[n].setAttribute('id',guid())}
if(undefined==navEls[n].getAttribute('data-zoomsrc')){navEls[n].setAttribute('data-zoomsrc',navEls[n].src)}
...
if(in_array("fileupload",behaviors)){if(window.File&&window.FileReader&&window.FileList&&window.Blob){navEls[n].setAttribute('_bgcolor',navEls[n].style.backgroundColor||'');addEventHandler(navEls[n],"dragover",function(evt){cancelBubble(evt)});addEventHandler(navEls[n],"dragexit",function(evt){cancelBubble(evt);var bgcolor='';if(undefined!=this.getAttribute('_dragcolor_out')){bgcolor=this.getAttribute('_dragcolor_out')}
this.style.backgroundColor=bgcolor});addEventHandler(navEls[n],"dragenter",function(evt){cancelBubble(evt);this.style.backgroundColor=this.getAttribute('data-color-over')||'green'},!1);addEventHandler(navEls[n],"drop",function(evt){evt=evt||window.event;cancelBubble(evt);this.style.backgroundColor=this.getAttribute('_bgcolor');fileUploadBehavior(evt,this)},!1)}else{setText(navEls[n],'Fileupload via dragdrop is not supported in your browser.')}}
if(in_array("float",behaviors)){var id=navEls[n].getAttribute('id');if(id){let top=navEls[n].dataset.top||navEls[n].getAttribute('top');floatDiv(id,top)}}
if(in_array("marquee",behaviors)){var attr=getAllAttributes(navEls[n]);if(undefined!=attr.id){marquee(attr.id)}}
if(in_array("pin",behaviors)){addEventHandler(navEls[n],'mouseout',function(e){if(undefined==e){e=fixE(e)}
if(undefined!=e){if(checkMouseLeave(this,e)){this.style.display='none';var onhide=this.getAttribute('onhide');if(onhide){let jsfunc=new Function(onhide);jsfunc()}}}})}else if(in_array("dropdown",behaviors)){var dname=navEls[n].dataset.display||navEls[n].getAttribute('display');if(dname){navEls[n].style.position='relative';addEventHandler(navEls[n],'click',function(e){let dname=navEls[n].dataset.display||navEls[n].getAttribute('display');dObj=getObject(dname);if(dObj){if(dObj.style.display=='block'){return!0}
dObj.style.display='block';dObj.style.position='absolute';dObj.style.bottom="-199px";dObj.style.height="200px";dObj.style.left="0px";dObj.style.zIndex='999999';dObj.style.backgroundColor='#FFF'}});addEventHandler(navEls[n],'mouseout',function(e){if(undefined==e){e=fixE(e)}
if(undefined!=e){if(checkMouseLeave(this,e)){let dname=navEls[n].dataset.display||navEls[n].getAttribute('display');dObj=getObject(dname);if(dObj){var hide=0;if(undefined!=dObj.className){if(dObj.className.indexOf("current")==-1){hide++}}else{var cclass=dObj.getAttribute('class');if(undefined==cclass){hide++}else{if(cclass.indexOf("current")==-1){hide++}}}
if(hide){dObj.style.display='none'}}
var onhide=this.getAttribute('onhide');if(onhide){let jsfunc=new Function(onhide);jsfunc()}}}})}}else if(in_array("menu",behaviors)){var dname=navEls[n].dataset.display||navEls[n].getAttribute('display');if(dname){addEventHandler(navEls[n],'click',function(e){var dname=this.dataset.display||this.getAttribute('display');dObj=getObject(dname);if(undefined!=dObj){if(undefined!=dObj.dataset.display){if(dObj.style.display==dObj.dataset.display){return!0}
dObj.style.display=dObj.dataset.display}else{if(dObj.style.display=='block'){return!0}
dObj.style.display='block'}}
var dmouse=this.getAttribute('mouse');var dx=this.getAttribute('x');var dy=this.getAttribute('y');if(undefined!=dmouse){var x=0;var y=0;if(dmouse.indexOf('x')!=-1){x=MouseX}else if(dmouse.indexOf('y')!=-1){y=MouseY}else{x=MouseX;y=MouseY}
if(undefined!=dx){xvalue=dx+'';if(xvalue.indexOf('+')!=-1){x=Math.round(MouseX+parseInt(xvalue))}else if(xvalue.indexOf('-')!=-1){x=Math.round(MouseX-Math.abs(parseInt(xvalue)))}else{x=Math.round(Math.abs(parseInt(xvalue)))}}
if(undefined!=dy){yvalue=dy+'';if(yvalue.indexOf('+')!=-1){y=Math.round(MouseY+parseInt(yvalue))}else if(yvalue.indexOf('-')!=-1){y=Math.round(MouseY-Math.abs(parseInt(yvalue)))}else{y=Math.round(Math.abs(parseInt(yvalue)))}}
dObj.style.position='absolute';dObj.style.top=y+"px";dObj.style.left=x+"px"}});addEventHandler(navEls[n],'mouseover',function(e){var dname=this.dataset.display||this.getAttribute('display');dObj=getObject(dname);if(undefined!=dObj){if(undefined!=dObj.dataset.display){if(dObj.style.display==dObj.dataset.display){return!0}
dObj.style.display=dObj.dataset.display}else{if(dObj.style.display=='block'){return!0}
dObj.style.display='block'}}
var dmouse=this.getAttribute('mouse');var dx=this.getAttribute('x');var dy=this.getAttribute('y');if(undefined!=dmouse){var x=0;var y=0;if(dmouse.indexOf('x')!=-1){x=MouseX}else if(dmouse.indexOf('y')!=-1){y=MouseY}else{x=MouseX;y=MouseY}
if(undefined!=dx){xvalue=dx+'';if(xvalue.indexOf('+')!=-1){x=Math.round(MouseX+parseInt(xvalue))}else if(xvalue.indexOf('-')!=-1){x=Math.round(MouseX-Math.abs(parseInt(xvalue)))}else{x=Math.round(Math.abs(parseInt(xvalue)))}}
if(undefined!=dy){yvalue=dy+'';if(yvalue.indexOf('+')!=-1){y=Math.round(MouseY+parseInt(yvalue))}else if(yvalue.indexOf('-')!=-1){y=Math.round(MouseY-Math.abs(parseInt(yvalue)))}else{y=Math.round(Math.abs(parseInt(yvalue)))}}
dObj.style.position='absolute';dObj.style.top=y+"px";dObj.style.left=x+"px"}});addEventHandler(navEls[n],'mouseout',function(e){if(undefined==e){e=fixE(e)}
if(undefined!=e){if(checkMouseLeave(this,e)){var dname=this.dataset.display||this.getAttribute('display');dObj=getObject(dname);if(dObj){var hide=0;if(undefined!=dObj.className){if(dObj.className.indexOf("current")==-1){hide++}}else{var cclass=dObj.getAttribute('class');if(undefined==cclass){hide++}else{if(cclass.indexOf("current")==-1){hide++}}}
if(hide){dObj.style.display='none'}}
var onhide=this.getAttribute('onhide');if(onhide){let jsfunc=new Function(onhide);jsfunc()}}}})}}
if(in_array("scrolltable",behaviors)){var id=navEls[n].getAttribute('id');var h=navEls[n].dataset.scrollheight||navEls[n].getAttribute('scrollheight');var w=navEls[n].dataset.scrollwidth||navEls[n].getAttribute('scrollwidth');if(id){scrollableTable(navEls[n],h,w)}}
if(in_array("slideshow",behaviors)){var id=navEls[n].getAttribute('id');if(id){addClass(navEls[n],'w_slideshow');var t=navEls[n].dataset.timer||navEls[n].getAttribute('timer')||15;var navobj=getObject(id+'_nav');if(undefined!=navobj){var tag=navEls[n].dataset.tag||navEls[n].getAttribute('tag')||'img';var objs=navEls[n].getElementsByTagName(tag);if(objs.length!=0){var txt='';for(var n=0;n<objs.length;n++){var navtitle=objs[n].getAttribute('title')||'';if(t){txt+='<div id="'+id+'_nav_'+n+'" data-tooltip="'+navtitle+'" data-tooltip_position="bottom" class="" onclick="slideShow(\''+id+'\','+n+','+t+');"></div>'}else{txt+='<div id="'+id+'_nav_'+n+'" data-tooltip="'+navtitle+'" data-tooltip_position="bottom" class=""  onclick="slideShow(\''+id+'\','+n+');"></div>'}}
setText(navobj,txt)}}
slideShow(id,0,t)}}
if(in_array("sticky",behaviors)){var pos=getPos(navEls[n]);var wh=getWidthHeight(navEls[n]);navEls[n].setAttribute('sticky_y',pos.y+wh[1]);navEls[n].setAttribute('sticky_x',pos.x+wh[0]);if(undefined!=navEls[n].style.zIndex){navEls[n].setAttribute('sticky_z',navEls[n].style.zIndex)}
if(calledStickyMenus==0){addEventHandler(window,'scroll',stickyMenus);calledStickyMenus=1}}
if(in_array("stopwatch",behaviors)){var id=navEls[n].getAttribute('id');if(id){stopWatch(id,0)}}else if(in_array("timer_verbose",behaviors)){var id=navEls[n].getAttribute('id');if(undefined!=id){var t=parseInt(getText(navEls[n]),10);if(!isNaN(t)){navEls[n].setAttribute('timer_verbose',t);timerVerbose(id,1)}}}else if(in_array("time_verbose",behaviors)){var s=getText(navEls[n]);if(!isNaN(s)){var secs=parseInt(s,10);if(!isNaN(secs)){var t=verboseTime(secs);if(t.length){setText(navEls[n],t)}}}}else if(in_array("time",behaviors)){var id=navEls[n].getAttribute('id');if(id){startClock(id,0)}}
if(in_array("tab",behaviors)){addEventHandler(navEls[n],'keydown',function(e){if(e.keyCode==9||e.which==9){e.preventDefault();var s=this.selectionStart;this.value=this.value.substring(0,this.selectionStart)+"\t"+this.value.substring(this.selectionEnd);this.selectionEnd=s+1}})}
for(b in behaviors){var behavior=behaviors[b];var id=navEls[n].getAttribute('id');var re=new RegExp('\@([a-z]+)[(](.+)[)]','igm');var res=re.exec(behavior);if(res&&res.length>0){var func=res[1].toLowerCase();var str=res[2].toLowerCase();switch(func){case 'sum':var result=0;var sids=str.split(/[,:\s]+/);var dec=navEls[n].dataset.decimal||0;for(let s=0;s<sids.length;s++){let sidobj=document.querySelector('#'+sids[s]);if(undefined==sidobj){continue}
if(undefined==sidobj.handler_set){sidobj.handler_set=1;addEventHandler(sidobj,'keyup',function(){initBehaviors()})}
result+=Math.round(getText(sids[s]),dec)}
result=Math.round(result,dec);setText(navEls[n],result);break;case 'avg':var tot=0;var cnt=0;var sids=str.split(/[,:\s]+/);for(let s=0;s<sids.length;s++){let sidobj=document.querySelector('#'+sids[s]);if(undefined==sidobj){continue}
if(undefined==sidobj.handler_set){sidobj.handler_set=1;addEventHandler(sidobj,'keyup',function(){initBehaviors()})}
tot+=Math.round(getText(sids[s]));cnt+=1}
var dec=navEls[n].dataset.decimal||0;var result=Math.round((tot/cnt),dec);setText(navEls[n],result);break;case 'max':var max=0;var sids=str.split(/[,:\s]+/);for(let s=0;s<sids.length;s++){let sidobj=document.querySelector('#'+sids[s]);if(undefined==sidobj){continue}
if(undefined==sidobj.handler_set){sidobj.handler_set=1;addEventHandler(sidobj,'keyup',function(){initBehaviors()})}
let sval=getText(sids[s]);let svalp=parseFloat(sval);if(svalp>max){max=sval}}
var dec=navEls[n].dataset.decimal||0;var result=Math.round(max,dec);setText(navEls[n],result);break;case 'min':var min=9999999999999;var sids=str.split(/[,:\s]+/);for(let s=0;s<sids.length;s++){let sidobj=document.querySelector('#'+sids[s]);if(undefined==sidobj){continue}
if(undefined==sidobj.handler_set){sidobj.handler_set=1;addEventHandler(sidobj,'keyup',function(){initBehaviors()})}
let sval=getText(sids[s]);let svalp=parseFloat(sval);if(svalp<min){min=sval}}
var dec=navEls[n].dataset.decimal||0;var result=Math.round(min,dec);setText(navEls[n],result);break;case 'math':doMath(id);break;case 'raid':var cObj=getObject(str);if(typeof(cObj)=='object'){setText(cObj,getText(navEls[n]));navEls[n].setAttribute('raidid',str);addEventHandler(navEls[n],'keyup',function(){var raidid=this.getAttribute('raidid');setText(raidid,getText(this))});addEventHandler(navEls[n],'change',function(){var raidid=this.getAttribute('raidid');setText(raidid,getText(this))});addEventHandler(navEls[n],'blur',function(){var raidid=this.getAttribute('raidid');setText(raidid,getText(this))})}
break}}}}
var tobs=GetElementsByAttribute('*','data-tooltip','.+');for(var i=0;i<tobs.length;i++){addEventHandler(tobs[i],'mouseover',function(event){tooltipDiv(this);event.stopPropagation();event.preventDefault()});addEventHandler(tobs[i],'focus',function(event){tooltipDiv(this);event.stopPropagation();event.preventDefault()});addEventHandler(tobs[i],'mouseout',function(event){tooltipDivObj='';fadeId('w_tooltip',1,1);event.stopPropagation();event.preventDefault()})}
var tobs=GetElementsByAttribute('*','data-tip','.+');for(var i=0;i<tobs.length;i++){addEventHandler(tobs[i],'mouseover',function(event){tooltipDiv(this);event.stopPropagation();event.preventDefault()});addEventHandler(tobs[i],'focus',function(event){tooltipDiv(this);event.stopPropagation();event.preventDefault()});addEventHandler(tobs[i],'mouseout',function(event){tooltipDivObj='';fadeId('w_tooltip',1,1);event.stopPropagation();event.preventDefault()})}}
function initEnlighterJs(){let els=document.querySelectorAll('[data-enlighter-language]');for(let i=0;i<els.length;i++){if(undefined==els[i].dataset.enlightened){let params={language:els[i].dataset.enlighterLanguage,rawButton:!1,infoButton:!1,windowButton:!1};if(undefined!=els[i].dataset.enlighterRawbutton&&els[i].dataset.enlighterRawbutton.indexOf('false')==-1){params.rawButton=!0}
if(undefined!=els[i].dataset.enlighterWindowbutton&&els[i].dataset.enlighterWindowbutton.indexOf('false')==-1){params.windowButton=!0}
if(undefined!=els[i].dataset.enlighterInfobutton&&els[i].dataset.enlighterInfobutton.indexOf('false')==-1){params.infoButton=!0}
if(undefined!=els[i].dataset.enlighterTheme){params.theme=els[i].dataset.enlighterTheme}
if(undefined!=els[i].dataset.enlighterIndent){params.indent=els[i].dataset.enlighterIndent}
if(undefined!=els[i].dataset.enlighterHighlight){params.highlight=els[i].dataset.enlighterHighlight}
EnlighterJS.enlight(els[i],params);els[i].dataset.enlightened=1}}
return!1}
function initFlatpickr(){let els=document.querySelectorAll('input[data-behavior="flatpickr"]');if(!els.length){return!1}
...
els[i].dataset.initflatpickr=1;let lang=els[i].dataset.lang||nlang;let config={errorHandler:function(err){alert(err);return!1}};config.prevArrow="<span class='icon-arrow-left'></span>";config.nextArrow="<span class='icon-arrow-right'></span>";for(k in els[i].dataset){let v=els[i].dataset[k];switch(k.toLowerCase()){case 'altformat':k='altFormat';break;case 'altinput':k='altInput';break;case 'altinputclass':k='altInputClass';break;case 'allowinput':k='allowInput';break;case 'allowinvalidpreload':k='allowInvalidPreload';break;case 'appendto':k='appendTo';v=document.querySelector(v)||getObject(v);break;case 'ariadateformat':k='ariaDateFormat';break;case 'conjunction':k='conjunction';break;case 'clickopens':k='clickOpens';break;case 'dateformat':k='dateFormat';break;case 'defaultdate':if(undefined!=els[i].value&&els[i].value.length){k=''}else{k='defaultDate'}
...
config.locale.firstDayOfWeek=v;continue;break;case 'hourincrement':k='hourIncrement';break;case 'maxdate':k='maxDate';if(!isNaN(v)){if(v<0){v=Math.abs(v);v=new Date(new Date().setDate(new Date().getDate()-v)).toLocaleDateString('en-CA')}else if(v>0){v=Math.abs(v);v=new Date(new Date().setDate(new Date().getDate()+v)).toLocaleDateString('en-CA')}}else if(v.toLowerCase()=='today'){v=new Date().toLocaleDateString('en-CA')}
els[i].dataset.maxdate_value=v;break;case 'mindate':k='minDate';if(!isNaN(v)){if(v<0){v=Math.abs(v);v=new Date(new Date().setDate(new Date().getDate()-v)).toLocaleDateString('en-CA')}else if(v>0){v=Math.abs(v);v=new Date(new Date().setDate(new Date().getDate()+v)).toLocaleDateString('en-CA')}}else if(v.toLowerCase()=='today'){v=new Date().toLocaleDateString('en-CA')}
...
switch(v.toLowerCase()){case 'true':case '1':v=!0;break;case 'false':case '0':v=!1;break}
if(k!=''){config[k]=v}}
switch(lang.toLowerCase()){case 'es':config.locale={};config.locale.months={};config.locale.months.longhand=['enero','febrero','marzo','abril','mayo','junio','julio','agosto','septiembre','octubre','noviembre','diciembre'];config.locale.months.shorthand=['enero','feb.','marzo','abr.','mayo','jun.','jul.','agosto','sept.','oct.','nov.','dic.'];config.locale.weekdays={};config.locale.weekdays.longhand=['Domingo','Lunes','Martes','Miércoles','Jueves','Viernes','Sábado'];config.locale.weekdays.shorthand=['Dom','Lun','Mar','Mié','Jue','Vie','Sáb'];config.locale.firstDayOfWeek=1;config.locale.rangeSeparator=" a ";config.locale.time_24hr=!0;config.locale.ordinal=function(){return"º"};break;case 'de':config.locale={};config.locale.months={};config.locale.months.longhand=['Januar','Februar','März','April','Mai','Juni','Juli','August','September','Oktober','November','Dezember'];config.locale.months.shorthand=['Jan.','Feb.','Marz','Apr.','Mai','Juni.','Juli','Aug.','Sept.','Okt.','Nov.','Dez.'];config.locale.weekdays={};config.locale.weekdays.longhand=['Sonntag','Montag','Dienstag','Mittwoch','Donnerstag','Freitag','Samstag'];config.locale.weekdays.shorthand=['Son','Mon','Die','Mit','Don','Fre','Sam'];config.locale.firstDayOfWeek=1;config.locale.weekAbbreviation="KW";config.locale.rangeSeparator=" bis ";config.locale.scrollTitle="Zum Ändern scrollen";config.locale.toggleTitle="Zum Umschalten klicken";config.locale.time_24hr=!0;break;case 'fr':config.locale={};config.locale.months={};config.locale.months.longhand=['janvier','février','mars','avril','mai','juin','juillet','août','septembre','octobre','novembre','décembre'];config.locale.months.shorthand=['janv.','févr.','mars','avril.','mai','juin','juil.','août','sept.','oct.','nov.','déc.'];config.locale.weekdays={};config.locale.weekdays.longhand=['Dimanche','Lundi','Mardi','Mercredi','Jeudi','Vendredi','Samedi'];config.locale.weekdays.shorthand=['Dim','Lun','Mar','Mer','Jeu','Ven','Sam'];config.locale.ordinal=function(nth){if(nth>1){return""}
return"er"};rangeSeparator=" au ";weekAbbreviation="Sem";scrollTitle="Défiler pour augmenter la valeur";toggleTitle="Cliquer pour basculer";time_24hr=!0;break;case 'it':config.locale={};config.locale.months={};config.locale.months.longhand=['gennaio','febbraio','marzo','aprile','maggio','giugno','luglio','agosto','settembre','ottobre','novembre','dicembre'];config.locale.months.shorthand=['genn.','febbr.','mar.','abr.','magg.','giugno','luglio','ag.','sett.','ott.','nov.','dic.'];config.locale.weekdays={};config.locale.weekdays.longhand=['Domenica','Lunedì','Martedì','Mercoledì','Giovedì','Venerdì','Sabato'];config.locale.weekdays.shorthand=['Dom','Lun','Mar','Mer','Gio','Ven','Sab'];config.locale.firstDayOfWeek=1;config.locale.ordinal=function(){return"°"};config.locale.rangeSeparator=" al ";config.locale.weekAbbreviation="Se";config.locale.scrollTitle="Scrolla per aumentare";config.locale.toggleTitle="Clicca per cambiare";config.locale.time_24hr=!0;break;case 'ja':config.locale={};config.locale.months={};config.locale.months.longhand=["1月","2月","3月","4月","5月","6月","7月","8月","9月","10月","11月","12月"];config.locale.months.shorthand=["1月","2月","3月","4月","5月","6月","7月","8月","9月","10月","11月","12月"];config.locale.weekdays={};config.locale.weekdays.longhand=["日曜日","月曜日","火曜日","水曜日","木曜日","金曜日","土曜日"];config.locale.weekdays.shorthand=["日","月","火","水","木","金","土"];config.locale.time_24hr=!0;config.locale.rangeSeparator=" から ";config.locale.monthAriaLabel="月";config.locale.amPM=["午前","午後"];config.locale.yearAriaLabel="年";config.locale.hourAriaLabel="時間";config.locale.minuteAriaLabel="分";break;case 'ko':config.locale={};config.locale.months={};config.locale.months.longhand=["1월","2월","3월","4월","5월","6월","7월","8월","9월","10월","11월","12월"];config.locale.months.shorthand=["1월","2월","3월","4월","5월","6월","7월","8월","9월","10월","11월","12월"];config.locale.weekdays={};config.locale.weekdays.longhand=["일요일","월요일","화요일","수요일","목요일","금요일","토요일"];config.locale.weekdays.shorthand=["일","월","화","수","목","금","토"];config.locale.rangeSeparator=" ~ ";config.locale.ordinal=function(){return"일"};break;case 'pt':config.locale={};config.locale.months={};config.locale.months.longhand=["Janeiro","Fevereiro","Março","Abril","Maio","Junho","Julho","Agosto","Setembro","Outubro","Novembro","Dezembro"];config.locale.months.shorthand=["Jan","Fev","Mar","Abr","Mai","Jun","Jul","Ago","Set","Out","Nov","Dez"];config.locale.weekdays={};config.locale.weekdays.longhand=["Domingo","Segunda-feira","Terça-feira","Quarta-feira","Quinta-feira","Sexta-feira","Sábado"];config.locale.weekdays.shorthand=["Dom","Seg","Ter","Qua","Qui","Sex","Sáb"];config.locale.rangeSeparator=" até ";config.locale.time_24hr=!0;break;case 'zh':config.locale={};config.locale.months={};config.locale.months.longhand=["一月","二月","三月","四月","五月","六月","七月","八月","九月","十月","十一月","十二月"];config.locale.months.shorthand=["一月","二月","三月","四月","五月","六月","七月","八月","九月","十月","十一月","十二月"];config.locale.weekdays={};config.locale.weekdays.longhand=["星期日","星期一","星期二","星期三","星期四","星期五","星期六"];config.locale.weekdays.shorthand=["周日","周一","周二","周三","周四","周五","周六"];config.locale.rangeSeparator=" 至 ";config.locale.weekAbbreviation="周";config.locale.scrollTitle="滚动切换";config.locale.toggleTitle="点击切换 12/24 小时时制";break}
flatpickr(els[i],config)}
return!1}
function initNavigate(){let navigate_check=document.querySelector('[data-navigate]');if(undefined==navigate_check){console.log('initNavigate - check failed');return}
let fels=[...document.querySelectorAll('[data-navigate-focus^="Ctrl+" i]')];for(let i=0;i<fels.length;i++){fels[i].title='Access Key = '+fels[i].dataset.navigateFocus;let span=fels[i].querySelector('.navigate-text');if(undefined==span){continue}
let letter=fels[i].dataset.navigateFocus.replace('Ctrl+','');if(letter.length==1){let str=span.innerText;if(undefined!=span.dataset.navigateColor){span.innerHTML=str.replace(letter,'<span style="border-bottom:1px solid '+span.dataset.navigateColor+';">'+letter+'</span>')}else{span.innerHTML=str.replace(letter,'<span style="border-bottom:1px solid #d70000;">'+letter+'</span>')}}}
fels=[...document.querySelectorAll('[data-navigate-focus^="Alt+" i]')];for(let i=0;i<fels.length;i++){fels[i].title='Access Key = '+fels[i].dataset.navigateFocus;let span=fels[i].querySelector('.navigate-text');if(undefined==span){continue}
let letter=fels[i].dataset.navigateFocus.replace('Alt+','');if(letter.length==1){let str=span.innerText;if(undefined!=span.dataset.navigateColor){span.innerHTML=str.replace(letter,'<span style="border-bottom: 1px solid '+span.dataset.navigateColor+';">'+letter+'</span>')}else{span.innerHTML=str.replace(letter,'<span style="border-bottom:1px solid #0000e8;">'+letter+'</span>')}}}
let navels=[...document.querySelectorAll('[data-navigate]')];let groupcounts=new Array();for(let i=0;i<navels.length;i++){if(undefined==navels[i].tabindex){navels[i].setAttribute('tabindex',i+1)}
navels[i].setAttribute('data-navigate',i);let group='navigate';if(undefined==navels[i].dataset.navigateGroup){navels[i].setAttribute('data-navigate-group',group)}else{group=navels[i].dataset.navigateGroup}
if(undefined==groupcounts[group]){groupcounts[group]=0}
let gi=parseInt(groupcounts[group])+1;groupcounts[group]=gi;navels[i].setAttribute('data-navigate-group-index',gi)}
document.onkeydown=function(e){e=e||window.event;if(undefined==e){return!0}
let nav={};nav.keycode=e.which||e.keyCode||e.charCode;nav.key=String.fromCharCode(nav.keycode);nav.shift=e.shiftKey;nav.alt=e.altKey;nav.ctrl=e.ctrlKey;nav.focus='';switch(nav.keycode){case 8:nav.key='Backspace';break;case 9:nav.key='Tab';break;case 13:nav.key='Enter';break;case 33:nav.key='PageUp';break;case 34:nav.key='PageDown';break;case 35:nav.key='End';break;case 36:nav.key='Home';break;case 37:nav.key='Left';break;case 38:nav.key='Up';break;case 39:nav.key='Right';break;case 40:nav.key='Down';break;case 45:nav.key='Insert';break;case 46:nav.key='Delete';break}
if(nav.shift){nav.focus='Shift+'+nav.key}else if(nav.ctrl&&nav.alt){nav.focus='Ctrl+Alt+'+nav.key}else if(nav.alt){nav.focus='Alt+'+nav.key}else if(nav.ctrl){nav.focus='Ctrl+'+nav.key}else{nav.focus=nav.key}
if(nav.focus.length){nav.el=document.querySelector('[data-navigate-focus="'+nav.focus+'" i]');if(undefined!=nav.el){e.preventDefault();e.stopPropagation();nav.el.focus();simulateEvent(nav.el,'click');return!1}}
let fel=document.activeElement;if(undefined==fel){return!0}
if(undefined==fel.dataset.navigate){return!0}
if(undefined!=fel.dataset.navigateAll){e.preventDefault();e.stopPropagation();let func=fel.dataset.navigateAll;if(function_exists(func)){window[func](fel)}else{eval(func)}
return!1}
let index=parseInt(fel.dataset.navigate);let group=fel.dataset.navigateGroup;let gindex=parseInt(fel.dataset.navigateGroupIndex);let next=gindex+1;let nextel=document.querySelector('[data-navigate-group="'+group+'"][data-navigate-group-index="'+next+'"]');let prev=gindex-1;let prevel=document.querySelector('[data-navigate-group="'+group+'"][data-navigate-group-index="'+prev+'"]');switch(parseInt(nav.keycode)){case 13:if(undefined!=fel.dataset.navigateEnter){e.preventDefault();e.stopPropagation();let func=fel.dataset.navigateEnter;let funcEl=document.querySelector(func);if((func.indexOf('Ctrl')==0||func.indexOf('Alt')==0)&&func.indexOf('+')!=-1){return navigateSetFocus(func)}else if(func.length==0||func=='false'||func.indexOf('return false')==0){}else if(func.indexOf('group:')==0){let group=func.replace('group:','');return navigateSetFocusGroup(group)}else if(function_exists(func)){window[func](fel)}else{eval(func)}
return!1}
break;case 13:if(undefined!=fel.dataset.navigateTab){e.preventDefault();e.stopPropagation();let func=fel.dataset.navigateTab;if((func.indexOf('Ctrl')==0||func.indexOf('Alt')==0)&&func.indexOf('+')!=-1){return navigateSetFocus(func)}else if(func.length==0||func=='false'||func.indexOf('return false')==0){}else if(func.indexOf('group:')==0){let group=func.replace('group:','');return navigateSetFocusGroup(group)}else if(function_exists(func)){window[func](fel)}else{eval(func)}
return!1}
break;case 38:if(undefined!=fel.dataset.navigateUp){e.preventDefault();e.stopPropagation();let func=fel.dataset.navigateUp;if((func.indexOf('Ctrl')==0||func.indexOf('Alt')==0)&&func.indexOf('+')!=-1){return navigateSetFocus(func)}else if(func.length==0||func=='false'||func.indexOf('return false')==0){}else if(func.indexOf('group:')==0){let group=func.replace('group:','');return navigateSetFocusGroup(group)}else if(function_exists(func)){window[func](fel)}else{eval(func)}
return!1}
if(undefined!=prevel){e.preventDefault();e.stopPropagation();simulateEvent(fel,'mouseout');prevel.focus();prevel.setAttribute('data-navigate-key',nav.keycode);if(undefined!=prevel.dataset.navigateClick){let clickFunc=prevel.dataset.navigateClick;if(clickFunc.length==0||clickFunc=='false'||clickFunc.indexOf('return false')==0){}else if(function_exists(clickFunc)){window[clickFunc](fel)}else{eval(clickFunc)}
return!1}
simulateEvent(prevel,'click');return!1}
break;case 40:if(undefined!=fel.dataset.navigateDown){e.preventDefault();e.stopPropagation();let func=fel.dataset.navigateDown;if((func.indexOf('Ctrl')==0||func.indexOf('Alt')==0)&&func.indexOf('+')!=-1){return navigateSetFocus(func)}else if(func.length==0||func=='false'||func.indexOf('return false')==0){}else if(func.indexOf('group:')==0){let group=func.replace('group:','');return navigateSetFocusGroup(group)}else if(function_exists(func)){window[func](fel)}else{eval(func)}
return!1}
if(undefined!=nextel){e.preventDefault();e.stopPropagation();simulateEvent(fel,'mouseout');nextel.focus();nextel.setAttribute('data-navigate-key',nav.keycode);if(undefined!=nextel.dataset.navigateClick){let clickFunc=nextel.dataset.navigateClick;if(clickFunc.length==0||clickFunc=='false'||clickFunc.indexOf('return false')==0){}else if(function_exists(clickFunc)){window[clickFunc](fel)}else{eval(clickFunc)}
return!1}
simulateEvent(nextel,'click');return!1}
break;case 39:if(undefined!=fel.dataset.navigateRight){e.preventDefault();e.stopPropagation();let func=fel.dataset.navigateRight;if((func.indexOf('Ctrl')==0||func.indexOf('Alt')==0)&&func.indexOf('+')!=-1){return navigateSetFocus(func)}else if(func.length==0||func=='false'||func.indexOf('return false')==0){}else if(func.indexOf('group:')==0){let group=func.replace('group:','');return navigateSetFocusGroup(group)}else if(function_exists(func)){window[func](fel)}else{eval(func)}
return!1}
if(undefined!=nextel){e.preventDefault();e.stopPropagation();simulateEvent(fel,'mouseout');nextel.focus();nextel.setAttribute('data-navigate-key',nav.keycode);if(undefined!=nextel.dataset.navigateClick){let clickFunc=nextel.dataset.navigateClick;if(clickFunc.length==0||clickFunc=='false'||clickFunc.indexOf('return false')==0){}else if(function_exists(clickFunc)){window[clickFunc](fel)}else{eval(clickFunc)}
return!1}
simulateEvent(nextel,'click');return!1}
break;case 37:if(undefined!=fel.dataset.navigateLeft){e.preventDefault();e.stopPropagation();let func=fel.dataset.navigateLeft;if((func.indexOf('Ctrl')==0||func.indexOf('Alt')==0)&&func.indexOf('+')!=-1){return navigateSetFocus(func)}else if(func.length==0||func=='false'||func.indexOf('return false')==0){}else if(func.indexOf('group:')==0){let group=func.replace('group:','');return navigateSetFocusGroup(group)}else if(function_exists(func)){window[func](fel)}else{eval(func)}
return!1}
if(undefined!=prevel){e.preventDefault();e.stopPropagation();simulateEvent(fel,'mouseout');prevel.focus();prevel.setAttribute('data-navigate-key',nav.keycode);if(undefined!=prevel.dataset.navigateClick){let clickFunc=prevel.dataset.navigateClick;if(clickFunc.length==0||clickFunc=='false'||clickFunc.indexOf('return false')==0){}else if(function_exists(clickFunc)){window[clickFunc](fel)}else{eval(clickFunc)}
return!1}
simulateEvent(prevel,'click');return!1}
break}};return!0}
function navigateSetFocusGroup(group){let el=document.querySelector('[data-navigate-group="'+group+'" i]');if(undefined!=el){el.focus();simulateEvent(el,'click')}
return!1}
function navigateHideAndSetFocusGroup(hide,group){hideId(hide);let el=document.querySelector('[data-navigate-group="'+group+'" i]');if(undefined!=el){el.focus();simulateEvent(el,'click')}
return!1}
function navigateSetFocus(focus){let el=document.querySelector('[data-navigate-focus="'+focus+'" i]');if(undefined!=el){el.focus();simulateEvent(el,'click')}
return!1}
function navigateHideAndSetFocus(hide,focus){hideId(hide);let el=document.querySelector('[data-navigate-focus="'+focus+'" i]');if(undefined!=el){el.focus();simulateEvent(el,'click')}
return!1}
function cancel(e){if(e.preventDefault){e.preventDefault()}
return!1}
function stickyMenus(){var list=GetElementsByAttribute('*','data-behavior','sticky');var scrollPosition=getWindowScrollPosition();for(var i=0;i<list.length;i++){var sticky_start=list[i].getAttribute('sticky_y');if(scrollPosition>=sticky_start&&list[i].style.position!="fixed"){list[i].style.position="fixed";list[i].style.top=0;list[i].style.width='100%';list[i].style.zIndex=99999;if(undefined!=list[i].id){fadeIn(list[i].id)}}
if(scrollPosition<sticky_start&&list[i].style.position!="relative"){list[i].style.position="relative";list[i].style.top="";list[i].style.width="";if(undefined!=list[i].getAttribute('sticky_z')){list[i].style.zIndex=list[i].getAttribute('sticky_z')}else{list[i].style.zIndex=''}}}}
function removeTinyMCE(id){if(tinyMCE.getInstanceById(id)){tinyMCE.execCommand('mceRemoveControl',!1,id)}}
addEventHandler(window,'load',initBehaviors);function addOnLoadEvent(f){addEventHandler(window,'load',f)}
function addEvent(elem,evnt,func){return addEventHandler(elem,evnt,func)}
function addEventHandler(elem,evnt,func){if(elem.addEventListener){elem.addEventListener(evnt,func,commonPassiveEventListener(!1))}else if(elem.attachEvent){elem.attachEvent("on"+evnt,func)}else{elem['on'+evnt]=func}}
function removeEvent(elem,evnt,func){return removeEventHandler(elem,evnt,func)}
function removeEventHandler(elem,evnt,func){if(elem.removeEventListener){elem.removeEventListener(evnt,func,!1)}else if(elem.detachEvent){elem.detachEvent("on"+evnt,func)}else{elem['on'+evnt]={}}}
function executeFunctionByName(functionName,context){if(undefined==context){context='window'}
var args=[].slice.call(arguments).splice(2);var namespaces=functionName.split(".");var func=namespaces.pop();for(var i=0;i<namespaces.length;i++){context=context[namespaces[i]]}
return context[func].apply(this,args)}
var tinymceInitialized=0;function tinymceInitialize(txtid,px){var bar1="bold,italic,underline,|,forecolor,backcolor,|,justifyleft,justifycenter,justifyright,|,bullist,numlist,outdent,indent,|,charmap,image,media,link,unlink,formatselect,code,fullscreen,fontsizeselect";var bar2="";var bar3="";var ok=tinyMCE.init({mode:"none",theme:"advanced",plugins:"table,advhr,inlinepopups,contextmenu,media,paste,fullscreen,visualchars,nonbreaking,xhtmlxtras,template,advlist,advimage",submit_patch:!1,paste_auto_cleanup_on_paste:!0,paste_text_replacements:[[/\u2026/g,"..."],[/[\x93\x94\u201c\u201d]/g,'"'],[/[\x60\x91\x92\u2018\u2019]/g,"'"],[/.*((https?|ssh|ftp|file):\/\/\S+).*/gi,'<a href="$1">$1</a>'],[/.*((https?|ssh|ftp|file):\/\/\S+((\.)bmp|gif|jpe?g|png|psd|tif?f)).*/gi,'<img src="$1" alt="image" />']],theme_advanced_buttons1:bar1,theme_advanced_buttons2:bar2,theme_advanced_buttons3:bar3,theme_advanced_buttons4:"",theme_advanced_toolbar_location:"top",theme_advanced_toolbar_align:"left",theme_advanced_resizing:!0})}
function fullScreenToggle(id){var obj=getObject(id);if(undefined==obj){return!1}
var set=obj.getAttribute('fullscreen');if(undefined==set||set==0){var wh=getWidthHeight(obj);obj.setAttribute('fullscreen',1);obj.setAttribute('width_ori',wh[0]);obj.setAttribute('height_ori',wh[1]);obj.style.width=document.body.clientWidth+'px';obj.style.height=document.body.clientHeight+'px'}else{obj.setAttribute('fullscreen',0);obj.style.width=obj.getAttribute('width_ori')+'px';obj.style.height=obj.getAttribute('height_ori')+'px'}}
function floatDiv(id,t,b){var obj=getObject(id);var top=0;if(undefined!=obj.parentNode&&parseInt(obj.parentNode.scrollTop)>0){top=parseInt(obj.parentNode.scrollTop)}else if(undefined!=document.documentElement&&parseInt(document.documentElement.scrollTop)>0){top=parseInt(document.documentElement.scrollTop)}else if(undefined!=document.body&&parseInt(document.body.scrollTop)>0){top=parseInt(document.body.scrollTop)}else{return!1}
...
function storeCaret(text){if(typeof(text.createTextRange)!='undefined'){text.caretPos=document.selection.createRange().duplicate()}}
function replaceText(text,textarea){if(typeof(textarea.caretPos)!="undefined"&&textarea.createTextRange){var caretPos=textarea.caretPos;caretPos.text=caretPos.text.charAt(caretPos.text.length-1)==' '?text+' ':text;caretPos.select()}else if(typeof(textarea.selectionStart)!="undefined"){var begin=textarea.value.substr(0,textarea.selectionStart);var end=textarea.value.substr(textarea.selectionEnd);var scrollPos=textarea.scrollTop;textarea.value=begin+text+end;if(textarea.setSelectionRange){textarea.focus();textarea.setSelectionRange(begin.length+text.length,begin.length+text.length)}
textarea.scrollTop=scrollPos}else{textarea.value+=text;textarea.focus(textarea.value.length-1)}}
function surroundText(text1,text2,textarea){if(typeof(textarea.caretPos)!="undefined"&&textarea.createTextRange){var caretPos=textarea.caretPos;caretPos.text=caretPos.text.charAt(caretPos.text.length-1)==' '?text1+caretPos.text+text2+' ':text1+caretPos.text+text2;caretPos.select()}else if(typeof(textarea.selectionStart)!="undefined"){var begin=textarea.value.substr(0,textarea.selectionStart);var selection=textarea.value.substr(textarea.selectionStart,textarea.selectionEnd-textarea.selectionStart);var end=textarea.value.substr(textarea.selectionEnd);var newCursorPos=textarea.selectionStart;var scrollPos=textarea.scrollTop;textarea.value=begin+text1+selection+text2+end;if(textarea.setSelectionRange){if(selection.length==0)
textarea.setSelectionRange(newCursorPos+text1.length,newCursorPos+text1.length);else textarea.setSelectionRange(newCursorPos,newCursorPos+text1.length+selection.length+text2.length);textarea.focus()}
textarea.scrollTop=scrollPos}else{textarea.value+=text1+text2;textarea.focus(textarea.value.length-1)}}
function insertAtCursor(myField,myValue){if(document.selection){myField.focus();sel=document.selection.createRange();sel.text=myValue}else if(myField.selectionStart||myField.selectionStart=='0'){var startPos=myField.selectionStart;var endPos=myField.selectionEnd;myField.value=myField.value.substring(0,startPos)+myValue+myField.value.substring(endPos,myField.value.length)}else{myField.value+=myValue}
return!1}
function isEmptyText(theField){var theValue=theField.value;while(theValue.length>0&&(theValue.charAt(0)==' '||theValue.charAt(0)=='\t'))
theValue=theValue.substring(1,theValue.length);while(theValue.length>0&&(theValue.charAt(theValue.length-1)==' '||theValue.charAt(theValue.length-1)=='\t'))
theValue=theValue.substring(0,theValue.length-1);if(theValue==''){return!0}else{return!1}}
function findPosX(xobj){var curleft=0;if(xobj.offsetParent){while(xobj.offsetParent){curleft+=xobj.offsetLeft;xobj=xobj.offsetParent}}else if(xobj.x){curleft+=xobj.x}
return curleft}
function findPosY(yobj){var curtop=0;if(yobj.offsetParent){while(yobj.offsetParent){curtop+=yobj.offsetTop;yobj=yobj.offsetParent}}else if(yobj.y){curtop+=yobj.y}
return curtop}
var TimoutArray=new Array();function slideShow(divid,idx,s){if(undefined==s){s=15}
var ms=Math.round(s*1000);idx=Math.round(idx+0)
var divobj=getObject(divid);var transition=navEls[n].dataset.transition||navEls[n].getAttribute('transition');id='slideshow'+divid;clearTimeout(TimoutArray[id]);if(isMouseOver(divid)){TimoutArray[id]=setTimeout("slideShow('"+divid+"',"+idx+","+s+")",ms);return}
var tag=divobj.getAttribute('data-tag');if(undefined==tag){tag='img'}
var objs=divobj.getElementsByTagName(tag);if(objs.length==0){alert('SlideShow Error: - No '+tag+' elements found');return!1}
if(idx==objs.length){idx=0}
for(var i=0;i<objs.length;i++){var caption=objs[i].getAttribute('data-caption');if(undefined!=transition&&undefined==objs[i].getAttribute('data-transition')){objs[i].setAttribute('data-transition',transition)}
objs[i].id='w_slide';setStyle(objs[i],'display','block');if(undefined==caption){caption=''}
var navobj=getObject(divid+'_nav');if(i==idx){addClass(objs[i],'opaque');if(undefined!=objs[i].getAttribute('data-function')){var functionName=objs[i].getAttribute('data-function');window[functionName](objs[i])}
if(undefined!=navobj){var navdiv=getObject(divid+'_nav_'+i);addClass(navdiv,'active')}}else{removeClass(objs[i],'opaque');if(undefined!=navobj){var navdiv=getObject(divid+'_nav_'+i);removeClass(navdiv,'active')}
caption=''}
setText(divid+'_caption',caption)}
idx=Math.round(idx+1);TimoutArray[id]=setTimeout("slideShow('"+divid+"',"+idx+","+s+")",ms)}
function stopWatch(id){clearTimeout(TimoutArray[id]);var obj=getObject(id);if(undefined==obj){return!1}
if(obj.dataset.behavior.indexOf('stopwatch')==-1){return!1}
if(undefined!=obj.dataset.stop&&parseInt(obj.dataset.stop)==1){return!1}
var stime=getText(id);var hour=0;var min=0;var sec=0;if(stime.length){var parts=stime.split(':');hour=parseInt(parts[0]);min=parseInt(parts[1]);sec=parseInt(parts[2])}
...
function ajaxTimer(id){clearTimeout(TimoutArray[id]);var obj=getObject(id);if(undefined==obj){return}
var attr=getAllAttributes(obj);var number;if(undefined!=attr['data-countdown']){number=parseInt(attr['data-countdown'])}else if(undefined!=attr.countdown){number=parseInt(attr.countdown)}else if(undefined!=attr['data-timer']){number=parseInt(attr['data-timer'])}else if(undefined!=attr.timer){number=parseInt(attr.timer)}else{number=75}
number--;obj.setAttribute('data-countdown',number);var timer;if(undefined!=attr['data-timer']){timer=parseInt(attr['data-timer'])}else if(undefined!=attr.timer){timer=parseInt(attr.timer)}else{timer=75}
var skip=0;if(undefined!=document.activeElement&&undefined!=document.activeElement.type&&document.activeElement.type.indexOf('text')!=-1){skip=1}else if(undefined!=getObject('skip_ajax_timer')){skip=1}
obj.setAttribute('data-skip',skip);if(skip==0&&number<=0){if(undefined!=attr.url||undefined!=attr['data-url']){var url;if(undefined!=attr.url){url=attr.url}else{url=attr['data-url']}
var parts=url.split('?');var params={};if(undefined!=parts[1]){params=JSON.parse('{"'+decodeURI(parts[1].replace(/&/g,"\",\"").replace(/=/g,"\":\""))+'"}')}
params.showprocessing=!1;params.timer=timer;ajaxGet(parts[0],attr.id,params)}else if(undefined!=attr['function']||undefined!=attr['data-function']){var func;if(undefined!=attr['function']){func=attr['function']}else{func=attr['data-function']}
let jsfunc=new Function(func);jsfunc()}
obj.setAttribute('data-countdown',timer)}
TimoutArray[id]=setTimeout("ajaxTimer('"+id+"')",1000)}
function countDown(id){clearTimeout(TimoutArray[id]);let obj=getObject(id);if(undefined==obj){return}
let debug=0;if(undefined!=obj.getAttribute('data-debug')){debug=parseInt(obj.getAttribute('data-debug'))}
if(debug==1){console.log(Array('countDown object',obj))}
let number=getText(obj);if(debug==1){console.log(Array('countDown number is ',number))}
number=parseInt(number);number--;setText(obj,number);if(debug==1){console.log(Array('countDown set number to ',number))}
let cb='';if(undefined!=obj.getAttribute('data-callback')){cb=obj.getAttribute('data-callback')}else if(undefined!=obj.getAttribute('callback')){cb=obj.getAttribute('callback')}
let stop=0;if(cb.length>0){let cbfunc=cb+"('"+id+"','"+number+"')";let jsfunc=new Function(cbfunc);jsfunc()}else if(number==0){if(undefined!=obj.dataset.end){let func=obj.dataset.end;console.log(func);if(function_exists(func)){window[func](obj)}else{let fn=new Function(func)();console.log(fn)}}else if(undefined!=obj.dataset.onzero){let func=obj.dataset.onzero;if(function_exists(func)){window[func](obj)}}
stop=1}
if(stop==0){TimoutArray[id]=setTimeout("countDown('"+id+"')",1000)}}
function countDownDate(divid,yr,m,d,hr,min,tz){let divobj=getObject(divid);if(undefined==divobj){return}
let a=new Date();let bstr=divobj.dataset.year+'-'+divobj.dataset.month+'-'+divobj.dataset.day;if(undefined!=divobj.dataset.hour){bstr=bstr+' '+divobj.dataset.hour+':'+divobj.dataset.minute}
if(undefined!=divobj.dataset.second){bstr=bstr+':'+divobj.dataset.second}else{bstr=bstr+':00'}
if(undefined!=divobj.dataset.tz){bstr=bstr+' '+divobj.dataset.tz}
let b=new Date(bstr)
if(undefined==b){setText(divobj,'Date Conversion Error: '+bstr);return}
let utca=Date.UTC(a.getFullYear(),a.getMonth(),a.getDate(),a.getHours(),a.getMinutes(),a.getSeconds());let utcb=Date.UTC(b.getFullYear(),b.getMonth(),b.getDate(),b.getHours(),b.getMinutes(),b.getSeconds());let dd=utcb-utca;let dday=Math.floor(dd/(60*60*1000*24)*1);let dhour=Math.floor((dd%(60*60*1000*24))/(60*60*1000)*1);let dmin=Math.floor(((dd%(60*60*1000*24))%(60*60*1000))/(60*1000)*1);let dsec=Math.floor((((dd%(60*60*1000*24))%(60*60*1000))%(60*1000))/1000*1);if(dday<=0&&dhour<=0&&dmin<=0&&dsec<=0){divobj.style.display='none';return}else{let dd=dday;let dh=dhour;let dm=dmin;let ds=dsec;if(dday<=9){dday="0"+dday}
if(dhour<=9){dhour="0"+dhour}
if(dmin<=9){dmin="0"+dmin}
if(dsec<=9){dsec="0"+dsec}
let rtn='<table  class="w_countdown">'+"\r\n";rtn+='<tr align="center">';if(dd>0){rtn+='<th>'+dday+'</th>'}
if(dh>0||dd>0){rtn+='<th>'+dhour+'</th>'}
if(dm>0||dh>0||dd>0){rtn+='<th>'+dmin+'</th>'}
if(ds>0||dm>0||dh>0||dd>0){rtn+='<th>'+dsec+'</th>'}
rtn+='</tr>'+"\r\n";rtn+='<tr align="center">';if(dd>0){rtn+='<td>Day</td>'}
if(dh>0||dd>0){rtn+='<td>Hour</td>'}
if(dm>0||dh>0||dd>0){rtn+='<td>Min</td>'}
if(ds>0||dm>0||dh>0||dd>0){rtn+='<td>Sec</td>'}
rtn+='</tr>'+"\r\n";rtn+='</table>'+"\r\n";setText(divobj,rtn);setTimeout(countDownDate,1000,divid)}}
function countdownTime(id){let el=getObject(id);if(undefined==el){return!1}
let timeArray=el.innerText.split(/[:]+/);let c=new Date();let repeat=1;if(undefined!=el.dataset.paused){if(parseInt(el.dataset.paused)==1){this.timer=window.setTimeout(countdownTime,1000,id);return}}
if(timeArray.length==3){let t=new Date(c.getFullYear(),c.getMonth(),c.getDay(),timeArray[0],timeArray[1],timeArray[2]-1);let h=t.getHours();let m=t.getMinutes();let s=t.getSeconds();if(h==0&&m==0&&s==0){repeat=0}
if(h.toString().length==1){h='0'+h}
if(m.toString().length==1){m='0'+m}
if(s.toString().length==1){s='0'+s}
if(repeat==0){el.innerHTML='00:00:00';if(undefined!=el.dataset.end){let func=el.dataset.end;if(function_exists(func)){window[func](el)}}}else{el.innerText=h+':'+m+':'+s}}else if(timeArray.length==2){let t=new Date(c.getFullYear(),c.getMonth(),c.getDay(),c.getHours(),timeArray[0],timeArray[1]-1);let m=t.getMinutes();let s=t.getSeconds();if(m==0&&s==0){repeat=0}
if(m.toString().length==1){m='0'+m}
if(s.toString().length==1){s='0'+s}
if(repeat==0){el.innerHTML='00:00';if(undefined!=el.dataset.end){let func=el.dataset.end;if(function_exists(func)){window[func](el)}}}else{el.innerText=m+':'+s}}
if(repeat==1){if(undefined!=this.timer){clearTimeout(this.timer)}
...
function timerVerbose(id,live){var obj=getObject(id);if(undefined==obj){return!1}
var s=parseInt(obj.getAttribute('timer_verbose'));setText(obj,verboseTime(s));s=s+1;obj.setAttribute('timer_verbose',s);clearTimeout(TimoutArray[id]);if(live){TimoutArray[id]=setTimeout("timerVerbose('"+id+"',"+live+")",1000)}}
function startUTCClock(id,live){if(undefined==live){live=!1}
clearTimeout(TimoutArray[id]);obj=getObject(id);var offset=Math.round(obj.getAttribute('data-offset'));var dt=new Date();if(dt.isDST()){offset=Math.round(obj.getAttribute('data-offset-dst'));if(undefined==offset){offset=Math.round(obj.getAttribute('data-offset'))}}
var h=dt.getUTCHours();if(dt.getDate()!=dt.getUTCDate()){h+=24}
var h=h+offset;var m=dt.getUTCMinutes();var s=dt.getUTCSeconds();var p=" am";if(h>24){h=h-24;p='am'}else if(h>12){h=h-12;p='pm'}else if(h==12){p='pm'}
var format=obj.getAttribute('data-format')||'h:m:s p';var t=1000;if(format.indexOf('s')==-1){t=15000}
var parts=new Array();if(format.indexOf('h')!=-1){format=format.replace('h',h)}
if(format.indexOf('m')!=-1){if(m<10){m='0'+m}
format=format.replace('m',m)}
if(format.indexOf('s')!=-1){if(s<10){s='0'+s}
format=format.replace('s',s)}
if(format.indexOf('p')!=-1){format=format.replace('p',p)}
setText(id,format);if(live){TimoutArray[id]=setTimeout("startUTCClock('"+id+"',"+live+")",t)}}
...
function doMath(id){var behavior=document.getElementById(id).getAttribute('data-behavior').toLowerCase();var re=new RegExp('^\@([a-z]+)[(](.+)[)]$','igm');var str;var res=re.exec(behavior);if(res&&res.length>0){var func=res[1].toLowerCase();var str=res[2].toLowerCase();var result=0;var mre=new RegExp('([a-z0-9\_]+)','igm');while(mres=mre.exec(str)){if(mres&&mres.length>0){var mname=mres[1];var txt=getText(mres[1]);var evalstr='str.replace(/'+mname+'/,\''+txt+'\')';let jsfunc=new Function(evalstr);str=jsfunc()}}
try{let jsfunc=new Function(str);str=jsfunc();setText(id,str)}catch(err){setText(id,err)}}}
...
function checkMouseLeave(element,evt){if(element.contains&&undefined!=evt.toElement){return!element.contains(evt.toElement)}else if(evt.relatedTarget){return!containsDOM(element,evt.relatedTarget)}}
function containsDOM(container,containee){var isParent=!1;do{if((isParent=container==containee)){break}
containee=containee.parentNode}
while(containee!=null);return isParent}
function isOver(dragId,containerId){var dragPos=getPos(dragId);var dw=getWidth(dragId);var dx=dragPos.x+parseInt(dw/2);var contPos=getPos(containerId);var w=getWidth(containerId);var h=getHeight(containerId);var lft=contPos.x+w;var h=contPos.y+h;if(dx>contPos.x&&dx<lft&&dragPos.y>contPos.y&&dragPos.y<h){return!0}
return!1}
var Drag={obj:null,init:function(o,oRoot,minX,maxX,minY,maxY,bSwapHorzRef,bSwapVertRef,fXMapper,fYMapper){o=getObject(o);o.onmousedown=Drag.start;o.hmode=bSwapHorzRef?!1:!0;o.vmode=bSwapVertRef?!1:!0;o.root=oRoot&&oRoot!=null?oRoot:o;if(o.hmode&&isNaN(parseInt(o.root.style.left))){o.root.style.left="0px"}
...
dropdiv.startX=Drag.obj.startX;dropdiv.startY=Drag.obj.startY;window[dofunc](targetdiv,dropdiv)}else{var dragover=navEls[n].getAttribute('_dragover');if(undefined!=dragover&&dragover==1&&undefined!=navEls[n].getAttribute('_ondragout')){navEls[n].setAttribute('_dragover',0);var dofunc=navEls[n].getAttribute('_ondragout');var targetdiv=new Object();for(var a=0;a<navEls[n].attributes.length;a++){var attrib=navEls[n].attributes[a];targetdiv[attrib.name]=attrib.value}
var dropdiv=new Object();for(var a=0;a<Drag.obj.root.attributes.length;a++){var attrib=Drag.obj.root.attributes[a];dropdiv[attrib.name]=attrib.value}
dropdiv.startX=Drag.obj.startX;dropdiv.startY=Drag.obj.startY;window[dofunc](targetdiv,dropdiv)}}}
return!1},end:function(){document.onmousemove=null;document.onmouseup=null;Drag.obj.root.onDragEnd(parseInt(Drag.obj.root.style[Drag.obj.hmode?"left":"right"]),parseInt(Drag.obj.root.style[Drag.obj.vmode?"top":"bottom"]));var dragendfunc=Drag.obj.getAttribute('data-ondragend');if(undefined==dragendfunc){dragendfunc=Drag.obj.getAttribute('_ondragend')}
if(undefined!=dragendfunc){var dropdiv=new Object();for(var a=0;a<Drag.obj.attributes.length;a++){var attrib=Drag.obj.attributes[a];dropdiv[attrib.name]=attrib.value}
dropdiv.drag_x=parseInt(Drag.obj.root.style[Drag.obj.hmode?"left":"right"]);dropdiv.drag_y=parseInt(Drag.obj.root.style[Drag.obj.vmode?"top":"bottom"]);window[dragendfunc](dropdiv)}
var navEls=GetElementsByAttribute('*','_ondrop','.+');for(var n=0;n<navEls.length;n++){if(isOver(Drag.obj,navEls[n])){var dropfunc=navEls[n].getAttribute('_ondrop');var targetdiv=new Object();for(var a=0;a<navEls[n].attributes.length;a++){var attrib=navEls[n].attributes[a];targetdiv[attrib.name]=attrib.value}
var dropdiv=new Object();for(var a=0;a<Drag.obj.root.attributes.length;a++){var attrib=Drag.obj.root.attributes[a];dropdiv[attrib.name]=attrib.value}
dropdiv.startX=Drag.obj.startX;dropdiv.startY=Drag.obj.startY;window[dropfunc](targetdiv,dropdiv)}}
Drag.obj=null},fixE:function(e){if(typeof e=='undefined')e=window.event;if(typeof e.layerX=='undefined')e.layerX=e.offsetX;if(typeof e.layerY=='undefined')e.layerY=e.offsetY;return e}}
...
var navEls=GetElementsByAttribute(tagname,tagatt,attval);for(var n=0;n<navEls.length;n++){addEventHandler(navEls[n],'mouseout',function(e){if(undefined==e){e=fixE(e)}
if(undefined!=e){if(checkMouseLeave(this,e)){this.style.display='none';var onhide=this.getAttribute('onhide');if(onhide){let jsfunc=new Function(onhide);jsfunc()}}}})}}
function wasqlMarkerForm(){var centerpop=getObject('centerpop');if(undefined!=centerpop){return}
ajaxGet('/php/index.php','centerpop',{_template:4,mousex:MouseX,mousey:MouseY,_marker:'click'})}
var wasqlmarkers={};var wasqlmarker={};function wasqlMarkerTagsJson(id){var jsondata=getText(id);wasqlmarkers=parseJSONString(jsondata);for(var i in wasqlmarkers){wasqlMarkerTag(wasqlmarkers[i].x,wasqlmarkers[i].y,wasqlmarkers[i].page,wasqlmarkers[i].priority)}}
function wasqlMarkerTag(x,y,page,priority){var _markernulldiv=getObject('_markernulldiv');if(undefined==_markernulldiv){return'no _markernulldiv div'}
var guid='_marker_'+x+'_'+y+'_'+page;var hex='#337ab7';switch(priority){case 0:hex='#bc1717';break;case 1:hex='#cf312c';break;case 2:hex='#337ab7';break;case 3:hex='#bcbcbc';break}
var icon=page==0?'icon-tags':'icon-tag';var txt='<span class="'+icon+' w_pointer" style="font-size:2rem;color:'+hex+';"></span>';var tag=getObject(guid);if(undefined!=tag){setText(tag,txt);return guid}
tag=document.createElement('div');tag.id=guid;tag.style.position='absolute';tag.style.display='inline';tag.style.zIndex=600;tag.style.top=y+'px';tag.style.left=x+'px';setText(tag,txt);document.body.appendChild(tag);return new Array(x,y,page,priority,guid,hex,txt)}
function wasqlMarkerLoad(){var _markernulldiv=getObject('_markernulldiv');if(undefined==_markernulldiv){var div_outer=document.createElement('div');div_outer.style.display='none';document.body.appendChild(div_outer);var div_inner=document.createElement('div');div_inner.id='_markernulldiv';div_outer.appendChild(div_inner);ajaxGet('/php/index.php','_markernulldiv',{_template:4,_marker:'load'})}}
function wasqlMarkerInit(el,cl){if(undefined==el){el=document.body}
if(undefined==cl){cl='icon-tags'}
el=getObject(el);wasqlMarkerLoad();fontIconCursor(el,cl);addEventHandler(el,'click',function(e){cancelBubble(e);if(e.altKey&&e.ctrlKey){wasqlMarkerForm();return!1}
return!1})}
function resizeSignatureWidthHeight(){var list=document.querySelectorAll('.w_signature');for(var i=0;i<list.length;i++){let w=parseInt(list[i].parentNode.style.width);let h=parseInt(list[i].parentNode.style.height);list[i].setAttribute('width',w);list[i].setAttribute('height',h)}}
eventJSInit();