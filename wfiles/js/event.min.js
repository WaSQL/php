...
tipdiv.innerHTML=txt;let pos=findPos(obj);let x=y=h=w=th=0;h=getHeight(obj);if(params.position=='bottom'){h=getHeight(obj);w=getWidth(obj);y=pos.y+h+6;x=pos.x}else{tipdiv.setAttribute("class",'left');w=getWidth(obj);y=pos.y;x=pos.x+w+6}
...
quills[i].processed=1;let qdiv=document.createElement('div');quills[i].insertAdjacentElement('afterEnd',qdiv);quills[i].style.display='none';let toolbarOptions=new Array();if(undefined==quills[i].dataset.toolbar){toolbarOptions=[['bold','italic','underline','strike'],['blockquote','code-block'],[{'header':1},{'header':2}],[{'list':'ordered'},{'list':'bullet'}],[{'align':[]}],[{'script':'sub'},{'script':'super'}],[{'indent':'-1'},{'indent':'+1'}],[{'direction':'rtl'}],[{'size':['small',!1,'large','huge']}],[{'header':[1,2,3,4,5,6,!1]}],['link','image','video'],[{'color':[]},{'background':[]}],[{'font':[]}],['clean']]}else{let bars=quills[i].dataset.toolbar.split(/[ ,]+/);for(let b=0;b<bars.length;b++){switch(bars[b].toLowerCase()){case 'h1':toolbarOptions.push({'header':1});break;case 'h2':toolbarOptions.push({'header':2});break;case 'background':toolbarOptions.push({'background':[]});break;case 'color':toolbarOptions.push({'color':[]});break;case 'font':toolbarOptions.push({'font':[]});break;case 'header':toolbarOptions.push({'header':[1,2,3,4,5,6,!1]});break;case 'size':toolbarOptions.push({'size':['small',!1,'large','huge']});break;case 'direction':toolbarOptions.push({'direction':'rtl'});break;case 'ol':toolbarOptions.push({'list':'ordered'});break;case 'ul':toolbarOptions.push({'list':'bullet'});break;case 'align':toolbarOptions.push({'align':[]});break;case 'sub':toolbarOptions.push({'script':'sub'});break;case 'sup':toolbarOptions.push({'script':'super'});break;case 'outdent':toolbarOptions.push({'indent':'-1'});break;case 'indent':toolbarOptions.push({'indent':'+1'});break;case 'bold':case 'italic':case 'underline':case 'strike':case 'blockquote':case 'code-block':case 'link':case 'image':case 'video':case 'clean':toolbarOptions.push(bars[b].toLowerCase());break}}}
let quill=new Quill(qdiv,{modules:{toolbar:toolbarOptions},theme:'snow'});if(quills[i].innerText.length){const delta=quill.clipboard.convert(quills[i].innerText);quill.setContents(delta,'silent')}
quill.textarea=quills[i];quill.on('text-change',function(delta,source){quill.textarea.innerHTML='<div class="ql-editor ql-snow">'+quill.root.innerHTML+'</div>'})}}
