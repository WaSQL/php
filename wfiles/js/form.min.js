...
let sets=filters.split(";");console.log(sets);for(let s=0;s<sets.length;s++){let fltrs=sets[s].split(" ");let dfield=fltrs.shift();if(dfield=='*'){dfield='Any Field'}
let doper=fltrs.shift();let dvalue='';if(fltrs.length){dvalue=fltrs.join(" ")}
let did=dfield+doper+dvalue;did=did.replace(/[^a-zA-Z0-9\_]+/g,"").toLowerCase();did='id'+did;let obj=frm.querySelector('#'+did);let filters=new Array();if(undefined!=obj){obj.style.display='inline-block'}else{let d=document.createElement('div');d.className='w_pagingfilter';d.setAttribute('data-field',dfield);d.setAttribute('data-operator',doper);d.setAttribute('data-value',dvalue);d.id=did;let dval='\''+dvalue+'\'';switch(doper){case 'ct':doper='Contains';break;case 'nct':doper='Not Contains';break;case 'ca':doper='Contains Any of These';break;case 'nca':doper='Not Contain Any of These';break;case 'eq':doper='Equals';break;case 'neq':doper='Not Equals';break;case 'ea':doper='Equals Any of These';break;case 'nea':doper='Not Equals Any of These';break;case 'gt':doper='Greater Than';break;case 'lt':doper='Less Than';break;case 'egt':doper='Equals or Greater than';break;case 'elt':doper='Less than or Equals';break;case 'ib':doper='Is Blank';dval='';break;case 'nb':doper='Is Not Blank';dval='';break}
d.innerHTML='<span class="icon-filter w_grey"></span> '+dfield+' '+doper+' '+dval+' <span class="icon-cancel w_danger w_pointer" onclick="removeId(\''+did+'\');"></span>';let p=frm.querySelector('#send_to_filters');p.appendChild(d)}
