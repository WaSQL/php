...
return year+'-'+month+'-'+day},parse(dateString,format){const parts=dateString.split('/');const day=parseInt(parts[0],10);const month=parseInt(parts[1],10)-1;const year=parseInt(parts[2],10);return new Date(year,month,day)}};let attrs=getAllAttributes(field);if(undefined!=attrs['data-maxdate']){switch(attrs['data-maxdate'].toLowerCase()){case 'now':case 'today':opts.maxDate=new Date();break;default:opts.maxDate=new Date(attrs['data-maxdate']);break}}
if(undefined!=attrs['data-mindate']){switch(attrs['data-mindate'].toLowerCase()){case 'now':case 'today':opts.minDate=new Date();break;default:opts.minDate=new Date(attrs['data-mindate']);break}}
if(undefined!=attrs['data-firstday']){opts.firstDay=attrs['data-firstday']}
