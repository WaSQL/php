function formChanged(frm){let els=frm.querySelectorAll('[data-displayif]');for(let i=0;i<els.length;i++){let ifel=frm.querySelectorAll('[name="'+els[i].dataset.displayif+'"]');if(undefined==ifel){continue}
if(ifel.length>0){let display=0;for(let f=0;f<ifel.length;f++){let cval='';switch(ifel[f].type.toLowerCase()){case 'select-one':cval=ifel[f].options[ifel[f].selectedIndex].value;if(cval.toLowerCase()==1){display=1}
...
break;default:cval=ifel[f].value;if(cval.toLowerCase()==1){display=1}
if(cval.toLowerCase()=='y'){display=1}
if(cval.toLowerCase()=='yes'){display=1}
break}}
if(display==1){console.log('display - block');console.log(els[i]);els[i].style.display='block'}else{console.log('display - none');console.log(els[i]);els[i].style.display='none'}}else{els[i].display='none'}}}
