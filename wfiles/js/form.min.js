...
function initPikadayCalendar(field,trigger,params){field=getObject(field);let opts={field:field,trigger:getObject(trigger),position:'bottom right',format:'YYYY-MM-DD',toString(date,format){let day=date.getDate();let month=date.getMonth()+1;let year=date.getFullYear();if(day<10){day='0'+day}
if(month<10){month='0'+month}
return year+'-'+month+'-'+day},parse(dateString,format){const parts=dateString.split('/');const day=parseInt(parts[0],10);const month=parseInt(parts[1],10)-1;const year=parseInt(parts[2],10);return new Date(year,month,day)}};let attrs=getAllAttributes(field);if(undefined!=attrs['data-maxdate']){if(attrs['data-maxdate'].indexOf('-')!=-1){let n=parseInt(attrs['data-maxdate']);let d=new Date();d.setDate(d.getDate()-n);opts.maxDate=d}else{switch(attrs['data-maxdate'].toLowerCase()){case 'now':case 'today':opts.maxDate=new Date();break;default:opts.maxDate=new Date(attrs['data-maxdate']);break}}}
if(undefined!=attrs['data-mindate']){if(attrs['data-mindate'].indexOf('-')!=-1){let n=parseInt(attrs['data-mindate']);let d=new Date();d.setDate(d.getDate()-n);opts.minDate=d}else{switch(attrs['data-mindate'].toLowerCase()){case 'now':case 'today':opts.minDate=new Date();break;default:opts.minDate=new Date(attrs['data-mindate']);break}}}
