...
let data=new FormData();let request=new XMLHttpRequest();request.formdata=data;let j=0;for(let i=0;i<theform.length;i++){if(undefined==theform[i].name){continue}
...
break}}
request.sid=sid;request.params=params;request.addEventListener('loadstart',function(e){if(this.sid.indexOf('centerpop')!=-1){var txt=getProcessingDiv(this.sid);popUpDiv('',{id:this.sid,width:300,height:50,notop:1,nobot:1,noborder:1,nobackground:1,bodystyle:"padding:0px;border:0px;background:none;"});var atitle='Processing Request';setCenterPopText(this.sid,txt,{title:atitle,drag:!1,close_bot:!1})}else if(this.sid=='modal'){let title='';if(undefined!=this.params.title){title=this.params.title}else{title='Success'}
let txt=getProcessingDiv(this.sid);let modal=wacss.modalPopup(txt,title,{overlay:1})}else{setText(this.sid,request.response)}});request.upload.sid=sid;request.upload.xhr=request;request.upload.addEventListener('progress',function(e){let percent_complete=parseInt((e.loaded/e.total)*100);let sidobj=getObject(this.sid);if(undefined==sidobj||percent_complete>10){this.xhr.abort();return!1}
if(this.sid.indexOf('centerpop')!=-1){console.log(percent_complete+'% for centerpop');setText(this.sid,percent_complete+'%')}else if(this.sid=='modal'){console.log(percent_complete+'% for model');let modal=wacss.modalPopup(percent_complete+'%')}else{setText(this.sid,percent_complete+'%')}});request.addEventListener('load',function(e){if(this.sid.indexOf('centerpop')!=-1){setCenterPopText(this.sid,request.response)}else if(this.sid=='modal'){let title='';if(undefined!=this.params.title){title=this.params.title}else{title='Success'}
let modal=wacss.modalPopup(txt,title,{overlay:1})}else{setText(this.sid,request.response)}});request.addEventListener('error',function(e){if(this.sid.indexOf('centerpop')!=-1){setCenterPopText(this.sid,request.response)}else if(this.sid=='modal'){let title='';if(undefined!=this.params.title){title=this.params.title}else{title='Error'}
let modal=wacss.modalPopup(request.response,title,{overlay:1})}else{setText(this.sid,request.response)}});request.addEventListener('abort',function(e){console.log('abort listener called');if(this.sid.indexOf('centerpop')!=-1){removeId(this.sid)}else if(this.sid=='modal'){wacss.modalClose}else{setText(this.sid,'')}});request.open('post',url);request.send(data);return!1}
