...
function pagingSubmit(frm,div){pagingAddFilter(frm);pagingSetFilters(frm);if(undefined!=div){return ajaxSubmitForm(frm,div)}
frm.submit();return!1}
function pagingSetOffset(frm,v){frm.filter_offset.value=v;return pagingSubmit(frm)}
function pagingSetOrder(frm,v){if(frm.filter_order.value==v&&frm.filter_order.value.indexOf('desc')==-1){v=v+' desc'}
frm.filter_order.value=v;return pagingSubmit(frm)}
...
var v=frm.filter_field.value;frm.filter_field.value='';pagingSetFilters(frm);frm.filter_field.value=v;frm.filter_bulkedit.value='1';frm.submit();return!0}
function pagingExport(frm){pagingAddFilter(frm);pagingSetFilters(frm);frm.filter_export.value='1';frm.submit();return!1}
...
function ajaxAbort(sid){if(typeof(AjaxRequest.ActiveAjaxGroupRequests[sid])!='undefined'){var req=AjaxRequest.ActiveAjaxGroupRequests[sid];if(undefined!=req.abort_callback&&req.abort_callback.length){req.status='aborted';window[req.abort_callback](req)}
