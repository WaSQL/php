...
let modal=wacss.modalPopup(request.response,title,{overlay:1})}else{setText(this.sid,request.response)}});request.addEventListener('abort',function(e){console.log('abort listener called');if(this.sid.indexOf('centerpop')!=-1){removeId(this.sid)}else if(this.sid=='modal'){wacss.modalClose}else{setText(this.sid,this.prevtxt)}});request.open('post',url);request.send(data);return!1}
