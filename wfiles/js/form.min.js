...
function formIsIfTrue(frm,ifstr){let oper='';let sets=ifstr.split(' and ');if(sets.length){oper='and'}
...
let tvals=new Array();for(let i=0;i<sets.length;i++){let parts=sets[i].split(':');let fld=parts[0];let formels=frm.querySelectorAll('[name="'+fld+'"],[name="'+fld+'[]"],[id="'+fld+'"]');if(formels.length==0){continue}
if(formels.length==1&&undefined==formels[0].type){continue}
let fvals=new Array();for(let f=0;f<formels.length;f++){if(undefined==formels[f].type){continue}
...
els=frm.querySelectorAll('[data-classif]');for(let i=0;i<els.length;i++){let parts=els[i].dataset.classif.split(':');let eclass=parts[0];let ifstr=parts[1]+':'+parts[2];if(formIsIfTrue(frm,ifstr)){els[i].classList.add(eclass)}else{els[i].classList.remove(eclass)}}
return}
