...
function formDictate(inp,ico,frm){inp=getObject(inp);if(undefined==inp){console.log('formDictate error: undefined input '+inp);return!1}
ico=getObject(ico);frm=getObject(frm);if(window.hasOwnProperty('webkitSpeechRecognition')){let recognition=new webkitSpeechRecognition();recognition.continuous=!1;recognition.interimResults=!1;recognition.lang="en-US";if(undefined!=ico){ico.classList.add('w_blink');ico.classList.add('w_success');recognition.ico=ico}
if(undefined!=frm){recognition.frm=frm}
recognition.inp=inp;recognition.start();recognition.onresult=function(e){this.inp.value=e.results[0][0].transcript;if(undefined!=this.ico){this.ico.classList.remove('w_blink');this.ico.classList.remove('w_success')}
this.stop();if(undefined!=this.frm){simulateEvent(this.frm,'submit')}};recognition.onerror=function(e){if(undefined!=this.ico){this.ico.classList.remove('w_blink');this.ico.classList.remove('w_success');this.ico.classList.add('w_danger')}
this.stop()}}
return!1}
