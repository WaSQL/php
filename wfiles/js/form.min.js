...
return year+'-'+month+'-'+day},parse(dateString,format){const parts=dateString.split('/');const day=parseInt(parts[0],10);const month=parseInt(parts[1],10)-1;const year=parseInt(parts[2],10);return new Date(year,month,day)}};let attrs=getAllAttributes(field);if(undefined!=attrs['data-maxdate']){if(attrs['data-maxdate'].indexOf('-')!=-1){let dateparts=attrs['data-maxdate'].split("-");if(dateparts.length==3){opts.maxDate=new Date(dateparts[0],dateparts[1],dateparts[2])}else{let n=parseInt(attrs['data-maxdate'])*-1;let d=new Date();d.setDate(d.getDate()-n);opts.maxDate=d}}else{switch(attrs['data-maxdate'].toLowerCase()){case 'now':case 'today':opts.maxDate=new Date();break;default:opts.maxDate=new Date(attrs['data-maxdate']);break}}}
if(undefined!=attrs['data-mindate']){if(attrs['data-mindate'].indexOf('-')!=-1){let dateparts=attrs['data-mindate'].split("-");if(dateparts.length==3){opts.minDate=new Date(dateparts[0],dateparts[1],dateparts[2])}else{let n=parseInt(attrs['data-mindate'])*-1;let d=new Date();d.setDate(d.getDate()-n);opts.minDate=d}}else{switch(attrs['data-mindate'].toLowerCase()){case 'now':case 'today':opts.minDate=new Date();break;default:opts.minDate=new Date(attrs['data-mindate']);break}}}
