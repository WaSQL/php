...
break}}
request.sid=sid;request.params=params;request.addEventListener('loadstart',function(e){console.log('Load: '+this.sid);var txt=getProcessingDiv(this.sid);if(this.sid.indexOf('centerpop')!=-1){console.log('setting a centerpop');popUpDiv('',{id:this.sid,width:300,height:50,notop:1,nobot:1,noborder:1,nobackground:1,bodystyle:"padding:0px;border:0px;background:none;"});var atitle='Processing Request';setCenterPopText(this.sid,txt,{title:atitle,drag:!1,close_bot:!1})}else if(this.sid=='modal'){let title='';if(undefined!=this.params.title){title=this.params.title}else{title='Processing Request'}
console.log('setting a modal');let modal=wacss.modalPopup(txt,title,{overlay:1})}else{console.log('setting a custom');this.prevtxt=getText(this.sid);setText(this.sid,txt)}});request.upload.sid=sid;request.upload.xhr=request;request.upload.addEventListener('progress',function(e){let percent_complete=parseInt((e.loaded/e.total)*100);console.log('Percent Complete: '+percent_complete+'%');console.log(this.sid);let pobj=document.querySelector('#processing_div .processing_message');if(undefined!=pobj){setText(pobj,percent_complete+'%');return!1}
let txt=getProcessingDiv(this.sid,percent_complete+'%');if(this.sid.indexOf('centerpop')!=-1){console.log(percent_complete+'% for centerpop x');let sidobj=getObject(this.sid);if(undefined==sidobj){this.xhr.abort();return!1}
...
console.log(percent_complete+'% for model');let modal=wacss.modalPopup(txt)}else{console.log(percent_complete+'% for custom sid');setText(this.sid,txt)}});request.addEventListener('load',function(e){console.log('Load: '+this.sid);if(this.sid.indexOf('centerpop')!=-1){updateCenterPopText(this.sid,request.response)}else if(this.sid=='modal'){let title='';if(undefined!=this.params.success_title){title=this.params.success_title}else{title='Success'}
