...
let img=theform.querySelector('label[for="'+fields[i].id+'"] img');if(undefined!=img){let name=fields[i].value.split('\\').pop().split('/').pop();if(undefined!=theform[fields[i].name]){fields[i].parentNode.removeChild(fields[i])}
let h=document.createElement('input');h.type='hidden';h.name=fields[i].name;h.value=name;theform.appendChild(h);h=document.createElement('textarea');h.style.display='none';h.name=fields[i].name+'_base64';h.value=name+';'+name+';'+img.src;theform.appendChild(h)}}
