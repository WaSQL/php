...
function initPikadayCalendar(field,trigger,params){new Pikaday({field:getObject(field),trigger:getObject(trigger),position:'bottom right',format:'YYYY-MM-DD',toString(date,format){let day=date.getDate();let month=date.getMonth()+1;let year=date.getFullYear();if(day<10){day='0'+day}
if(month<10){month='0'+month}
return year+'-'+month+'-'+day},parse(dateString,format){const parts=dateString.split('/');const day=parseInt(parts[0],10);const month=parseInt(parts[1],10)-1;const year=parseInt(parts[2],10);return new Date(year,month,day)}})}
