...
function formIsIfTrue(frm,ifstr){if(undefined==ifstr||ifstr.length==0){return!1}
...
if(oper=='or'){if(tvals.length>0){console.log(ifstr+' = true (or)');return!0}
console.log(ifstr+' = false (or)');return!1}
if(tvals.length==sets.length){console.log(ifstr+' = true (and)');return!0}
console.log(ifstr+' = false (and)');return!1}
function formChanged(frm,debug){if(undefined==debug){debug=0}
if(debug==1){console.log('formChanged')}
let els=frm.querySelectorAll('[data-displayif]');for(let i=0;i<els.length;i++){if(formIsIfTrue(frm,els[i].dataset.displayif)){if(undefined!=els[i].dataset.display){els[i].style.display=els[i].dataset.display}else{els[i].style.display='initial'}}else{els[i].style.display='none'}}
els=frm.querySelectorAll('[data-hideif]');for(let i=0;i<els.length;i++){if(formIsIfTrue(frm,els[i].dataset.hideif)){els[i].style.display='none'}else{if(undefined!=els[i].dataset.display){els[i].style.display=els[i].dataset.display}else{els[i].style.display='initial'}}}
...
els[i].setAttribute('readonly','readonly')}else{els[i].removeAttribute('readonly');if(els[i].hasAttribute('onclickx')){els[i].removeAttribute('onclick');els[i].removeAttribute('onclickx')}}}
els=frm.querySelectorAll('[data-requiredif]');for(let i=0;i<els.length;i++){if(formIsIfTrue(frm,els[i].dataset.requiredif)){els[i].setAttribute('required','required')}else{els[i].removeAttribute('required');if(els[i].hasAttribute('data-required')){els[i].removeAttribute('data-required')}}}
...
els[i].innerHTML='';break;default:els[i].dataset.blankx=els[i].value;els[i].value='';break}}else{if(undefined!=els[i].dataset.blankx){switch(els[i].type.toLowerCase()){case 'radio':case 'checkbox':console.error('blankif does not support checkbox and radio inputs');break;case 'textarea':if(undefined!=els[i].codemirror){els[i].codemirror.getDoc().setValue(els[i].dataset.blankx)}
els[i].innerHTML='';els[i].innerHTML=els[i].dataset.blankx;break;default:els[i].value=els[i].dataset.blankx;break}}}}
els=frm.querySelectorAll('[data-classif]');for(let i=0;i<els.length;i++){let parts=els[i].dataset.classif.split(':');let eclass=parts.shift();let ifstr=parts.join(':');if(formIsIfTrue(frm,ifstr)){els[i].classList.add(eclass)}else{els[i].classList.remove(eclass)}}
return}
...
if(undefined!=tmeout&&tmeout=='novalidation'){tmeout=36000000}else{var ok=submitForm(theform,1,0,1);if(!ok){return!1}}
...
let returnid=!1;if(undefined!=theform.returnid){if(theform.returnid.value.toLowerCase()=='true'){returnid=!0}
if(theform.returnid.value.toLowerCase()=='1'){returnid=!0}}
...
return!1}
function ajaxGet(url,sid,xparams,callback,tmeout,nosetprocess,returnreq,newtitle,newurl,abort_callback,append){var params='';var cp_title='';var showprocessingdiv=sid;var showprocessing=!0;let returnid=!1;if(typeof(xparams)=='object'){if(undefined!=xparams.callback){callback=xparams.callback}
