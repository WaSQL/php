...
if(requiredif.length>0){let parts=requiredif.split(':');let rval='';requiredif=theForm.querySelector('[name="'+parts[0]+'"]');if(parts.length==1){if(undefined!=requiredif){switch(requiredif.type.toLowerCase()){case 'select-one':rval=requiredif.options[requiredif.selectedIndex].value;if(rval.length){required=1}
...
break}}}else{cvals=parts[1].split(',');if(undefined!=requiredif){switch(requiredif.type.toLowerCase()){case 'select-one':rval=requiredif.options[requiredif.selectedIndex].value;for(let c=0;c<cvals.length;c++){if(rval==cvals[c]){required=1;break}}
break;case 'select-multiple':let rvals=Array.from(requiredif.selectedOptions).map(el=>el.value);for(let r=0;r<rvals.length;r++){rval=rvals[r];for(let c=0;c<cvals.length;c++){if(rval==cvals[c]){required=1;break}}}
break;case 'radio':case 'checkbox':let rels=theForm.querySelectorAll('[type="'+requiredif.type+'"][name="'+requiredif.name+'"]');for(let r=0;r<rels.length;r++){if(rels[r].checked){rval=rels[r].value||1;for(let c=0;c<cvals.length;c++){if(rval==cvals[c]){required=1;break}}}}
break;case 'textarea':rval=requiredif.innerText;for(let c=0;c<cvals.length;c++){if(rval==cvals[c]){required=1;break}}
break
default:rval=requiredif.value;for(let c=0;c<cvals.length;c++){if(rval==cvals[c]){required=1;break}}
break}}}}
