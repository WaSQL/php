...
return!1}
ico=getObject(ico);if(window.hasOwnProperty('webkitSpeechRecognition')){let SpeechRecognition=window.SpeechRecognition||window.webkitSpeechRecognition;var recognition=new SpeechRecognition();recognition.continuous=continuous||!1;recognition.interimResults=!1;recognition.lang="en-US";if(undefined!=ico){ico.classList.add('w_blink');ico.classList.add('w_success');recognition.ico=ico}
if(undefined!=frm){recognition.frm=frm}
recognition.inp=inp;recognition.start()=>VM187:1 err:SpeechRecognitionErrorEvent{isTrusted:!0,error:"not-allowed",message:"",type:"error",target:SpeechRecognition};recognition.debug=debug;recognition.onresult=function(e){if(undefined!=this.frm&&undefined!=window[this.frm]){try{window[this.frm](e.results[0][0].transcript);this.stop();this.start()}catch(e){}}
...
recognition.onerror=function(e){if(this.continuous){if(undefined!=e.error&&e.error=='no-speech'){try{this.restart()}catch(e){}}else{console.log(e);if(this.debug){this.debug=!1;alert('ERROR: '+e.error)}}}else{try{this.stop()}catch(e){if(this.debug){this.debug=!1;alert('ERROR: '+e.error)}}
if(undefined!=this.ico){this.ico.classList.remove('w_blink');this.ico.classList.remove('w_success')}}}}else if(debug){alert('webkitSpeechRecognition property not found on this device')}
return!1}
