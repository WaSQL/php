function setInputFileName(fld){if(fld.files&&fld.files.length>1){fileName=(fld.getAttribute('data-multiple-caption')||'').replace('{count}',fld.files.length)}else{fileName=fld.value.split('\\').pop()}
var label=document.querySelector('label[for='+fld.id+']');console.log(fileName,label);if(fileName){setText(label,'<span class="icon-upload w_big w_success"></span> '+fileName);label.className='btn btn-default'}else{setText(label,'<span class="icon-upload w_big w_danger"></span> '+fld.getAttribute('data-text'));label.className='btn btn-default'}}
...
if(undefined==msg){msg='Processing ...'}
var str='';str+='<span id="processing_div">';str+='<span class="icon-spin7 w_spin"></span>';str+=' <span class="w_grey">'+msg+'</span>';if(cancel==1){str+='<span class="icon-cancel-circled w_danger w_pointer" onclick="return ajaxAbort(\''+id+'\');"></span>'}
str+='</span>';return str}
...
submitFormAlert(msg,popup,5);theForm[i].focus();return!1}}else if(theForm[i].type=='radio'){var checkboxlist=GetElementsByAttribute('input','name',theForm[i].name);var isChecked=0;for(var c=0;c<checkboxlist.length;c++){if(checkboxlist[c].type=='radio'&&checkboxlist[c].checked){isChecked++}else if(checkboxlist[c].type=='text'&&checkboxlist[c].value!=''){isChecked++}}
if(isChecked==0){var msg=dname+" is required";if(undefined!=requiredmsg){msg=requiredmsg}
submitFormAlert(msg,popup,5);theForm[i].focus();return!1}}else if(theForm[i].type=='textarea'){var cval=trim(getText(theForm[i]));if(cval.length==0){var msg=dname+" is required";if(undefined!=requiredmsg){msg=requiredmsg}
submitFormAlert(msg,popup,5);theForm[i].focus();return!1}}else if(theForm[i].type=='select-one'){var cval=theForm[i].options[theForm[i].selectedIndex].value;if(cval.length==0){var msg=dname+" is required";if(undefined!=requiredmsg){msg=requiredmsg}
submitFormAlert(msg,popup,5);theForm[i].focus();return!1}}else if(theForm[i].value==''){var msg=dname+" is required";if(undefined!=requiredmsg){msg=requiredmsg}
...
frm.filter_bulkedit.value='1';frm.submit()}
function pagingExport(frm){frm.filter_export.value='1';frm.submit()}
function pagingAddFilter(frm){if(frm.filter_field.value.length==0){alert('select a filter field');return!1}
...
frm._filters.value=implode("\r\n",filters);if(undefined!=frm.filter_bulkedit){frm.filter_bulkedit.value=''}
...
if(undefined!=tmeout&&tmeout=='novalidation'){tmeout=1800000}else{var ok=submitForm(theform,1,0,1);if(!ok){return!1}}
...
var showprocessing=!0;if(undefined!=theform.showprocessing){if(theform.setprocessing.value.toLowerCase()=='false'){showprocessing=!1}
if(theform.setprocessing.value.toLowerCase()=='0'){showprocessing=!1}}else if(undefined!=theform.noprocessing){if(theform.noprocessing.value.toLowerCase()=='true'){showprocessing=!1}
...
if(cb!=''){if(cb.indexOf('popupdiv')!=-1){popUpDiv(val,{id:dname,center:1,drag:1});centerObject(dname)}else if(lname=='pop'){popText(this.popNumber,val,this.var2)}else if(cb.indexOf('centerpop')!=-1){setCenterPopText(dname,val)}else{var str=this.callback+'(req);';eval(str)}}else{if(lname.indexOf('popupdiv')!=-1){popUpDiv(val,{id:dname,center:1,drag:1});centerObject(dname)}else if(lname=='pop'){popText(this.popNumber,val,this.var2)}else if(lname.indexOf('centerpop')!=-1){setCenterPopText(dname,val)}else if(document.getElementById(dname)){if(undefined==document.getElementById(dname).style.display||document.getElementById(dname).style.display=='none'){document.getElementById(dname).style.display='inline'}
...
if(undefined==returnreq){returnreq=!1}
var guid=getCookie('GUID');if(undefined==tmeout){tmeout=1800000}
if(tmeout<180000){tmeout=180000}
...
rawValue=value;prevX=e.clientX;this.addEventListener('mousemove',onDrag,!0);this.addEventListener('mouseup',onDragEnd,!0)}
