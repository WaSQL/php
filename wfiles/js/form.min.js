...
let hours=container.querySelectorAll('#frequency_hour');let months=container.querySelectorAll('#frequency_month');let days=container.querySelectorAll('#frequency_day');let field=container.querySelector('#'+fid);let nv={hour:[],month:[],day:[]};if(undefined!=v){if(typeof(v)=='string'){v=JSON.parse(v)}
...
setText(field,JSON.stringify(nv))}else{for(let x=0;x<hours.length;x++){let hval=parseInt(hours[x].value);if(hours[x].checked){nv.hour.push(hval)}}
