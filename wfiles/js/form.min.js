...
container.querySelector('span[title="clear minutes"]').style.color='#6c757d';break;case 'hour':for(let x=0;x<hours.length;x++){hours[x].checked=!1}
container.querySelector('span[title="clear hours"]').style.color='#6c757d';break;case 'day':for(let x=0;x<days.length;x++){days[x].checked=!1}
container.querySelector('span[title="clear days"]').style.color='#6c757d';break;case 'month':for(let x=0;x<months.length;x++){months[x].checked=!1}
container.querySelector('span[title="clear months"]').style.color='#6c757d';break}}}
for(let x=0;x<minutes.length;x++){let hval=parseInt(minutes[x].value);if(v.minute[0]==-1){minutes[x].checked=!0;nv.minute=[-1]}else if(in_array(hval,v.minute)){minutes[x].checked=!0;nv.minute.push(hval)}else{minutes[x].checked=!1}}
for(let x=0;x<hours.length;x++){let hval=parseInt(hours[x].value);if(v.hour[0]==-1){hours[x].checked=!0;nv.hour=[-1]}else if(in_array(hval,v.hour)){hours[x].checked=!0;nv.hour.push(hval)}else{hours[x].checked=!1}}
for(let x=0;x<months.length;x++){let hval=parseInt(months[x].value);if(v.month[0]==-1){months[x].checked=!0;nv.month=[-1]}else if(in_array(hval,v.month)){months[x].checked=!0;nv.month.push(hval)}else{months[x].checked=!1}}
for(let x=0;x<days.length;x++){let hval=parseInt(days[x].value);if(v.day[0]==-1){days[x].checked=!0;nv.day=[-1]}else if(in_array(hval,v.day)){days[x].checked=!0;nv.day.push(hval)}else{days[x].checked=!1}}
...
function formSetFrequencyDisplay(fid,s){let wizard=document.querySelector('#'+fid+'_wizard');if(undefined==wizard){return!1}
if(s){wizard.style.display='inline-block'}else{wizard.style.display='none'}
return!1}
