...
if(ifel.length>0){let display=0;for(let f=0;f<ifel.length;f++){let cval='';switch(ifel[f].type.toLowerCase()){case 'select-one':cval=ifel[f].options[ifel[f].selectedIndex].value;for(let v=0;v<vals.length;v++){if(cval.toLowerCase()==vals[v]){display=1}}
break;case 'radio':case 'checkbox':if(ifel[f].checked){cval=ifel[f].value||1}
for(let v=0;v<vals.length;v++){if(cval.toLowerCase()==vals[v]){display=1}}
break;case 'textarea':if(trim(ifel.innerText).length){display=1}
break;default:cval=ifel[f].value;for(let v=0;v<vals.length;v++){if(cval.toLowerCase()==vals[v]){display=1}
