...
let els=frm.querySelectorAll('[data-displayif]');if(debug==1){console.log(' - displayif el count:'+els.length)}
let display_count=0;for(let i=0;i<els.length;i++){let edebug=0;if(undefined!=els[i].dataset.debug&&parseInt(els[i].dataset.debug)==1){edebug=1;console.log('edebug set');console.log(els[i])}
let parts=els[i].dataset.displayif.split(':');let name=els[i].dataset.displayif;let vals=new Array('1','y','yes','notblank');if(parts.length==2){name=parts[0];vals=parts[1].split(',')}
for(let v=0;v<vals.length;v++){if(Number.isInteger(vals[v])){vals[v]=vals[v].toString()}}
let ifel=frm.querySelectorAll('[name="'+name+'"], [name="'+name+'[]"]');if(debug==1||edebug==1){console.log(' - i:'+i+', name:'+name+', form el count:'+ifel.length)}
if(undefined==ifel){continue}
if(ifel.length>0){let display=0;for(let f=0;f<ifel.length;f++){let cval='';switch(ifel[f].type.toLowerCase()){case 'select-one':cval=ifel[f].options[ifel[f].selectedIndex].value;for(let v=0;v<vals.length;v++){if(vals[v].toLowerCase()=='null'&&cval.length==0){display=1}else if(vals[v].toLowerCase()=='notblank'&&cval.length>0){display=1}else if(cval.toLowerCase()==vals[v].toLowerCase()){display=1}}
break;case 'radio':case 'checkbox':if(ifel[f].checked){cval=ifel[f].value||1}
for(let v=0;v<vals.length;v++){if(vals[v].toLowerCase()=='null'&&cval.length==0){display=1}else if(vals[v].toLowerCase()=='notblank'&&cval.length>0){display=1}else if(cval.toLowerCase()==vals[v].toLowerCase()){display=1}}
break;case 'textarea':if(trim(ifel.innerText).length){display=1}
break;default:cval=ifel[f].value;for(let v=0;v<vals.length;v++){if(vals[v].toLowerCase()=='null'&&cval.length==0){display=1}else if(vals[v].toLowerCase()=='notblank'&&cval.length>0){display=1}else if(cval.toLowerCase()==vals[v].toLowerCase()){display=1}}
break}
if(display==1){break}}
if(debug==1||edebug==1){console.log(' - display:'+display+', name:'+name+', vals:'+vals);console.log(' ---------------------------')}
if(display==1){if(debug==1||edebug==1){console.log(els[i]);console.log(els[i].style)}
if(undefined!=els[i].dataset.display){els[i].style.display=els[i].dataset.display;if(debug==1||edebug==1){console.log(' - set 1 style.display:'+els[i].dataset.display)}}else{els[i].style.display='initial';if(debug==1||edebug==1){console.log(' - set 2 style.display: initial')}}
display_count=display_count+1}else{els[i].style.display='none';if(debug==1||edebug==1){console.log(' - set 3 style.display: none')}}}else{els[i].style.display='none';if(debug==1||edebug==1){console.log(' - set 4 style.display: none')}}
if(debug==1||edebug==1){console.log(' ---------------------------')}}
els=frm.querySelectorAll('[data-hideif]');if(debug==1){console.log(' - hideif el count:'+els.length)}
let hide_count=0;for(let i=0;i<els.length;i++){let parts=els[i].dataset.hideif.split(':');let name=els[i].dataset.hideif;let vals=new Array('1','y','yes','notblank');if(parts.length==2){name=parts[0];vals=parts[1].split(',')}
for(let v=0;v<vals.length;v++){if(Number.isInteger(vals[v])){vals[v]=vals[v].toString()}}
let ifel=frm.querySelectorAll('[name="'+name+'"], [name="'+name+'[]"]');if(undefined==ifel){continue}
if(ifel.length>0){let hide=0;for(let f=0;f<ifel.length;f++){let cval='';switch(ifel[f].type.toLowerCase()){case 'select-one':cval=ifel[f].options[ifel[f].selectedIndex].value;for(let v=0;v<vals.length;v++){if(vals[v].toLowerCase()=='null'&&cval.length==0){hide=1}else if(vals[v].toLowerCase()=='notblank'&&cval.length>0){hide=1}else if(cval.toLowerCase()==vals[v].toLowerCase()){hide=1}}
break;case 'radio':case 'checkbox':if(ifel[f].checked){cval=ifel[f].value||1}
for(let v=0;v<vals.length;v++){if(vals[v].toLowerCase()=='null'&&cval.length==0){hide=1}else if(vals[v].toLowerCase()=='notblank'&&cval.length>0){hide=1}else if(cval.toLowerCase()==vals[v].toLowerCase()){hide=1}}
break;case 'textarea':if(trim(ifel.innerText).length){hide=1}
break;default:cval=ifel[f].value;for(let v=0;v<vals.length;v++){if(vals[v].toLowerCase()=='null'&&cval.length==0){hide=1}else if(vals[v].toLowerCase()=='notblank'&&cval.length>0){hide=1}else if(cval.toLowerCase()==vals[v].toLowerCase()){hide=1}}
break}
if(hide==1){break}}
if(hide==0){if(undefined!=els[i].dataset.display){els[i].style.display=els[i].dataset.display}else{els[i].style.display='initial'}}else{els[i].style.display='none'}}}
els=frm.querySelectorAll('[data-readonlyif]');if(debug==1){console.log(' - readonlyif el count:'+els.length)}
let readonly_count=0;for(let i=0;i<els.length;i++){let edebug=0;if(undefined!=els[i].dataset.debug&&parseInt(els[i].dataset.debug)==1){edebug=1;console.log('edebug set');console.log(els[i])}
let parts=els[i].dataset.readonlyif.split(':');let name=els[i].dataset.readonlyif;let vals=new Array('1','y','yes','notblank');if(parts.length==2){name=parts[0];vals=parts[1].split(',')}
for(let v=0;v<vals.length;v++){if(Number.isInteger(vals[v])){vals[v]=vals[v].toString()}}
let ifel=frm.querySelectorAll('[name="'+name+'"], [name="'+name+'[]"]');if(debug==1||edebug==1){console.log(' - i:'+i+', name:'+name+', form el count:'+ifel.length)}
if(undefined==ifel){continue}
if(ifel.length>0){let readonly=0;for(let f=0;f<ifel.length;f++){let cval='';switch(ifel[f].type.toLowerCase()){case 'select-one':cval=ifel[f].options[ifel[f].selectedIndex].value;for(let v=0;v<vals.length;v++){if(vals[v].toLowerCase()=='null'&&cval.length==0){readonly=1}else if(vals[v].toLowerCase()=='notblank'&&cval.length>0){readonly=1}else if(cval.toLowerCase()==vals[v].toLowerCase()){readonly=1}}
break;case 'radio':case 'checkbox':if(ifel[f].checked){cval=ifel[f].value||1}
for(let v=0;v<vals.length;v++){if(vals[v].toLowerCase()=='null'&&cval.length==0){readonly=1}else if(vals[v].toLowerCase()=='notblank'&&cval.length>0){readonly=1}else if(cval.toLowerCase()==vals[v].toLowerCase()){readonly=1}}
break;case 'textarea':if(trim(ifel.innerText).length){readonly=1}
break;default:cval=ifel[f].value;for(let v=0;v<vals.length;v++){if(vals[v].toLowerCase()=='null'&&cval.length==0){readonly=1}else if(vals[v].toLowerCase()=='notblank'&&cval.length>0){readonly=1}else if(cval.toLowerCase()==vals[v].toLowerCase()){readonly=1}}
break}
if(readonly==1){break}}
if(debug==1||edebug==1){console.log(' - readonly:'+readonly+', name:'+name+', vals:'+vals);console.log(' ---------------------------')}
if(readonly==1){if(!els[i].hasAttribute('onclick')){els[i].setAttribute('onclick','return false');els[i].setAttribute('onclickx','1')}
els[i].setAttribute('readonly','readonly')}else{els[i].removeAttribute('readonly');if(els[i].hasAttribute('onclickx')){els[i].removeAttribute('onclick');els[i].removeAttribute('onclickx')}}}else{els[i].removeAttribute('readonly');if(els[i].hasAttribute('onclickx')){els[i].removeAttribute('onclick');els[i].removeAttribute('onclickx')}}}
els=frm.querySelectorAll('[data-changeif]');if(debug==1){console.log(' - changeif el count:'+els.length)}
let change_count=0;for(let i=0;i<els.length;i++){if(undefined==els[i].dataset.value){if(debug==1){console.log(' - setting dataset.value for: '+els[i].type.toLowerCase())}
switch(els[i].type.toLowerCase()){case 'select-one':els[i].dataset.value=els[i].options[els[i].selectedIndex].value||'';break;case 'textarea':els[i].dataset.value=els[i].innerText||'';break;default:els[i].dataset.value=els[i].value||'';break}}
let changeif=els[i].dataset.changeif;let matches=changeif.split(',');if(undefined!=matches){let matchvalue=matches[1];let conditionstr=matches[0];let conditions=conditionstr.split(' and ');if(conditions.length>1){let fail=0;let success=0;for(let c=0;c<conditions.length;c++){let parts=conditions[c].split('=');let field=parts.shift();let fieldval=parts.join('=');let fieldobj=frm.querySelector('[name="'+field+'"], [name="'+field+'[]"]');if(undefined==fieldobj){fails=1;break}
switch(fieldobj.type.toLowerCase()){case 'select-one':if(fieldval==fieldobj.options[fieldobj.selectedIndex].value){success=1}else{fail=1}
break;case 'textarea':if(fieldval.toLowerCase()==trim(fieldobj.innerText.toLowerCase())){success=1}else{fail=1}
break;default:if(fieldval.toLowerCase()==fieldobj.value.toLowerCase()){success=1}else{fail=1}
break}}
if(success==1&&fail==0){if(debug==1){console.log(' - AND set value for '+els[i].name+' to '+matchvalue)}
els[i].value=matchvalue}else{if(debug==1){console.log(' - AND set value for '+els[i].name+' to '+els[i].dataset.value)}
els[i].value=els[i].dataset.value}}else{conditions=conditionstr.split(' or ');if(conditions.length>1){let success=0;for(let c=0;c<conditions.length;c++){let parts=conditions[c].split('=');let field=parts.shift();let fieldval=parts.join('=');let fieldobj=frm.querySelector('[name="'+field+'"], [name="'+field+'[]"]');if(undefined==fieldobj){break}
switch(fieldobj.type.toLowerCase()){case 'select-one':if(fieldval==fieldobj.options[fieldobj.selectedIndex].value){success=1}
break;case 'textarea':if(fieldval.toLowerCase()==trim(fieldobj.innerText.toLowerCase())){success=1}
break;default:if(fieldval.toLowerCase()==fieldobj.value.toLowerCase()){success=1}
break}}
if(success==1){if(debug==1){console.log(' - OR set value for '+els[i].name+' to '+matchvalue)}
els[i].value=matchvalue}else{if(debug==1){console.log(' - OR set value for '+els[i].name+' to '+els[i].dataset.value)}
els[i].value=els[i].dataset.value}}else{success=0;let parts=conditions[0].split('=');let field=parts.shift();let fieldval=parts.join('=');let fieldobj=frm.querySelector('[name="'+field+'"], [name="'+field+'[]"]');if(debug==1){console.log(' - SINGLE field:'+field+', fieldval:'+fieldval)}
if(undefined==fieldobj){break}
switch(fieldobj.type.toLowerCase()){case 'select-one':let cval=fieldobj.options[fieldobj.selectedIndex].value;if(debug==1){console.log(' - SINGLE cval:'+cval)}
if(fieldval==cval){success=1}
break;case 'textarea':if(fieldval.toLowerCase()==trim(fieldobj.innerText.toLowerCase())){success=1}
break;default:if(fieldval.toLowerCase()==fieldobj.value.toLowerCase()){success=1}
break}
if(debug==1){console.log(' - SINGLE success:'+success)}
if(success==1){if(debug==1){console.log(' - SINGLE 1 set value for '+els[i].name+' to '+matchvalue)}
els[i].value=matchvalue}else{if(debug==1){console.log(' - SINGLE 0 set value for '+els[i].name+' to '+els[i].dataset.value)}
els[i].value=els[i].dataset.value}}}}}
return}
function formRecorderAudioControl(el){let textbox=document.querySelector('#'+el.dataset.id+'_base64');if(undefined==textbox){return!1}
...
player=document.createElement('audio');let fname=el.dataset.id+'.webm';let str=fname+';'+fname+';';let base64String=textbox.textContent.replace(str,'');player.src=base64String;player.id=el.dataset.id+'_player';player.type='audio/webm';audiobox.appendChild(player);el.classList.remove('icon-play');el.classList.add('icon-pause');player.play()}else if(el.className.indexOf('icon-pause')>=0){el.classList.add('icon-play');el.classList.remove('icon-pause');player.pause()}else if(el.className.indexOf('icon-reset')>=0){let prev=document.querySelector('#'+el.dataset.id+'_prev');if(undefined!=prev){textbox.textContent=prev.textContent;el.parentNode.removeChild(el)}
return!1}
player.el=el;player.addEventListener('ended',function(){this.el.classList.add('icon-play');this.el.classList.remove('icon-pause')});return!1}
...
if(undefined!=el.mix){el.mix.pause()}}else{el.stream=null;try{el.stream=await navigator.mediaDevices.getUserMedia({audio:!0})}catch(err){console.log(err);return!1}
...
if(this.f==this.fmax&&undefined==labeltxt.querySelector('span.icon-erase')){let span=document.createElement('span');span.className='w_danger icon-erase';span.style.fontSize='16px';span.style.marginLeft='10px';span.title='Clear';span.fld=fld;span.labeltxt=labeltxt;span.onclick=function(e){cancelBubble(e);this.fld.value='';setText(this.labeltxt,this.labeltxt.dataset.text)}
labeltxt.appendChild(span)}}
reader.readAsDataURL(fld.files[f])}}
function formClearFileField(id){}
function formSendPhoneAuth(el){let phone_id=el.getAttribute('data-username_id');let phone=getText(phone_id);if(phone.length<10){alert('Enter Valid Phone Number to authorize');return!1}
return ajaxGet('/php/index.php',el.id,{send_phone_auth:1,phone:phone})}
function formShowPassword(id,sh){let obj=getObject(id);if(undefined==id){return!1}
if(undefined==sh){return!1}
if(undefined==obj){return!1}
if(sh==1){obj.setAttribute('type','text')}else{obj.setAttribute('type','password')}
return!1}
...
function formSetFrequency(fid,v){let container=document.querySelector('#'+fid+'_container');if(undefined==container){console.log('formSetFrequency - no container');return!1}
...
if(typeof(v)=='string'){if(v.length>0){v=JSON.parse(v)}else{v={minute:[],hour:[],month:[],day:[],dayname:[]}}}
...
break;case 'dayname':for(let x=0;x<daynames.length;x++){daynames[x].checked=!1}
...
console.log(v);for(let x=0;x<daynames.length;x++){let hval=parseInt(daynames[x].value);if(undefined!=v.dayname&&undefined!=v.dayname[0]&&v.dayname[0]==-1){daynames[x].checked=!0;nv.dayname=[-1]}else if(in_array(hval,v.dayname)){daynames[x].checked=!0;nv.dayname.push(hval)}else{daynames[x].checked=!1}}
...
for(let x=0;x<daynames.length;x++){let hval=parseInt(daynames[x].value);console.log(daynames[x]);if(daynames[x].checked){nv.dayname.push(hval)}}
if(daynames.length==nv.dayname.length){nv.dayname=[-1]}else if(nv.dayname.length==0){nv.dayname=[1]}
...
return!1}
function formSetFrequencyDisplay(fid,s){let ev=this.event||window.event;ev.stopPropagation();let fidstr='#'+fid+'_wizard';let wizard=document.querySelector(fidstr);if(undefined==wizard){return!1}
...
function formDictate(inp,ico,frm,continuous){inp=getObject(inp);if(undefined==inp){let err='formDictate error: undefined input '+inp;console.log(err);return!1}
ico=getObject(ico);if(window.hasOwnProperty('webkitSpeechRecognition')){var recognition=new webkitSpeechRecognition();recognition.continuous=continuous||!1;recognition.interimResults=!1;recognition.lang="en-US";if(undefined!=ico){ico.classList.add('w_blink');ico.classList.add('w_success');recognition.ico=ico}
if(undefined!=frm){recognition.frm=frm}
recognition.inp=inp;recognition.start();recognition.onresult=function(e){if(undefined!=this.frm&&undefined!=window[this.frm]){try{window[this.frm](e.results[0][0].transcript);this.stop();this.start()}catch(e){}}
...
if(undefined!=this.frm){simulateEvent(this.frm,'submit')}};recognition.onend=function(e){if(this.continuous){try{this.start()}catch(e){}}else{try{this.stop()}catch(e){}
...
switch(e.error.toLowerCase()){case 'no-speech':if(this.continuous){try{this.restart()}catch(e){}}else{try{this.stop()}catch(e){}
if(undefined!=this.ico){this.ico.classList.remove('w_blink');this.ico.classList.remove('w_success')}}
break;case 'aborted':if(undefined!=this.inp.value){this.inp.value=e.message}else{this.inp.textContent=e.message}
break;case 'audio-capture':if(undefined!=this.inp.value){this.inp.value=e.message}else{this.inp.textContent=e.message}
break;case 'network':if(undefined!=this.inp.value){this.inp.value=e.message}else{this.inp.textContent=e.message}
break;case 'not-allowed':if(undefined!=this.inp.value){this.inp.value=e.message}else{this.inp.textContent=e.message}
break;case 'service-not-allowed':if(undefined!=this.inp.value){this.inp.value=e.message}else{this.inp.textContent=e.message}
break;case 'bad-grammar':if(undefined!=this.inp.value){this.inp.value=e.message}else{this.inp.textContent=e.message}
break;case 'language-not-supported':if(undefined!=this.inp.value){this.inp.value=e.message}else{this.inp.textContent=e.message}
break}}}else{let msg='webkitSpeechRecognition property not found on this device';if(undefined!=this.inp.value){this.inp.value=msg}else{this.inp.textContent=msg}}
return!1}
...
function initPikadayCalendar(field,params){field=getObject(field);let attrs=getAllAttributes(field);let opts={};if(undefined!=attrs['data-showtime']){opts={field:field,showTime:!0,showMinutes:!0,showSeconds:!1,use24hour:!1,incrementHourBy:1,incrementMinuteBy:5,position:'bottom right',format:'YYYY-MM-DD HH:mm',toString(date,format){let day=date.getDate();let month=date.getMonth()+1;let year=date.getFullYear();let hour=date.getHours();let min=date.getMinutes();if(day<10){day='0'+day}
if(month<10){month='0'+month}
if(hour<10){hour='0'+hour}
if(min<10){min='0'+min}
return year+'-'+month+'-'+day+' '+hour+':'+min+':00'},parse(dateString,format){let parts=dateString.split(/[\/\ \:\-]+/);let year=parseInt(parts[0],10);let month=parseInt(parts[1],10)-1;let day=parseInt(parts[2],10);let hour=parseInt(parts[3],10);let min=parseInt(parts[4],10);return new Date(year,month,day,hour,min,0)}}}else{opts={field:field,position:'bottom right',format:'YYYY-MM-DD',toString(date,format){let day=date.getDate();let month=date.getMonth()+1;let year=date.getFullYear();if(day<10){day='0'+day}
if(month<10){month='0'+month}
...
if(undefined!=attrs['data-maxdate']){if(attrs['data-maxdate'].indexOf('-')!=-1){let dateparts=attrs['data-maxdate'].split("-");if(dateparts.length==3){dateparts[1]=parseInt(dateparts[1])-1;opts.maxDate=new Date(dateparts[0],dateparts[1],dateparts[2])}else{let n=parseInt(attrs['data-maxdate'])*-1;let d=new Date();d.setDate(d.getDate()-n);opts.maxDate=d}}else{switch(attrs['data-maxdate'].toLowerCase()){case 'now':case 'today':opts.maxDate=new Date();break;default:opts.maxDate=new Date(attrs['data-maxdate']);break}}}
if(undefined!=attrs['data-mindate']){if(attrs['data-mindate'].indexOf('-')!=-1){let dateparts=attrs['data-mindate'].split("-");if(dateparts.length==3){dateparts[1]=parseInt(dateparts[1])-1;opts.minDate=new Date(dateparts[0],dateparts[1],dateparts[2])}else{let n=parseInt(attrs['data-mindate'])*-1;let d=new Date();d.setDate(d.getDate()-n);opts.minDate=d}}else{switch(attrs['data-mindate'].toLowerCase()){case 'now':case 'today':opts.minDate=new Date();break;default:opts.minDate=new Date(attrs['data-mindate']);break}}}
...
if(undefined!=attrs['data-showweeknumber']){opts.showWeekNumber=!0}
...
return!1}}
function formSetMultiSelectStatus(fld){if(undefined==fld.getAttribute('data-id')){return!1}
let id=fld.getAttribute('data-id');let oid=id+'_options';let options=getObject(oid);if(undefined==options){return!1}
...
let btn_id=id+'_button';let bobj=getObject(btn_id);if(vals.length==0){bobj.innerText=bobj.getAttribute('data-dname')}else{let str='('+vals.length+') '+implode(', ',vals);bobj.innerText=str}
return!0}
...
if(function_exists(wacss.geoLocation)){wacss.geoLocation(fld,opts)}else{console.log('Error: You must enable wacss in your template to use getGeoLocation')}}
...
submitFormAlert('Password is required',popup,5);theForm[i].focus();return!1}
...
submitFormAlert('Email is required',popup,3);theForm[i].focus();return!1}
...
break}}}else{let not=0;let cvals=parts[1].split(',');if(notparts.length==2){not=1;cvals=notparts[1].split(',')}
if(undefined!=requiredif){switch(requiredif.type.toLowerCase()){case 'select-one':rval=requiredif.options[requiredif.selectedIndex].value;for(let c=0;c<cvals.length;c++){if(not==1){if(rval!=cvals[c]){required=1;break}}else{if(rval==cvals[c]){required=1;break}}}
break;case 'select-multiple':let rvals=Array.from(requiredif.selectedOptions).map(el=>el.value);for(let r=0;r<rvals.length;r++){rval=rvals[r];for(let c=0;c<cvals.length;c++){if(not==1){if(rval!=cvals[c]){required=1;break}}else{if(rval==cvals[c]){required=1;break}}}}
break;case 'radio':case 'checkbox':let rels=theForm.querySelectorAll('[type="'+requiredif.type+'"][name="'+requiredif.name+'"]');for(let r=0;r<rels.length;r++){if(rels[r].checked){rval=rels[r].value||1;for(let c=0;c<cvals.length;c++){if(not==1){if(rval!=cvals[c]){required=1;break}}else{if(rval==cvals[c]){required=1;break}}}}}
break;case 'textarea':rval=requiredif.innerText;for(let c=0;c<cvals.length;c++){if(not==1){if(rval!=cvals[c]){required=1;break}}else{if(rval==cvals[c]){required=1;break}}}
break
default:rval=requiredif.value;for(let c=0;c<cvals.length;c++){if(not==1){if(rval!=cvals[c]){required=1;break}}else{if(rval==cvals[c]){required=1;break}}}
break}}}}
...
wacss.blink(theForm[i].getAttribute('data-blink'))}else{submitFormAlert(msg,popup,5)}
...
wacss.blink(theForm[i].getAttribute('data-blink'))}else{submitFormAlert(msg,popup,5)}
if(debug==1){console.log(msg)}
theForm[i].focus();return!1}}else if(theForm[i].type=='file'){var cval=theForm[i].files[0];if(cval.length==0){let tmp=theForm.querySelector('span#'+theForm[i].name+'_text a');if(undefined!=tmp){cval=tmp.getAttribute('href')}}
if(cval.length==0){var msg=dname+" is required";if(undefined!=requiredmsg){msg=requiredmsg}
if(typeof wacss.blink==="function"&&undefined!=theForm[i].getAttribute('data-blink')){if(typeof wacss.toast==="function"){wacss.toast(msg)}
wacss.blink(theForm[i].getAttribute('data-blink'))}else{submitFormAlert(msg,popup,5)}
if(debug==1){console.log(msg)}
return!1}}else if(theForm[i].value==''){var msg=dname+" is required";if(undefined!=requiredmsg){msg=requiredmsg}
if(typeof wacss.blink==="function"&&undefined!=theForm[i].getAttribute('data-blink')){if(typeof wacss.toast==="function"){wacss.toast(msg)}
wacss.blink(theForm[i].getAttribute('data-blink'))}else{submitFormAlert(msg,popup,5)}
if(debug==1){console.log(msg)}
...
if(mask=='ccnumber'){if(isCreditCardNumber(theForm[i].value)==!1){let msg=fldmsg||dname+" must be of valid credit card number ";submitFormAlert(msg,popup,5);if(debug==1){console.log(msg)}
theForm[i].focus();return!1}}else if(mask=='date'){if(isDate(theForm[i].value)==!1){let msg=fldmsg||dname+" must be a valid date ";submitFormAlert(msg,popup,5);if(debug==1){console.log(msg)}
theForm[i].focus();return!1}}else if(mask=='futuredate'){if(isFutureDate(theForm[i].value)==!1){let msg=fldmsg||dname+" must be of valid date in the future ";submitFormAlert(msg,popup,5);if(debug==1){console.log(msg)}
theForm[i].focus();return!1}}else if(mask=='pastdate'){if(isPastDate(theForm[i].value)==!1){let msg=fldmsg||dname+" must be of valid date in the past ";submitFormAlert(msg,popup,5);if(debug==1){console.log(msg)}
theForm[i].focus();return!1}}else if(mask=='intlphone'){if(checkInternationalPhone(theForm[i].value)==!1){let msg=fldmsg||dname+" must be a valid phone number";submitFormAlert(msg,popup,5);if(debug==1){console.log(msg)}
theForm[i].focus();return!1}}else if(mask!='searchandreplace'){if(mask=='phone'){theForm[i].value=formatPhone(theForm[i].value)}else if(mask=='calendar'){theForm[i].value=formatCalendar(theForm[i].value)}
var rmask=mask;if(undefined!=quickmask[mask]){rmask=quickmask[mask]+''}
var re=new RegExp(rmask,'i');if(re.test(theForm[i].value)==!1){let msg=fldmsg||dname+" must be of type "+mask;submitFormAlert(msg,popup,5);theForm[i].focus();if(debug==1){console.log(msg)}
return!1}}}
...
submitFormAlert(msg,popup,5);if(debug==1){console.log(msg)}
theForm[i].focus();return!1}}
if(theForm[i].type=='file'&&theForm[i].getAttribute('accept')&&theForm[i].value.length){let allow=theForm[i].getAttribute('accept');if(debug==1){alert(" -- File type allowed exts:"+allow)}
let exts=allow.split(',');let valid=0;for(s=0;s<exts.length;s++){if(theForm[i].value.lastIndexOf(exts[s])!=-1){valid++}}
if(valid==0){let msg=dname+" must be of valid file type:  "+allow;if(theForm[i].getAttribute('acceptmsg')){msg=theForm[i].getAttribute('acceptmsg')}
submitFormAlert(msg,popup,5);if(debug==1){console.log(msg)}
theForm[i].focus();return!1}}}
...
if(undefined!=theForm.show_upload_progress){formShowUploadProgress(theForm)}else{let dlist=theForm.querySelectorAll('.w_disable_on_submit');for(let d=0;d<dlist.length;d++){dlist[d].setAttribute('disabled','disabled')}
...
function pagingClearFilters(frm){var f=frm.querySelectorAll('.w_pagingfilter');if(undefined==f){return!1}
...
function ajaxPost(theform,sid,tmeout,callback,returnreq,abort_callback){let fields=theform.querySelectorAll('input[type="file"]');for(let i=0;i<fields.length;i++){if(undefined==fields[i].id){continue}
...
if(undefined==theform){alert("No form object passed to ajaxPost");return!1}
...
var txt='';if(cptext.indexOf('w_centerpop_content')!=-1){txt=cptext}else{txt+='<div class="w_centerpop">'+"\n";if(params.title.length){txt+='	<div class="w_centerpop_title">'+params.title+'</div>'+"\n"}
