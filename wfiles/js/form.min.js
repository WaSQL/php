function formJsonPretty(id){let txt=getText(id);let jsonobj=JSON.parse(txt);let ptxt=JSON.stringify(jsonobj,null,'\t');setText(id,ptxt);}
function formIsIfTrue(frm,ifstr){if(undefined==ifstr||ifstr.length==0){return false;}
...
return false;}
function formChanged(frm,debug){if(undefined==debug){debug=0;}
if(debug==1){console.log('formChanged');}
if(undefined==frm){return false;}
let els=frm.querySelectorAll('[data-displayif]');for(let i=0;i<els.length;i++){if(formIsIfTrue(frm,els[i].dataset.displayif)){if(undefined!=els[i].dataset.display){els[i].style.display=els[i].dataset.display;}
else{els[i].style.display='initial';}}
else{els[i].style.display='none';}}
els=frm.querySelectorAll('[data-hideif]');for(let i=0;i<els.length;i++){if(formIsIfTrue(frm,els[i].dataset.hideif)){els[i].style.display='none';}
else{if(undefined!=els[i].dataset.display){els[i].style.display=els[i].dataset.display;}
else{els[i].style.display='initial';}}}
els=frm.querySelectorAll('[data-readonlyif]');for(let i=0;i<els.length;i++){if(formIsIfTrue(frm,els[i].dataset.readonlyif)){if(!els[i].hasAttribute('onclick')){els[i].setAttribute('onclick','return false');els[i].setAttribute('onclickx','1');}
...
els=frm.querySelectorAll('[data-requiredif]');for(let i=0;i<els.length;i++){if(formIsIfTrue(frm,els[i].dataset.requiredif)){els[i].setAttribute('required','required');if(undefined!=els[i].dataset.requiredmsg){els[i].setCustomValidity(els[i].dataset.requiredmsg);}}
else{els[i].removeAttribute('required');if(els[i].hasAttribute('data-required')){els[i].removeAttribute('data-required');}}}
els=frm.querySelectorAll('[data-blankif]');for(let i=0;i<els.length;i++){if(formIsIfTrue(frm,els[i].dataset.blankif)){switch(els[i].type.toLowerCase()){case'radio':case'checkbox':console.error('blankif does not support checkbox and radio inputs');break;case'textarea':els[i].dataset.blankx=trim(els[i].innerHTML);if(undefined!=els[i].codemirror){els[i].codemirror.getDoc().setValue('');}
els[i].innerHTML='';break;default:els[i].dataset.blankx=els[i].value;els[i].value='';break;}}
else{if(undefined!=els[i].dataset.blankx){switch(els[i].type.toLowerCase()){case'radio':case'checkbox':console.error('blankif does not support checkbox and radio inputs');break;case'textarea':if(undefined!=els[i].codemirror){els[i].codemirror.getDoc().setValue(els[i].dataset.blankx);}
els[i].innerHTML='';els[i].innerHTML=els[i].dataset.blankx;break;default:els[i].value=els[i].dataset.blankx;break;}}}}
els=frm.querySelectorAll('[data-classif]');for(let i=0;i<els.length;i++){let parts=els[i].dataset.classif.split(':');let eclass=parts.shift();let ifstr=parts.join(':');if(formIsIfTrue(frm,ifstr)){els[i].classList.add(eclass);}
else{els[i].classList.remove(eclass);}}
return;}
function formRecorderAudioControl(el){let textbox=document.querySelector('#'+el.dataset.id+'_base64');if(undefined==textbox){return false;}
let audiobox=document.querySelector('#'+el.dataset.id+'_audiobox');let player=document.querySelector('#'+el.dataset.id+'_player');if(el.className.indexOf('icon-play')>=0){if(undefined!=player){audiobox.removeChild(player);}
player=document.createElement('audio');let fname=el.dataset.id+'.webm';let str=fname+';'+fname+';';let base64String=textbox.textContent.replace(str,'');player.src=base64String;player.id=el.dataset.id+'_player';player.type='audio/webm';audiobox.appendChild(player);el.classList.remove('icon-play');el.classList.add('icon-pause');player.play();}
else if(el.className.indexOf('icon-pause')>=0){el.classList.add('icon-play');el.classList.remove('icon-pause');player.pause();}
else if(el.className.indexOf('icon-reset')>=0){let prev=document.querySelector('#'+el.dataset.id+'_prev');if(undefined!=prev){textbox.textContent=prev.textContent;el.parentNode.removeChild(el);}
return false;}
player.el=el;player.addEventListener('ended',function(){this.el.classList.add('icon-play');this.el.classList.remove('icon-pause');});return false;}
async function formRecorderAudio(el){if(undefined==el.recorder){el.recorder={};}
if(undefined==el.recordedchunks){el.recordedchunks=[];}
let mic=document.querySelector('#'+el.dataset.id+'_mic');if(undefined!=el.recorder.state&&el.recorder.state=="recording"){el.recorder.stop();el.stream.getAudioTracks()[0].stop();if(undefined!=mic){mic.classList.remove('w_red');mic.classList.remove('w_blink');}
if(undefined!=el.dataset.record){el.textContent=el.dataset.record;}
if(undefined!=el.mix){el.mix.pause();}}
else{el.stream=null;try{el.stream=await navigator.mediaDevices.getUserMedia({audio:true});}catch(err){console.log(err);return false;}
if(undefined!=el.stream){el.recorder=new MediaRecorder(el.stream);el.recorder.el=el;el.recorder.ondataavailable=function(e){if(e.data.size>0){this.el.recordedchunks.push(e.data);let blob=new Blob(this.el.recordedchunks,{type:"audio/webm"});let url=URL.createObjectURL(blob);let reader=new FileReader();reader.readAsDataURL(blob);reader.el=this.el;reader.onloadend=function(){let base64String=this.result;let fld=document.querySelector('#'+el.dataset.id+'_base64');let audiobox=document.querySelector('#'+this.el.dataset.id+'_audiobox');if(undefined!=fld){let fname=this.el.dataset.id+'.webm';let str=fname+';'+fname+';';fld.textContent=str+base64String;let reset=document.querySelector('#'+this.el.dataset.id+'_reset');let prev=document.querySelector('#'+el.dataset.id+'_prev');if(undefined==reset&&undefined!=prev){reset=document.createElement('span');reset.dataset.id=this.el.dataset.id;reset.id=this.el.dataset.id+'_reset';reset.title='reset';reset.style='padding:0.375rem 0.75rem;border-left:1px solid #CCC;';reset.className='icon-reset w_pointer';reset.setAttribute('onclick','formRecorderAudioControl(this);');audiobox.appendChild(reset);}}
this.el.recordedchunks=[];let play=document.querySelector('#'+this.el.dataset.id+'_play');if(undefined==play){play=document.createElement('span');play.dataset.id=this.el.dataset.id;play.id=this.el.dataset.id+'_play';play.style='padding:0.375rem 0.75rem;border-left:1px solid #CCC;';play.className='icon-play w_pointer';play.setAttribute('onclick','formRecorderAudioControl(this);');audiobox.appendChild(play);}}}};if(undefined!=el.dataset.mix){let mix=document.createElement('audio');mix.src=el.dataset.mix;mix.el=el;el.mix=mix;mix.addEventListener('canplaythrough',function(){this.play();this.el.recorder.start();});}
else{el.recorder.start();}
if(undefined!=mic){mic.classList.add('w_red');mic.classList.add('w_blink');}
if(undefined!=el.dataset.stop){el.textContent=el.dataset.stop;}}}}
function setInputFileName(fld){let multiple=0;if(fld.multiple){multiple=1;}
let label=document.querySelector('label[for='+fld.id+']');let labeltxt=document.querySelector('label[for='+fld.id+'] span.input_file_text');let pform=getParentForm(fld);if(undefined==labeltxt.dataset.text){labeltxt.dataset.text=getText(labeltxt);}
if(undefined==fld.files||fld.files.length==0){setText(labeltxt,labeltxt.dataset.text);return true;}
if(multiple==0){setText(labeltxt,'');}
for(let f=0;f<fld.files.length;f++){if(undefined!=fld.files[f].size&&fld.files[f].size>500000){setText(labeltxt,fld.files[f].name);let span=document.createElement('span');span.className='w_danger icon-erase';span.style.fontSize='16px';span.style.marginLeft='25px';span.title='Clear';span.fld=fld;span.labeltxt=labeltxt;span.onclick=function(e){cancelBubble(e);this.fld.value='';setText(this.labeltxt,this.labeltxt.dataset.text);}
labeltxt.appendChild(span);console.log('setInputFileName: skipping due to size: '+fld.files[f].name);continue;}
let reader=new FileReader();reader.label=label;reader.labeltxt=labeltxt;reader.fld=fld;reader.cfile=fld.files[f];reader.filename=fld.files[f].name;reader.f=f;reader.pform=pform;reader.fmax=fld.files.length-1;reader.onload=function(e){let h=document.createElement('textarea');h.style.display='none';h.id=this.fld.id+'_base64_content';h.innerHTML=this.filename+';'+this.filename+';'+e.target.result;this.pform.appendChild(h);let ext=this.filename.split('.').pop().toLowerCase();if(this.result.indexOf('data:image')==0){let img=document.createElement('img');img.style.display='inline';img.style.height='24px';img.src=e.target.result;img.title=this.filename;this.labeltxt.appendChild(img);}
else if(this.result.indexOf('data:audio')==0){let span=document.createElement('span');span.className='w_gray icon-file-audio';span.style.fontSize='26px';span.title=this.filename;this.labeltxt.appendChild(span);}
else if(this.result.indexOf('data:video')==0){let span=document.createElement('span');span.className='w_gray icon-file-video';span.style.fontSize='26px';span.title=this.filename;this.labeltxt.appendChild(span);}
else{let cname='w_gray ';switch(ext){case'pdf':cname=cname+'icon-file-pdf2';break;case'xls':case'xlsx':cname=cname+'icon-file-excel';break;case'doc':case'docx':cname=cname+'icon-file-word';break;case'zip':case'gz':cname=cname+'icon-file-zip';break;case'txt':case'csv':cname=cname+'icon-file-txt';break;default:cname=cname+'icon-file-doc';break;}
let span=document.createElement('span');span.className=cname;span.style.fontSize='26px';span.title=this.filename;this.labeltxt.appendChild(span);}
if(this.f==this.fmax&&undefined==labeltxt.querySelector('span.icon-erase')){let span=document.createElement('span');span.className='w_danger icon-erase';span.style.fontSize='16px';span.style.marginLeft='25px';span.title='Clear';span.fld=fld;span.labeltxt=labeltxt;span.onclick=function(e){cancelBubble(e);this.fld.value='';setText(this.labeltxt,this.labeltxt.dataset.text);}
labeltxt.appendChild(span);}}
reader.readAsDataURL(fld.files[f]);}}
function formClearFileField(id){}
function formSendPhoneAuth(el){let phone_id=el.getAttribute('data-username_id');let phone=getText(phone_id);if(phone.length<10){alert('Enter Valid Phone Number to authorize');return false;}
return ajaxGet('/php/index.php',el.id,{send_phone_auth:1,phone:phone});}
function formShowPassword(id,sh){let obj=getObject(id);if(undefined==id){return false;}
if(undefined==sh){return false;}
if(undefined==obj){return false;}
if(sh==1){obj.setAttribute('type','text');}
else{obj.setAttribute('type','password');}
return false;}
...
var allow_multiple=false;if(undefined!=fld.getAttribute('multiple')){allow_multiple=true;}
for(var i=0;i<filesSelected.length;i++){var fileToLoad=filesSelected[i];var fileReader=new FileReader();if(allow_multiple){fileReader.cname=fld.name+'_base64[]';}
else{fileReader.cname=fld.name+'_base64';}
fileReader.cid=pname+'_'+fld.name+'_base64';fileReader.allow_multiple=allow_multiple;fileReader.fname=fileToLoad.name;fileReader.fid=pname+'_'+fld.name+'_base64_filename';fileReader.onload=function(fileLoadedEvent){if(this.allow_multiple){var txtarea=document.createElement("textarea");txtarea.name=this.cname;txtarea.id=this.cid;txtarea.setAttribute('data-filename',this.fname);txtarea.innerHTML='filename:'+this.fname+';'+fileLoadedEvent.target.result;txtarea.style.display='none';parentForm.appendChild(txtarea);}
else{var txtobj=getObject(this.cid);if(undefined==txtobj){txtarea=document.createElement("textarea");txtarea.name=this.cname;txtarea.id=this.cid;txtarea.style.display='none';parentForm.appendChild(txtarea);}
...
function formSetFrequency(fid,v){let container=document.querySelector('#'+fid+'_container');if(undefined==container){console.log('formSetFrequency - no container');return false;}
...
break;}}
formSetFrequency(field.id);return false;}
...
else if(in_array(hval,v.minute)){minutes[x].checked=true;nv.minute.push(hval);}
...
else if(in_array(hval,v.hour)){hours[x].checked=true;nv.hour.push(hval);}
...
else if(in_array(hval,v.month)){months[x].checked=true;nv.month.push(hval)}
...
else if(in_array(hval,v.day)){days[x].checked=true;nv.day.push(hval);}
...
else if(in_array(hval,v.dayname)){daynames[x].checked=true;nv.dayname.push(hval);}
...
return false;}
function formSetFrequencyDisplay(fid,s){let ev=this.event||window.event;ev.stopPropagation();let fidstr='#'+fid+'_wizard';let wizard=document.querySelector(fidstr);if(undefined==wizard){return false;}
if(s){wizard.style.display='block';commonOpenClose(wizard,'display','none');}
...
ajaxPopup('/php/index.php',params,{id:'centerpop'});return false;}
...
function formDictate(inp,ico,frm,continuous){inp=getObject(inp);if(undefined==inp){let err='formDictate error: undefined input '+inp;console.log(err);return false;}
ico=getObject(ico);if(window.hasOwnProperty('webkitSpeechRecognition')){var recognition=new webkitSpeechRecognition();recognition.continuous=continuous||false;recognition.interimResults=false;recognition.lang="en-US";if(undefined!=ico){ico.classList.add('w_blink');ico.classList.add('w_success');recognition.ico=ico;}
if(undefined!=frm){recognition.frm=frm;}
recognition.inp=inp;recognition.start();recognition.onresult=function(e){if(undefined!=this.frm&&undefined!=window[this.frm]){try{window[this.frm](e.results[0][0].transcript);this.stop();this.start();}
catch(e){}}
this.inp.value=e.results[0][0].transcript;if(undefined!=this.ico){this.ico.classList.remove('w_blink');this.ico.classList.remove('w_success');}
try{this.stop();}catch(e){}
if(undefined!=this.frm){simulateEvent(this.frm,'submit');}};recognition.onend=function(e){if(this.continuous){try{this.start();}catch(e){}}
else{try{this.stop();}catch(e){}
if(undefined!=this.ico){this.ico.classList.remove('w_blink');this.ico.classList.remove('w_success');}}}
recognition.onerror=function(e){if(undefined==e.error){e.error='';}
switch(e.error.toLowerCase()){case'no-speech':if(this.continuous){try{this.restart();}catch(e){}}
else{try{this.stop();}catch(e){}
if(undefined!=this.ico){this.ico.classList.remove('w_blink');this.ico.classList.remove('w_success');}}
break;case'aborted':if(undefined!=this.inp.value){this.inp.value=e.message;}
else{this.inp.textContent=e.message;}
break;case'audio-capture':if(undefined!=this.inp.value){this.inp.value=e.message;}
else{this.inp.textContent=e.message;}
break;case'network':if(undefined!=this.inp.value){this.inp.value=e.message;}
else{this.inp.textContent=e.message;}
break;case'not-allowed':if(undefined!=this.inp.value){this.inp.value=e.message;}
else{this.inp.textContent=e.message;}
break;case'service-not-allowed':if(undefined!=this.inp.value){this.inp.value=e.message;}
else{this.inp.textContent=e.message;}
break;case'bad-grammar':if(undefined!=this.inp.value){this.inp.value=e.message;}
else{this.inp.textContent=e.message;}
break;case'language-not-supported':if(undefined!=this.inp.value){this.inp.value=e.message;}
else{this.inp.textContent=e.message;}
break;}};}
else{let msg='webkitSpeechRecognition property not found on this device';if(undefined!=this.inp.value){this.inp.value=msg;}
else{this.inp.textContent=msg;}}
return false;}
...
function initPikadayCalendar(field,params){field=getObject(field);let attrs=getAllAttributes(field);let opts={};if(undefined!=attrs['data-showtime']){opts={field:field,showTime:true,showMinutes:true,showSeconds:false,use24hour:false,incrementHourBy:1,incrementMinuteBy:5,position:'bottom right',format:'YYYY-MM-DD HH:mm',toString(date,format){let day=date.getDate();let month=date.getMonth()+1;let year=date.getFullYear();let hour=date.getHours();let min=date.getMinutes();if(day<10){day='0'+day;}
if(month<10){month='0'+month;}
if(hour<10){hour='0'+hour;}
if(min<10){min='0'+min;}
return year+'-'+month+'-'+day+' '+hour+':'+min+':00';},parse(dateString,format){let parts=dateString.split(/[\/\ \:\-]+/);let year=parseInt(parts[0],10);let month=parseInt(parts[1],10)-1;let day=parseInt(parts[2],10);let hour=parseInt(parts[3],10);let min=parseInt(parts[4],10);return new Date(year,month,day,hour,min,0);}};}
else{opts={field:field,position:'bottom right',format:'YYYY-MM-DD',toString(date,format){let day=date.getDate();let month=date.getMonth()+1;let year=date.getFullYear();if(day<10){day='0'+day;}
if(month<10){month='0'+month;}
return year+'-'+month+'-'+day;},parse(dateString,format){let parts=dateString.split(/[\/\ \:\-]+/);if(parts.length<2){return'';}
let year=parseInt(parts[0],10);let month=parseInt(parts[1],10)-1;let day=parseInt(parts[2],10);return new Date(year,month,day);}};}
if(undefined!=attrs['data-maxdate']){if(attrs['data-maxdate'].indexOf('-')!=-1){let dateparts=attrs['data-maxdate'].split("-");if(dateparts.length==3){dateparts[1]=parseInt(dateparts[1])-1;opts.maxDate=new Date(dateparts[0],dateparts[1],dateparts[2])}
else{let n=parseInt(attrs['data-maxdate'])*-1;let d=new Date();d.setDate(d.getDate()-n);opts.maxDate=d;}}
else{switch(attrs['data-maxdate'].toLowerCase()){case'now':case'today':opts.maxDate=new Date();break;default:opts.maxDate=new Date(attrs['data-maxdate']);break;}}}
if(undefined!=attrs['data-mindate']){if(attrs['data-mindate'].indexOf('-')!=-1){let dateparts=attrs['data-mindate'].split("-");if(dateparts.length==3){dateparts[1]=parseInt(dateparts[1])-1;opts.minDate=new Date(dateparts[0],dateparts[1],dateparts[2])}
else{let n=parseInt(attrs['data-mindate'])*-1;let d=new Date();d.setDate(d.getDate()-n);opts.minDate=d;}}
else{switch(attrs['data-mindate'].toLowerCase()){case'now':case'today':opts.minDate=new Date();break;default:opts.minDate=new Date(attrs['data-mindate']);break;}}}
...
if(undefined!=attrs['data-showweeknumber']){opts.showWeekNumber=true;}
if(undefined!=attrs['data-disableweekends']){opts.disableWeekends=true;}
if(undefined!=attrs['data-showalldays']){opts.showDaysInNextAndPreviousMonths=true;opts.enableSelectionDaysInNextAndPreviousMonths=true;}
...
if(undefined!=attrs['data-pickwholeweek']){opts.pickWholeWeek=true;}
if(undefined!=attrs['data-theme']){opts.theme=attrs['data-theme'];}
if(undefined!=attrs['data-format']){opts.format=attrs['data-format'];}
if(undefined!=attrs['data-yearrange']){let r=attrs['data-yearrange'].split(/\,/);if(r.length==1){let n=parseInt(attrs['data-yearrange']);opts.yearRange=n;}
else{r[0]=parseInt(r[0]);r[1]=parseInt(r[1]);opts.yearRange=r;}}
if(undefined!=attrs['data-debug']){console.log('Pikaday calendar opts');console.log(opts);}
...
var tag=document.createElement("input");tag.type='checkbox';tag.id=params.id;var onclick='';if(undefined!=params['onchange']){onclick=params['onchange'];}
else if(undefined!=params['onclick']){onclick=params['onclick'];}
...
var taglabel=document.createElement("label");taglabel.setAttribute('for',tag.id);taglabel.innerHTML='&nbsp;'+params.label;tagdiv.appendChild(taglabel);if(undefined!=params['-parent']){var pobj=getObject(params['-parent']);if(undefined!=pobj){pobj.appendChild(tagdiv);}
...
if(undefined!=params['value']){var vtype=typeof(params['value']);if(vtype.toLowerCase()=='string'){var vals=params['value'].split(':');for(v=0;v<vals.length;v++){params['-values'][vals[v]]=true;}}
else{params['-values']=params['value'];}
params['value']='';}
if(undefined!=params['-values']){var pvtype=typeof(params['-values']);if(pvtype.toLowerCase()=='string'){var pvals=params['-values'].split(':');params['-values']={};for(v=0;v<pvals.length;v++){params['-values'][pvals[v]]=true;}}}
...
ctag.value=tval;if(undefined!=params['-values'][tval]){ctag.checked=true;}
...
if(undefined!=params['-parent']){var pobj=getObject(params['-parent']);if(undefined!=pobj){pobj.appendChild(rowdiv);}
else{console.log(params['-parent']+' does not exist');}}
return rowdiv;}
...
tagdiv.appendChild(taglist);if(undefined!=params['-parent']){var pobj=getObject(params['-parent']);if(undefined!=pobj){pobj.appendChild(tagdiv);}
else{console.log(params['-parent']+' does not exist');}}
return tagdiv;}
...
if(undefined!=params.value&&params.value.length){tag.setAttribute('value',params.value);}
else{tag.setAttribute('value','');}
tag.classname=params.classname;tag.setAttribute('data-type',params['-control']);tag.placeholder=params.placeholder;tagdiv.appendChild(tag);var tagspan=document.createElement("span");tagspan.id=iconid;tagspan.className=spanclass;tagspan.style.color=iconcolor+';padding-left:3px !important;padding-right:6px !important;';tagspan.title=params.title;if(params['-control']=='datetime'){tagspan.innerHTML='<span class="icon-clock"></span>';}
tagdiv.appendChild(tagspan);if(undefined!=params['-parent']){var pobj=getObject(params['-parent']);if(undefined!=pobj){pobj.appendChild(tagdiv);}
else{console.log(params['-parent']+' does not exist');}}
if(params['-control']=='date'){}
tagdiv.setAttribute('data-tagid',tag.id);tagdiv.setAttribute('data-spanid',tagspan.id);return tagdiv;}
...
if(undefined==params['-formname']){params['-formname']='addedit';}
if(undefined==params.id){params.id=params['-formname']+'_'+fieldname;}
var tag=document.createElement("input");tag.type='hidden';tag.name=fieldname;if(undefined!=params.value){tag.setAttribute('value',params.value);}
else{tag.setAttribute('value','');}
tag.id=params.id;if(undefined!=params['-parent']){var pobj=getObject(params['-parent']);if(undefined!=pobj){pobj.appendChild(tag);}
else{console.log(params['-parent']+' does not exist');}}
return tag;}
...
tag.id=params.id;tag.className=params.classname;if(undefined!=params.value){tag.setAttribute('value',params.value);}
else{tag.setAttribute('value','');}
if(undefined!=params['-parent']){var pobj=getObject(params['-parent']);if(undefined!=pobj){pobj.appendChild(tag);}
else{console.log(params['-parent']+' does not exist');}}
return tag;}
function buildFormRadio(fieldname,opts,params){if(undefined==params){params={};}
params['-radio']=true;return buildFormCheckbox(fieldname,opts,params);}
...
tag.name=fieldname;tag.id=params.id;if(undefined!=params['-parent']){var pobj=getObject(params['-parent']);if(undefined!=pobj){pobj.appendChild(tag);}
else{console.log(params['-parent']+' does not exist');}}
return tag;}
...
if(undefined!=params.value){tag.innerHTML=params.value;}
else{tag.innerHTML='';}
...
tag.name=fieldname;tag.id=params.id;for(var tval in opts){var coption=document.createElement("OPTION");coption.value=tval;coption.innerHTML=opts[tval];if(undefined!=params.value&&tval==params.value){coption.setAttribute('selected',true);}
...
return false;}}
function formSetMultiSelectStatus(fld){if(undefined==fld.getAttribute('data-id')){return false;}
let id=fld.getAttribute('data-id');let oid=id+'_options';let options=getObject(oid);if(undefined==options){return false;}
let list=options.querySelectorAll('input');let vals=new Array();for(let i=0;i<list.length;i++){if(undefined==list[i].value){continue;}
if(list[i].value==''){continue;}
if(undefined==list[i].id){continue;}
if(list[i].id==''){continue;}
if(list[i].checked){let label=document.querySelector('label[for="'+list[i].id+'"]');vals.push(label.innerText);}}
let btn_id=id+'_button';let bobj=getObject(btn_id);if(vals.length==0){bobj.innerText=bobj.getAttribute('data-dname');}
else{let str='('+vals.length+') '+implode(', ',vals);bobj.innerText=str;}
return true;}
function fielddataChange(fld){var parentObj=getParentForm(fld);return false;}
...
var dname=fld.name;if(fld.getAttribute('data-displayname')){dname=fld.getAttribute('data-displayname');}
...
ajaxGet('/php/index.php',divid,params);return false;}
function changeTriggerType(srcObj){let theForm=getElementForm(srcObj);let content='';if(undefined!=theForm["_id"]){content=getText(theForm["functions"]);if(content.length>0){return;}}
let tablename=theForm["name"].value;content='';if(srcObj.value==0){}
else{content+='<?php'+"\r\n";content+='/* Available functions: Each function should return the array that was passed in.'+"\r\n";content+=''+"\r\n";switch(tablename.toLowerCase()){case'_users':content+=' //_usersLogin -(user rec is passed in)'+"\r\n";content+=' function '+tablename+'Login($user_rec=array()){}'+"\r\n";content+=' //_usersLogout -(user rec is passed in)'+"\r\n";content+=' function '+tablename+'Logout($user_rec=array()){}'+"\r\n";content+=''+"\r\n";break;}
...
if(undefined==msg){msg='';}
let pdiv=document.querySelector('#setprocessing_custom');if(undefined!=pdiv){return pdiv.innerHTML;}
...
function colorSelector(id){var obj=getObject(id);if(undefined==obj){alert(id+' does not exist');return false;}
var wobj=getObject(id+'_wrapper');if(undefined!=wobj){removeId(wobj);return false;}
...
return false;}
function colorSelectorSet(id){var setid=id+'_set';var wrapper=id+'_wrapper';var iconid=id+'_icon';var hex=getText(setid);setText(id,hex);setStyle(iconid,'color',hex);removeId(wrapper);return false;}
...
function setRadioLabel(rname){var rtags=GetElementsByAttribute('input','name','^'+rname+'$');for(var x=0;x<rtags.length;x++){var rtagid=rtags[x].id;var cust=GetElementsByAttribute('label','for','^'+rtagid+'$');for(var i=0;i<cust.length;i++){if(rtags[x].checked){setClassName(cust[i],'w_checklist_checked');}
else{setClassName(cust[i],'w_checklist');}}}}
function setLabelChecked(att,val,ck,classn,classc){var cust=GetElementsByAttribute('label',att,'^'+val+'$');var cnt=0;if(undefined!=classn&&undefined!=classc){for(var i=0;i<cust.length;i++){if(ck){setClassName(cust[i],classc);}
else{setClassName(cust[i],classn);}}}
else{for(var i=0;i<cust.length;i++){if(ck){setClassName(cust[i],'w_checklist_checked');}
else{setClassName(cust[i],'w_checklist');}}}
return true;}
function checkAllElements(att,val,ck){let list;switch(att.toLowerCase()){case'class':list=document.querySelectorAll('input[type="checkbox"].'+val);break;case'id':list=document.querySelectorAll('input[type="checkbox"]#'+val);break;default:list=document.querySelectorAll('input[type="checkbox"]['+att+'="'+val+'"]');break;}
for(var i=0;i<list.length;i++){if(undefined!=list[i].dataset.onclick){simulateEvent(list[i],'click');}
list[i].checked=ck;}
return false;}
...
oTextbox.focus();}
function getGeoLocation(fld,opts){if(undefined==opts){opts={};}
if(function_exists(wacss.geoLocation)){wacss.geoLocation(fld,opts);}
else{console.log('Error: You must enable wacss in your template to use getGeoLocation');}}
...
popup=1;var quickmask=new Array();quickmask['alpha']='^[a-zA-Z_\\-\\?\\ \\\']+$';quickmask['alphanumeric']='^[0-9a-zA-Z_\\-\\.\\?\\ \\\']+$';quickmask['calendar']='^[0-9]{1,2}\\-[0-9]{1,2}\\-[0-9]{1,4}$';quickmask['email']='.+@.+\\..{2,6}';quickmask['integer']='^[0-9]+$';quickmask['hexcolor']='^#[abcdef0-9]{6,6}$';quickmask['decimal']='^[0-9]*\\.[0-9]+$';quickmask['number']='(^\-{0,1}[0-9]+$)|(^\-{0,1}\\.[0-9]+$)|(^\-{0,1}[0-9]+\\.[0-9]+$)';quickmask['phone']='^([0-9]{3,3}[\\-\\.][0-9]{3,3}[\\-\\.][0-9]{4,4}|\\([0-9]{3,3}\\)\\ [0-9]{3,3}[\\-][0-9]{4,4})$';quickmask['time']='^[0-9]{1,2}\\:[0-9]{2}$';quickmask['ssn']='^[0-9]{3,3}\\-[0-9]{2,2}\\-[0-9]{4,4}$';quickmask['zipcode']='^[0-9]{5,5}(\\\-[0-9]{4,4})*$';if(theForm.length==0){return false;}
if(debug==1){console.log("submitForm Debug Begin. Form length: "+theForm.length);}
var formfields=new Array();for(var i=0;i<theForm.length;i++){if(debug==1){console.log(" - Checking "+theForm[i].name+" of type "+theForm[i].type);}
let atts=getAllAttributes(theForm[i]);if(undefined!=atts.disabled){continue;}
...
if(theForm[i].name=='password'&&undefined!=theForm.password_confirm){if(theForm[i].value.length==0||theForm.password_confirm.value.length==0){if(debug==1){console.log(theForm[i].name+' is required');}
submitFormAlert('Password is required',popup,5);theForm[i].focus();return false;}
...
if(theForm[i].name=='email'&&undefined!=theForm.email_confirm){if(theForm[i].value.length==0||theForm.email_confirm.value.length==0){if(debug==1){console.log(theForm[i].name+' is required');}
submitFormAlert('Email is required',popup,3);theForm[i].focus();return false;}
if(theForm[i].value!=theForm.email_confirm.value){submitFormAlert('Emails do not match.',popup,3);theForm[i].focus();return false;}}
var dname=theForm[i].name;if(theForm[i].getAttribute('data-displayname')){dname=theForm[i].getAttribute('data-displayname');}
else if(theForm[i].getAttribute('displayname')){dname=theForm[i].getAttribute('displayname');}
var required=0;if(undefined!=theForm[i].getAttribute('_required')){required=theForm[i].getAttribute('_required');}
...
var requiredif='';if(undefined!=theForm[i].getAttribute('_requiredif')){requiredif=theForm[i].getAttribute('_requiredif');}
...
if(requiredif.length>0){let parts=requiredif.split(':');let notparts=requiredif.split('!');let rval='';requiredif=theForm.querySelector('[name="'+parts[0]+'"],[name="'+parts[0]+'[]"]');if(parts.length==1){if(undefined!=requiredif){switch(requiredif.type.toLowerCase()){case'select-one':rval=requiredif.options[requiredif.selectedIndex].value;if(rval.length){required=1;}
break;case'select-multiple':let rvals=Array.from(requiredif.selectedOptions).map(el=>el.value);if(rvals.length){required=1;}
break;case'radio':case'checkbox':let rels=theForm.querySelectorAll('[type="'+requiredif.type+'"][name="'+requiredif.name+'"]');for(let r=0;r<rels.length;r++){if(rels[r].checked){required=1;break;}}
break;case'textarea':if(requiredif.innerText.length){required=1;}
break;default:if(requiredif.value.length){required=1;}
break;}}}
else{let not=0;let cvals=parts[1].split(',');if(notparts.length==2){not=1;cvals=notparts[1].split(',');}
if(undefined!=requiredif){switch(requiredif.type.toLowerCase()){case'select-one':rval=requiredif.options[requiredif.selectedIndex].value;for(let c=0;c<cvals.length;c++){if(not==1){if(rval!=cvals[c]){required=1;break;}}
else{if(rval==cvals[c]){required=1;break;}}}
break;case'select-multiple':let rvals=Array.from(requiredif.selectedOptions).map(el=>el.value);for(let r=0;r<rvals.length;r++){rval=rvals[r];for(let c=0;c<cvals.length;c++){if(not==1){if(rval!=cvals[c]){required=1;break;}}
else{if(rval==cvals[c]){required=1;break;}}}}
break;case'radio':case'checkbox':let rels=theForm.querySelectorAll('[type="'+requiredif.type+'"][name="'+requiredif.name+'"]');for(let r=0;r<rels.length;r++){if(rels[r].checked){rval=rels[r].value||1;for(let c=0;c<cvals.length;c++){if(not==1){if(rval!=cvals[c]){required=1;break;}}
else{if(rval==cvals[c]){required=1;break;}}}}}
break;case'textarea':rval=requiredif.innerText;for(let c=0;c<cvals.length;c++){if(not==1){if(rval!=cvals[c]){required=1;break;}}
else{if(rval==cvals[c]){required=1;break;}}}
break
default:rval=requiredif.value;for(let c=0;c<cvals.length;c++){if(not==1){if(rval!=cvals[c]){required=1;break;}}
else{if(rval==cvals[c]){required=1;break;}}}
break;}}}}
if(required==1||required=='required'){var requiredmsg=theForm[i].getAttribute('data-requiredmsg');if(undefined==requiredmsg){requiredmsg=theForm[i].getAttribute('requiredmsg');}
...
if(typeof wacss.blink==="function"&&undefined!=theForm[i].getAttribute('data-blink')){if(typeof wacss.toast==="function"){wacss.toast(msg);}
wacss.blink(theForm[i].getAttribute('data-blink'));}
else{submitFormAlert(msg,popup,5);}
if(debug==1){console.log(msg);}
theForm[i].focus();return false;}}
...
if(debug==1){console.log(msg);}
theForm[i].focus();return false;}}
else if(theForm[i].type=='textarea'){var cval=trim(getText(theForm[i]));if(cval.length==0){var msg=dname+" is required";if(undefined!=requiredmsg){msg=requiredmsg;}
if(typeof wacss.blink==="function"&&undefined!=theForm[i].getAttribute('data-blink')){if(typeof wacss.toast==="function"){wacss.toast(msg);}
wacss.blink(theForm[i].getAttribute('data-blink'));}
else{submitFormAlert(msg,popup,5);}
if(debug==1){console.log(msg);}
theForm[i].focus();return false;}}
else if(theForm[i].type=='select-one'){var cval=theForm[i].options[theForm[i].selectedIndex].value;if(cval.length==0){var msg=dname+" is required";if(undefined!=requiredmsg){msg=requiredmsg;}
if(typeof wacss.blink==="function"&&undefined!=theForm[i].getAttribute('data-blink')){if(typeof wacss.toast==="function"){wacss.toast(msg);}
wacss.blink(theForm[i].getAttribute('data-blink'));}
else{submitFormAlert(msg,popup,5);}
if(debug==1){console.log(msg);}
theForm[i].focus();return false;}}
else if(theForm[i].type=='file'){var cval='';if(undefined!=theForm[i].files[0]){cval=theForm[i].files[0];}
if(cval.length==0){let tmp=theForm.querySelector('span#'+theForm[i].name+'_text a');if(undefined!=tmp){cval=tmp.getAttribute('href');}}
if(cval.length==0){var msg=dname+" is required";if(undefined!=requiredmsg){msg=requiredmsg;}
if(typeof wacss.blink==="function"&&undefined!=theForm[i].getAttribute('data-blink')){if(typeof wacss.toast==="function"){wacss.toast(msg);}
wacss.blink(theForm[i].getAttribute('data-blink'));}
else{submitFormAlert(msg,popup,5);}
if(debug==1){console.log(msg);}
return false;}}
else if(theForm[i].value==''){var msg=dname+" is required";if(undefined!=requiredmsg){msg=requiredmsg;}
if(typeof wacss.blink==="function"&&undefined!=theForm[i].getAttribute('data-blink')){if(typeof wacss.toast==="function"){wacss.toast(msg);}
wacss.blink(theForm[i].getAttribute('data-blink'));}
else{submitFormAlert(msg,popup,5);}
if(debug==1){console.log(msg);}
theForm[i].focus();return false;}}
let mask=theForm[i].getAttribute('pattern');if(undefined==mask){mask=theForm[i].getAttribute('mask');}
if(undefined==mask){mask=theForm[i].getAttribute('data-mask');}
if(undefined!=mask&&mask!=''&&theForm[i].value!=''){let fldmsg=theForm[i].getAttribute('data-pattern_msg');if(undefined==fldmsg){fldmsg=theForm[i].getAttribute('data-pattern-msg');}
if(undefined==fldmsg){fldmsg=theForm[i].getAttribute('maskmsg');}
if(undefined==fldmsg){fldmsg=theForm[i].getAttribute('data-mask_msg');}
if(mask=='ccnumber'){if(isCreditCardNumber(theForm[i].value)==false){let msg=fldmsg||dname+" must be of valid credit card number ";submitFormAlert(msg,popup,5);if(debug==1){console.log(msg);}
theForm[i].focus();return false;}}
else if(mask=='date'){if(isDate(theForm[i].value)==false){let msg=fldmsg||dname+" must be a valid date ";submitFormAlert(msg,popup,5);if(debug==1){console.log(msg);}
theForm[i].focus();return false;}}
else if(mask=='futuredate'){if(isFutureDate(theForm[i].value)==false){let msg=fldmsg||dname+" must be of valid date in the future ";submitFormAlert(msg,popup,5);if(debug==1){console.log(msg);}
theForm[i].focus();return false;}}
else if(mask=='pastdate'){if(isPastDate(theForm[i].value)==false){let msg=fldmsg||dname+" must be of valid date in the past ";submitFormAlert(msg,popup,5);if(debug==1){console.log(msg);}
theForm[i].focus();return false;}}
else if(mask=='intlphone'){if(checkInternationalPhone(theForm[i].value)==false){let msg=fldmsg||dname+" must be a valid phone number";submitFormAlert(msg,popup,5);if(debug==1){console.log(msg);}
theForm[i].focus();return false;}}
else if(mask!='searchandreplace'){if(mask=='phone'){theForm[i].value=formatPhone(theForm[i].value);}
else if(mask=='calendar'){theForm[i].value=formatCalendar(theForm[i].value);}
var rmask=mask;if(undefined!=quickmask[mask]){rmask=quickmask[mask]+'';}
var re=new RegExp(rmask,'i');if(re.test(theForm[i].value)==false){let msg=fldmsg||dname+" must be of type "+mask;submitFormAlert(msg,popup,5);theForm[i].focus();if(debug==1){console.log(msg);}
return false;}}}
if(theForm[i].type=='textarea'&&theForm[i].getAttribute('maxlength')){let len=theForm[i].value.length;let max=Math.abs(theForm[i].getAttribute('maxlength'));if(len>max){let msg=dname+" must be less than "+max+" characters\nYou entered "+len+" characters.";if(theForm[i].getAttribute('maxlengthmsg')){msg=theForm[i].getAttribute('maxlengthmsg');}
submitFormAlert(msg,popup,5);if(debug==1){console.log(msg);}
theForm[i].focus();return false;}}
if(theForm[i].getAttribute('minlength')){let len=theForm[i].value.length;let minlength=Math.abs(theForm[i].getAttribute('minlength'));if(len<minlength){let msg=dname+" must be at least "+minlength+" characters.\nYou entered "+len+" characters.";if(theForm[i].getAttribute('minlengthmsg')){msg=theForm[i].getAttribute('minlengthmsg');}
submitFormAlert(msg,popup,5);if(debug==1){console.log(msg);}
theForm[i].focus();return false;}}
if(theForm[i].getAttribute('minwords')){let cnt=getWordCount(theForm[i]);let minwords=Math.abs(theForm[i].getAttribute('minwords'));if(len<minwords){let msg=dname+" must be at least "+minwords+" words in length.\nYou entered "+cnt+" words.";if(theForm[i].getAttribute('minwordsmsg')){msg=theForm[i].getAttribute('minwordsmsg');}
submitFormAlert(msg,popup,5);if(debug==1){console.log(msg);}
theForm[i].focus();return false;}}
if(theForm[i].getAttribute('maxwords')){let cnt=getWordCount(theForm[i]);let maxwords=Math.abs(theForm[i].getAttribute('maxwords'));if(len<maxwords){let msg=dname+" must be less than "+maxwords+" words in length.\nYou entered "+cnt+" words.";if(theForm[i].getAttribute('maxwordsmsg')){msg=theForm[i].getAttribute('maxwordsmsg');}
submitFormAlert(msg,popup,5);if(debug==1){console.log(msg);}
theForm[i].focus();return false;}}}
for(var i=0;i<theForm.length;i++){let behavior=theForm[i].getAttribute('data-behavior');let id=theForm[i].getAttribute('id');if(undefined!=behavior&&(behavior=='wysiwyg'||behavior=='richtext'||behavior=='tinymce'||behavior=='nicedit')&&undefined!=nicEditors[id]){if(ajax==1){nicEditors.findEditor(id).saveContent();}
...
if(undefined!=theForm.show_upload_progress){formShowUploadProgress(theForm);}
else{let dlist=theForm.querySelectorAll('.w_disable_on_submit');for(let d=0;d<dlist.length;d++){dlist[d].setAttribute('disabled','disabled');}
let hlist=document.querySelectorAll('.w_hide_on_submit');for(let h=0;h<hlist.length;h++){hlist[h].style.display='none';}}
return true;}
function formShowUploadProgress(theForm){let dlist=theForm.querySelectorAll('.w_disable_on_submit');for(let d=0;d<dlist.length;d++){dlist[d].setAttribute('disabled','disabled');}
let hlist=document.querySelectorAll('.w_hide_on_submit');for(let h=0;h<hlist.length;h++){hlist[h].style.display='none';}
html='';html+='<div class="w_centerpop_title">Uploading...</div>'+"\n";html+='<div class="w_centerpop_content">'+"\n";html+=' <div class="w_padtop"><span class="icon-spin4 w_rotate w_spin"></span> Uploading ...</div>'+"\n";html+='</div>'+"\n";centerpopDiv(html,0);}
...
function formFieldHasValue(fld){fld=getObject(fld);if(undefined==fld){return false;}
if(undefined==fld.type){return false;}
if(fld.type=='checkbox'||fld.type=='radio'){if(fld.checked){return true;}}
else if(fld.type=='textarea'){var cval=trim(getText(fld));if(cval.length){return true;}}
else if(fld.value!=''){return true;}
return false;}
function imposeMaxlength(obj,max){return(obj.value.length<=max);}
function pagingSetProcessing(obj){let s=obj.querySelector('span');if(undefined!=s){s.className='icon-spin7 w_spin';}
else{setText(obj,'<span class="icon-spin7 w_spin"></span>');}}
function pagingSubmit(frm,div){pagingAddFilter(frm);pagingSetFilters(frm);if(undefined!=div){frm.setAttribute('pagingSetFilters',0);return ajaxSubmitForm(frm,div);}
...
function pagingBulkEdit(frm){if(frm.filter_field.value.length==0||frm.filter_field.value=='*'){alert('select a field to edit');return false;}
var editval='';if(frm.filter_value.value.length==0){editval='NULL';}
...
function pagingExport(frm){let div=frm._export_formname.value+'_exportbutton';let obj=getObject(div);if(undefined==div){return false;}
obj.innerHTML='<span class="icon-spin7 w_spin" style="margin-top:5px;"></span>';obj.style.display='inline-block';let exportForm=document.createElement('FORM');exportForm.method='POST';exportForm.action='/php/index.php';let inp=document.createElement('INPUT');inp.type='hidden';inp.name='setprocessing';inp.value='0';exportForm.appendChild(inp);let inp1=document.createElement('INPUT');inp1.type='hidden';inp1.name='_pushexport';inp1.value='1';exportForm.appendChild(inp1);let inp2=document.createElement('TEXTAREA');inp2.name='_pushparams';inp2.innerText=frm._export_params_.innerText;exportForm.appendChild(inp2);document.body.appendChild(exportForm);ajaxSubmitForm(exportForm,div);exportForm.parentNode.removeChild(exportForm);return false;}
function pagingAddFilter(frm){if(undefined!=frm.bulkedit){return false;}
...
if(frm.filter_field.value=='*'&&(frm.filter_operator.value=='ib'||frm.filter_operator.value=='nb')){alert('select a field to check for null values on');frm.filter_field.focus();return false;}
else if(frm.filter_field.value!='*'&&(frm.filter_operator.value=='ib'||frm.filter_operator.value=='nb')){}
else if(frm.filter_value.value.length==0&&frm.filter_operator.value!='null'){frm.filter_value.focus();return false;}
let str=frm.filter_field.value+' '+frm.filter_operator.value+' '+frm.filter_value.value;pagingAddFilters(frm,str);frm.filter_value.value='';frm.filter_value.focus();}
function pagingAddFilters(frm,filters,clear){if(undefined==clear){clear=0;}
if(clear==1){pagingClearFilters(frm);}
let sets=filters.split(";");for(let s=0;s<sets.length;s++){let fltrs=sets[s].split(" ");let dfield=fltrs.shift();if(dfield=='*'){dfield='Any Field';}
let doper=fltrs.shift();let dvalue='';if(fltrs.length){dvalue=fltrs.join(" ");}
let did=dfield+doper+dvalue;did=did.replace(/[^a-zA-Z0-9\_]+/g,"");did='id_'+did.toLowerCase();let obj=frm.querySelector('#'+did);let filters=new Array();if(undefined!=obj){obj.style.display='inline-block';}
else{let filter_element=document.createElement('div');filter_element.className='w_pagingfilter';filter_element.setAttribute('data-field',dfield);filter_element.setAttribute('data-operator',doper);filter_element.setAttribute('data-value',dvalue);filter_element.setAttribute('id',did);let dval='\''+dvalue+'\'';switch(doper){case'ct':doper='Contains';break;case'nct':doper='Not Contains';break;case'ca':doper='Contains Any of These';break;case'nca':doper='Not Contain Any of These';break;case'eq':doper='Equals';break;case'neq':doper='Not Equals';break;case'ea':doper='Equals Any of These';break;case'nea':doper='Not Equals Any of These';break;case'gt':doper='Greater Than';break;case'lt':doper='Less Than';break;case'egt':doper='Equals or Greater than';break;case'elt':doper='Less than or Equals';break;case'ib':doper='Is Blank';dval='';break;case'nb':doper='Is Not Blank';dval='';break;}
filter_element.innerHTML='<span class="icon-filter w_grey"></span> '+dfield+' '+doper+' '+dval+' <span class="icon-cancel w_danger w_pointer" onclick="removeId(\''+filter_element.id+'\');"></span>';let p=frm.querySelector('#send_to_filters');p.appendChild(filter_element);}
let f=frm.querySelectorAll('.w_pagingfilter');let filter_count=0;for(let i=0;i<f.length;i++){if(f[i].style.display=='none'){continue;}
if(undefined==f[i].getAttribute('data-field')||f[i].getAttribute('data-field')=='null'){continue;}
filter_count=filter_count+1;}
if(filter_count>0){let obj=frm.querySelector('#paging_clear_filters');if(undefined!=obj){removeId(obj);}
let d=document.createElement('div');d.className='w_pagingfilter icon-erase w_big w_danger';d.id='paging_clear_filters';d.setAttribute('title','Clear All Filters');d.onclick=function(){pagingClearFilters(getParent(this,'form'));};let p=frm.querySelector('#send_to_filters');p.appendChild(d);}
frm.filter_value.value='';frm.filter_value.focus();}
if(clear==1){simulateEvent(frm,'submit');}}
function pagingSetFilters(frm){var f=frm.querySelectorAll('.w_pagingfilter');var filters=new Array();for(var i=0;i<f.length;i++){if(f[i].style.display=='none'){continue;}
if(undefined==f[i].getAttribute('data-field')||f[i].getAttribute('data-field')=='null'){continue;}
...
frm._filters.value=implode("\r\n",filters);if(undefined!=frm.getAttribute('pagingSetFilters')){let check=frm.getAttribute('pagingSetFilters');if(check==1||check=='1'){return false;}}
...
function pagingClearFilters(frm){var f=frm.querySelectorAll('.w_pagingfilter');if(undefined==f){return false;}
for(var i=0;i<f.length;i++){removeId(f[i]);}
...
document.getElementById(pfield).innerHTML='<img src="/wfiles/busy.gif" title="checking Perl syntax" width="12" height="12" alt="checking Perl syntax" />';document._perlcheck.perlcheck.value=value;ajaxSubmitForm(document._perlcheck,pfield,5);return false;}
function cloneObj(c){var obj=getObject(c);var inc=obj.getAttribute('inc');if(undefined==inc){inc=1;}
else{inc=parseFloat(inc);inc++;}
obj.setAttribute('inc',inc);var cloneObj=obj.cloneNode(true);if(undefined!=cloneObj.id){cloneObj.setAttribute('id',cloneObj.id+'_'+inc);}
...
function ajaxSubmitMultipartForm(theform,sid,params){if(undefined==theform){alert("No form object passed to ajaxSubmitMultipartForm");return false;}
if(undefined=='sid'){sid='centerpop';}
if(undefined==params){params={};}
let url=theform.getAttribute('action');let ok=submitForm(theform,1,0,1);if(!ok){return false;}
...
break;case'file':if(theform[i].files.length==1){data.append(theform[i].name,theform[i].files[0]);}
else{for(j=0;j<theform[i].files.length;j++){data.append(theform[i].name+'_'+j,theform[i].files[j]);}}
break;}}
request.sid=sid;request.params=params;request.addEventListener('loadstart',function(e){if(undefined==this.params.setprocessing){if(this.params.setprocessing==0){return false;}}
var txt=getProcessingDiv(this.sid);if(undefined==this.params.setprocessing){setText(this.params.setprocessing,txt);return false;}
if(this.sid.indexOf('centerpop')!=-1){popUpDiv('',{id:this.sid,width:300,height:50,notop:1,nobot:1,noborder:1,nobackground:1,bodystyle:"padding:0px;border:0px;background:none;"});var atitle='Processing Request';setCenterPopText(this.sid,txt,{title:atitle,drag:false,close_bot:false});}
else if(this.sid=='modal'){let title='';if(undefined!=this.params.title){title=this.params.title;}
else{title='Processing Request';}
let modal=wacss.modalPopup(txt,title,{overlay:1});}
else{this.prevtxt=getText(this.sid);setText(this.sid,txt);}});request.upload.sid=sid;request.upload.params=params;request.upload.xhr=request;request.upload.addEventListener('progress',function(e){let percent_complete=parseInt((e.loaded / e.total)*100);let pobj=document.querySelector('#processing_div .processing_message');if(undefined!=pobj){setText(pobj,percent_complete+'%');return false;}
let txt=getProcessingDiv(this.sid,percent_complete+'%');if(this.sid.indexOf('centerpop')!=-1){let sidobj=getObject(this.sid);if(undefined==sidobj){this.xhr.abort();return false;}
updateCenterPopText(this.sid,txt);}
else if(this.sid=='modal'){let sidobj=getObject('wacss_modal');if(undefined==sidobj){this.xhr.abort();return false;}
let modal=wacss.modalPopup(txt);}
else{setText(this.sid,txt);}});request.addEventListener('load',function(e){console.log('Load: '+this.sid);if(this.sid.indexOf('centerpop')!=-1){updateCenterPopText(this.sid,request.response);}
else if(this.sid=='modal'){let title='';if(undefined!=this.params.success_title){title=this.params.success_title;}
else{title='Success';}
let modal=wacss.modalPopup(request.response,title);}
else{setText(this.sid,request.response);}});request.addEventListener('error',function(e){if(this.sid.indexOf('centerpop')!=-1){updateCenterPopText(this.sid,request.response,'Error');}
else if(this.sid=='modal'){let title='';if(undefined!=this.params.error_title){title=this.params.error_title;}
else{title='Error';}
let modal=wacss.modalPopup(request.response,title,{overlay:1});}
else{setText(this.sid,request.response);}});request.addEventListener('abort',function(e){if(this.sid.indexOf('centerpop')!=-1){removeId(this.sid);}
else if(this.sid=='modal'){wacss.modalClose();}
else{setText(this.sid,this.prevtxt);}});request.open('post',url);request.send(data);return false;}
function ajaxPost(theform,sid,tmeout,callback,returnreq,abort_callback){if(undefined==theform){alert("No form object passed to ajaxPost");return false;}
if(undefined!=tmeout&&tmeout=='novalidation'){tmeout=36000000;}
else{let ok=submitForm(theform,1,0,1);if(!ok){return false;}}
let fields=theform.querySelectorAll('input[type="file"]');for(let i=0;i<fields.length;i++){if(undefined==fields[i].id){continue;}
let cid=fields[i].id+'_base64_content';let cobj=theform.querySelector('#'+cid);if(undefined==cobj){continue;}
if(cobj.innerHTML.length==0){continue;}
cobj.name=fields[i].name+'_base64';if(undefined!=theform[fields[i].name]){fields[i].parentNode.removeChild(fields[i]);}
let name=fields[i].value.split('\\').pop().split('/').pop();let h=document.createElement('input');h.type='hidden';h.name=fields[i].name;h.value=name;theform.appendChild(h);}
if(undefined==callback){if(undefined!=theform.callback&&theform.callback.type=='hidden'&&theform.callback.value.length){callback=theform.callback.value;}
else{callback='';}}
if(undefined==returnreq){returnreq=false;}
if(undefined==abort_callback){if(undefined!=theform.abort_callback&&theform.abort_callback.type=='hidden'&&theform.abort_callback.value.length){abort_callback=theform.abort_callback.value;}
else{abort_callback='';}}
if(undefined==tmeout){tmeout=36000000;}
if(tmeout<180000){tmeout=180000;}
var lcsid=sid.toLowerCase();var cb=callback.toLowerCase();if(undefined==document.getElementById(sid)&&lcsid!='pop'&&lcsid!='null'&&lcsid!='modal'&&cb.indexOf('popupdiv')==-1&&cb.indexOf('centerpop')==-1&&lcsid.indexOf('popupdiv')==-1&&lcsid.indexOf('centerpop')==-1){alert('Error1 in ajaxPost\n'+sid+' is not defined as a valid object id');return false;}
if(typeof(AjaxRequest.ActiveAjaxGroupRequests[sid])!='undefined'){ajaxAbort(sid);}
let returnid=false;if(undefined!=theform.returnid){if(theform.returnid.value.toLowerCase()=='true'){returnid=true;}
if(theform.returnid.value.toLowerCase()=='1'){returnid=true;}}
var showprocessing=true;if(undefined!=theform.setprocessing){if(theform.setprocessing.value.toLowerCase()=='false'){showprocessing=false;}
if(theform.setprocessing.value.toLowerCase()=='0'){showprocessing=false;}}
else if(undefined!=theform.showprocessing){if(theform.showprocessing.value.toLowerCase()=='false'){showprocessing=false;}
if(theform.showprocessing.value.toLowerCase()=='0'){showprocessing=false;}}
else if(undefined!=theform.noprocessing){if(theform.noprocessing.value.toLowerCase()=='true'){showprocessing=false;}
if(theform.noprocessing.value.toLowerCase()=='1'){showprocessing=false;}}
var showprocessingdiv=sid;if(undefined!=theform.showprocessingdiv){showprocessingdiv=theform.showprocessingdiv.value;}
else if(undefined!=theform.setprocessing){showprocessingdiv=theform.setprocessing.value;}
var cp_title='';if(undefined!=theform.cp_title){cp_title=theform.cp_title.value;}
else if(undefined!=theform.centerpop_title){cp_title=theform.centerpop_title.value;}
var title='';if(undefined!=theform.title){title=theform.title.value;}
var AJUid=new Date().getTime()+"";var h=document.createElement('input');h.type='hidden';h.name='AjaxRequestUniqueId';h.value=AJUid;theform.appendChild(h);var getReq=AjaxRequest.submit(theform,{'groupName':sid,'timeout':tmeout,'callback':callback,'var2':cp_title,'var3':title,'abort_callback':abort_callback,'showprocessing':showprocessing,'showprocessingdiv':showprocessingdiv,'AjaxRequestUniqueId':AJUid,'onGroupBegin':function(req){var dname=this.groupName;var lname=dname.toLowerCase();var cb=this.callback;cb=cb.toLowerCase();if(cb.indexOf('centerpop')!=-1||lname.indexOf('centerpop')!=-1){var txt=getProcessingDiv(sid);popUpDiv('',{id:dname,width:300,height:50,notop:1,nobot:1,noborder:1,nobackground:1,bodystyle:"padding:0px;border:0px;background:none;"});var atitle='Processing Request';setCenterPopText(dname,txt,{title:atitle,drag:false,close_bot:false});}
else if(lname=='modal'){let modal=wacss.modalPopup('<span class="icon-spin7 w_spin"></span>','Processing Request',{overlay:1});}
else if(lname=='pop'){this.popNumber=popWindow(getProcessingDiv(sid),'processing request...');}
else if(lname=='null'){}
else if(this.showprocessing){setProcessing(this.showprocessingdiv);}},'onGroupEnd':function(req){},'onTimeout':function(req){var dname=this.groupName;var lname=dname.toLowerCase();var cb=this.callback;cb=cb.toLowerCase();var val="<b style=\"color:red\">ajaxPost Timed Out Error</b>";if(cb.indexOf('centerpop')!=-1||dname.indexOf('centerpop')!=-1){setCenterPopText(dname,val);}
else if(lname=='modal'){let modal=wacss.modalPopup(val,'Timeout Error',{overlay:1});}
else if(lname=='pop'){popText(this.popNumber,val,'timed out...');}
else if(lname=='null'){removeId('centerpop');}
...
else if(lname=='modal'){let modal=wacss.modalPopup(val,'ajaxPost Error',{overlay:1});}
else if(lname=='pop'){popText(this.popNumber,val,'error');}
else if(lname=='null'){removeId('centerpop');}
else{setText(dname,val);}
...
for(i=0;i<ajaxids.length;i++){var cid=ajaxids[i].id;tinyMCE.execCommand('mceRemoveControl',false,cid);}
let rel=document.createElement('html');rel.innerHTML=req.responseText;let scripts=rel.getElementsByTagName('script');for(let s=0;s<scripts.length;s++){eval(scripts[s].innerHTML);}
if(undefined!=theform.setprocessing){setText(theform.setprocessing.value,'');}
if(cb!=''){if(cb.indexOf('popupdiv')!=-1){popUpDiv(val,{id:dname,center:1,drag:1});centerObject(dname);}
else if(lname=='pop'){popText(this.popNumber,val,this.var2);}
else if(lname=='null'){removeId('centerpop');}
else if(cb.indexOf('centerpop')!=-1){setCenterPopText(dname,val);}
else if(cb=='modal'){let title='';if(this.var2.length){title=this.var2;}
else if(this.var3.length){title=this.var3;}
else{title='Success';}
let modal=wacss.modalPopup(val,title,{overlay:1});}
else{window[this.callback](req,dname);}}
else{if(lname.indexOf('popupdiv')!=-1){popUpDiv(val,{id:dname,center:1,drag:1});centerObject(dname);}
else if(lname=='pop'){popText(this.popNumber,val,this.var2);}
else if(lname=='null'){removeId('centerpop');}
else if(lname.indexOf('centerpop')!=-1){setCenterPopText(dname,val);}
else if(lname=='modal'){let title='';if(undefined!=this.var2&&this.var2.length){title=this.var2;}
else if(undefined!=this.var3&&this.var3.length){title=this.var3;}
else{title='Success';}
let modal=wacss.modalPopup(val,title,{overlay:1});}
else if(document.getElementById(dname)){if(undefined==document.getElementById(dname).style.display||document.getElementById(dname).style.display=='none'){document.getElementById(dname).style.display='inline';}
...
initBehaviors(dname);if(undefined!=typeof wacss&&typeof wacss['initOnloads']==='function'){wacss.initOnloads();}}});if(returnreq){return getReq;}
if(returnid){return getReq.AjaxRequestUniqueId;}
return false;}
function updateCenterPopText(cpid,cptext,cptitle){let cp=getObject(cpid);if(undefined==cp){console.log('updateCenterPopText Error: No cp');return false;}
if(undefined==cptext){cptext='';}
if(undefined==cptitle){cptitle='';}
console.log(cp);if(cptext.length){let cpt=cp.querySelector('.w_centerpop_content');if(undefined!=cpt){console.log('updating centerpop content to'+cptext);setText(cpt,cptext);}
else{console.log('updateCenterPopText Error: No cpt');}}
if(cptitle.length){let cptt=cp.querySelector('.w_centerpop_title');if(undefined!=cptt){setText(cptt,cptitle);}
else{console.log('updateCenterPopText Error: No cptt');}}}
...
var txt='';if(cptext.indexOf('w_centerpop_content')!=-1){txt=cptext;}
else{txt+='<div class="w_centerpop">'+"\n";if(params.title.length){txt+=' <div class="w_centerpop_title">'+params.title+'</div>'+"\n";}
...
txt+='</div>'+"\n";cptext=txt;}
setText(cpid,txt);if(!cptext.includes('w_centerpop_close_top')&&cptext.includes('w_centerpop_title')){let cpobj=getObject(cpid);if(undefined!=cpobj){let tobj=cpobj.querySelector('div.w_centerpop_title');if(undefined!=tobj){let div=document.createElement("div");div.setAttribute("class",'w_centerpop_close_top icon-cancel');div.setAttribute("title",'Click to close');div.setAttribute("onclick",'ajaxAbort(\''+cpid+'\');');tobj.appendChild(div);}}}
if(params.center){centerObject(cpid);}
...
function callWaSQL(id,name,params){var url='/php/admin.php';ajaxGet(url,name,'&_view='+id+'&'+params);}
function ajaxAbort(sid){console.log('ajaxAbort'+sid);if(sid=='modal'){wacss.modalClose();return false;}
...
req.xmlHttpRequest.abort();delete(req);if(sid.indexOf('centerpop')!=-1){removeId(sid);}
else{setText(sid,req.prevValue);}
return false;}
if(sid.indexOf('centerpop')!=-1){removeId(sid);}
else{setText(sid,'');}
return false;}
function ajaxGet(url,sid,xparams,callback,tmeout,nosetprocess,returnreq,newtitle,newurl,abort_callback,append){var params='';var cp_title='';var showprocessingdiv=sid;var showprocessing=true;let returnid=false;if(typeof(xparams)=='object'){if(undefined!=xparams.callback){callback=xparams.callback;}
...
if(undefined!=xparams.nosetprocess){if(xparams.nosetprocess){showprocessing=false;}}
else if(undefined!=xparams.showprocessing){if(!xparams.showprocessing){showprocessing=false;}}
if(undefined!=xparams.setprocessing){showprocessingdiv=xparams.setprocessing;}
else if(undefined!=xparams.showprocessingdiv){showprocessingdiv=xparams.showprocessingdiv;}
...
if(undefined==newtitle){if(undefined!=xparams&&undefined!=xparams["-newtitle"]){newtitle=xparams["-newtitle"];}
else{newtitle='';}}
if(undefined==newurl){if(undefined!=xparams&&undefined!=xparams["-newurl"]){newurl=xparams["-newurl"];}
else{newurl='';}}
...
else if(lname=='modal'){let modal=wacss.modalPopup('ajaxGet Timed Out','Timed out Processing Request',{overlay:1});}
else if(lname=='pop'){popText(this.popNumber,val,'timed out...');}
else{setText(dname,val);}},'onError':function(req){var dname=this.groupName;var lname=dname.toLowerCase();var cb=this.callback.toLowerCase();var val='<div style="display:none" id="ajaxOnError">'+req.responseText+'</div>';if(cb.indexOf('centerpop')!=-1||lname.indexOf('centerpop')!=-1){setCenterPopText(dname,val);}
else if(lname=='modal'){let modal=wacss.modalPopup(req.responseText,'Error Processing Request',{overlay:1});}
else if(lname=='pop'){popText(this.popNumber,val,'error');}
...
if(cb!=''){if(cb.indexOf('popupdiv')!=-1){var val=req.responseText;popUpDiv(val,{id:dname,center:1,drag:1});centerObject(dname);}
else if(lname=='pop'){popText(this.popNumber,req.responseText,this.var2);}
else if(cb.indexOf('centerpop')!=-1){if(undefined!=this.var2&&this.var2.length>0){setCenterPopText(dname,req.responseText,{title:this.var2});}
else{setCenterPopText(dname,req.responseText);}}
else if(lname=='modal'){let title='';if(this.var2.length){title=this.var2;}
else if(this.var3.length){title=this.var3;}
else{title='Success';}
let modal=wacss.modalPopup(req.responseText,title,{overlay:1});}
else{window[this.callback](req,dname);}}
else{var val=req.responseText;if(lname.indexOf('popupdiv')!=-1){popUpDiv(val,{id:dname,center:1,drag:1});centerObject(dname);}
else if(lname=='pop'){popText(this.popNumber,req.responseText,this.var2);}
else if(lname.indexOf('centerpop')!=-1){if(undefined!=this.var2&&this.var2.length>0){setCenterPopText(dname,req.responseText,{title:this.var2});}
else{setCenterPopText(dname,req.responseText);}}
else if(lname=='modal'){let title='';if(this.var2.length){title=this.var2;}
else if(this.var3.length){title=this.var3;}
else{title='Success';}
...
setText(dname,val);if(undefined!=document.getElementById(dname).getAttribute('onshow')&&document.getElementById(dname).getAttribute('onshow')=='center'){document.getElementById(dname).style.position='absolute';centerObject(dname);}}
else{}}
initBehaviors(dname);if(undefined!=typeof wacss&&typeof wacss['initOnloads']==='function'){wacss.initOnloads();}}});if(returnreq){return getReq;}
if(returnid){return getReq.AjaxRequestUniqueId;}
return false;}
...
function wpassInput(v){if(!isNum(v)){return true;}
var obj=getObject('wpass_search');if(undefined==obj){return;}
obj.value='';ajaxGet('/php/index.php','wpass_info',{_wpass:v});return false;}
function AjaxRequest(){var req=new Object();req.timeout=null;req.generateUniqueUrl=true;req.url=window.location.href;req.method="GET";req.async=true;req.username=null;req.password=null;req.xAttribute=null;req.xValue=null;req.xName=null;req.xAction=null;req.xId=null;req.prevValue=null;req.parameters=new Object();req.requestIndex=AjaxRequest.numAjaxRequests++;req.responseReceived=false;req.showprocessing=true;req.showprocessingdiv=null;req.groupName=null;req.callback=null;req.abort_callback=null;req.var1=null;req.var2=null;req.var3=null;req.var4=null;req.var5=null;req.AjaxRequestUniqueId=null;req.queryString="";req.responseText=null;req.responseXML=null;req.status=null;req.statusText=null;req.aborted=false;req.xmlHttpRequest=null;req.onTimeout=null;req.onLoading=null;req.onLoaded=null;req.onInteractive=null;req.onComplete=null;req.onSuccess=null;req.onError=null;req.onGroupBegin=null;req.onGroupEnd=null;req.xmlHttpRequest=AjaxRequest.getXmlHttpRequest();if(req.xmlHttpRequest==null){return null;}
...
if(req.groupName!=null){if(typeof(AjaxRequest.numActiveAjaxGroupRequests[req.groupName])=="undefined"){AjaxRequest.numActiveAjaxGroupRequests[req.groupName]=0;AjaxRequest.ActiveAjaxGroupRequests[req.groupName]=req;}
else{AjaxRequest.ActiveAjaxGroupRequests[req.groupName]=req;}
...
break;default:break;}}}
return queryString;};AjaxRequest.numActiveAjaxRequests=0;AjaxRequest.numActiveAjaxGroupRequests=new Object();AjaxRequest.ActiveAjaxGroupRequests=new Object();AjaxRequest.numAjaxRequests=0;(function(){var test=document.createElement('input');try{test.type='range';if(test.type=='range'){return;}}
catch(e){return;}
...
var scale;var isMac=navigator.platform=='MacIntel';var thumb={radius:isMac?9:6,width:isMac?22:12,height:isMac?16:20};var track='linear-gradient(transparent '+(isMac?'6px, #999 6px, #999 7px, #ccc 8px, #bbb 9px, #bbb 10px, transparent 10px':'9px, #999 9px, #bbb 10px, #fff 11px, transparent 11px')+', transparent)';var styles={'min-width':thumb.width+'px','min-height':thumb.height+'px','max-height':thumb.height+'px',padding:'0 0 '+(isMac?'2px':'1px'),border:0,'border-radius':0,cursor:'default','text-indent':'-999999px'};var options={attributes:true,attributeFilter:['min','max','step','value']};var forEach=Array.prototype.forEach;var onChange=document.createEvent('HTMLEvents');onChange.initEvent('change',true,false);if(document.readyState=='loading'){document.addEventListener('DOMContentLoaded',initialize,commonPassiveEventListener(true));}
else{initialize();}
...
forEach.call(node.querySelectorAll('input'),check);});});}).observe(document,{childList:true,subtree:true});}
...
rawValue=value;prevX=e.clientX;this.addEventListener('mousemove',onDrag,commonPassiveEventListener(true));this.addEventListener('mouseup',onDragEnd,commonPassiveEventListener(true));}
