...
return!1}
ico=getObject(ico);if(window.hasOwnProperty('webkitSpeechRecognition')){var recognition=new webkitSpeechRecognition();recognition.continuous=continuous||!1;recognition.interimResults=!1;recognition.lang="en-US";if(debug){alert(recognition)}
if(undefined!=ico){ico.classList.add('w_blink');ico.classList.add('w_success');recognition.ico=ico}
if(undefined!=frm){recognition.frm=frm}
recognition.inp=inp;recognition.start();recognition.debug=debug;recognition.onresult=function(e){if(this.debug){alert('onresult')}
if(undefined!=this.frm&&undefined!=window[this.frm]){try{window[this.frm](e.results[0][0].transcript);this.stop();this.start()}catch(e){}}
...
recognition.onerror=function(e){if(this.continuous){if(undefined!=e.error&&e.error=='no-speech'){try{this.restart()}catch(e){}}else{console.log(e);if(this.debug){alert('ERROR: '+e.error)}}}else{try{this.stop()}catch(e){if(this.debug){alert('ERROR: '+e.error)}}
