...
ico=getObject(ico);if(window.hasOwnProperty('webkitSpeechRecognition')){let recognition=new webkitSpeechRecognition();recognition.continuous=continuous||!1;recognition.interimResults=!1;recognition.lang="en-US";if(undefined!=ico){ico.classList.add('w_blink');ico.classList.add('w_success');recognition.ico=ico}
if(undefined!=frm){recognition.frm=frm}
recognition.inp=inp;recognition.start();recognition.onresult=function(e){if(undefined!=this.frm&&undefined!=window[this.frm]){try{window[this.frm](e.results[0][0].transcript);this.stop();this.start()}catch(e){this.stop();this.start()}
return}
