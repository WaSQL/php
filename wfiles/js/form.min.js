...
els=frm.querySelectorAll('[data-requiredif]');for(let i=0;i<els.length;i++){if(formIsIfTrue(frm,els[i].dataset.requiredif)){els[i].setAttribute('required','required');if(undefined!=els[i].dataset.requiredmsg){els[i].setCustomValidity(els[i].dataset.requiredmsg)}}else{els[i].removeAttribute('required');if(els[i].hasAttribute('data-required')){els[i].removeAttribute('data-required')}}}
...
if(undefined!=tmeout&&tmeout=='novalidation'){tmeout=36000000}else{let ok=submitForm(theform,1,0,1);if(!ok){return!1}}
