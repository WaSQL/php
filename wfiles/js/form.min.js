...
function isIf(frm,ifstr){let oper='';let sets=ifstr.split(' and ');if(sets.length){oper='and'}
...
let tvals=new Array();for(let i=0;i<sets.length;i++){let parts=sets[i].split(':');let fld=parts[0];if(undefined==frm[fld]){continue}
if(undefined==frm[fld].type){continue}
let fval='';switch(frm[fld].type.toLowerCase()){case 'select-one':fval=frm[fld].options[frm[fld].selectedIndex].value;break;case 'radio':case 'checkbox':if(frm[fld].checked){fval=frm[fld].value||1}
break;case 'textarea':fval=trim(frm[fld].innerText);break;default:fval=frm[fld].value;break}
if(undefined!=parts[1]){let vals=parts[1].split(',');for(let v=0;v<vals.length;v++){if(fval==vals[v]){tvals.push(1);break}}}else{if(fval.length){tvals.push(1)}}}
...
let els=frm.querySelectorAll('[data-displayif]');for(let i=0;i<els.length;i++){if(isIf(frm,els[i].dataset.displayif)){if(undefined!=els[i].dataset.display){els[i].style.display=els[i].dataset.display}else{els[i].style.display='initial'}}else{els[i].style.display='none'}}
els=frm.querySelectorAll('[data-hideif]');for(let i=0;i<els.length;i++){if(isIf(frm,els[i].dataset.hideif)){els[i].style.display='none'}else{if(undefined!=els[i].dataset.display){els[i].style.display=els[i].dataset.display}else{els[i].style.display='initial'}}}
els=frm.querySelectorAll('[data-readonlyif]');for(let i=0;i<els.length;i++){if(isIf(frm,els[i].dataset.readonlyif)){if(!els[i].hasAttribute('onclick')){els[i].setAttribute('onclick','return false');els[i].setAttribute('onclickx','1')}
...
els=frm.querySelectorAll('[data-requiredif]');for(let i=0;i<els.length;i++){console.log(els[i]);if(isIf(frm,els[i].dataset.requiredif)){console.log('true');els[i].setAttribute('required','required')}else{console.log('false');els[i].removeAttribute('required');if(els[i].hasAttribute('data-required')){els[i].removeAttribute('data-required')}}}
els=frm.querySelectorAll('[data-blankif]');for(let i=0;i<els.length;i++){if(isIf(frm,els[i].dataset.blankif)){switch(els[i].type.toLowerCase()){case 'radio':case 'checkbox':if(els[i].checked){els[i].dataset.blankx=els[i].value||1}
break;case 'textarea':els[i].dataset.blankx=trim(els[i].innerHTML);els[i].innerHTML='';break;default:els[i].dataset.blankx=els[i].value;els[i].value='';break}}else{if(undefined!=els[i].dataset.blankx){switch(els[i].type.toLowerCase()){case 'radio':case 'checkbox':break;case 'textarea':els[i].innerHTML='';els[i].innerHTML=els[i].dataset.blankx;break;default:els[i].value=els[i].dataset.blankx;break}}}}
els=frm.querySelectorAll('[data-classif]');for(let i=0;i<els.length;i++){console.log(els[i]);let parts=els[i].dataset.classif.split(':');console.log(parts);let eclass=parts[0];let ifstr=parts[1]+':'+parts[2];console.log(eclass);console.log(ifstr);if(isIf(frm,ifstr)){console.log('true');els[i].classList.add(eclass)}else{console.log('false');els[i].classList.remove(eclass)}}
return}
