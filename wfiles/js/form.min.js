...
let label=document.querySelector('label[for='+fld.id+']');if(undefined!=fld.dataset.thumbnail){let reader=new FileReader();reader.label=label;reader.filename=fileName;reader.onload=function(e){console.log(e.target);let img=document.createElement('img');img.style='margin-top:2px;display:block;max-width:200px;max-height:200px;border-radius:3px;border:1px solid #ccc;';img.src=e.target.result;setText(this.label,this.filename);this.label.style='padding:0px;border:0px;background:transparent;';this.label.appendChild(img)}
reader.readAsDataURL(fld.files[0])}else if(fileName){setText(label,'<span class="icon-upload w_big w_success"></span> '+fileName);label.className='btn btn-default'}else{setText(label,'<span class="icon-upload w_big w_danger"></span> '+fld.getAttribute('data-text'));label.className='btn btn-default'}
if(undefined!=fld.getAttribute('data-remove_checkbox')){let robj=getObject(fld.getAttribute('data-remove_checkbox'));robj.checked=!0}
if(undefined!=fld.getAttribute('data-remove_display')){let robj=getObject(fld.getAttribute('data-remove_display'));robj.style.border='1px solid #dc354b'}}
