function setInputFileName(fld){if(fld.files&&fld.files.length>1){fileName=(fld.getAttribute('data-multiple-caption')||'').replace('{count}',fld.files.length);}
else{fileName=fld.value.split('\\').pop();}
var label=document.querySelector('label[for='+fld.id+']');console.log(fileName,label);if(fileName){setText(label,'<span class="icon-upload w_big w_success"></span> '+fileName);label.className='btn btn-default';}
else{setText(label,'<span class="icon-upload w_big w_danger"></span> '+fld.getAttribute('data-text'));label.className='btn btn-default';}}
...
var tablename=theForm["name"].value;content='';if(srcObj.value==0){}
...
if(undefined==msg){msg='Processing ...';}
var str='';str+='<span id="processing_div">';str+='<span class="w_loader"></span>';str+='<span class="w_grey">'+msg+'</span>';if(cancel==1){str+='<span class="icon-cancel-circled w_danger w_pointer" onclick="return ajaxAbort(\''+id+'\');"></span>';}
str+='</span>';return str;}
...
return true;}
function checkAllElements(att,val,ck){var cust=GetElementsByAttribute('input',att,'^'+val+'$');var cnt=0;for(var i=0;i<cust.length;i++){if(cust[i].type=='checkbox'){if(undefined!=cust[i].getAttribute('onclick')){simulateEvent(cust[i],'click');console.log(cust[i],'click');}
...
popup=1;var quickmask=new Array();quickmask['alpha']='^[a-zA-Z_\\-\\?\\ \\\']+$';quickmask['alphanumeric']='^[0-9a-zA-Z_\\-\\.\\?\\ \\\']+$';quickmask['calendar']='^[0-9]{1,2}\\-[0-9]{1,2}\\-[0-9]{1,4}$';quickmask['email']='.+@.+\\..{2,6}';quickmask['integer']='^[0-9]+$';quickmask['hexcolor']='^#[abcdef0-9]{6,6}$';quickmask['decimal']='^[0-9]*\\.[0-9]+$';quickmask['number']='(^[0-9]+$)|(^\\.[0-9]+$)|(^[0-9]+\\.[0-9]+$)';quickmask['phone']='^([0-9]{3,3}[\\-\\.][0-9]{3,3}[\\-\\.][0-9]{4,4}|\\([0-9]{3,3}\\)\\ [0-9]{3,3}[\\-][0-9]{4,4})$';quickmask['time']='^[0-9]{1,2}\\:[0-9]{2}$';quickmask['ssn']='^[0-9]{3,3}\\-[0-9]{2,2}\\-[0-9]{4,4}$';quickmask['zipcode']='^[0-9]{5,5}(\\\-[0-9]{4,4})*$';if(theForm.length==0){return false;}
...
if(theForm[i].name=='email'&&undefined!=theForm.email_confirm){if(theForm[i].value.length==0||theForm.email_confirm.value.length==0){submitFormAlert('Email is required',popup,3);theForm[i].focus();return false;}
...
if(mask=='ccnumber'){if(isCreditCardNumber(theForm[i].value)==false){var msg=dname+" must be of valid credit card number ";if(undefined!=fldmsg){msg=fldmsg;}
submitFormAlert(msg,popup,5);theForm[i].focus();return false;}}
else if(mask=='date'){if(isDate(theForm[i].value)==false){var msg=dname+" must be a valid date ";if(undefined!=fldmsg){msg=fldmsg;}
submitFormAlert(msg,popup,5);theForm[i].focus();return false;}}
else if(mask=='futuredate'){if(isFutureDate(theForm[i].value)==false){var msg=dname+" must be of valid date in the future ";if(undefined!=fldmsg){msg=fldmsg;}
submitFormAlert(msg,popup,5);theForm[i].focus();return false;}}
else if(mask=='pastdate'){if(isPastDate(theForm[i].value)==false){var msg=dname+" must be of valid date in the past ";if(undefined!=fldmsg){msg=fldmsg;}
submitFormAlert(msg,popup,5);theForm[i].focus();return false;}}
else if(mask=='intlphone'){if(checkInternationalPhone(theForm[i].value)==false){var msg=dname+" must be a valid phone number";if(undefined!=fldmsg){msg=fldmsg;}
submitFormAlert(msg,popup,5);theForm[i].focus();return false;}}
...
var re=new RegExp(rmask,'i');if(re.test(theForm[i].value)==false){var msg=dname+" must be of type "+mask;if(undefined!=fldmsg){msg=fldmsg;}
submitFormAlert(msg,popup,5);theForm[i].focus();return false;}}}
...
if(ffound==0){var ffield=document.createElement("input");ffield.type='hidden';ffield.name='_formfields';ffield.value=formfieldstr;theForm.appendChild(ffield);}
return true;}
...
frm._bulkedit.value='1';frm.submit();}
function pagingExport(frm){frm._export.value='1';frm.submit();}
function pagingAddFilter(frm){if(frm.filter_field.value.length==0){alert('select a filter field');return false;}
...
var id=frm.filter_field.value+frm.filter_operator.value+frm.filter_value.value;var obj=getObject(id);var filters=new Array();if(undefined!=obj){obj.style.display='inline-block';}
else{var d=document.createElement('div');d.className='w_pagingfilter';d.setAttribute('data-field',frm.filter_field.value);d.setAttribute('data-operator',frm.filter_operator.value);d.setAttribute('data-value',frm.filter_value.value);d.id=id;var dfield=frm.filter_field.value;if(dfield=='*'){dfield='Any Field';}
...
obj=getObject('paging_clear_filters');if(undefined!=obj){removeId(obj);}
var d=document.createElement('div');d.className='w_pagingfilter  icon-erase w_danger';d.id='paging_clear_filters';d.setAttribute('title','Clear All Filters');d.onclick=function(){pagingClearFilters();};var p=getObject('send_to_filters');p.appendChild(d);frm.filter_value.value='';frm.filter_value.focus();}
...
frm._filters.value=implode("\r\n",filters);if(undefined!=frm._bulkedit){frm._bulkedit.value='';}
...
if(undefined!=tmeout&&tmeout=='novalidation'){tmeout=600000;}
else{var ok=submitForm(theform,1,0,1);if(!ok){return false;}}
...
var lcsid=sid.toLowerCase();var cb=callback.toLowerCase();if(undefined==document.getElementById(sid)&&lcsid!='pop'&&cb.indexOf('popupdiv')==-1&&cb.indexOf('centerpop')==-1&&lcsid.indexOf('popupdiv')==-1&&lcsid.indexOf('centerpop')==-1){alert('Error in ajaxPost\n'+sid+' is not defined as a valid object id');return false;}
if(typeof(AjaxRequest.ActiveAjaxGroupRequests[sid])!='undefined'){ajaxAbort(sid);}
var showprocessing=true;if(undefined!=theform.showprocessing){if(theform.setprocessing.value.toLowerCase()=='false'){showprocessing=false;}
...
function ajaxAbort(sid){if(typeof(AjaxRequest.ActiveAjaxGroupRequests[sid])!='undefined'){var req=AjaxRequest.ActiveAjaxGroupRequests[sid];if(undefined!=req.abort_callback&&req.abort_callback.length){req.status='aborted';var str=req.abort_callback+'(req);';eval(str);}
...
return false;}
function ajaxGet(url,sid,xparams,callback,tmeout,nosetprocess,returnreq,newtitle,newurl,abort_callback,append){var params='';var cp_title='';var showprocessingdiv=sid;var showprocessing=true;if(typeof(xparams)=='object'){if(undefined!=xparams.callback){callback=xparams.callback;}
...
if(undefined==returnreq){returnreq=false;}
var guid=getCookie('GUID');if(undefined==tmeout){tmeout=600000;}
if(tmeout<180000){tmeout=180000;}
if(undefined==nosetprocess){if(nosetprocess){showprocessing=false;}}
var lcsid=sid.toLowerCase();var cb=callback.toLowerCase();if(undefined==document.getElementById(sid)&&lcsid!='pop'&&cb.indexOf('popupdiv')==-1&&cb.indexOf('centerpop')==-1&&lcsid.indexOf('popupdiv')==-1&&lcsid.indexOf('centerpop')==-1){alert('Error in ajaxGet\n'+sid+' is not defined as a valid object id in the DOM');return false;}
if(typeof(AjaxRequest.ActiveAjaxGroupRequests[sid])!='undefined'){ajaxAbort(sid);}
