...
function formChanged(frm){let els=frm.querySelectorAll('[data-displayif]');for(let i=0;i<els.length;i++){let parts=els[i].dataset.displayif.split(':');let name=els[i].dataset.displayif;let vals=new Array(1,'y','yes');if(parts.length==2){name=parts[0];vals=parts[1].split(',')}
...
break}
break}
if(display==1){if(undefined!=els[i].dataset.display){els[i].style.display=els[i].dataset.display}else{els[i].style.display='initial'}}else{els[i].style.display='none'}}else{els[i].display='none'}}}
