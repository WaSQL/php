...
if(requiredif.length>0){let parts=requiredif.split(':');let notparts=requiredif.split('!');let rval='';requiredif=theForm.querySelector('[name="'+parts[0]+'"]');if(parts.length==1){if(undefined!=requiredif){switch(requiredif.type.toLowerCase()){case 'select-one':rval=requiredif.options[requiredif.selectedIndex].value;if(rval.length){required=1}
