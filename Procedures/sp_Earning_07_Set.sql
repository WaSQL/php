drop Procedure Commissions.sp_Earning_07_Set;
create Procedure Commissions.sp_Earning_07_Set(
					 pn_Period_id		int
...
Begin
	...
	declare ln_Unicap		decimal(18,8);
    
	Update period_batch
	Set beg_date_Earning_7 = current_timestamp
      ,end_date_Earning_7 = Null
	...
	-- Get Volumes
	select round(c.vol_13,2), round(c.vol_13 * (r1.value_2/100) + (ifnull(r2.value_2,0)) + (ifnull(:ln_Unicap,0)), 2)
	...
	if :ln_Count = 0 then
		insert into pool_head (period_id, batch_id, pool_id, count) values (:pn_Period_id, :pn_Period_Batch_id, 3, 0);
		commit;
	end if;
	...
	-- Set Earning Detail
	replace Earning_07 (period_id, batch_id, customer_id, rank_id, shares, shares_extra, from_currency, to_currency, exchange_rate, bonus, bonus_exchanged)
	select 
		  a.period_id
		, a.batch_id
		, a.customer_id
	...
	commit;
   	...
   	replace customer_history (customer_id, period_id, batch_id, Earning_7)
   	select 
    	 customer_id
    	,period_id
   		,batch_id
	...
	group by customer_id,period_id,batch_id;
   	...
   	Update period_batch
   	Set end_date_Earning_7 = current_timestamp
   	Where period_id = :pn_Period_id
   	and batch_id = :pn_Period_Batch_id;
