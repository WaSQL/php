drop Procedure Commissions.sp_Earning_08_Set;
create Procedure Commissions.sp_Earning_08_Set(
					 pn_Period_id		int
...
Begin
	...
	Update period_batch
	Set beg_date_Earning_8 = current_timestamp
      ,end_date_Earning_8 = Null
	...
	if :ln_Count = 0 then
		insert into pool_head (period_id, batch_id, pool_id, count) values (:pn_Period_id, :pn_Period_Batch_id, 4, 0);
		commit;
	end if;
	...
	-- Set Earning Detail
	replace Earning_08 (period_id, batch_id, customer_id, rank_id, shares, shares_extra, from_currency, to_currency, exchange_rate, bonus, bonus_exchanged)
	select 
		  a.period_id
		, a.batch_id
		, a.customer_id
	...
	commit;
   	...
   	replace customer_history (customer_id, period_id, batch_id, Earning_8)
   	select 
    	 customer_id
    	,period_id
   		,batch_id
	...
	group by customer_id,period_id,batch_id;
   	...
   	Update period_batch
   	Set end_date_Earning_8 = current_timestamp
   	Where period_id = :pn_Period_id
   	and batch_id = :pn_Period_Batch_id;
