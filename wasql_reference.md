...
```php
$rows = dbQueryResults('postgres_ods', $sql);
...
```
  ...
  `dbGetLast()` with no key returns the whole set — it also carries `query`, `start`/`stop`/`time` and the calling `function`, which is the cheapest way to time a remote query. Without this check a page reports `select * from no_such_table` as a successful empty result — that bug was live in the dexpdq `mcp` server's `run_query` tool until 2026-08-17.
