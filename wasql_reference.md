...
### Same-server cross-database reads — schema-qualified `-table` (no `config.xml` entry)
When the *other* database is another schema on the **same MySQL server** and the site's DB user has `SELECT` on it, you don't need a `config.xml` connection or the `db*` family at all — a **schema-qualified table name is a first-class citizen** of the standard data-access layer. `isDBTable()`, `getDBFields()`, `getDBFieldInfo()`, `getDBQuery()` and `databaseListRecords()` all `preg_split('/\./',$table)` and emit `from {schema}.{table}` (unquoted). So:
```php
$recs = getDBRecords(['-table'=>'otherdb.members','-where'=>'site_id IN (4,5,6)','-nocache'=>1]);
$grid = databaseListRecords(['-table'=>'otherdb.members','-where'=>$where,
    '-listview'=>getView('member_record'),'-results_eval'=>'myExtra','-searchfields'=>'last_name,first_name']);
...
`getCalendar()` (core `php/common.php`) takes **either or both** `-event_table` (queries one local table itself) **and** `-events` (an array you already fetched, from anywhere — a cross-DB schema-qualified read, an API, hand-built records). Both feed the *same* per-day bucket, so you can show a foreign/other-source calendar alongside a local table without touching core or hand-rolling a `UNION` query:
```php
$calendar = getCalendar($monthyear, [
    '-event_table'         => 'calendar',
    '-event_table_filters' => ['site_id'=>commonSiteId()],
    '-events'               => myOtherSourceEvents(),   // array of ['name'=>...,'startdate'=>...] + any extra keys
]);
```
Each `-events` record only *needs* `startdate` + `name`; everything else you set on it (a custom `is_foo` flag, `group`, `icon`) rides through untouched into `$day['events']` for the view to read. If you don't set `_id`, one is generated with `getGuid()` (same as holidays/iCal entries) — a **non-numeric `_id` is the established in-codebase convention for "this calendar entry isn't a row in the local events table"**: the view's `event_time` block is already gated on `isNum($event['_id'])`, and giving your merged rows a non-numeric id (e.g. `'stake_'.$realId`) guarantees they can never collide with a real local `_id` if something later does `(int)$event['_id']`. **Caveat:** the view's edit-click affordance (`data-nav=".../addedit/<?=$event['_id'];?>"`) is *not* gated the same way — it's pre-existing behavior (holidays have always been clickable this way) so a user with edit rights can click a merged entry and land on a confusing add/edit form for a nonexistent row. Not a security issue (the id just matches nothing), but worth an explicit `renderViewIf` guard on `event_edit`-style views in new code if you want to avoid the rough edge.
...
> ⚠️ **The PHP/ODBC path has no working query timeout — confirmed empirically, don't re-attempt this without a different mechanism.** `ctreeSetTimeouts()` is a no-op and `odbc_setoption(…, SQL_QUERY_TIMEOUT, …)` is silently ignored by the FairCom driver (both known from the code's own comments). FairCom's docs also state `QUERY_TIMEOUT=N` in the connect string should bound execution and take precedence over the DSN/ODBC.INI setting — but it doesn't in practice: live-tested 2026-09-04 by adding `;QUERY_TIMEOUT=30` to a `<database>` connect string and running `select top 8000 * from admin.dstdb` (one plain `odbc_prepare()`/`odbc_execute()` call — not a `SELECTPAGINATE` query, so `ctreeQueryResults()`'s batching loop never engages, ruling out "the timeout only bounds one batch" as the explanation). It ran **4.75 minutes** and returned normally, no timeout error at any point. **There is currently no in-process way to bound a c-tree/ODBC query's runtime.** A runaway query just runs until it finishes or PHP's own `max_execution_time` kills it with a fatal (`ctree.php:10-11`, 4200s). The only real fix would be moving long queries off the Apache request thread onto an out-of-band worker process (`WaSQL_Worker_1`) — not attempted, large effort, currently on hold. Full investigation, evidence, and the six related bugs: `ctree_fixes.md` + `/bugs` #38-#43.
>
> This is softened by a separate fix worth knowing about: **SQL Prompt used to let one long-running query wedge every other request from the same user** (`php/sessions.php`'s default `files` session handler holds an exclusive lock for the whole request, and nothing on the `admin.php`/SQL-Prompt path ever called `session_write_close()` before running a query — so a second tab, a reload, or a cancel attempt all blocked in `session_start()` behind the query they were trying to work around). Fixed in `sqlprompt_controller.php`: `session_write_close()` before every call that can run long (the `php>`/`py>`/`lua>`/`cmd>` escapes, and the py/Groovy/default query-execution branches), reopening briefly afterward only where a later request needs `$_SESSION['sql_last_count']`/`debugValue_lastm']` persisted — captured into a local var first, since `session_start()` on reopen repopulates `$_SESSION` from the persisted store and would otherwise discard whatever accumulated during the closed window. The general pattern (**bracket any admin.php action that can run long with `session_write_close()`/`session_start()`, capture-before-reopen for anything written mid-call**) applies to any future long-running admin action, not just SQL Prompt.
...
- **Python side (`python/snowflakedb.py`)** mirrors the same `dbcert`/`dbcertpass`/`dbauth` attributes, but the connector is `snowflake-connector-python`, not ODBC: the key is loaded with `cryptography` and handed over as DER bytes in `private_key` (plus `authenticator=SNOWFLAKE_JWT`, and `password` is dropped). If `cryptography` isn't installed it falls back to `private_key_file`/`private_key_file_pwd`, which needs connector 3.6+. Helpers match the php names — `certFileCheck`, `certValue` (params, then `CONFIG['dbcert_snowflake']`/`CONFIG['snowflake_dbcert']`), `loadPrivateKey`, `maskConnectParams`.
...
- **A bare top-level `function name(){…}` in a page's `body`/`controller` ALWAYS fatals with "Cannot redeclare", on every single request — not intermittent.** `evalPHP()` (`php/common.php` ~10716-10729) runs each `<?php ?>` block through TWO eval() calls: a syntax-check dry run (`$evalcheck = "return true;\n".$evalcode; eval($evalcheck);`) meant to catch parse errors before anything executes, then — if that was clean — the real `eval($evalcode)`. The `return true;` is supposed to make the dry run a no-op, but PHP hoists (pre-registers) an *unconditionally* top-level function declaration at compile time regardless of unreachable code after a `return` — so the dry run silently declares the function anyway, and the real run's identical declaration then collides. Confirmed by isolating the exact two-eval pattern standalone. This is unrelated to (and easy to confuse with) the cross-page `loadDBFunctions` collision two bullets up — this one fires from a SINGLE page's own body/controller, with no second page involved, and 100% reproducibly rather than only when two pages both load the same helper. **Not affected:** a page's `functions` field (and any page loaded via `loadDBFunctions`) — those run through `includeDBOnce`/`commonIncludeFunctionCode`, a single-pass `include_once`, so ordinary top-level function declarations there are fine. Fix in body/controller: guard it — `if(!function_exists('name')){ function name(){...} }` — a conditional declaration is never hoisted, so the dry run's dead-code path never touches it. Logged as core bug #49 (unfixed in core — the real fix belongs in `evalPHP()`, e.g. drop the dry-run pass in favor of wrapping the single real eval in `try/catch(\Throwable)`). Found fixing wired's `/cron_send_article` after `cronSendArticleToSubscribers()` was refactored into a named top-level function in the page body.
...
- **`getCachedValue()` caches whatever the producer returns on a miss — including the "don't run the scan" placeholder** — and with `$hrs = 0` (no expiry) that placeholder then lives forever. The common shape for an expensive on-demand scan is a producer that checks a `$GLOBALS['RUNSCAN']` flag and returns an empty array when the caller only wanted a cached read; the first ordinary page view is a cache MISS, so the producer runs, returns `array()`, and that empty array is written to APCu/the `gsv` file (6 bytes, `a:0:{}`) and served to every later read. It is recoverable — a `$force = true` call overwrites it — but it means **an empty cached value is indistinguishable from "never scanned"**, so never render it as a silently blank column: detect the empty result and say *not scanned* (plus a button that forces the scan) instead. Also note the cache key is `sha256(namespace . '|' . producerString)`, so a producer that takes an argument (`'return pageFooScan("lrp");'`) is a **separate cache entry per argument** — forcing one scope leaves all the others empty. Inspect entries directly at `sys_get_temp_dir() . '/gsv/<sha256>.gsv'` (plain `serialize()` output) when APCu is off.
