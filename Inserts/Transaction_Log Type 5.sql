insert into transaction_log (transaction_log_id,transaction_log_ref_id,customer_id,source_key_id,source_id,period_id,transaction_date,transaction_type_id,transaction_category_id,currency_code,value_2,value_4)
select 
	...
	,5
	,1
	,(select currency_code from country where country_code = c.country)
	,b.pv-(c.vol_1+c.vol_4)
	,b.cv-(c.vol_6+c.vol_9)
from customer_history c, orabwt b
...
and c.customer_id in (
	...
	from HIERARCHY ( 
		 	SOURCE ( select customer_id AS node_id, sponsor_id AS parent_id, aa.*, round(aa.vol_1+aa.vol_4,2) as agg_pv
		             from (select * from customer_history where period_id = 12) aa)
			Start where customer_id = 3) ac, orabwt ab
