...
*/
//---------- begin function snowflakeIsResourceOrObject--------------------
/**
* @describe returns true if $e is a resource OR an object.
*	Local clone of common.php's commonIsResourceOrObject so this extra does not
*	depend on the host site running a common.php new enough to define it.
* @param e mixed - element to check
* @return boolean - true if $e is a resource or an object
* @usage if(snowflakeIsResourceOrObject($dbh_snowflake)){...}
*/
function snowflakeIsResourceOrObject($e){
...
function snowflakeGetDBRecordById($table='',$id=0,$relate=1,$fields=""){
	...
	$recopts=array('-table'=>$table,'_id'=>$id,'-limit'=>1);
	if($relate){$recopts['-relate']=1;}
	if(strlen($fields)){$recopts['-fields']=$fields;}
	$recs=snowflakeGetDBRecords($recopts);
	return isset($recs[0]) ? $recs[0] : array();
}
...
function snowflakeDelDBRecordById($table='',$id=0){
	...
	$params=array();
	$params['-table']=$table;
	$query="DELETE FROM {$table} WHERE _id in ({$idstr})";
	return snowflakeExecuteSQL($query,$params);
}
...
function snowflakeDBConnect($params=array()){
	...
	if(isset($params['-single'])){
		...
		if(!snowflakeIsResourceOrObject($dbh_snowflake_single)){
			$e=odbc_errormsg();
	...
	if(snowflakeIsResourceOrObject($dbh_snowflake)){return $dbh_snowflake;}
	...
	try{
		...
		if(!snowflakeIsResourceOrObject($dbh_snowflake)){
...
ENDOFQUERY;
	...
	try{
		if(!isset($snowflakeAddDBRecordCache[$params['-table']]['stmt'])){
			...
			if(!snowflakeIsResourceOrObject($snowflakeAddDBRecordCache[$params['-table']]['stmt'])){
				$e=odbc_errormsg();
...
ENDOFQUERY;
	...
	try{
		...
		if(!snowflakeIsResourceOrObject($snowflake_stmt)){
			$e=odbc_errormsg();
...
function snowflakeGetDBSchemas($params=array()){
	...
	try{
		...
		if(!$result){
			...
			$error=array("snowflakeGetDBSchemas Error",$e);
			debugValue($error);
			return json_encode($error);
		}
	}
...
function snowflakeGetDBTables($params=array()){
	...
	try{
		$result=odbc_tables($dbh_snowflake);
		if(!snowflakeIsResourceOrObject($result)){
			$e=odbc_errormsg($dbh_snowflake);
...
function snowflakeGetDBCount($params=array()){
	...
	if(!isset($recs[0]['cnt'])){
		debugValue(array("snowflakeGetDBCount Error",$recs));
		return 0;
	}
	return $recs[0]['cnt'];
}
...
function snowflakeQueryResults($query,$params=array()){
	...
	if ($snowsql==1) {
		...
		$accountname='';
	if(isset($DATABASE[$db]['dbaccount'])){
		$accountname=$DATABASE[$db]['dbaccount'];
	}
...
ENDOFCON;
	...
	try{
		...
		if(!$result){
			...
			if(stringContains($e,'session not connected') || stringContains($e,'Receive Error')){
				...
				$DATABASE['_lastquery']['error']='exec failed: '.$e;
				debugValue($DATABASE['_lastquery']);
		    	return 0;
			}
		}
		...
		$DATABASE['_lastquery']['error']='exec failed: '.$e->getMessage();
		debugValue($DATABASE['_lastquery']);
    	return 0;
	}
	...
	if(isset($params['-filename'])){
    	...
    	if(!isset($fh) || !snowflakeIsResourceOrObject($fh)){
			odbc_free_result($result);
