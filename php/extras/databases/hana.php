...
function hanaAddDBRecordsProcess($recs,$params=array()){
	...
	if(isset($params['-upsert']) && isset($params['-upserton'])){
		...
		*/
		...
		if(isset($params['-upsertwhere'])){
			$condition=PHP_EOL." AND {$params['-upsertwhere']}";
		}
		$query .= ') '.PHP_EOL."WHEN MATCHED {$condition} THEN UPDATE SET ".PHP_EOL;
		$flds=array();
	...
	odbc_errormsg($dbh_hana);
	//echo $query;exit;
	$stmt = odbc_prepare($dbh_hana, $query);
	if(!$stmt){
		debugValue(array("odbc_prepare error",odbc_errormsg($dbh_hana),$query,$fields));
		return 0;
	}
	$ok = odbc_execute($stmt, $pvalues);
	//echo printValue($ok).odbc_errormsg($dbh_hana).printValue($pvalues);exit;
	...
	if(!$ok){
		...
		debugValue(array("odbc_execute error",odbc_errormsg($dbh_hana),$query,$drecs));
		return 0;
	}
	if(isset($params['-debug'])){
