...
* @usage
*	<?=mssqlListRecords(array('-table'=>'notes'));?>
*	<?=mssqlListRecords(array('-list'=>$recs));?>
*	<?=mssqlListRecords("select * from myschema.mytable where ...");?>
*/
function mssqlListRecords($params=array()){
	...
	if(!empty($params['-table'])){
		...
		if(!is_array($info) || !count($info)){
			return "mssqlListRecords error: No fields found for {$params['-table']}";
		}
	...
	if(!is_array($params['-list']) || count($params['-list'])==0){
		return "mssqlListRecords error: ".$params['-list'];
	}
