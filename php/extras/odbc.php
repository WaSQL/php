...
//---------- begin function odbcAddDBRecordsFromCSV ----------
/**
* @describe imports data from csv into a ODBC table
* @param $table string - table to import into
* @param $csv - csvfile to import (path visible by the ODBC server)
...
* @usage 
*	loadExtras('odbc');
*	$ok=odbcAddDBRecordsFromCSV('stgschema.testtable','/mnt/dtxodbc/test.csv');
*/
function odbcAddDBRecordsFromCSV($table,$csvfile,$params=array()){
	//error log with same name as csvfile in same path so ODBC can write to it.
	 ...
	 * 	https://blogs.sap.com/2013/04/08/best-practices-for-sap-odbc-data-loads/
	 * 	https://blogs.sap.com/2014/02/12/8-tips-on-pre-processing-flat-files-with-sap-odbc/
	 *
...
*/
function odbcAddDBRecordsOLD($table,$recs){
	global $USER;
	if(!strlen($table)){return 'odbcAddDBRecords error: No table.';}
	$fields=odbcGetDBFieldInfo($table,$params);
	$opts=array();
...
ENDOFQUERY;
	...
	try{
		...
		$success = odbc_execute($odbc_stmt,$opts['values']);
		if(!$success){
			$e=odbc_errormsg();
			debugValue(array("odbcAddDBRecord Execute Error",$e));
...
//---------- begin function odbcManageDBSessions ----------
/**
* @describe cleans up idle sessions since ODBC does not clean up for you.
...
* @usage 
*	loadExtras('odbc');
*	$cnt=odbcManageDBSessions('all');
*/
function odbcManageDBSessions($username='',$idle=1800000,$params=array()){
	global $USER;
...
ENDOFQUERY;
	...
	foreach($recs as $rec){
    	...
    	$ok=odbcQueryResults($query,$params);
	}
	return count($recs);
}
...
//---------- begin function odbcGetDBSystemTables ----------
/**
...
* @usage 
*	loadExtras('odbc');
*	$systemtables=odbcGetDBSystemTables();
*/
function odbcGetDBSystemTables($params=array()){
	$params['-table_schema']='S';
	return odbcGetDBTables($params);
}
...
* @usage
*	loadExtras('odbc'); 
*	$fields=odbcGetDBTablePrimaryKeys();
*/
function odbcGetDBTablePrimaryKeys($table,$params=array()){
	...
	$tmp = odbcQueryResults($query,$params);
	$keys=array();
	foreach($tmp as $rec){
...
//---------- begin function odbcBuildPreparedInsertStatement ----------
/**
* @describe creates the query needed for a prepared Insert Statement
* @param $table string - tablename
* @param $fieldinfo array - field info obtained from odbcGetDBFieldInfo function
* @return $query string
* @usage
*	loadExtras('odbc'); 
*	$query=odbcBuildPreparedInsertStatement($table,$fieldinfo,$primary_keys);
*/
function odbcBuildPreparedInsertStatement($table,$fieldinfo=array(),$params=array()){
	if(!is_array($fieldinfo)){
		$fieldinfo=odbcGetDBFieldInfo($table,$params);
	}
	$fields=array();
	$binds=array();
...
//---------- begin function odbcBuildPreparedReplaceStatement ----------
/**
...
* @usage 
*	loadExtras('odbc');
*	$query=odbcBuildPreparedReplaceStatement($table,$fieldinfo,$primary_keys);
*/
function odbcBuildPreparedReplaceStatement($table,$fieldinfo=array(),$keys=array(),$params=array()){
	if(!is_array($keys) || !count($keys)){
		echo "odbcBuildPreparedReplaceStatement error - missing keys.  Table: {$table}";
		exit;
	}
	if(!is_array($fieldinfo)){
		$fieldinfo=odbcGetDBFieldInfo($table,$params);
...
//---------- begin function odbcBuildPreparedUpdateStatement ----------
/**
...
* @usage 
*	loadExtras('odbc');
*	$query=odbcBuildPreparedUpdateStatement($table,$fieldinfo,$primary_keys);
*/
function odbcBuildPreparedUpdateStatement($table,$fieldinfo=array(),$keys=array(),$params=array()){
	if(!is_array($keys) || !count($keys)){
		echo "odbcBuildPreparedUpdateStatement error - missing keys.  Table: {$table}";
		exit;
	}
	if(!is_array($fieldinfo)){
		$fieldinfo=odbcGetDBFieldInfo($table,$params);
...
//---------- begin function odbcBuildPreparedDeleteStatement ----------
/**
...
* @usage 
*	loadExtras('odbc');
*	$query=odbcBuildPreparedDeleteStatement($table,$fieldinfo,$primary_keys);
*/
function odbcBuildPreparedDeleteStatement($table,$fieldinfo=array(),$keys=array(),$params=array()){
	if(!is_array($keys) || !count($keys)){
		echo "odbcBuildPreparedDeleteStatement error - missing keys.  Table: {$table}";
		exit;
	}
	if(!is_array($fieldinfo)){
		$fieldinfo=odbcGetDBFieldInfo($table,$params);
