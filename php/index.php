	...
	if(!is_array($ruser)){
		...
		if(isset($CONFIG['smtp'])){
			...
			if($sendopts['ok']==true || (isNum($sendopts['ok']) && $sendopts['ok']==1)){
				...
				echo '<div style="display:none">'.printValue($sendopts).'</div>';
				exit;
            }
            else{
...
if(is_array($PAGE) && $PAGE['_id'] > 0){
	...
	if(isset($TEMPLATE['post_render']) && function_exists($TEMPLATE['post_render'])){
		$htm=call_user_func($TEMPLATE['post_render'],$htm);
	}
