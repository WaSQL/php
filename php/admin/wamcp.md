...
- The `query` tool only permits read-only statements (SELECT, SHOW, EXPLAIN, DESCRIBE, WITH) — write operations are rejected. The one exception is `addpage`, a narrow, purpose-built insert into `_pages` (name + template_id only, via the framework's own record-add path) — it cannot run arbitrary SQL.
