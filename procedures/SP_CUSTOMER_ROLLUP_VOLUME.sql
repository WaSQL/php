drop procedure Commissions.sp_Customer_Rollup_Volume;
create procedure Commissions.sp_Customer_Rollup_Volume(
	...
	if ifnull(:pn_PV,0) <> 0 or ifnull(:pn_CV,0) <> 0 then
		...
		loop
			...
			if ifnull(:ln_Realtime_Rank,1) = 1 then
				call Commissions.sp_Customer_Rollup_Rank(:ln_Customer_id);
			end if;
